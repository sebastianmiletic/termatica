...
- (void)clearWideCellAtX:(NSUInteger)x row:(TCell *)row __attribute__((objc_direct)) {
    ...
    else if((row[x].flags&TContinuation)&&x){row[x-1]=[self blankCell];}
}
- (void)sanitizeWideCellsInRow:(TCell *)row __attribute__((objc_direct)) {if(!row)return;for(NSUInteger x=0;x<_cols;x++){if(row[x].flags&TWide){if(x+1<_cols&&(row[x+1].flags&TContinuation)){x++;continue;}row[x]=[self blankCell];}else if(row[x].flags&TContinuation)row[x]=[self blankCell];}}
...
static int TRunTerminalSelfTest(void) {
    ...
    fprintf(stdout,"terminal-self-test ok history=%lu rows=%lu columns=%lu keyboard=kitty+legacy mouse=native-selection+buttons+drag+motion+tui-wheel selection=document-anchored wheel=normal+hyprland codex-redraw=wide-cell-clean+row-clipped tabs=surface-stable layout=independent-mixed-splits+arbitrary-slot-swap motion=translation-only rail=bounded\n",(unsigned long)[initial[@"history"] unsignedIntegerValue],(unsigned long)[initial[@"rows"] unsignedIntegerValue],(unsigned long)[initial[@"columns"] unsignedIntegerValue]);
    return 0;
}
