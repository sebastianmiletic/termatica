...
- (void)configureRenderBackend {
    ...
    if([desired isEqual:@"metal"]&&!_metalFailed){
        ...
        _metalFailed=YES;_metalFailureCount++;TLog(@"Metal renderer unavailable: %@",error.localizedDescription?:@"unknown error");
    }
...
- (void)fallbackToAppKitForError:(NSError *)error {
    ...
    TLog(@"Metal renderer failure, falling back to AppKit: %@",error.localizedDescription?:@"unknown error");_metalFailed=YES;_metalFailureCount++;[_renderBackend shutdown];_renderBackend=nil;_metalBackend=nil;
...
- (NSDictionary *)diagnosticState {@synchronized(self){return @{@"history":@(_historyCount),@"offset":@(_historyOffset),@"cursorX":@(_cursorX),@"cursorY":@(_cursorY),@"activeTerminal":@(self.activeTerminal),@"firstResponder":@(self.window.firstResponder==self),@"displayCursorVisible":@(_displaySnapshot.cursorVisible),@"alternate":@(_alternateScreen),@"inlineViewport":@(_inlineViewportMode),@"inlineViewportTop":@(_inlineViewportTop),@"synchronizedUpdates":@(_synchronizedUpdates),@"selecting":@(_selecting),@"selection":@(_hasSelection),@"rows":@(_rows),@"columns":@(_cols),@"mouseMode":@(_mouseTrackingMode),@"mouseEncoding":_pixelMouse?@"pixel-sgr":(_sgrMouse?@"sgr":(_urxvtMouse?@"urxvt":(_utf8Mouse?@"utf8":@"legacy"))),@"renderer":_renderBackend.name?:@"none",@"renderGeneration":@(_renderBackend.lastPresentedGeneration),@"metalFailed":@(_metalFailed),@"metalFailureCount":@(_metalFailureCount),@"presentationRecoveryCount":@(_presentationRecoveryCount),@"lastPresentationRecovery":_lastPresentationRecovery?:@"none",@"secondsSincePresentationRecovery":@(_lastPresentationRecoveryAt>0?MAX(0,CFAbsoluteTimeGetCurrent()-_lastPresentationRecoveryAt):-1),@"metalFrameChecksum":@(_metalBackend.lastFrameChecksum),@"metalFrameVaried":@(_metalBackend.lastFrameVariedPixels),@"snapshotBuildMs":@(_lastSnapshotBuildMilliseconds),@"metalSnapshotWaitMs":@(_metalBackend.lastSnapshotWaitMilliseconds),@"metalCPUEncodeMs":@(_metalBackend.lastCPUEncodeMilliseconds),@"metalGPUExecutionMs":@(_metalBackend.lastGPUExecutionMilliseconds),@"metalGPUCompletionMs":@(_metalBackend.lastGPUCompletionMilliseconds),@"metalPresentIntervalMs":@(_metalBackend.lastPresentIntervalMilliseconds)};}}
