...
- (NSDictionary *)automationReply:(NSDictionary *)request {NSString *action=[request[@"action"] isKindOfClass:NSString.class]?[request[@"action"] lowercaseString]:@"",*text=[request[@"text"] isKindOfClass:NSString.class]?request[@"text"]:nil,*rawDirectory=[request[@"cwd"] isKindOfClass:NSString.class]?request[@"cwd"]:nil,*directory=[self automationDirectory:rawDirectory];if(rawDirectory.length&&!directory)return @{@"ok":@NO,@"error":@"cwd must be an existing directory"};if(text.length>4096)return @{@"ok":@NO,@"error":@"text exceeds the 4096 character limit"};TWindowController *controller=[self active];if([action isEqual:@"status"]){NSMutableArray *windows=[NSMutableArray array];for(TWindowController *window in self.windows)[windows addObject:[window automationStatus]];NSUInteger active=[self.windows indexOfObject:controller];return @{@"ok":@YES,@"schemaVersion":@1,@"windows":windows,@"activeWindow":@(active==NSNotFound?0:active+1),@"privacy":@{@"terminalContentIncluded":@NO,@"scrollbackIncluded":@NO,@"commandHistoryIncluded":@NO}};}if([action isEqual:@"activate"]){[NSApp activateIgnoringOtherApps:YES];[controller.window makeKeyAndOrderFront:nil];[controller focusTerminal:controller.terminal];return @{@"ok":@YES};}if([action isEqual:@"new-window"]){[self newWindowWithDirectory:directory command:text];return @{@"ok":@YES};}if(!controller)return @{@"ok":@NO,@"error":@"no terminal window is available"};if([action isEqual:@"new-tab"]){[controller addTabWithDirectory:directory command:text];return @{@"ok":@YES};}if([action isEqual:@"split"]){NSString *direction=[request[@"direction"] isKindOfClass:NSString.class]?[request[@"direction"] lowercaseString]:@"horizontal";if(![@[@"horizontal",@"vertical"] containsObject:direction])return @{@"ok":@NO,@"error":@"split direction must be horizontal or vertical"};[controller addSplitWithVerticalDivider:[direction isEqual:@"vertical"] directory:directory command:text];return @{@"ok":@YES};}if([action isEqual:@"focus"]){NSString *target=[request[@"target"] isKindOfClass:NSString.class]?[request[@"target"] lowercaseString]:@"";BOOL ok=NO;if([target isEqual:@"next"]||[target isEqual:@"previous"]){[controller focusAdjacentSplit:[target isEqual:@"next"]?1:-1];ok=YES;}else if([target isEqual:@"tab"])ok=[controller focusAutomationTab:[request[@"index"] integerValue]];else if([target isEqual:@"pane"])ok=[controller focusAutomationPane:[request[@"index"] integerValue]];return ok?@{@"ok":@YES}:@{@"ok":@NO,@"error":@"focus target or index is invalid"};}if([action isEqual:@"send"]||[action isEqual:@"run"]){if(!text.length)return @{@"ok":@NO,@"error":@"text is required"};[controller.terminal sendString:[action isEqual:@"run"]?[text stringByAppendingString:@"\n"]:text];return @{@"ok":@YES};}if([action isEqual:@"key"]){NSDictionary *keys=@{@"enter":@"\r",@"return":@"\r",@"escape":@"\033",@"tab":@"\t",@"up":@"\033[A",@"down":@"\033[B",@"right":@"\033[C",@"left":@"\033[D",@"home":@"\033[H",@"end":@"\033[F",@"page-up":@"\033[5~",@"page-down":@"\033[6~",@"ctrl-c":@"\003",@"ctrl-d":@"\004"};NSString *value=keys[[request[@"key"] lowercaseString]];if(!value)return @{@"ok":@NO,@"error":@"unsupported key"};[controller.terminal sendString:value];return @{@"ok":@YES};}if([action isEqual:@"recipe"]){NSArray *commands=[request[@"commands"] isKindOfClass:NSArray.class]?request[@"commands"]:@[];NSString *direction=[request[@"direction"] isEqual:@"vertical"]?@"vertical":@"horizontal";if(!commands.count||commands.count>9)return @{@"ok":@NO,@"error":@"recipe must contain 1 to 9 commands"};for(id command in commands)if(![command isKindOfClass:NSString.class]||[command length]>4096)return @{@"ok":@NO,@"error":@"recipe command is invalid"};[controller.terminal sendString:[commands[0] stringByAppendingString:@"\n"]];for(NSUInteger i=1;i<commands.count;i++)[controller addSplitWithVerticalDivider:[direction isEqual:@"vertical"] directory:nil command:commands[i]];return @{@"ok":@YES,@"panes":@(commands.count)};}return @{@"ok":@NO,@"error":@"unknown automation action"};}
- (void)handleCLIRequest:(NSDictionary *)request {NSString *command=request[@"command"];if([command isEqual:@"automation"])TSendCLIReply(request[@"replyPath"],[self automationReply:request]);else if([command isEqual:@"newTab"])[self automationReply:@{@"action":@"new-tab"}];else if([command isEqual:@"newWindow"])[self automationReply:@{@"action":@"new-window"}];else if([command isEqual:@"splitHorizontal"])[self automationReply:@{@"action":@"split",@"direction":@"horizontal"}];else if([command isEqual:@"splitVertical"])[self automationReply:@{@"action":@"split",@"direction":@"vertical"}];else if([command isEqual:@"sendText"])[self automationReply:@{@"action":@"send",@"text":request[@"text"]?:@""}];else if([command isEqual:@"reload"])[self reloadAll];else if([command isEqual:@"activate"])[self automationReply:@{@"action":@"activate"}];else if([command isEqual:@"run"]&&![[self active] executeExtensionNamed:request[@"name"] query:request[@"query"]])TLog(@"extension command not found: %@",request[@"name"]);else if([command isEqual:@"rendererReport"])TSendCLIReply(request[@"replyPath"],[self rendererFieldReport]);else if([command isEqual:@"rendererRetry"])TSendCLIReply(request[@"replyPath"],[self rendererRetryReport]);else if([command isEqual:@"benchmark"]){TWindowController *controller=[self active];NSUInteger terminals=0;for(TWindowController *window in self.windows)terminals+=window.terminals.count;NSDictionary *reply=controller?TRunInAppBenchmark(self.config,controller.terminal,self.windows.count,terminals):@{@"ok":@NO};TSendCLIReply(request[@"replyPath"],reply);}else if([command isEqual:@"showBenchmark"]){NSString *path=[request[@"artifactPath"] isKindOfClass:NSString.class]?request[@"artifactPath"]:nil;BOOL directory=NO;if(path.length&&[NSFileManager.defaultManager fileExistsAtPath:path isDirectory:&directory]&&directory){self.benchmarkResults=[[TBenchmarkResultsController alloc]initWithArtifactPath:path current:[request[@"current"] isKindOfClass:NSDictionary.class]?request[@"current"]:@{}];[NSApp activateIgnoringOtherApps:YES];[self.benchmarkResults showWindow:nil];[self.benchmarkResults.window makeKeyAndOrderFront:nil];}}else if([command isEqual:@"benchmarkDiagnostics"])TSendCLIReply(request[@"replyPath"],self.benchmarkResults?[self.benchmarkResults diagnostics]:@{@"ok":@NO,@"error":@"benchmark window is not open"});else if([command isEqual:@"quit"])[NSApp terminate:nil];}
- (void)buildMenu {
...
static int TRunConfigWatcherSelfTest(void) {
    TConfig *config=[TConfig new];TAppDelegate *delegate=[TAppDelegate new];delegate.config=config;delegate.windows=[NSMutableArray array];[delegate startConfigWatcher];
    for(NSNumber *size in @[@18,@23]){NSMutableDictionary *dictionary=[TReadJSONDictionaryAtPath(config.path) mutableCopy];dictionary[@"fontSize"]=size;NSError *error=nil;if(!TWriteJSONDictionary(dictionary,config.path,&error)){fprintf(stderr,"config-watcher-self-test failed stage=write error=%s\n",error.localizedDescription.UTF8String);return 180;}NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:2];while(config.fontSize!=size.doubleValue&&deadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];if(config.fontSize!=size.doubleValue){fprintf(stderr,"config-watcher-self-test failed stage=reload expected=%.0f actual=%.0f\n",size.doubleValue,config.fontSize);return 181;}}
    fputs("config-watcher-self-test ok repeated-atomic-edits=2 selected-file-rearmed=yes\n",stdout);return 0;
}
