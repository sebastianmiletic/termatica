...
static int TRunRendererSelfTest(void) {
    ...
    NSDictionary *state=terminal.diagnosticState;
    if(!headlessCI){
    ...
    fprintf(stdout,"renderer-self-test ok mode=%s metal-generation=%llu checksum=%llu fallback=appkit\n",headlessCI?"headless-ci":"pixel-readback",(unsigned long long)[state[@"renderGeneration"] unsignedLongLongValue],(unsigned long long)[state[@"metalFrameChecksum"] unsignedLongLongValue]);return 0;
}
