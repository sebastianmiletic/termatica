...
static NSString *TCompletionScript(NSString *shell) {
    NSString *commands=@"ssh system-monitor monitor benchmark bench config config-file update reload editor run completions help version sm b c cf u r e x h v",*configActions=@"list get set create use rename delete",*sshActions=@"list add set show rename remove connect command check split tile config known-hosts keys keygen help",*editors=@"vim nvim emacs nano micro hx";
...
static int TRunCLI(int argc, const char *argv[]) {
    ...
    NSDictionary *quick=@{@"sm":@"system-monitor",@"b":@"benchmark",@"c":@"config",@"cf":@"config-file",@"u":@"update",@"r":@"reload",@"e":@"editor",@"x":@"run",@"h":@"help",@"v":@"version"};
    arg=quick[arg]?:arg;
    if([arg isEqual:@"help"]){
...
- (void)addTab {[self cancelTileAnimation];BOOL animate=_terminals.count>0&&_terminals.count<9,wasTiled=[self usesTiledLayout];TTerminalView *anchor=self.terminal,*terminal=[self newTerminal];terminal.launchDirectory=anchor?[anchor workingDirectory]:_cwd;[terminal startShell];[_terminals addObject:terminal];[_root addSubview:terminal positioned:NSWindowBelow relativeTo:_tabRail];self.terminal=terminal;self.extensions.activeTerminal=terminal;_animateTabLayout=NO;if(wasTiled!=[self usesTiledLayout])[self applyAppearance];else{[self rebuildTabs];[self layoutTabs];}if(animate)[self animateNewTerminal:terminal];[self focusTerminal:terminal];}
- (void)addSplitWithVerticalDivider:(BOOL)verticalDivider {[self cancelTileAnimation];TTerminalView *anchor=self.terminal;if(!anchor){[self addTab];return;}BOOL animate=_terminals.count<9,wasTiled=[self usesTiledLayout];TTerminalView *terminal=[self newTerminal];terminal.launchDirectory=[anchor workingDirectory]?:_cwd;terminal.verticalSplit=verticalDivider;terminal.splitAnchor=anchor;[terminal startShell];NSUInteger index=[_terminals indexOfObject:anchor];[_terminals insertObject:terminal atIndex:index==NSNotFound?_terminals.count:index+1];if(animate&&[self animationsEnabled])_enteringTerminal=terminal;[_root addSubview:terminal positioned:NSWindowBelow relativeTo:_tabRail];self.terminal=terminal;self.extensions.activeTerminal=terminal;_animateTabLayout=animate;if(wasTiled!=[self usesTiledLayout])[self applyAppearance];else{[self rebuildTabs];[self layoutTabs];}[self focusTerminal:terminal];}
