...
static NSString *TCompletionScript(NSString *shell) {
    NSString *commands=@"ssh system-monitor monitor benchmark bench renderer-report renderer-qualification renderer-campaign qualify renderer-retry retry config config-file update reload editor run completions help version sm b rr rq rc rt c cf u r e x h v",*configActions=@"list get set create use rename delete",*sshActions=@"list add set show rename remove connect command check split tile config known-hosts keys keygen help",*editors=@"vim nvim emacs nano micro hx";
...
static int TRunCLI(int argc, const char *argv[]) {
    ...
    NSDictionary *quick=@{@"sm":@"system-monitor",@"b":@"benchmark",@"c":@"config",@"cf":@"config-file",@"u":@"update",@"r":@"reload",@"rr":@"renderer-report",@"rq":@"renderer-qualification",@"rc":@"renderer-campaign",@"rt":@"renderer-retry",@"e":@"editor",@"x":@"run",@"h":@"help",@"v":@"version"};
    arg=quick[arg]?:arg;
    if([arg isEqual:@"--help"]||[arg isEqual:@"-h"]||[arg isEqual:@"help"]){
        fprintf(stdout,"Termatica %s\n\nUSAGE\n  t <command> [arguments]\n  termatica <command> [arguments]\n\nQUICK\n  t ssh              Open the SSH manager\n  t ssh tile ...     Open saved hosts across split panes\n  t sm               Open the live system monitor\n  t b                Benchmark Termatica now; use saved competitor results\n  t b a              Benchmark all installed comparison terminals now\n  t c                Open config files and settings\n  t cf               Open the selected config file\n  t u [check]        Update or check for an update\n  t r                Reload saved configuration\n  t rr               Print a privacy-safe renderer field report\n  t rq               Show observed field qualification and open gates\n  t rc export [file] Export an integrity-digested field report\n  t rc aggregate ... Validate and combine exported reports\n  t rt               Retry quarantined Metal panes in place\n  t e <name> ...     Run a terminal editor\n  t x <name> [text]  Run an enabled extension command\n  t h / t v          Help / version\n\nCONFIGURATION\n  config             Open the complete categorized terminal config UI\n  config-file        Open the selected authoritative JSON file\n  config-file path   Print the selected config path\n\nUPDATES\n  update             Download, verify, and install the latest GitHub release\n  update check       Check GitHub without installing\n\nTOOLS\n  ssh                Manage, validate, split, tile, and connect SSH profiles\n  system-monitor     Live CPU, memory, storage, network, and process statistics\n  benchmark [all]    Benchmark Termatica only, or all comparison terminals\n  renderer-report    Report display and renderer health without terminal text\n  renderer-qualification  Report observed field gates without claiming synthetic proof\n  renderer-campaign export [file]  Export a privacy-safe report; stdout if no file\n  renderer-campaign aggregate <file ...>  Validate, deduplicate, and combine reports\n  renderer-retry     Retry only quarantined Metal panes without changing config\n  reload             Reload saved configuration in the running app\n  editor <name> ...  Run Vim, Neovim, Emacs, Nano, Micro, or Helix\n  run <name> [text]  Run an enabled extension command\n  completions        Generate or install shell completions\n\nFLAGS\n  --help             Show this guide\n  --version          Print the version\n",TCurrentVersion().UTF8String);return 0;
    }
    ...
    if([arg isEqual:@"renderer-report"]){if(argc>2){fputs("termatica renderer-report: no arguments expected\n",stderr);return 2;}NSDictionary *reply=TRequestCLI(@{@"command":@"rendererReport"},3);if(!reply){fputs("termatica renderer-report: the current Termatica app did not reply; restart it once after updating.\n",stderr);return 1;}NSData *json=[NSJSONSerialization dataWithJSONObject:reply options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if(!json)return 1;fwrite(json.bytes,1,json.length,stdout);fputc('\n',stdout);return [reply[@"ok"] boolValue]?0:1;}
    if([arg isEqual:@"renderer-qualification"]||[arg isEqual:@"qualify"]){if(argc>2){fputs("termatica renderer-qualification: no arguments expected\n",stderr);return 2;}NSDictionary *reply=TRequestCLI(@{@"command":@"rendererReport"},3);if(!reply){fputs("termatica renderer-qualification: the current Termatica app did not reply; restart it once after updating.\n",stderr);return 1;}NSDictionary *output=@{@"ok":reply[@"ok"]?:@NO,@"app":reply[@"app"]?:@{},@"system":reply[@"system"]?:@{},@"displays":reply[@"displays"]?:@[],@"fieldEvidence":reply[@"fieldEvidence"]?:@{},@"fieldQualification":reply[@"fieldQualification"]?:@{}};NSData *json=[NSJSONSerialization dataWithJSONObject:output options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if(!json)return 1;fwrite(json.bytes,1,json.length,stdout);fputc('\n',stdout);return [reply[@"ok"] boolValue]?0:1;}
    if([arg isEqual:@"renderer-campaign"]){if(argc<3){fputs("termatica renderer-campaign: expected export [file] or aggregate <file ...>\n",stderr);return 2;}NSString *action=[[NSString stringWithUTF8String:argv[2]] lowercaseString];if([action isEqual:@"export"]){if(argc>4){fputs("termatica renderer-campaign export: expected at most one file\n",stderr);return 2;}NSDictionary *reply=TRequestCLI(@{@"command":@"rendererReport"},3);if(!reply){fputs("termatica renderer-campaign export: the current Termatica app did not reply; restart it once after updating.\n",stderr);return 1;}NSDictionary *envelope=TFieldCampaignEnvelope(reply);NSData *json=[NSJSONSerialization dataWithJSONObject:envelope options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if(!json)return 1;if(argc==4){NSString *path=[[NSString stringWithUTF8String:argv[3]] stringByExpandingTildeInPath];NSError *error=nil;if(![json writeToFile:path options:NSDataWritingAtomic error:&error]){fprintf(stderr,"termatica renderer-campaign export: %s\n",error.localizedDescription.UTF8String);return 1;}chmod(path.fileSystemRepresentation,0600);fprintf(stdout,"Field report exported: %s\nIntegrity is SHA-256; machine and operator authenticity are not asserted.\n",path.UTF8String);}else{fwrite(json.bytes,1,json.length,stdout);fputc('\n',stdout);}return 0;}if([action isEqual:@"aggregate"]){if(argc<4){fputs("termatica renderer-campaign aggregate: provide one or more exported report files\n",stderr);return 2;}NSMutableArray *reports=[NSMutableArray array];for(int index=3;index<argc;index++){NSString *path=[[NSString stringWithUTF8String:argv[index]] stringByExpandingTildeInPath];NSDictionary *attributes=[NSFileManager.defaultManager attributesOfItemAtPath:path error:nil];unsigned long long size=[attributes[NSFileSize] unsignedLongLongValue];NSData *data=size>0&&size<=1048576?[NSData dataWithContentsOfFile:path options:NSDataReadingMappedIfSafe error:nil]:nil;id value=data?[NSJSONSerialization JSONObjectWithData:data options:0 error:nil]:nil;if([value isKindOfClass:NSDictionary.class])[reports addObject:value];else [reports addObject:@{}];}NSDictionary *aggregate=TAggregateFieldCampaignReports(reports);NSData *json=[NSJSONSerialization dataWithJSONObject:aggregate options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if(!json)return 1;fwrite(json.bytes,1,json.length,stdout);fputc('\n',stdout);return [aggregate[@"acceptedReports"] unsignedIntegerValue]>0?0:1;}fputs("termatica renderer-campaign: expected export or aggregate\n",stderr);return 2;}
    if([arg isEqual:@"renderer-retry"]||[arg isEqual:@"retry"]){if(argc>2){fputs("termatica renderer-retry: no arguments expected\n",stderr);return 2;}NSDictionary *reply=TRequestCLI(@{@"command":@"rendererRetry"},8);if(!reply){fputs("termatica renderer-retry: the current Termatica app did not reply; restart it once after updating.\n",stderr);return 1;}NSData *json=[NSJSONSerialization dataWithJSONObject:reply options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if(!json)return 1;fwrite(json.bytes,1,json.length,stdout);fputc('\n',stdout);return [reply[@"ok"] boolValue]?0:1;}
...
- (void)animateNewTerminal:(TTerminalView *)terminal {
    if(!self.config.tabAnimations)return;CAMediaTimingFunction *ease=[CAMediaTimingFunction functionWithControlPoints:0.16f :1.0f :0.3f :1.0f];
...
- (void)selectTabNumber:(NSInteger)number {
    ...
    TTerminalView *next=_terminals[(NSUInteger)index];self.terminal=next;_cwd=[next workingDirectory];self.extensions.activeTerminal=next;if([self usesTiledLayout]){[self updateTabSelectionAnimated:YES];[self focusTerminal:next];[next setNeedsDisplay:YES];return;}[self layoutTabs];[self updateTabSelectionAnimated:self.config.tabAnimations];[self focusTerminal:next];[next setNeedsDisplay:YES];
}
- (void)rebuildTabs {
...
- (void)layoutTabs {
    ...
    for(NSUInteger i=0;i<_terminals.count;i++){
        ...
        BOOL animateTerminal=tile&&animate&&(!_enteringTerminal||terminal==_enteringTerminal);
