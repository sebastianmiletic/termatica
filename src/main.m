...
static int TRunTerminalSelfTest(void) {
    ...
    fprintf(stdout,"terminal-self-test ok history=%lu rows=%lu columns=%lu keyboard=kitty+legacy mouse=native-selection+buttons+drag+motion+tui-wheel selection=document-anchored wheel=normal+hyprland tabs=surface-stable layout=independent-mixed-splits+arbitrary-slot-swap motion=translation-only rail=bounded\n",(unsigned long)[initial[@"history"] unsignedIntegerValue],(unsigned long)[initial[@"rows"] unsignedIntegerValue],(unsigned long)[initial[@"columns"] unsignedIntegerValue]);
    return 0;
}
