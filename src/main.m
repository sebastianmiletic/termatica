...
- (TRenderSnapshot *)renderSnapshot {
    @synchronized(self){
        ...
        BOOL cursorActive=self.activeTerminal,pinnedInlineCursor=_inlineViewportMode&&_inlineViewportTop>0&&_cursorY>=_inlineViewportTop,cursorShown=_cursorVisible&&(_historyOffset==0||pinnedInlineCursor)&&(_cursorBlink?_cursorBlinkVisible:YES)&&cursorActive;
        NSFont *fallback=[NSFont monospacedSystemFontOfSize:self.config.fontSize weight:NSFontWeightRegular];
        BOOL cursorFocused=cursorActive&&self.window.isKeyWindow&&self.window.firstResponder==self;
...
static int TRunTerminalSelfTest(void) {
    ...
    for(TTerminalView *focusTarget in @[leftRoot,rightRoot,quarter,leftRoot,quarter]){
        ...
        if(tiles.terminal!=focusTarget||tiles.window.firstResponder!=focusTarget||activeCount!=1||visibleCursorCount!=1||displayCursorCount!=1)return 68;
    }
...
static int TRunRendererReliabilitySelfTest(void) {
    ...
    for(NSString *requested in requestedRenderers){
        ...
        for(NSUInteger cycle=0;cycle<120;cycle++){
            ...
            if(controller.terminal!=target||controller.window.firstResponder!=target||active!=1||cursors!=1||displayCursors!=1){[controller.window close];fprintf(stderr,"renderer-reliability-self-test failed stage=cursor requested=%s cycle=%lu active=%lu snapshots=%lu displayed=%lu\n",requested.UTF8String,(unsigned long)cycle,(unsigned long)active,(unsigned long)cursors,(unsigned long)displayCursors);return 111;}
        }
