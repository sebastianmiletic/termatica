...
static int TRunTerminalSelfTest(void) {
    ...
    CABasicAnimation *quarterMotion=(CABasicAnimation *)[quarter.layer animationForKey:@"termatica.hypr.snap"],*rightMotion=(CABasicAnimation *)[rightRoot.layer animationForKey:@"termatica.hypr.snap"];if(!quarterMotion&&!rightMotion)return 56;
    for(CABasicAnimation *motion in @[quarterMotion?:rightMotion,rightMotion?:quarterMotion]){CATransform3D from=[motion.fromValue CATransform3DValue];if(fabs(from.m11-1)>0.0001||fabs(from.m22-1)>0.0001||fabs(from.m33-1)>0.0001)return 175;}
