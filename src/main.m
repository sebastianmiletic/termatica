...
- (instancetype)initWithConfig:(TConfig *)config extensions:(TExtensionHost *)extensions directory:(NSString *)directory command:(NSString *)command {
    if((self=[super initWithWindow:nil])){_config=config;_extensions=extensions;_terminals=[NSMutableArray array];_tabButtons=[NSMutableArray array];
        TTerminalView *terminal=[self newTerminalDeferred:YES];terminal.launchDirectory=directory;[_terminals addObject:terminal];[terminal startShell];if(command.length)[self sendAutomationCommand:command toTerminal:terminal];self.terminal=terminal;self.extensions.activeTerminal=terminal;
...
- (NSArray<NSValue *> *)hyprlandFrames {
    ...
    NSMutableArray<TTerminalView *> *roots=[NSMutableArray array];if(self.config.hyprlandLayout){for(TTerminalView *terminal in _terminals)if(!terminal.splitAnchor||![_terminals containsObject:terminal.splitAnchor])[roots addObject:terminal];}else{TTerminalView *root=[self rootTerminalForTerminal:self.terminal];if(root)[roots addObject:root];}
