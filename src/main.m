...
- (void)sanitizeWideCellsInRow:(TCell *)row __attribute__((objc_direct)) {
    if(!row)return;NSUInteger physical=[self physicalRowForRow:_cursorY];TCell blank=[self blankCell];
    for(NSUInteger x=0;x<_cols;x++){
        ...
        row[x]=blank;if(_underlineStyles)_underlineStyles[physical*_cols+x]=0;if(_linksByCell.count)[_linksByCell removeObjectForKey:@(physical*_cols+x)];
    }
}
...
- (void)addVerticalTab {[self addSplitWithVerticalDivider:NO];}
