...
@implementation TWindowController { NSString *_cwd; NSView *_root; NSVisualEffectView *_effect; TTabRailView *_tabRail; TTabEdgeView *_tabEdge; NSMutableArray<TTabButton *> *_tabButtons; BOOL _animateTabLayout; BOOL _hyprlandApplied,_adjustingScreenFrame; NSRect _preHyprlandFrame,_preHyprlandVisibleFrame,_lastScreenVisibleFrame; TTerminalView *_draggingTerminal; TTerminalView *_dragSwapTarget; NSPoint _dragOffset; TTerminalView *_enteringTerminal; NSTimer *_tabHideTimer; NSTrackingArea *_tabHoverArea; NSRect _tabRailTargetFrame; BOOL _tabRailVisible; BOOL _mouseInTabArea; BOOL _revealRailAfterLayout; }
...
- (void)assignFrame:(NSRect)frame toTerminal:(TTerminalView *)terminal frames:(NSMutableArray<NSValue *> *)frames {
    ...
    NSRect own=frame;
    for(TTerminalView *child in children){BOOL vertical=child.verticalSplit;CGFloat extent=vertical?NSWidth(own):NSHeight(own),gap=MIN(self.config.tileGap,MAX(0,extent-2)),half=MAX(1,floor((extent-gap)/2));NSRect childFrame=own;if(vertical){own.size.width=half;childFrame.origin.x=NSMaxX(own)+gap;childFrame.size.width=MAX(1,extent-gap-half);}else{own.origin.y=NSMaxY(own)-half;own.size.height=half;childFrame.size.height=MAX(1,extent-gap-half);}[self assignFrame:childFrame toTerminal:child frames:frames];}
...
- (NSArray<NSValue *> *)hyprlandFrames {
    ...
    if(self.config.hyprlandLayout){for(TTerminalView *item in _terminals)if([self rootTerminalForTerminal:item]==item)[roots addObject:item];}
...
- (void)closeTab {[self cancelTileAnimation];TTerminalView *closing=self.terminal;if(!closing)return;if(_terminals.count<=1){[_terminals removeAllObjects];self.terminal=nil;self.extensions.activeTerminal=nil;[closing stopShellTerminating:YES];[closing removeFromSuperview];[self.window close];return;}BOOL wasTiled=[self usesTiledLayout],animate=_terminals.count<=9;NSUInteger index=[_terminals indexOfObject:closing];NSMutableArray<TTerminalView *> *children=[NSMutableArray array];for(TTerminalView *item in _terminals)if(item.splitAnchor==closing)[children addObject:item];TTerminalView *parent=closing.splitAnchor,*promoted=children.firstObject;if(!parent&&promoted){promoted.splitAnchor=nil;for(NSUInteger i=1;i<children.count;i++)children[i].splitAnchor=promoted;}else for(TTerminalView *child in children)child.splitAnchor=parent;[_terminals removeObjectAtIndex:index];[closing stopShellTerminating:YES];[closing removeFromSuperview];self.terminal=promoted?:_terminals[MIN(index,_terminals.count-1)];self.extensions.activeTerminal=self.terminal;_animateTabLayout=animate;if(wasTiled!=[self usesTiledLayout])[self applyAppearance];else{[self rebuildTabs];[self layoutTabs];}[self focusTerminal:self.terminal];}
