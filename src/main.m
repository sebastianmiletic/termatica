...
static BOOL TCommitUnifiedConfig(TConfig *config,NSMutableDictionary *dictionary,NSString **message) {
    TNormalizeConfigToggles(dictionary);NSError *error=nil;if(!TWriteJSONDictionary(dictionary,config.path,&error)){if(message)*message=[NSString stringWithFormat:@"FAILED: %@",error.localizedDescription?:@"could not write config"];return NO;}[config ensureEditableFile];[config reload];TPostCLICommand(@"reloadIfChanged");if(message)*message=@"SAVED + RELOADED";return YES;
}
...
- (void)layoutTabs {
    ...
    for(NSUInteger i=0;i<_terminals.count;i++){
        ...
        terminal.hidden=!shown;terminal.leadingOverlayInset=0;terminal.topContentInset=self.config.topBar?0:6;terminal.activeTerminal=shown&&terminal==self.terminal;terminal.tiledRendering=shown&&tile;
        ...
        terminal.wantsLayer=(shown&&tile)||animateTerminal||[terminal usesMetalRenderer];terminal.layer.cornerRadius=shown&&tile?self.config->tileCornerRadius:0;terminal.layer.masksToBounds=shown&&tile;[terminal.layer removeAnimationForKey:@"termatica.hypr.snap"];if(stableTiledCompositing)terminal.layer.mask=nil;
...
static int TRunConfigWatcherSelfTest(void) {
    ...
    NSUInteger expectedApplications=1;
    for(NSNumber *size in @[@18,@23]){
        ...
        expectedApplications++;NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:2];
        ...
        NSDate *settled=[NSDate dateWithTimeIntervalSinceNow:0.4];while(settled.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:settled];NSDictionary *diagnostics=[delegate configReloadDiagnostics];
        if(config.fontSize!=size.doubleValue||[diagnostics[@"applications"] unsignedIntegerValue]!=expectedApplications){fprintf(stderr,"config-watcher-self-test failed stage=reload expected=%.0f actual=%.0f diagnostics=%s\n",size.doubleValue,config.fontSize,diagnostics.description.UTF8String);return 181;}
    }
    ...
    NSDictionary *diagnostics=[delegate configReloadDiagnostics];fprintf(stdout,"config-watcher-self-test ok repeated-atomic-edits=2 invalid-preserved=yes last-good-recovered=yes theme-inheritance=yes selected-file-rearmed=yes applied=%lu unchanged-skipped=%lu\n",(unsigned long)[diagnostics[@"applications"] unsignedIntegerValue],(unsigned long)[diagnostics[@"skips"] unsignedIntegerValue]);return 0;
}
