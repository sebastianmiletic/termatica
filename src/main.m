...
static NSString *TCaptureConfigShortcut(void) {
    fputs("\n\033[38;2;255;224;131m  READING... press the desired shortcut   ESC cancel\033[0m",stdout);fputs("\033]777;term-keybinding-capture\a",stdout);fflush(stdout);NSMutableData *line=[NSMutableData data];while(!TMenuInterrupted&&line.length<256){unsigned char byte=0;if(read(STDIN_FILENO,&byte,1)!=1)continue;if(byte=='\n'||byte=='\r')break;[line appendBytes:&byte length:1];}fputs("\033]777;term-keybinding-cancel\a",stdout);fflush(stdout);NSString *response=[[NSString alloc]initWithData:line encoding:NSUTF8StringEncoding];NSString *prefix=@"TERMATICABIND:";if(![response hasPrefix:prefix])return nil;NSString *shortcut=[response substringFromIndex:prefix.length];return shortcut.length?shortcut:nil;
}
...
@implementation TTerminalView {
    ...
    BOOL _keybindingCaptureRequested;
...
- (void)finishOSC:(NSString *)osc __attribute__((objc_direct)) {
    ...
    if([osc isEqual:@"777;term-keybinding-capture"]){_keybindingCaptureRequested=YES;return;}
    if([osc isEqual:@"777;term-keybinding-cancel"]){_keybindingCaptureRequested=NO;return;}
...
- (BOOL)captureKeybindingEvent:(NSEvent *)event {
    if(!_keybindingCaptureRequested)return NO;_keybindingCaptureRequested=NO;NSString *shortcut=TShortcutForEvent(event);if([shortcut isEqual:@"escape"])shortcut=@"";[self sendString:[NSString stringWithFormat:@"TERMATICABIND:%@\n",shortcut?:@""]];return YES;
}
...
- (BOOL)captureKeybindingEvent:(NSEvent *)event {return self.terminal?[self.terminal captureKeybindingEvent:event]:NO;}
