...
static NSString *TRuntimeArchitecture(void) {
...
static BOOL TProcessIsTranslated(void) {int translated=0;size_t size=sizeof(translated);return sysctlbyname("sysctl.proc_translated",&translated,&size,NULL,0)==0&&translated==1;}
static NSDictionary *TFieldQualification(NSDictionary *evidence) {
    NSString *architecture=evidence[@"processArchitecture"]?:@"unknown";BOOL translated=[evidence[@"translated"] boolValue],appleSilicon=[architecture isEqual:@"arm64"]||translated,intel=[architecture isEqual:@"x86_64"]&&!translated;
    NSDictionary *gates=@{
      @"metalSessionHealthy":@([evidence[@"metalPanes"] unsignedIntegerValue]>0&&[evidence[@"quarantinedPanes"] unsignedIntegerValue]==0),
      @"multiDisplayObserved":@([evidence[@"maximumDisplayCount"] unsignedIntegerValue]>=2),
      @"displayHotPlugObserved":@([evidence[@"displayCountChanges"] unsignedIntegerValue]>0),
      @"backingScaleChangeObserved":@([evidence[@"observedBackingScaleChanges"] unsignedIntegerValue]>0),
      @"screenChangeObserved":@([evidence[@"observedScreenChanges"] unsignedIntegerValue]>0),
      @"wakeRecoveryObserved":@([evidence[@"wakeEvents"] unsignedIntegerValue]>0||[evidence[@"screenWakeEvents"] unsignedIntegerValue]>0),
      @"multipleRefreshRatesObserved":@([evidence[@"observedRefreshRateCount"] unsignedIntegerValue]>=2),
      @"appleSiliconNativeObserved":@(appleSilicon&&!translated),
      @"intelNativeObserved":@(intel),
      @"physicalVisualInspectionConfirmed":@NO
    };
    NSMutableArray *blockers=[NSMutableArray array];for(NSString *key in gates)if(![gates[key] boolValue])[blockers addObject:key];[blockers sortUsingSelector:@selector(compare:)];
    return @{@"scope":@"current-process-session",@"evidenceSource":evidence[@"evidenceSource"]?:@"observed-runtime-events",@"syntheticEvidenceAccepted":@NO,@"defaultChangeEligible":@(blockers.count==0),@"status":blockers.count?@"open":@"eligible",@"gates":gates,@"blockers":blockers};
}
#if TERMATICA_BENCHMARKS
static NSData *TCanonicalJSON(id object) {if(!object)return nil;return [NSJSONSerialization isValidJSONObject:object]?[NSJSONSerialization dataWithJSONObject:object options:NSJSONWritingSortedKeys error:nil]:nil;}
static NSString *TSHA256Hex(NSData *data) {if(!data)return nil;unsigned char digest[CC_SHA256_DIGEST_LENGTH];CC_SHA256(data.bytes,(CC_LONG)data.length,digest);NSMutableString *hex=[NSMutableString stringWithCapacity:CC_SHA256_DIGEST_LENGTH*2];for(NSUInteger index=0;index<CC_SHA256_DIGEST_LENGTH;index++)[hex appendFormat:@"%02x",digest[index]];return hex;}
static NSDictionary *TFieldCampaignPayload(NSDictionary *report) {
    NSDictionary *raw=[report[@"fieldEvidence"] isKindOfClass:NSDictionary.class]?report[@"fieldEvidence"]:@{};NSArray *keys=@[@"evidenceSource",@"fieldSessionIdentifier",@"sessionSeconds",@"processArchitecture",@"translated",@"minimumDisplayCount",@"maximumDisplayCount",@"observedDisplayCountCount",@"observedScaleCount",@"observedRefreshRateCount",@"displayParameterEvents",@"displayCountChanges",@"wakeEvents",@"screenWakeEvents",@"screenTransitionEvents",@"backingScaleEvents",@"observedScreenChanges",@"observedBackingScaleChanges",@"metalPanes",@"quarantinedPanes"];NSMutableDictionary *evidence=[NSMutableDictionary dictionary];for(NSString *key in keys)if(raw[key])evidence[key]=raw[key];NSDictionary *app=[report[@"app"] isKindOfClass:NSDictionary.class]?report[@"app"]:@{};
    return @{@"campaignSchemaVersion":@1,@"reportSchemaVersion":report[@"schemaVersion"]?:@0,@"generatedAt":report[@"generatedAt"]?:@"unknown",@"app":@{@"version":app[@"version"]?:@"unknown",@"build":app[@"build"]?:@"unknown"},@"fieldEvidence":evidence,@"fieldQualification":TFieldQualification(evidence),@"privacy":@{@"terminalContentIncluded":@NO,@"workingDirectoriesIncluded":@NO,@"configPathIncluded":@NO,@"displayNamesIncluded":@NO,@"machineIdentifierIncluded":@NO,@"ephemeralSessionIdentifierIncluded":@YES}};
}
static NSDictionary *TFieldCampaignEnvelope(NSDictionary *report) {NSDictionary *payload=TFieldCampaignPayload(report);NSString *digest=TSHA256Hex(TCanonicalJSON(payload));return @{@"format":@"termaticafield-campaign-report",@"integrity":@"sha256-canonical-json",@"authenticity":@"not-asserted",@"payload":payload,@"sha256":digest?:@""};}
static NSDictionary *TValidateFieldCampaignEnvelope(NSDictionary *envelope) {
    if(![envelope isKindOfClass:NSDictionary.class]||![envelope[@"format"] isEqual:@"termaticafield-campaign-report"]||![envelope[@"integrity"] isEqual:@"sha256-canonical-json"]||![envelope[@"authenticity"] isEqual:@"not-asserted"])return @{@"ok":@NO,@"error":@"invalid-envelope"};NSDictionary *payload=[envelope[@"payload"] isKindOfClass:NSDictionary.class]?envelope[@"payload"]:nil;NSString *expected=[envelope[@"sha256"] isKindOfClass:NSString.class]?envelope[@"sha256"]:@"",*actual=TSHA256Hex(TCanonicalJSON(payload));if(!payload||expected.length!=64||![expected isEqual:actual])return @{@"ok":@NO,@"error":@"digest-mismatch"};if([payload[@"campaignSchemaVersion"] unsignedIntegerValue]!=1||[payload[@"reportSchemaVersion"] unsignedIntegerValue]!=4)return @{@"ok":@NO,@"error":@"unsupported-schema"};NSDictionary *app=[payload[@"app"] isKindOfClass:NSDictionary.class]?payload[@"app"]:nil,*evidence=[payload[@"fieldEvidence"] isKindOfClass:NSDictionary.class]?payload[@"fieldEvidence"]:nil,*privacy=[payload[@"privacy"] isKindOfClass:NSDictionary.class]?payload[@"privacy"]:nil;if(!app||!evidence||!privacy||![app[@"version"] isKindOfClass:NSString.class]||![app[@"build"] isKindOfClass:NSString.class])return @{@"ok":@NO,@"error":@"invalid-payload"};if(![evidence[@"evidenceSource"] isEqual:@"observed-runtime-events"])return @{@"ok":@NO,@"error":@"non-observed-source"};NSString *session=[evidence[@"fieldSessionIdentifier"] isKindOfClass:NSString.class]?evidence[@"fieldSessionIdentifier"]:@"",*architecture=[evidence[@"processArchitecture"] isKindOfClass:NSString.class]?evidence[@"processArchitecture"]:@"";if(session.length<8||session.length>64)return @{@"ok":@NO,@"error":@"invalid-session-identifier"};if(![@[@"arm64",@"x86_64"] containsObject:architecture]||![evidence[@"translated"] isKindOfClass:NSNumber.class])return @{@"ok":@NO,@"error":@"invalid-architecture"};for(NSString *key in @[@"sessionSeconds",@"minimumDisplayCount",@"maximumDisplayCount",@"observedDisplayCountCount",@"observedScaleCount",@"observedRefreshRateCount",@"displayParameterEvents",@"displayCountChanges",@"wakeEvents",@"screenWakeEvents",@"screenTransitionEvents",@"backingScaleEvents",@"observedScreenChanges",@"observedBackingScaleChanges",@"metalPanes",@"quarantinedPanes"])if(![evidence[key] isKindOfClass:NSNumber.class]||[evidence[key] doubleValue]<0)return @{@"ok":@NO,@"error":@"invalid-evidence"};for(NSString *key in @[@"terminalContentIncluded",@"workingDirectoriesIncluded",@"configPathIncluded",@"displayNamesIncluded",@"machineIdentifierIncluded"])if(![privacy[key] isKindOfClass:NSNumber.class]||[privacy[key] boolValue])return @{@"ok":@NO,@"error":@"privacy-violation"};return @{@"ok":@YES,@"digest":actual,@"sessionIdentifier":session,@"payload":payload};
}
static NSDictionary *TAggregateFieldCampaignReports(NSArray<NSDictionary *> *envelopes) {
    NSMutableArray *accepted=[NSMutableArray array],*rejected=[NSMutableArray array];NSMutableSet *digests=[NSMutableSet set],*sessions=[NSMutableSet set];NSUInteger duplicates=0;for(NSUInteger index=0;index<envelopes.count;index++){NSDictionary *validated=TValidateFieldCampaignEnvelope(envelopes[index]);if(![validated[@"ok"] boolValue]){[rejected addObject:@{@"index":@(index),@"error":validated[@"error"]?:@"invalid"}];continue;}NSString *digest=validated[@"digest"],*session=validated[@"sessionIdentifier"];if([digests containsObject:digest]||[sessions containsObject:session]){duplicates++;continue;}[digests addObject:digest];[sessions addObject:session];[accepted addObject:validated[@"payload"]];}
    BOOL allHealthy=accepted.count>0,multiDisplay=NO,hotPlug=NO,scale=NO,screen=NO,wake=NO,refresh=NO,apple=NO,intel=NO;NSMutableSet *versions=[NSMutableSet set],*architectures=[NSMutableSet set];for(NSDictionary *payload in accepted){NSDictionary *evidence=payload[@"fieldEvidence"],*qualification=TFieldQualification(evidence),*gates=qualification[@"gates"];allHealthy&=[gates[@"metalSessionHealthy"] boolValue];multiDisplay|=[gates[@"multiDisplayObserved"] boolValue];hotPlug|=[gates[@"displayHotPlugObserved"] boolValue];scale|=[gates[@"backingScaleChangeObserved"] boolValue];screen|=[gates[@"screenChangeObserved"] boolValue];wake|=[gates[@"wakeRecoveryObserved"] boolValue];refresh|=[gates[@"multipleRefreshRatesObserved"] boolValue];apple|=[gates[@"appleSiliconNativeObserved"] boolValue];intel|=[gates[@"intelNativeObserved"] boolValue];[versions addObject:payload[@"app"][@"version"]?:@"unknown"];[architectures addObject:evidence[@"processArchitecture"]?:@"unknown"];}NSDictionary *gates=@{@"allAcceptedMetalSessionsHealthy":@(allHealthy),@"multiDisplayObserved":@(multiDisplay),@"displayHotPlugObserved":@(hotPlug),@"backingScaleChangeObserved":@(scale),@"screenChangeObserved":@(screen),@"wakeRecoveryObserved":@(wake),@"multipleRefreshRatesObserved":@(refresh),@"appleSiliconNativeObserved":@(apple),@"intelNativeObserved":@(intel),@"physicalVisualInspectionConfirmed":@NO};NSMutableArray *blockers=[NSMutableArray array];for(NSString *key in gates)if(![gates[key] boolValue])[blockers addObject:key];[blockers sortUsingSelector:@selector(compare:)];return @{@"campaignSchemaVersion":@1,@"scope":@"validated-exported-reports",@"integrity":@"sha256-canonical-json",@"authenticity":@"not-asserted",@"inputReports":@(envelopes.count),@"acceptedReports":@(accepted.count),@"duplicateReports":@(duplicates),@"rejectedReports":rejected,@"appVersions":[versions.allObjects sortedArrayUsingSelector:@selector(compare:)],@"architectures":[architectures.allObjects sortedArrayUsingSelector:@selector(compare:)],@"gates":gates,@"blockers":blockers,@"fieldEvidenceComplete":@(blockers.count==1&&[blockers.firstObject isEqual:@"physicalVisualInspectionConfirmed"]),@"defaultRendererChangeEligible":@NO,@"defaultRenderer":@"appkit",@"operatorReviewRequired":@YES};
}
#endif
...
static NSString *TConfigProfilesPath(void) {return [TConfigDirectoryPath() stringByAppendingPathComponent:@"configs"];}
static NSString *TConfigSelectionPath(void) {return [TConfigDirectoryPath() stringByAppendingPathComponent:@"current"];}
static NSString *TLegacyConfigPath(void) {return [TConfigDirectoryPath() stringByAppendingPathComponent:@"config.json"];}
static NSString *TConfigFilename(NSString *input) {
    if(![input isKindOfClass:NSString.class])return nil;NSString *name=[input stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];if([name.pathExtension.lowercaseString isEqual:@"json"])name=name.stringByDeletingPathExtension;if(!TSafeIdentifier(name)||[name isEqual:@"session"]||[name isEqual:@"config"])return nil;return [name stringByAppendingPathExtension:@"json"];
}
static NSString *TCanonicalConfigPath(NSString *filename) {NSString *valid=TConfigFilename(filename);return valid?[TConfigProfilesPath() stringByAppendingPathComponent:valid]:nil;}
static NSString *TReadSelectedConfigFilename(void) {
    NSString *value=[NSString stringWithContentsOfFile:TConfigSelectionPath() encoding:NSUTF8StringEncoding error:nil];return TConfigFilename(value);
}
static NSString *TActiveConfigFingerprint(void) {
    NSString *selected=TReadSelectedConfigFilename()?:@"";NSData *data=[NSData dataWithContentsOfFile:TCanonicalConfigPath(selected)]?:NSData.data;unsigned char digest[CC_SHA256_DIGEST_LENGTH];CC_SHA256(data.bytes,(CC_LONG)data.length,digest);NSMutableString *hex=[NSMutableString stringWithCapacity:CC_SHA256_DIGEST_LENGTH*2];for(NSUInteger index=0;index<CC_SHA256_DIGEST_LENGTH;index++)[hex appendFormat:@"%02x",digest[index]];return [NSString stringWithFormat:@"%@:%@",selected,hex];
}
static BOOL TWriteSelectedConfigFilename(NSString *filename,NSError **error) {
    NSString *valid=TConfigFilename(filename);if(!valid){if(error)*error=[NSError errorWithDomain:@"TermaticaConfig" code:1 userInfo:@{NSLocalizedDescriptionKey:@"invalid config filename"}];return NO;}NSData *data=[[valid stringByAppendingString:@"\n"] dataUsingEncoding:NSUTF8StringEncoding];BOOL ok=[data writeToFile:TConfigSelectionPath() options:NSDataWritingAtomic error:error];if(ok)[NSFileManager.defaultManager setAttributes:@{NSFilePosixPermissions:@0600} ofItemAtPath:TConfigSelectionPath() error:nil];return ok;
}
static NSDictionary *TReadJSONDictionaryAtPath(NSString *path) {
    NSData *data=[NSData dataWithContentsOfFile:path];id value=data?[NSJSONSerialization JSONObjectWithData:data options:0 error:nil]:nil;return [value isKindOfClass:NSDictionary.class]?value:nil;
}
static BOOL TWriteJSONDictionary(NSDictionary *dictionary,NSString *path,NSError **error);
...
static id TResolveThemeValue(id configured,id themed) {
    return [configured isKindOfClass:NSString.class]&&[configured isEqual:@"theme"]?themed:(configured?:themed);
}
...
- (instancetype)init {
    if ((self = [super init])) {
        ...
        _path = TCanonicalConfigPath(@"default.json");
...
- (NSDictionary *)defaults {
    static NSDictionary *base;static dispatch_once_t once;dispatch_once(&once,^{
        NSString *json=@"{\"shell\":\"/bin/zsh\",\"shellArguments\":[\"-l\"],\"fontName\":\"SF Mono\",\"fontFeatures\":[],\"fontSize\":11,\"padding\":12,\"scrollback\":60000,\"theme\":\"ghost-glass\",\"themeOptions\":[\"ghost-glass\",\"green-screen\",\"terminal-default\",\"amber-crt\"],\"textColorMode\":\"ansi\","
        "\"plugins\":{\"hello\":\"off\",\"pi-bridge\":\"off\",\"editor-deck\":\"off\",\"vim-control\":\"off\",\"neovim-control\":\"off\",\"emacs-control\":\"off\",\"nano-control\":\"off\",\"micro-control\":\"off\",\"helix-control\":\"off\",\"hidden-path\":\"on\",\"hyprland-layout\":\"on\",\"unicode-rendering\":\"on\",\"osc-integration\":\"on\",\"borderless-window\":\"on\"},"
        "\"appearance\":{\"backgroundOpacity\":\"theme\",\"windowOpacity\":0.90000000000000024,\"blur\":\"on\",\"blurMaterial\":\"hud\",\"glow\":\"theme\",\"scanlines\":\"theme\",\"vignette\":\"theme\",\"cursorStyle\":\"theme\",\"renderer\":\"appkit\"},"
        ...
        "\"tabs\":{\"railWidth\":34,\"animations\":\"on\",\"animationSpeed\":1.35,\"autoHide\":\"on\",\"hideDelay\":5,\"tileGap\":10,\"screenInset\":18,\"hyprlandBlur\":\"theme\",\"railMargin\":8,\"railCornerRadius\":11,\"buttonCornerRadius\":8,\"buttonFontSize\":11,\"buttonInset\":4,\"minimumHeight\":20,\"maximumHeight\":28,\"selectedOpacity\":0.22,\"hoverOpacity\":0.68,\"railOpacity\":0.96,\"railBlurOpacity\":0.26,\"edgeOpacity\":0.58,\"collapsedPeek\":7},"
        ...
        "\"keybindings\":{\"openConfig\":\"cmd+,\",\"newWindow\":\"cmd+n\",\"newTab\":\"cmd+t\",\"closeTab\":\"cmd+w\",\"clearTerminal\":\"cmd+k\",\"searchScrollback\":\"cmd+shift+f\",\"splitHorizontal\":\"cmd+d\",\"splitVertical\":\"cmd+shift+d\",\"nextSplit\":\"cmd+]\",\"previousSplit\":\"cmd+[\",\"previousPrompt\":\"cmd+shift+p\",\"nextPrompt\":\"cmd+option+p\",\"reload\":\"cmd+r\",\"copy\":\"cmd+c\",\"paste\":\"cmd+v\",\"selectAll\":\"cmd+a\",\"zoomIn\":\"cmd+plus\",\"zoomOut\":\"cmd+-\",\"zoomReset\":\"cmd+0\"}}";
...
- (void)reload {
    ...
    if (data) {
        NSError *parseError=nil;id parsed = [NSJSONSerialization JSONObjectWithData:data options:0 error:&parseError];
        if ([parsed isKindOfClass:NSDictionary.class]) {
            user = parsed;
            NSMutableDictionary *merged = [d mutableCopy];
            [merged addEntriesFromDictionary:parsed];
            d = merged;
        } else if(_hasLoadedConfiguration) {
            TLog(@"config reload rejected invalid JSON at %@: %@",self.path,parseError.localizedDescription?:@"root must be a JSON object");
            return;
        }
    }
    ...
    NSDictionary *userTabs=[user[@"tabs"] isKindOfClass:NSDictionary.class]?user[@"tabs"]:@{};id configuredHyprlandBlur=userTabs[@"hyprlandBlur"];if((!configuredHyprlandBlur||[configuredHyprlandBlur isEqual:@"theme"])&&appearance[@"hyprlandBlur"])self.hyprlandBlur=TConfigBool(appearance[@"hyprlandBlur"],NO);
    ...
    self.background = [THexColor(TResolveThemeValue(colorOverrides[@"background"],theme[@"background"]), THexColor(@"#101216", NSColor.blackColor)) colorWithAlphaComponent:self.backgroundOpacity];
    self.foreground = THexColor(TResolveThemeValue(colorOverrides[@"foreground"],theme[@"foreground"]), THexColor(@"#D8DEE9", NSColor.textColor));
    self.cursor = THexColor(TResolveThemeValue(colorOverrides[@"cursor"],theme[@"cursor"]), THexColor(@"#EEF1F5", NSColor.textColor));
    self.accent = THexColor(TResolveThemeValue(colorOverrides[@"accent"],theme[@"accent"]), self.cursor);
    self.panel=THexColor(TResolveThemeValue(colorOverrides[@"panel"],theme[@"panel"]),THexColor(@"#151820",NSColor.windowBackgroundColor));self.muted=THexColor(TResolveThemeValue(colorOverrides[@"muted"],theme[@"muted"]),THexColor(@"#6B7280",NSColor.secondaryLabelColor));self.selection=THexColor(TResolveThemeValue(colorOverrides[@"selection"],theme[@"selection"]),THexColor(@"#2B3445",self.accent));
...
- (NSMutableDictionary *)normalizedDictionary:(NSDictionary *)source configName:(NSString *)configName {
    ...
    for(NSString *identifier in [NSFileManager.defaultManager contentsOfDirectoryAtPath:extensionRoot error:nil]?:@[]){
        ...
        if(!plugins[identifier])plugins[identifier]=configuredPlugins?@"off":TConfigToggle(![legacyDisabled containsObject:identifier]);
    }
    ...
    [normalized removeObjectForKey:@"configName"];
    TNormalizeConfigToggles(normalized);
    return normalized;
}
- (void)ensureEditableFile {
    NSFileManager *fm=NSFileManager.defaultManager;[fm createDirectoryAtPath:TConfigDirectoryPath() withIntermediateDirectories:YES attributes:nil error:nil];[fm createDirectoryAtPath:TConfigProfilesPath() withIntermediateDirectories:YES attributes:nil error:nil];
    NSString *selected=TReadSelectedConfigFilename(),*legacyPath=TLegacyConfigPath(),*legacyTarget=[fm destinationOfSymbolicLinkAtPath:legacyPath error:nil];BOOL selectionNeedsWrite=!selected.length;NSDictionary *legacy=TReadJSONDictionaryAtPath(legacyPath);BOOL legacyRegular=!legacyTarget&&[fm fileExistsAtPath:legacyPath];
    if(!selected.length){NSString *legacyName=[legacy[@"configName"] isKindOfClass:NSString.class]?legacy[@"configName"]:nil;selected=TConfigFilename(legacyName)?:@"default.json";}
    NSString *selectedPath=TCanonicalConfigPath(selected);BOOL selectedExists=[fm fileExistsAtPath:selectedPath];NSDictionary *selectedDictionary=TReadJSONDictionaryAtPath(selectedPath);
    if(legacyRegular&&legacy){selectedDictionary=legacy;}
    if(!selectedDictionary&&!selectedExists)selectedDictionary=[self defaults];
    NSError *writeError=nil;if(selectedDictionary){NSMutableDictionary *selectedNormalized=[self normalizedDictionary:selectedDictionary configName:nil];if(![selectedNormalized isEqualToDictionary:selectedDictionary]&&!TWriteJSONDictionary(selectedNormalized,selectedPath,&writeError))TLog(@"could not prepare selected config %@: %@",selected,writeError.localizedDescription);}else TLog(@"preserving invalid selected config %@; continuing with the last known-good settings",selectedPath);
...
- (void)writeEditableDictionary:(NSDictionary *)dictionary {
    NSMutableDictionary *normalized=[self normalizedDictionary:dictionary configName:nil];NSData *data=[NSJSONSerialization dataWithJSONObject:normalized options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if([data writeToFile:self.path options:NSDataWritingAtomic error:nil])[NSFileManager.defaultManager setAttributes:@{NSFilePosixPermissions:@0600} ofItemAtPath:self.path error:nil];[self ensureEditableFile];[self reload];
}
...
static NSDictionary *TRequestCLI(NSDictionary *request,NSTimeInterval timeout) {
    ...
    NSDictionary *result=nil;if(TPostCLIRequest(payload)){struct pollfd wait={.fd=fd,.events=POLLIN};int ready=0;CFAbsoluteTime deadline=CFAbsoluteTimeGetCurrent()+MAX(0.02,timeout);do{int remaining=(int)MAX(1,(deadline-CFAbsoluteTimeGetCurrent())*1000);ready=poll(&wait,1,remaining);}while(ready<0&&errno==EINTR&&CFAbsoluteTimeGetCurrent()<deadline);if(ready>0&&(wait.revents&POLLIN)){uint8_t buffer[8192];ssize_t count=recv(fd,buffer,sizeof(buffer),0);if(count>0){id value=[NSJSONSerialization JSONObjectWithData:[NSData dataWithBytes:buffer length:(NSUInteger)count] options:0 error:nil];if([value isKindOfClass:NSDictionary.class])result=value;}}}
...
static NSString *TConfigProfilePath(NSString *name){return TCanonicalConfigPath(name);}
static NSArray<NSString *> *TConfigProfileNames(void){NSMutableArray<NSString *> *names=[NSMutableArray array];for(NSString *file in [NSFileManager.defaultManager contentsOfDirectoryAtPath:TConfigProfileDirectory() error:nil]?:@[]){NSString *valid=TConfigFilename(file);if(valid&&[valid isEqual:file])[names addObject:file];}[names sortUsingSelector:@selector(localizedCaseInsensitiveCompare:)];NSString *current=TReadSelectedConfigFilename();NSUInteger index=current.length?[names indexOfObject:current]:NSNotFound;if(index!=NSNotFound&&index){[names removeObjectAtIndex:index];[names insertObject:current atIndex:0];}return names;}
static BOOL TConfigProfileIsValid(NSString *name){return TReadJSONDictionaryAtPath(TConfigProfilePath(name))!=nil;}
...
static NSString *TActiveConfigName(TConfig *config){[config ensureEditableFile];return config.path.lastPathComponent?:@"default.json";}
static NSString *TSaveConfigNamed(NSString *name,TConfig *config){NSString *file=TConfigFilename(name);if(!file)return @"[ INVALID ] use letters, numbers, dot, dash or underscore";NSString *path=TConfigProfilePath(file);if([NSFileManager.defaultManager fileExistsAtPath:path])return [NSString stringWithFormat:@"[ EXISTS ] %@",file];NSMutableDictionary *profile=[config normalizedDictionary:[config defaults] configName:nil];NSError *error=nil;if(!TWriteJSONDictionary(profile,path,&error))return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not create config"];if(!TWriteSelectedConfigFilename(file,&error)){[NSFileManager.defaultManager removeItemAtPath:path error:nil];return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not select config"];}[config ensureEditableFile];[config reload];TPostCLICommand(@"reloadIfChanged");return [NSString stringWithFormat:@"[ CREATED + CURRENT ] %@",file];}
static NSString *TUseConfigNamed(NSString *name,TConfig *config){NSString *file=TConfigFilename(name);if(!file)return @"[ INVALID ] config filename";NSDictionary *value=TReadJSONDictionaryAtPath(TConfigProfilePath(file));if(!value)return [NSString stringWithFormat:@"[ INVALID OR MISSING ] %@",file];NSMutableDictionary *profile=[config normalizedDictionary:value configName:nil];NSError *error=nil;if(!TWriteJSONDictionary(profile,TConfigProfilePath(file),&error)||!TWriteSelectedConfigFilename(file,&error))return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not select config"];[config ensureEditableFile];[config reload];TPostCLICommand(@"reloadIfChanged");return [NSString stringWithFormat:@"[ CURRENT ] %@",file];}
static NSString *TRenameConfig(NSString *oldName,NSString *newName,TConfig *config){NSString *oldFile=TConfigFilename(oldName),*newFile=TConfigFilename(newName);if(!oldFile||!newFile)return @"[ INVALID ] config filename";NSString *oldPath=TConfigProfilePath(oldFile),*newPath=TConfigProfilePath(newFile);NSFileManager *fm=NSFileManager.defaultManager;if(![fm fileExistsAtPath:oldPath])return [NSString stringWithFormat:@"[ NOT FOUND ] %@",oldFile];if([fm fileExistsAtPath:newPath])return [NSString stringWithFormat:@"[ EXISTS ] %@",newFile];BOOL wasActive=[TActiveConfigName(config) isEqual:oldFile];NSError *error=nil;if(![fm moveItemAtPath:oldPath toPath:newPath error:&error])return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not rename config"];if(wasActive&&!TWriteSelectedConfigFilename(newFile,&error)){[fm moveItemAtPath:newPath toPath:oldPath error:nil];return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not update selection"];}[config ensureEditableFile];[config reload];if(wasActive)TPostCLICommand(@"reloadIfChanged");return [NSString stringWithFormat:@"[ RENAMED ] %@ -> %@",oldFile,newFile];}
static NSString *TDeleteConfig(NSString *name,TConfig *config){NSString *file=TConfigFilename(name);if(!file)return @"[ INVALID ] config filename";NSString *path=TConfigProfilePath(file);NSFileManager *fm=NSFileManager.defaultManager;if(![fm fileExistsAtPath:path])return [NSString stringWithFormat:@"[ NOT FOUND ] %@",file];BOOL wasActive=[TActiveConfigName(config) isEqual:file];NSArray *others=[TConfigProfileNames() filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(NSString *candidate,NSDictionary *bindings){return ![candidate isEqual:file]&&TConfigProfileIsValid(candidate);}]];if(wasActive&&!others.count)return @"[ REQUIRED ] cannot delete the only valid config";NSError *error=nil;NSString *replacement=wasActive?others.firstObject:nil;if(wasActive&&!TWriteSelectedConfigFilename(replacement,&error))return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not select replacement"];if(![fm removeItemAtPath:path error:&error]){if(wasActive)TWriteSelectedConfigFilename(file,nil);return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not delete config"];}[config ensureEditableFile];[config reload];if(wasActive)TPostCLICommand(@"reloadIfChanged");return wasActive?[NSString stringWithFormat:@"[ DELETED ] %@  [ CURRENT ] %@",file,replacement]:[NSString stringWithFormat:@"[ DELETED ] %@",file];}
...
static NSArray<NSDictionary *> *TUnifiedConfigSections(TConfig *config) {
    NSDictionary *active=TReadActiveConfig(config);
    NSSet *integratedPlugins=[NSSet setWithArray:@[@"hyprland-layout",@"unicode-rendering",@"osc-integration",@"borderless-window"]];NSMutableArray *pluginRows=[NSMutableArray array];for(NSString *identifier in [config.pluginStates.allKeys sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)])if(![integratedPlugins containsObject:identifier])[pluginRows addObject:TSetting(identifier.uppercaseString,[@"plugins." stringByAppendingString:identifier],@"bool",nil,nil,nil,nil)];
    NSMutableArray *bindingRows=[NSMutableArray array];for(NSString *identifier in [config.keybindings.allKeys sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)]){if([identifier isEqual:@"newVerticalTab"])continue;NSString *path=[@"keybindings." stringByAppendingString:identifier];[bindingRows addObject:TSetting(identifier,path,@"shortcut",nil,nil,nil,nil)];}
    ...
    return @[
      @{@"title":@"APPEARANCE",@"detail":@"theme, type, colour, transparency and effects",@"rows":@[
        TSetting(@"Theme",@"theme",@"option",themes,nil,nil,nil),TSetting(@"Font",@"fontName",@"option",TMonospacedFontOptions(config.fontName),nil,nil,nil),TSetting(@"Font size",@"fontSize",@"number",nil,@8,@48,@1),
        TSetting(@"Font features",@"fontFeatures",@"option",TOptionsIncludingCurrent(fontFeatures,@[@[],@[@"calt"],@[@"liga"],@[@"calt",@"liga"],@[@"calt",@"liga",@"zero"],@[@"ss01"],@[@"ss02"]]),nil,nil,nil),TSetting(@"Text colour mode",@"textColorMode",@"option",@[@"ansi",@"spectrum"],nil,nil,nil),TSetting(@"Content padding",@"padding",@"number",nil,@0,@40,@1),
        TSetting(@"Background colour",@"colors.background",@"option",TOptionsIncludingCurrent(background,colours),nil,nil,nil),TSetting(@"Foreground colour",@"colors.foreground",@"option",TOptionsIncludingCurrent(foreground,colours),nil,nil,nil),TSetting(@"Cursor colour",@"colors.cursor",@"option",TOptionsIncludingCurrent(cursor,colours),nil,nil,nil),TSetting(@"Accent colour",@"colors.accent",@"option",TOptionsIncludingCurrent(accent,colours),nil,nil,nil),TSetting(@"Panel colour",@"colors.panel",@"option",TOptionsIncludingCurrent(panel,colours),nil,nil,nil),TSetting(@"Muted colour",@"colors.muted",@"option",TOptionsIncludingCurrent(muted,colours),nil,nil,nil),TSetting(@"Selection colour",@"colors.selection",@"option",TOptionsIncludingCurrent(selection,colours),nil,nil,nil),TSetting(@"ANSI palette",@"colors.palette",@"option",TOptionsIncludingCurrent(palette,@[@"theme",TStandardPaletteHex()]),nil,nil,nil),
        TSetting(@"Background opacity",@"appearance.backgroundOpacity",@"number-theme",nil,@0.08,@1,@0.05),TSetting(@"Window opacity",@"appearance.windowOpacity",@"number-theme",nil,@0.20,@1,@0.05),TSetting(@"Blur",@"appearance.blur",@"bool-theme",nil,nil,nil,nil),TSetting(@"Blur material",@"appearance.blurMaterial",@"option-theme",@[@"hud",@"popover",@"sidebar",@"menu",@"under-window"],nil,nil,nil),TSetting(@"Glow",@"appearance.glow",@"number-theme",nil,@0,@1,@0.05),TSetting(@"Scanlines",@"appearance.scanlines",@"number-theme",nil,@0,@1,@0.05),TSetting(@"Scanline spacing",@"terminalUI.scanlineSpacing",@"number",nil,@1,@16,@1),TSetting(@"Scanline thickness",@"terminalUI.scanlineThickness",@"number",nil,@1,@16,@1),TSetting(@"Vignette",@"appearance.vignette",@"number-theme",nil,@0,@1,@0.05),TSetting(@"Vignette layers",@"terminalUI.vignetteLayers",@"number",nil,@1,@24,@1),TSetting(@"Cursor style",@"appearance.cursorStyle",@"option-theme",@[@"block",@"bar",@"underline"],nil,nil,nil),TSetting(@"Cursor thickness",@"terminalUI.cursorThickness",@"number",nil,@1,@8,@1),TSetting(@"Block cursor opacity",@"terminalUI.cursorBlockOpacity",@"number",nil,@0.05,@1,@0.05),TSetting(@"Inactive cursor opacity",@"terminalUI.cursorInactiveOpacity",@"number",nil,@0.02,@1,@0.05)
      ]},
      @{@"title":@"PERFORMANCE",@"detail":@"renderer, memory and compatibility work",@"rows":@[
        TSetting(@"Renderer",@"appearance.renderer",@"option",@[@"appkit",@"metal"],nil,nil,nil),TSetting(@"Scrollback lines",@"scrollback",@"number",nil,@100,@100000,@500),TSetting(@"Unicode rendering",@"plugins.unicode-rendering",@"bool",nil,nil,nil,nil),TSetting(@"OSC integration",@"plugins.osc-integration",@"bool",nil,nil,nil,nil)
      ]},
      @{@"title":@"TABS & TILING",@"detail":@"tabs, rail and Hyprland layout",@"rows":@[
        TSetting(@"Hyprland layout",@"plugins.hyprland-layout",@"bool",nil,nil,nil,nil),TSetting(@"Rail width",@"tabs.railWidth",@"number",nil,@20,@120,@1),TSetting(@"Auto hide rail",@"tabs.autoHide",@"bool",nil,nil,nil,nil),TSetting(@"Hide delay",@"tabs.hideDelay",@"number",nil,@1,@30,@1),TSetting(@"Tile gap",@"tabs.tileGap",@"number",nil,@0,@48,@1),TSetting(@"Screen inset",@"tabs.screenInset",@"number",nil,@0,@120,@1),TSetting(@"Hyprland blur",@"tabs.hyprlandBlur",@"bool-theme",nil,nil,nil,nil),TSetting(@"Rail margin",@"tabs.railMargin",@"number",nil,@0,@48,@1),TSetting(@"Rail corner radius",@"tabs.railCornerRadius",@"number",nil,@0,@48,@1),TSetting(@"Button corner radius",@"tabs.buttonCornerRadius",@"number",nil,@0,@32,@1),TSetting(@"Button font size",@"tabs.buttonFontSize",@"number",nil,@8,@28,@1),TSetting(@"Button inset",@"tabs.buttonInset",@"number",nil,@0,@16,@1),TSetting(@"Minimum item height",@"tabs.minimumHeight",@"number",nil,@12,@64,@1),TSetting(@"Maximum item height",@"tabs.maximumHeight",@"number",nil,@12,@96,@1),TSetting(@"Selected opacity",@"tabs.selectedOpacity",@"number",nil,@0,@1,@0.05),TSetting(@"Hover opacity",@"tabs.hoverOpacity",@"number",nil,@0,@1,@0.05),TSetting(@"Rail opacity",@"tabs.railOpacity",@"number",nil,@0,@1,@0.05),TSetting(@"Blur rail opacity",@"tabs.railBlurOpacity",@"number",nil,@0,@1,@0.05),TSetting(@"Edge opacity",@"tabs.edgeOpacity",@"number",nil,@0,@1,@0.05),TSetting(@"Collapsed rail peek",@"tabs.collapsedPeek",@"number",nil,@0,@24,@1)
      ]},
      @{@"title":@"WINDOW",@"detail":@"dimensions, corners and native surface",@"rows":@[
        ...
        TSetting(@"Window shadow",@"window.shadow",@"bool",nil,nil,nil,nil),TSetting(@"Borderless window",@"plugins.borderless-window",@"bool",nil,nil,nil,nil)
      ]},
      @{@"title":@"TERMINAL & INPUT",@"detail":@"shell, clipboard, bell, scrolling and search",@"rows":@[
        TSetting(@"Shell",@"shell",@"option",TShellOptions(config.shell),nil,nil,nil),TSetting(@"Shell arguments",@"shellArguments",@"option",TOptionsIncludingCurrent(shellArguments,@[@[@"-l"],@[],@[@"-i"],@[@"-l",@"-i"]]),nil,nil,nil),TSetting(@"Unsafe paste protection",@"system.pasteProtection",@"bool",nil,nil,nil,nil),TSetting(@"Secure password input",@"system.secureKeyboard",@"bool",nil,nil,nil,nil),TSetting(@"Shell integration",@"system.shellIntegration",@"bool",nil,nil,nil,nil),TSetting(@"Clipboard reads",@"system.clipboardRead",@"option",@[@"ask",@"allow",@"deny"],nil,nil,nil),TSetting(@"Clipboard writes",@"system.clipboardWrite",@"option",@[@"ask",@"allow",@"deny"],nil,nil,nil),TSetting(@"Bell style",@"system.bellStyle",@"option",@[@"sound",@"visual",@"both",@"none"],nil,nil,nil),TSetting(@"Scrollbar width",@"terminalUI.scrollbarWidth",@"number",nil,@1,@16,@0.5),
        ...
        TSetting(@"Scrollbar opacity",@"terminalUI.scrollbarOpacity",@"number",nil,@0.05,@1,@0.05),
      ...
      @{@"title":@"MOTION",@"detail":@"animation switches, speed and timing",@"rows":@[
        TSetting(@"Animations",@"tabs.animations",@"bool",nil,nil,nil,nil),TSetting(@"Animation speed",@"tabs.animationSpeed",@"number",nil,@0.25,@4,@0.10),TSetting(@"Launch duration",@"motion.launchDuration",@"number",nil,@0.05,@2,@0.05),TSetting(@"Terminal duration",@"motion.terminalDuration",@"number",nil,@0.05,@2,@0.05),
        TSetting(@"Layout duration",@"motion.layoutDuration",@"number",nil,@0.03,@1,@0.03)
      ]},
      @{@"title":@"EXTENSIONS",@"detail":@"optional installed terminal capabilities",@"rows":pluginRows},
      @{@"title":@"UPDATES",@"detail":@"release checks and source repository",@"rows":@[TSetting(@"Check on launch",@"updates.checkOnLaunch",@"bool",nil,nil,nil,nil),TSetting(@"Update repository",@"updates.repository",@"option",TOptionsIncludingCurrent(config.updateRepository,@[@"sebastianmiletic/termatica"]),nil,nil,nil)]},
...
static BOOL TCommitUnifiedConfig(TConfig *config,NSMutableDictionary *dictionary,NSString **message) {
    TNormalizeConfigToggles(dictionary);NSError *error=nil;if(!TWriteJSONDictionary(dictionary,config.path,&error)){if(message)*message=[NSString stringWithFormat:@"FAILED: %@",error.localizedDescription?:@"could not write config"];return NO;}[config ensureEditableFile];[config reload];struct stat socketInfo={0};BOOL listener=lstat(TCLISocketPath().fileSystemRepresentation,&socketInfo)==0&&S_ISSOCK(socketInfo.st_mode)&&socketInfo.st_uid==getuid();NSDictionary *reply=listener?TRequestCLI(@{@"command":@"reloadIfChanged"},0.35):nil;if(message)*message=reply?@"SAVED + APPLIED":@"SAVED";return YES;
}
...
static NSString *TReadKeybindingCaptureLine(int timeoutMilliseconds) {
    NSMutableData *line=[NSMutableData data];while(!TMenuInterrupted&&line.length<512){struct pollfd descriptor={.fd=STDIN_FILENO,.events=POLLIN};int ready=poll(&descriptor,1,timeoutMilliseconds);if(ready==0)return nil;if(ready<0){if(errno==EINTR)continue;return nil;}unsigned char byte=0;if(read(STDIN_FILENO,&byte,1)!=1)continue;if(byte=='\n'||byte=='\r')break;[line appendBytes:&byte length:1];}return [[NSString alloc]initWithData:line encoding:NSUTF8StringEncoding];
}
static NSString *TCaptureConfigShortcut(NSString **status) {
    NSString *token=[NSUUID.UUID.UUIDString stringByReplacingOccurrencesOfString:@"-" withString:@""];fprintf(stdout,"\033]777;term-keybinding-capture;%s\a",token.UTF8String);fflush(stdout);NSString *ready=TReadKeybindingCaptureLine(2000),*expectedReady=[NSString stringWithFormat:@"TERMATICABIND:READY:%@",token];if(![ready isEqual:expectedReady]){fprintf(stdout,"\033]777;term-keybinding-cancel;%s\a",token.UTF8String);fflush(stdout);if(status)*status=@"CAPTURE UNAVAILABLE — RESTART TERMATICA AND TRY AGAIN";return nil;}fputs("\n\033[38;2;255;224;131m  READING... press the desired shortcut   ESC cancel\033[0m",stdout);fflush(stdout);NSString *response=TReadKeybindingCaptureLine(30000);fprintf(stdout,"\033]777;term-keybinding-cancel;%s\a",token.UTF8String);fflush(stdout);NSString *prefix=[NSString stringWithFormat:@"TERMATICABIND:VALUE:%@:",token];if(![response hasPrefix:prefix]){if(status)*status=response?@"INVALID SHORTCUT — TRY AGAIN":@"CAPTURE TIMED OUT";return nil;}NSString *shortcut=[response substringFromIndex:prefix.length];if(!shortcut.length&&status)*status=@"CANCELLED";return shortcut.length?shortcut:nil;
}
static NSUInteger TConfigMenuVisibleRows(NSUInteger reservedRows) {
    struct winsize size={0};NSUInteger terminalRows=24;if(ioctl(STDOUT_FILENO,TIOCGWINSZ,&size)==0&&size.ws_row)terminalRows=size.ws_row;return terminalRows>reservedRows+3?terminalRows-reservedRows:4;
}
static void TDrawConfigFilesHome(NSArray<NSString *> *names,NSUInteger selected,TConfig *config,NSString *message) {
    NSString *current=TActiveConfigName(config);fputs("\033[2J\033[H\033[38;2;122;162;247m  TERMATICA CONFIG / CONFIG FILES\033[0m\n",stdout);fprintf(stdout,"\033[38;2;107;114;128m  folder  %s\n  current %s\033[0m\n\n",TConfigProfileDirectory().fileSystemRepresentation,current.UTF8String);
    NSUInteger visible=TConfigMenuVisibleRows(8),start=selected>=visible?selected-visible+1:0;if(names.count>visible)start=MIN(start,names.count-visible);NSUInteger end=MIN(names.count,start+visible);if(start)fprintf(stdout,"\033[38;2;107;114;128m  ... %lu files above ...\033[0m\n",(unsigned long)start);for(NSUInteger i=start;i<end;i++){NSString *file=names[i];BOOL highlighted=selected==i,isCurrent=[file isEqual:current],valid=TConfigProfileIsValid(file);NSString *state=!valid?@"INVALID":(isCurrent?@"CURRENT":@"SAVED");NSString *action=!valid?@"repair JSON before selecting":(isCurrent?@"open settings":@"select and open settings");fputs(highlighted?"\033[48;2;43;52;69m\033[38;2;238;241;245m":"\033[38;2;216;222;233m",stdout);fprintf(stdout," %c  %-8s  %-28.28s  %s\033[0m\n",highlighted?'>':' ',state.UTF8String,file.UTF8String,action.UTF8String);}if(end<names.count)fprintf(stdout,"\033[38;2;107;114;128m  ... %lu files below ...\033[0m\n",(unsigned long)(names.count-end));
    fputs("\n\033[38;2;107;114;128m  ENTER select/open   N new   R rename   D delete   ESC/Q quit\033[0m",stdout);if(message.length)fprintf(stdout,"\n\033[38;2;152;195;121m  %s\033[0m",message.UTF8String);fflush(stdout);
}
static BOOL TRunConfigFilesPanel(TConfig *config,struct termios original,struct termios raw,NSString **lastMessage) {
    NSUInteger selected=0;NSString *message=*lastMessage;while(!TMenuInterrupted){NSArray *names=TConfigProfileNames();NSUInteger count=names.count;if(!count){[config ensureEditableFile];names=TConfigProfileNames();count=names.count;}NSString *current=TActiveConfigName(config);NSUInteger currentIndex=[names indexOfObject:current];if(selected>=count)selected=currentIndex==NSNotFound?0:currentIndex;TDrawConfigFilesHome(names,selected,config,message);unsigned char key=0;NSInteger direction=0;if(!TReadMenuKey(&key,&direction))continue;if(key=='q'||key=='Q'){*lastMessage=message;return NO;}if(key=='j'||key=='J')selected=count?(selected+1)%count:0;else if(key=='k'||key=='K')selected=count?(selected+count-1)%count:0;else if((key=='\r'||key=='\n')&&count){NSString *file=names[selected];if(!TConfigProfileIsValid(file)){message=[NSString stringWithFormat:@"[ INVALID JSON ] %@",file];continue;}if(![file isEqual:current])message=TUseConfigNamed(file,config);*lastMessage=message;return YES;}else if(key=='n'||key=='N'){NSString *name=TConfigPrompt(original,raw,@"new config filename (.json optional): ");message=name.length?TSaveConfigNamed(name,config):@"CANCELLED";if([message hasPrefix:@"[ CREATED"]){*lastMessage=message;return YES;}}else if((key=='r'||key=='R')&&count){NSString *oldName=names[selected];NSString *name=TConfigPrompt(original,raw,[NSString stringWithFormat:@"rename %@ to (.json optional): ",oldName]);message=name.length?TRenameConfig(oldName,name,config):@"CANCELLED";}else if((key=='d'||key=='D')&&count){NSString *name=names[selected];NSString *answer=TConfigPrompt(original,raw,[NSString stringWithFormat:@"delete %@? [y/N]: ",name]);message=[answer.lowercaseString isEqual:@"y"]?TDeleteConfig(name,config):@"CANCELLED";}}
    *lastMessage=message;return NO;
}
static NSString *TRunSettingsPanel(TConfig *config,NSDictionary *section) {
    NSUInteger selected=0;NSString *message=nil;while(YES){NSArray *rows=section[@"rows"];selected=MIN(selected,rows.count?rows.count-1:0);NSMutableDictionary *dictionary=TReadActiveConfig(config);fputs("\033[2J\033[H",stdout);fprintf(stdout,"\033[38;2;122;162;247m  TERMATICA CONFIG / %s\033[0m\n\033[38;2;107;114;128m  file  %s\033[0m\n\n",[section[@"title"] UTF8String],TActiveConfigName(config).UTF8String);
        NSUInteger visible=TConfigMenuVisibleRows(7),start=selected>=visible?selected-visible+1:0;if(rows.count>visible)start=MIN(start,rows.count-visible);NSUInteger end=MIN(rows.count,start+visible);if(start)fprintf(stdout,"\033[38;2;107;114;128m  ... %lu settings above ...\033[0m\n",(unsigned long)start);for(NSUInteger i=start;i<end;i++){NSDictionary *row=rows[i];BOOL active=i==selected;NSString *value=TConfigDisplayValue(TConfigValueAtPath(dictionary,row[@"path"]));fputs(active?"\033[48;2;43;52;69m\033[38;2;238;241;245m":"\033[38;2;216;222;233m",stdout);fprintf(stdout," %c  %-24.24s  %-43.43s\033[0m\n",active?'>':' ',[row[@"label"] UTF8String],value.UTF8String);}if(end<rows.count)fprintf(stdout,"\033[38;2;107;114;128m  ... %lu settings below ...\033[0m\n",(unsigned long)(rows.count-end));
        BOOL shortcutRow=rows.count&&[rows[selected][@"type"] isEqual:@"shortcut"];fputs(shortcutRow?"\n\033[38;2;107;114;128m  UP/DOWN move   ENTER record shortcut   ESC/Q back\033[0m":"\n\033[38;2;107;114;128m  UP/DOWN move   LEFT/RIGHT change   ENTER next choice   ESC/Q back\033[0m",stdout);if(message.length)fprintf(stdout,"\n\033[38;2;152;195;121m  %s\033[0m",message.UTF8String);fflush(stdout);unsigned char key=0;NSInteger direction=0;if(!TReadMenuKey(&key,&direction))continue;if(key=='q'||key=='Q')return message;if(!rows.count)continue;if(key=='j'||key=='J')selected=(selected+1)%rows.count;else if(key=='k'||key=='K')selected=(selected+rows.count-1)%rows.count;else if(shortcutRow&&(key=='\r'||key=='\n')){NSString *shortcut=TCaptureConfigShortcut(&message);if(shortcut.length)TChangeConfigSetting(config,rows[selected],1,shortcut,&message);else if(!message.length)message=@"CANCELLED";}else if(shortcutRow&&(key=='h'||key=='H'||key=='l'||key=='L'))message=@"PRESS ENTER TO RECORD";else if(key=='h'||key=='H'||key=='l'||key=='L'||key=='\r'||key=='\n')TChangeConfigSetting(config,rows[selected],direction?:((key=='h'||key=='H')?-1:1),nil,&message);}
}
static int TRunUnifiedConfigCLI(int argc,const char *argv[],TConfig *config) {
    if(argc>=3){NSString *action=[[NSString stringWithUTF8String:argv[2]] lowercaseString];if([action isEqual:@"list"]){NSString *current=TActiveConfigName(config);for(NSString *name in TConfigProfileNames())fprintf(stdout,"%s\t%s\n",!TConfigProfileIsValid(name)?"invalid":([name isEqual:current]?"current":"saved"),name.UTF8String);return 0;}if([action isEqual:@"get"]&&argc==4){id value=TConfigValueAtPath(TReadActiveConfig(config),[NSString stringWithUTF8String:argv[3]]);if(!value)return 1;fprintf(stdout,"%s\n",TConfigDisplayValue(value).UTF8String);return 0;}if([action isEqual:@"set"]&&argc>=5){NSString *path=[NSString stringWithUTF8String:argv[3]];NSMutableArray *parts=[NSMutableArray array];for(int i=4;i<argc;i++)[parts addObject:[NSString stringWithUTF8String:argv[i]]];NSString *input=[parts componentsJoinedByString:@" "];NSMutableDictionary *dictionary=TReadActiveConfig(config);id current=TConfigValueAtPath(dictionary,path),value=nil;BOOL toggle=[path hasPrefix:@"plugins."]||([current isKindOfClass:NSNumber.class]&&CFGetTypeID((__bridge CFTypeRef)current)==CFBooleanGetTypeID())||([current isKindOfClass:NSString.class]&&([@[@"on",@"off",@"true",@"false",@"yes",@"no"] containsObject:[current lowercaseString]]));if(toggle)value=TParseConfigInput(input,@"bool");else{NSData *data=[input dataUsingEncoding:NSUTF8StringEncoding];value=[NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingFragmentsAllowed error:nil]?:input;}if(!value){fputs("termatica config: expected ON or OFF\n",stderr);return 2;}TConfigSetValueAtPath(dictionary,path,value);NSString *message=nil;if(!TCommitUnifiedConfig(config,dictionary,&message))return 1;id committed=TConfigValueAtPath(TReadActiveConfig(config),path);fprintf(stdout,"%s\t%s\n",path.UTF8String,TConfigDisplayValue(committed).UTF8String);return 0;}NSString *result=nil;if([action isEqual:@"create"]&&argc==4)result=TSaveConfigNamed([NSString stringWithUTF8String:argv[3]],config);else if([action isEqual:@"use"]&&argc==4)result=TUseConfigNamed([NSString stringWithUTF8String:argv[3]],config);else if([action isEqual:@"rename"]&&argc==5)result=TRenameConfig([NSString stringWithUTF8String:argv[3]],[NSString stringWithUTF8String:argv[4]],config);else if([action isEqual:@"delete"]&&argc==4)result=TDeleteConfig([NSString stringWithUTF8String:argv[3]],config);else{fputs("usage: termatica config [list|get PATH|set PATH VALUE|create NAME|use NAME|rename OLD NEW|delete NAME]\n",stderr);return 2;}fprintf(stdout,"%s\n",result.UTF8String);return [result containsString:@"FAILED"]||[result containsString:@"INVALID"]||[result containsString:@"NOT FOUND"]||[result containsString:@"EXISTS"]||[result containsString:@"REQUIRED"]?1:0;}
...
static int TRunSystemMonitorCLI(int argc,const char *argv[]) {
    BOOL once=argc==3&&!strcmp(argv[2],"--once");if(argc>2&&!once){fputs("termatica system-monitor: expected no arguments or --once\n",stderr);return 2;}
    NSString *script=[NSBundle.mainBundle.resourcePath stringByAppendingPathComponent:@"SystemMonitor/system-monitor.zsh"];
    if(![NSFileManager.defaultManager isReadableFileAtPath:script]){fprintf(stderr,"termatica system-monitor: monitor resource is missing at %s\n",script.fileSystemRepresentation);return 1;}
    const char *zsh="/bin/zsh";char *arguments[4]={(char *)zsh,(char *)script.fileSystemRepresentation,once?"--once":NULL,NULL};execv(zsh,arguments);fprintf(stderr,"termatica system-monitor: could not launch: %s\n",strerror(errno));return 1;
}

static NSString *TCompletionScript(NSString *shell) {
    NSString *commands=@"ssh automation system-monitor monitor benchmark bench config config-file update reload editor run completions help version sm a b c cf u r e x h v",*configActions=@"list get set create use rename delete",*sshActions=@"list add set show rename remove connect command check split tile config known-hosts keys keygen help",*editors=@"vim nvim emacs nano micro hx";
    if([shell isEqual:@"zsh"])return [NSString stringWithFormat:@"#compdef termatica t\nlocal -a commands\ncommands=(%@)\nif (( CURRENT == 2 )); then compadd -- $commands; return; fi\ncase $words[2] in\n  ssh) (( CURRENT == 3 )) && _values 'action' %@ ;;\n  automation|a) (( CURRENT == 3 )) && _values 'action' status new-tab new-window close split focus send run key activate recipe ;;\n  benchmark|bench|b) (( CURRENT == 3 )) && _values 'scope' all a ;;\n  config|c) (( CURRENT == 3 )) && _values 'action' %@ ;;\n  config-file|cf) (( CURRENT == 3 )) && _values 'action' path ;;\n  update|u) (( CURRENT == 3 )) && _values 'action' check ;;\n  editor|e) (( CURRENT == 3 )) && _values 'editor' %@ || _files ;;\n  completions) _values 'shell' zsh bash fish install path ;;\nesac\n",commands,sshActions,configActions,editors];
    if([shell isEqual:@"bash"])return [NSString stringWithFormat:@"_termatica_complete() {\n  local cur=\"${COMP_WORDS[COMP_CWORD]}\" command=\"${COMP_WORDS[1]}\" words=\"%@\"\n  if (( COMP_CWORD == 1 )); then COMPREPLY=( $(compgen -W \"$words\" -- \"$cur\") ); return; fi\n  case \"$command\" in\n    ssh) words=\"%@\" ;;\n    automation|a) words=\"status new-tab new-window close split focus send run key activate recipe\" ;;\n    benchmark|bench|b) words=\"all a\" ;;\n    config|c) words=\"%@\" ;;\n    config-file|cf) words=\"path\" ;;\n    update|u) words=\"check\" ;;\n    editor|e) words=\"%@\" ;;\n    completions) words=\"zsh bash fish install path\" ;;\n  esac\n  COMPREPLY=( $(compgen -W \"$words\" -- \"$cur\") )\n}\ncomplete -F _termatica_complete termatica t\n",commands,sshActions,configActions,editors];
    if([shell isEqual:@"fish"])return [NSString stringWithFormat:@"for __termaticacmd in termatica t\ncomplete -c $__termaticacmd -f -n 'not __fish_seen_subcommand_from %@' -a '%@'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from ssh' -a '%@'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from automation a' -a 'status new-tab new-window close split focus send run key activate recipe'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from benchmark bench b' -a 'all a'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from config c' -a '%@'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from config-file cf' -a 'path'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from update u' -a 'check'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from editor e' -a '%@'\ncomplete -c $__termaticacmd -f -n '__fish_seen_subcommand_from completions' -a 'zsh bash fish install path'\nend\n",commands,commands,sshActions,configActions,editors];
    return nil;
}
...
static NSString *TReleaseSigningRequirement(void) {return @"=certificate leaf = H\"f95605c333732a3aa6c9fcd24e1170b03b19dce7\" and identifier \"com.termatica.Termatica\"";}
...
static NSString *TSSHProfilesPath(void){return [TConfigDirectoryPath() stringByAppendingPathComponent:@"ssh-hosts.json"];}
static BOOL TSSHSafeToken(NSString *value,NSString *extra){if(![value isKindOfClass:NSString.class]||!value.length||value.length>512)return NO;NSMutableCharacterSet *allowed=[[NSCharacterSet alphanumericCharacterSet] mutableCopy];[allowed addCharactersInString:[@"._-" stringByAppendingString:extra?:@""]];return [value rangeOfCharacterFromSet:allowed.invertedSet].location==NSNotFound;}
static NSMutableDictionary *TSSHReadStore(void){NSDictionary *raw=TReadJSONDictionaryAtPath(TSSHProfilesPath());NSDictionary *profiles=[raw[@"profiles"] isKindOfClass:NSDictionary.class]?raw[@"profiles"]:@{};return [@{@"schemaVersion":@1,@"profiles":[profiles mutableCopy]} mutableCopy];}
static BOOL TSSHWriteStore(NSDictionary *store,NSError **error){NSFileManager *fm=NSFileManager.defaultManager;[fm createDirectoryAtPath:TConfigDirectoryPath() withIntermediateDirectories:YES attributes:@{NSFilePosixPermissions:@0700} error:nil];NSData *data=[NSJSONSerialization dataWithJSONObject:store options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:error];BOOL ok=data&&[data writeToFile:TSSHProfilesPath() options:NSDataWritingAtomic error:error];if(ok)[fm setAttributes:@{NSFilePosixPermissions:@0600} ofItemAtPath:TSSHProfilesPath() error:nil];return ok;}
static NSArray<NSString *> *TSSHManagedNames(void){NSDictionary *profiles=TSSHReadStore()[@"profiles"];return [profiles.allKeys sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];}
static NSArray<NSString *> *TSSHConfigAliases(void){NSMutableSet<NSString *> *aliases=[NSMutableSet set];NSMutableArray<NSString *> *files=[NSMutableArray arrayWithObject:[NSHomeDirectory() stringByAppendingPathComponent:@".ssh/config"]];NSString *fragments=[NSHomeDirectory() stringByAppendingPathComponent:@".ssh/config.d"];for(NSString *name in [NSFileManager.defaultManager contentsOfDirectoryAtPath:fragments error:nil]?:@[])[files addObject:[fragments stringByAppendingPathComponent:name]];for(NSString *path in files){NSString *content=[NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];for(NSString *line in [content componentsSeparatedByCharactersInSet:NSCharacterSet.newlineCharacterSet]){NSString *trim=[line stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceCharacterSet],*lower=trim.lowercaseString;if(![lower hasPrefix:@"host "]&&![lower hasPrefix:@"host\t"])continue;NSArray *parts=[trim componentsSeparatedByCharactersInSet:NSCharacterSet.whitespaceCharacterSet];for(NSUInteger index=1;index<parts.count;index++){NSString *name=parts[index];if(name.length&&![name containsString:@"*"]&&![name containsString:@"?"]&&![name hasPrefix:@"!"])[aliases addObject:name];}}}return [aliases.allObjects sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];}
static NSString *TSSHProfileTarget(NSDictionary *profile){NSString *host=profile[@"host"],*user=profile[@"user"];return user.length?[NSString stringWithFormat:@"%@@%@",user,host]:host;}
static NSArray<NSString *> *TSSHArguments(NSDictionary *profile,NSArray<NSString *> *extra){NSMutableArray<NSString *> *args=[NSMutableArray array];NSNumber *port=profile[@"port"];NSString *identity=profile[@"identityFile"],*jump=profile[@"proxyJump"];if(port)[args addObjectsFromArray:@[@"-p",port.stringValue]];if(identity.length)[args addObjectsFromArray:@[@"-i",identity.stringByExpandingTildeInPath]];if(jump.length)[args addObjectsFromArray:@[@"-J",jump]];for(NSString *spec in profile[@"localForwards"]?:@[])[args addObjectsFromArray:@[@"-L",spec]];for(NSString *spec in profile[@"remoteForwards"]?:@[])[args addObjectsFromArray:@[@"-R",spec]];for(NSString *spec in profile[@"dynamicForwards"]?:@[])[args addObjectsFromArray:@[@"-D",spec]];NSDictionary *options=[profile[@"options"] isKindOfClass:NSDictionary.class]?profile[@"options"]:@{};for(NSString *key in [options.allKeys sortedArrayUsingSelector:@selector(compare:)])[args addObjectsFromArray:@[@"-o",[NSString stringWithFormat:@"%@=%@",key,options[key]]]];[args addObject:TSSHProfileTarget(profile)];if(extra.count)[args addObjectsFromArray:extra];return args;}
static NSString *TSSHShellCommand(NSDictionary *profile,NSArray<NSString *> *extra){NSMutableArray *parts=[NSMutableArray arrayWithObject:@"/usr/bin/ssh"];for(NSString *argument in TSSHArguments(profile,extra))[parts addObject:TShellQuote(argument)];return [parts componentsJoinedByString:@" "];}
static NSDictionary *TSSHProfileForName(NSString *name){NSDictionary *profile=TSSHReadStore()[@"profiles"][name];return [profile isKindOfClass:NSDictionary.class]?profile:nil;}
static NSString *TLaunchRecipesPath(void){return [TConfigDirectoryPath() stringByAppendingPathComponent:@"launch-recipes.json"];}
static NSMutableDictionary *TLaunchRecipeStore(void){NSDictionary *raw=TReadJSONDictionaryAtPath(TLaunchRecipesPath());NSDictionary *recipes=[raw[@"recipes"] isKindOfClass:NSDictionary.class]?raw[@"recipes"]:@{};return [@{@"schemaVersion":@1,@"recipes":[recipes mutableCopy]} mutableCopy];}
static BOOL TWriteLaunchRecipeStore(NSDictionary *store,NSError **error){NSFileManager *fm=NSFileManager.defaultManager;[fm createDirectoryAtPath:TConfigDirectoryPath() withIntermediateDirectories:YES attributes:@{NSFilePosixPermissions:@0700} error:nil];NSData *data=[NSJSONSerialization dataWithJSONObject:store options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:error];BOOL ok=data&&[data writeToFile:TLaunchRecipesPath() options:NSDataWritingAtomic error:error];if(ok)[fm setAttributes:@{NSFilePosixPermissions:@0600} ofItemAtPath:TLaunchRecipesPath() error:nil];return ok;}
static NSDictionary *TLaunchRecipeNamed(NSString *name){NSDictionary *recipe=TLaunchRecipeStore()[@"recipes"][name];return [recipe isKindOfClass:NSDictionary.class]?recipe:nil;}
static NSArray<NSString *> *TLaunchRecipeCommands(NSDictionary *recipe,NSString **failure){NSArray *profiles=[recipe[@"profiles"] isKindOfClass:NSArray.class]?recipe[@"profiles"]:@[];if(!profiles.count||profiles.count>9){if(failure)*failure=@"recipe must contain 1 to 9 SSH profiles";return nil;}NSMutableArray *commands=[NSMutableArray array];NSUInteger total=0;for(id value in profiles){if(![value isKindOfClass:NSString.class]||!TSafeIdentifier(value)){if(failure)*failure=@"recipe contains an invalid SSH profile name";return nil;}NSDictionary *profile=TSSHProfileForName(value);if(!profile){if(failure)*failure=[NSString stringWithFormat:@"SSH profile '%@' does not exist",value];return nil;}NSString *command=TSSHShellCommand(profile,@[]);total+=command.length;if(command.length>4096||total>6000){if(failure)*failure=@"recipe commands exceed the local automation packet limit";return nil;}[commands addObject:command];}return commands;}
static NSString *TSSHSaveProfile(NSString *name,NSDictionary *profile,BOOL replace){if(!TSafeIdentifier(name))return @"invalid profile name";NSString *host=profile[@"host"],*user=profile[@"user"],*jump=profile[@"proxyJump"],*identity=profile[@"identityFile"];NSInteger port=[profile[@"port"] integerValue];if(!TSSHSafeToken(host,@":[]%")||(user.length&&!TSSHSafeToken(user,@""))||(jump.length&&!TSSHSafeToken(jump,@"@:[]%"))||(port<1||port>65535)||(identity.length&&[identity rangeOfCharacterFromSet:NSCharacterSet.controlCharacterSet].location!=NSNotFound))return @"invalid host, user, port, jump host, or identity path";for(NSString *key in @[@"localForwards",@"remoteForwards",@"dynamicForwards"])for(NSString *spec in profile[key]?:@[])if(!TSSHSafeToken(spec,@":[]%@"))return @"invalid forwarding specification";NSDictionary *options=profile[@"options"]?:@{};for(NSString *key in options)if(!TSSHSafeToken(key,@"")||![options[key] isKindOfClass:NSString.class]||[options[key] rangeOfCharacterFromSet:NSCharacterSet.controlCharacterSet].location!=NSNotFound)return @"invalid OpenSSH option";NSMutableDictionary *store=TSSHReadStore(),*profiles=store[@"profiles"];if(profiles[name]&&!replace)return @"profile already exists; use ssh set to replace it";profiles[name]=profile;NSError *error=nil;if(!TSSHWriteStore(store,&error))return error.localizedDescription?:@"could not save profile";return nil;}
static NSDictionary *TSSHProfileFromArguments(int argc,const char *argv[],int start,NSString **failure){if(start>=argc){*failure=@"host is required";return nil;}NSString *host=[NSString stringWithUTF8String:argv[start++]],*user=@"",*identity=@"",*jump=@"";NSInteger port=22;NSMutableArray *local=[NSMutableArray array],*remote=[NSMutableArray array],*dynamic=[NSMutableArray array];NSMutableDictionary *options=[NSMutableDictionary dictionary];while(start<argc){NSString *flag=[NSString stringWithUTF8String:argv[start++]];if(start>=argc){*failure=[NSString stringWithFormat:@"%@ requires a value",flag];return nil;}NSString *value=[NSString stringWithUTF8String:argv[start++]];if([flag isEqual:@"--user"]||[flag isEqual:@"-l"])user=value;else if([flag isEqual:@"--port"]||[flag isEqual:@"-p"])port=value.integerValue;else if([flag isEqual:@"--identity"]||[flag isEqual:@"-i"])identity=value;else if([flag isEqual:@"--jump"]||[flag isEqual:@"-J"])jump=value;else if([flag isEqual:@"--local-forward"]||[flag isEqual:@"-L"])[local addObject:value];else if([flag isEqual:@"--remote-forward"]||[flag isEqual:@"-R"])[remote addObject:value];else if([flag isEqual:@"--dynamic-forward"]||[flag isEqual:@"-D"])[dynamic addObject:value];else if([flag isEqual:@"--option"]||[flag isEqual:@"-o"]){NSRange split=[value rangeOfString:@"="];if(split.location==NSNotFound){*failure=@"--option requires KEY=VALUE";return nil;}options[[value substringToIndex:split.location]]=[value substringFromIndex:split.location+1];}else{*failure=[NSString stringWithFormat:@"unknown profile option %@",flag];return nil;}}NSMutableDictionary *profile=[@{@"host":host,@"port":@(port)} mutableCopy];if(user.length)profile[@"user"]=user;if(identity.length)profile[@"identityFile"]=identity;if(jump.length)profile[@"proxyJump"]=jump;if(local.count)profile[@"localForwards"]=local;if(remote.count)profile[@"remoteForwards"]=remote;if(dynamic.count)profile[@"dynamicForwards"]=dynamic;if(options.count)profile[@"options"]=options;return profile;}
static NSString *TSSHPromptValue(struct termios original,struct termios raw,NSString *label,NSString *current){NSString *prompt=current.length?[NSString stringWithFormat:@"%@ [%@]: ",label,current]:[label stringByAppendingString:@": "];NSString *value=TConfigPrompt(original,raw,prompt);return value.length?value:(current?:@"");}
static NSString *TSSHPromptProfile(struct termios original,struct termios raw,NSString *existingName,NSDictionary *existing){NSString *name=TSSHPromptValue(original,raw,@"profile name",existingName),*host=TSSHPromptValue(original,raw,@"host or address",existing[@"host"]),*user=TSSHPromptValue(original,raw,@"user (optional)",existing[@"user"]),*portText=TSSHPromptValue(original,raw,@"port",[existing[@"port"] stringValue]?:@"22"),*identity=TSSHPromptValue(original,raw,@"identity file (optional)",existing[@"identityFile"]),*jump=TSSHPromptValue(original,raw,@"proxy jump (optional)",existing[@"proxyJump"]);NSMutableDictionary *profile=[@{@"host":host,@"port":@(portText.integerValue)} mutableCopy];if(user.length)profile[@"user"]=user;if(identity.length)profile[@"identityFile"]=identity;if(jump.length)profile[@"proxyJump"]=jump;for(NSString *key in @[@"localForwards",@"remoteForwards",@"dynamicForwards",@"options"])if(existing[key])profile[key]=existing[key];NSString *error=TSSHSaveProfile(name,profile,existing!=nil);return error?[NSString stringWithFormat:@"[ FAILED ] %@",error]:[NSString stringWithFormat:@"[ SAVED ] %@",name];}
static int TSSHExecProfile(NSDictionary *profile,NSArray<NSString *> *extra){NSArray<NSString *> *arguments=TSSHArguments(profile,extra);char **values=calloc(arguments.count+2,sizeof(char *));values[0]=(char *)"ssh";for(NSUInteger index=0;index<arguments.count;index++)values[index+1]=(char *)arguments[index].UTF8String;execv("/usr/bin/ssh",values);fprintf(stderr,"termatica ssh: could not start OpenSSH: %s\n",strerror(errno));free(values);return 127;}
static int TRunSSHManager(void){if(!isatty(STDIN_FILENO)||!isatty(STDOUT_FILENO)){fputs("termatica ssh requires a terminal; use 'termatica ssh list' for scripts.\n",stderr);return 2;}struct termios original;if(tcgetattr(STDIN_FILENO,&original)!=0)return 2;struct termios raw=original;raw.c_lflag&=~(ICANON|ECHO);raw.c_iflag&=~(IXON|ICRNL);raw.c_cc[VMIN]=1;raw.c_cc[VTIME]=0;tcsetattr(STDIN_FILENO,TCSAFLUSH,&raw);void(*previous)(int)=signal(SIGINT,TMenuSignal);TMenuInterrupted=0;NSUInteger selected=0;NSString *message=nil;fputs("\033[?25l\033[?1049h",stdout);while(!TMenuInterrupted){NSArray *managed=TSSHManagedNames(),*configured=TSSHConfigAliases();NSMutableArray<NSDictionary *> *rows=[NSMutableArray array];for(NSString *name in managed)[rows addObject:@{@"name":name,@"kind":@"MANAGED",@"profile":TSSHProfileForName(name)}];for(NSString *name in configured)if(![managed containsObject:name])[rows addObject:@{@"name":name,@"kind":@"OPENSSH"}];selected=rows.count?MIN(selected,rows.count-1):0;fputs("\033[2J\033[H\033[38;2;122;162;247m  TERMATICA / SSH MANAGER\033[0m\n\033[38;2;107;114;128m  profiles are password-free and stored with mode 0600\033[0m\n\n",stdout);NSUInteger visible=TConfigMenuVisibleRows(9),start=selected>=visible?selected-visible+1:0;if(rows.count>visible)start=MIN(start,rows.count-visible),visible=MIN(visible,rows.count-start);for(NSUInteger index=start;index<MIN(rows.count,start+visible);index++){NSDictionary *row=rows[index],*profile=row[@"profile"];NSString *target=profile?TSSHProfileTarget(profile):row[@"name"],*detail=profile?[NSString stringWithFormat:@"%@:%@",target,profile[@"port"]]:@"uses ~/.ssh/config";fputs(index==selected?"\033[48;2;43;52;69m\033[38;2;238;241;245m":"\033[38;2;216;222;233m",stdout);fprintf(stdout," %c  %-8s  %-22.22s  %-42.42s\033[0m\n",index==selected?'>':' ',[row[@"kind"] UTF8String],[row[@"name"] UTF8String],detail.UTF8String);}if(!rows.count)fputs("  No profiles yet. Press N to add one, or configure ~/.ssh/config.\n",stdout);fputs("\n\033[38;2;107;114;128m  ENTER connect   N new   E edit   D delete   C config   I identities   Q quit\033[0m",stdout);if(message.length)fprintf(stdout,"\n\033[38;2;152;195;121m  %s\033[0m",message.UTF8String);fflush(stdout);unsigned char key=0;NSInteger direction=0;if(!TReadMenuKey(&key,&direction))continue;if(key=='q'||key=='Q')break;if(key=='j'||key=='J')selected=rows.count?(selected+1)%rows.count:0;else if(key=='k'||key=='K')selected=rows.count?(selected+rows.count-1)%rows.count:0;else if(key=='n'||key=='N')message=TSSHPromptProfile(original,raw,nil,nil);else if((key=='e'||key=='E')&&rows.count){NSDictionary *row=rows[selected],*profile=row[@"profile"];message=profile?TSSHPromptProfile(original,raw,row[@"name"],profile):@"[ OPENSSH ] edit this alias in ~/.ssh/config";}else if((key=='d'||key=='D')&&rows.count){NSDictionary *row=rows[selected];if(!row[@"profile"])message=@"[ OPENSSH ] delete this alias from ~/.ssh/config";else{NSString *answer=TConfigPrompt(original,raw,[NSString stringWithFormat:@"delete %@? [y/N]: ",row[@"name"]]);if([answer.lowercaseString isEqual:@"y"]){NSMutableDictionary *store=TSSHReadStore();[store[@"profiles"] removeObjectForKey:row[@"name"]];NSError *error=nil;message=TSSHWriteStore(store,&error)?@"[ DELETED ]":error.localizedDescription;}}}else if(key=='c'||key=='C'){tcsetattr(STDIN_FILENO,TCSAFLUSH,&original);TOpenPath([NSHomeDirectory() stringByAppendingPathComponent:@".ssh/config"]);tcsetattr(STDIN_FILENO,TCSAFLUSH,&raw);message=@"opened ~/.ssh/config";}else if(key=='i'||key=='I'){message=@"run 't ssh keys' for identity paths and fingerprints";}else if((key=='\r'||key=='\n')&&rows.count){NSDictionary *row=rows[selected],*profile=row[@"profile"]?:@{@"host":row[@"name"]};fputs("\033[?1049l\033[?25h\033[0m",stdout);tcsetattr(STDIN_FILENO,TCSAFLUSH,&original);signal(SIGINT,previous);return TSSHExecProfile(profile,@[]);}}fputs("\033[?1049l\033[?25h\033[0m\n",stdout);tcsetattr(STDIN_FILENO,TCSAFLUSH,&original);signal(SIGINT,previous);return TMenuInterrupted?130:0;}
static int TRunSSHCLI(int argc,const char *argv[]){if(argc<3)return TRunSSHManager();NSString *action=[[NSString stringWithUTF8String:argv[2]] lowercaseString];if([action isEqual:@"help"]){fputs("usage: termatica ssh [list|add|set|show|rename|remove|connect|command|check|split|tile|config|known-hosts|keys|keygen]\n",stdout);return 0;}if([action isEqual:@"list"]){BOOL json=argc==4&&!strcmp(argv[3],"--json");NSDictionary *profiles=TSSHReadStore()[@"profiles"];if(json){NSData *data=[NSJSONSerialization dataWithJSONObject:profiles options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];fwrite(data.bytes,1,data.length,stdout);fputc('\n',stdout);}else for(NSString *name in TSSHManagedNames()){NSDictionary *profile=profiles[name];fprintf(stdout,"%-20s  %-36s  port %ld%s\n",name.UTF8String,TSSHProfileTarget(profile).UTF8String,(long)[profile[@"port"] integerValue],[profile[@"proxyJump"] length]?[[NSString stringWithFormat:@"  via %@",profile[@"proxyJump"]] UTF8String]:"");}return 0;}if([action isEqual:@"add"]||[action isEqual:@"set"]){if(argc<5){fputs("usage: termatica ssh add NAME HOST [--user USER] [--port PORT] [--identity PATH] [--jump HOST] [-L SPEC] [-R SPEC] [-D SPEC] [-o KEY=VALUE]\n",stderr);return 2;}NSString *failure=nil,*name=[NSString stringWithUTF8String:argv[3]];NSDictionary *profile=TSSHProfileFromArguments(argc,argv,4,&failure);if(profile&&!failure)failure=TSSHSaveProfile(name,profile,[action isEqual:@"set"]);if(failure){fprintf(stderr,"termatica ssh: %s\n",failure.UTF8String);return 2;}fprintf(stdout,"saved SSH profile %s in %s\n",name.UTF8String,TSSHProfilesPath().fileSystemRepresentation);return 0;}if([action isEqual:@"show"]&&argc==4){NSDictionary *profile=TSSHProfileForName([NSString stringWithUTF8String:argv[3]]);if(!profile)return 1;NSData *data=[NSJSONSerialization dataWithJSONObject:profile options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];fwrite(data.bytes,1,data.length,stdout);fputc('\n',stdout);return 0;}if([action isEqual:@"rename"]&&argc==5){NSString *old=[NSString stringWithUTF8String:argv[3]],*next=[NSString stringWithUTF8String:argv[4]];if(!TSafeIdentifier(next)){fputs("termatica ssh: invalid new profile name\n",stderr);return 2;}NSMutableDictionary *store=TSSHReadStore(),*profiles=store[@"profiles"];if(!profiles[old]||profiles[next])return 1;profiles[next]=profiles[old];[profiles removeObjectForKey:old];return TSSHWriteStore(store,nil)?0:1;}if(([action isEqual:@"remove"]||[action isEqual:@"delete"])&&argc==4){NSString *name=[NSString stringWithUTF8String:argv[3]];NSMutableDictionary *store=TSSHReadStore(),*profiles=store[@"profiles"];if(!profiles[name])return 1;[profiles removeObjectForKey:name];return TSSHWriteStore(store,nil)?0:1;}if([@[@"config",@"known-hosts"] containsObject:action]){NSString *path=[NSHomeDirectory() stringByAppendingPathComponent:[action isEqual:@"config"]?@".ssh/config":@".ssh/known_hosts"];TOpenPath(path);fprintf(stdout,"opened %s\n",path.fileSystemRepresentation);return 0;}if([action isEqual:@"keys"]){NSString *root=[NSHomeDirectory() stringByAppendingPathComponent:@".ssh"];for(NSString *name in [[NSFileManager.defaultManager contentsOfDirectoryAtPath:root error:nil]?:@[] sortedArrayUsingSelector:@selector(compare:)])if([name hasPrefix:@"id_"]&&![name hasSuffix:@".pub"]){NSString *private=[root stringByAppendingPathComponent:name],*public=[private stringByAppendingString:@".pub"];int status=0;NSData *data=[NSFileManager.defaultManager fileExistsAtPath:public]?TRunTaskOutput(@"/usr/bin/ssh-keygen",@[@"-lf",public],nil,&status,nil):nil;NSString *fingerprint=data?[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]:@"private key (no .pub file)\n";fprintf(stdout,"%s\n  %s",private.fileSystemRepresentation,fingerprint.UTF8String);}return 0;}if([action isEqual:@"keygen"]&&argc>=4){NSString *name=[NSString stringWithUTF8String:argv[3]],*type=argc>=5?[NSString stringWithUTF8String:argv[4]]:@"ed25519";if(!TSafeIdentifier(name)||![@[@"ed25519",@"ecdsa",@"rsa"] containsObject:type]){fputs("usage: termatica ssh keygen NAME [ed25519|ecdsa|rsa]\n",stderr);return 2;}NSString *sshRoot=[NSHomeDirectory() stringByAppendingPathComponent:@".ssh"];[NSFileManager.defaultManager createDirectoryAtPath:sshRoot withIntermediateDirectories:YES attributes:@{NSFilePosixPermissions:@0700} error:nil];NSString *path=[sshRoot stringByAppendingPathComponent:[@"id_" stringByAppendingString:name]];execl("/usr/bin/ssh-keygen","ssh-keygen","-t",type.UTF8String,"-f",path.fileSystemRepresentation,NULL);return 127;}if([@[@"connect",@"command",@"check",@"split"] containsObject:action]){if(argc<4)return 2;NSString *name=[NSString stringWithUTF8String:argv[3]];NSDictionary *profile=TSSHProfileForName(name)?:@{@"host":name};int extraStart=4;if(extraStart<argc&&!strcmp(argv[extraStart],"--"))extraStart++;NSMutableArray *extra=[NSMutableArray array];for(int index=extraStart;index<argc;index++)[extra addObject:[NSString stringWithUTF8String:argv[index]]];if([action isEqual:@"command"]){fputs([[TSSHShellCommand(profile,extra) stringByAppendingString:@"\n"] UTF8String],stdout);return 0;}if([action isEqual:@"check"]){NSMutableArray *args=[@[@"-o",@"BatchMode=yes",@"-o",@"ConnectTimeout=5"] mutableCopy];[args addObjectsFromArray:TSSHArguments(profile,@[@"exit"] )];int status=0;TRunTaskOutput(@"/usr/bin/ssh",args,nil,&status,nil);return status;}if([action isEqual:@"split"]){BOOL vertical=extra.count&&[[(NSString *)extra.firstObject lowercaseString] isEqual:@"vertical"];TPostCLICommand(vertical?@"splitVertical":@"splitHorizontal");usleep(80000);BOOL sent=TPostCLIRequest(@{@"command":@"sendText",@"text":[TSSHShellCommand(profile,@[]) stringByAppendingString:@"\n"]});return sent?0:1;}return TSSHExecProfile(profile,extra);}if([action isEqual:@"tile"]){if(argc<4)return 2;BOOL vertical=argc>4&&!strcmp(argv[3],"--vertical");int start=vertical?4:3;for(int index=start;index<argc;index++){NSString *name=[NSString stringWithUTF8String:argv[index]];NSDictionary *profile=TSSHProfileForName(name)?:@{@"host":name};if(index>start){TPostCLICommand(vertical?@"splitVertical":@"splitHorizontal");usleep(80000);}if(!TPostCLIRequest(@{@"command":@"sendText",@"text":[TSSHShellCommand(profile,@[]) stringByAppendingString:@"\n"]}))return 1;usleep(80000);}return 0;}fputs("termatica ssh: unknown action; run 'termatica ssh help'\n",stderr);return 2;}

static void TPrintJSON(id value){NSData *data=[NSJSONSerialization dataWithJSONObject:value options:NSJSONWritingPrettyPrinted|NSJSONWritingSortedKeys error:nil];if(data){fwrite(data.bytes,1,data.length,stdout);fputc('\n',stdout);}}
static int TAutomationRequest(NSDictionary *request,BOOL print){NSDictionary *reply=TRequestCLI(request,5);if(!reply){fputs("termatica automation: the Termatica app is not running or did not reply\n",stderr);return 1;}if(print)TPrintJSON(reply);else if(![reply[@"ok"] boolValue])fprintf(stderr,"termatica automation: %s\n",[reply[@"error"]?:@"request failed" UTF8String]);return [reply[@"ok"] boolValue]?0:1;}
static int TRunLaunchRecipeCLI(int argc,const char *argv[]){if(argc<4){fputs("usage: termatica automation recipe [list|show|save|run|remove] ...\n",stderr);return 2;}NSString *action=[[NSString stringWithUTF8String:argv[3]] lowercaseString];NSMutableDictionary *store=TLaunchRecipeStore(),*recipes=store[@"recipes"];if([action isEqual:@"list"]){for(NSString *name in [recipes.allKeys sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)]){NSDictionary *recipe=recipes[name];fprintf(stdout,"%-20s  %s  %lu SSH profile(s)\n",name.UTF8String,[recipe[@"direction"] UTF8String],(unsigned long)[recipe[@"profiles"] count]);}return 0;}if([action isEqual:@"show"]&&argc==5){NSDictionary *recipe=recipes[[NSString stringWithUTF8String:argv[4]]];if(!recipe)return 1;TPrintJSON(recipe);return 0;}if([action isEqual:@"save"]&&argc>=6){NSString *name=[NSString stringWithUTF8String:argv[4]],*direction=@"horizontal";int start=5;if(!strcmp(argv[start],"--vertical")){direction=@"vertical";start++;}else if(!strcmp(argv[start],"--horizontal"))start++;if(!TSafeIdentifier(name)||start>=argc||argc-start>9){fputs("termatica automation recipe: use a safe name and 1 to 9 SSH profiles\n",stderr);return 2;}NSMutableArray *profiles=[NSMutableArray array];for(int i=start;i<argc;i++){NSString *profile=[NSString stringWithUTF8String:argv[i]];if(!TSSHProfileForName(profile)){fprintf(stderr,"termatica automation recipe: unknown SSH profile %s\n",profile.UTF8String);return 2;}[profiles addObject:profile];}recipes[name]=@{@"kind":@"ssh-layout",@"direction":direction,@"profiles":profiles};NSError *error=nil;if(!TWriteLaunchRecipeStore(store,&error)){fprintf(stderr,"termatica automation recipe: %s\n",error.localizedDescription.UTF8String);return 1;}fprintf(stdout,"saved launch recipe %s in %s\n",name.UTF8String,TLaunchRecipesPath().fileSystemRepresentation);return 0;}if(([@"remove" isEqual:action]||[@"delete" isEqual:action])&&argc==5){NSString *name=[NSString stringWithUTF8String:argv[4]];if(!recipes[name])return 1;[recipes removeObjectForKey:name];return TWriteLaunchRecipeStore(store,nil)?0:1;}if([action isEqual:@"run"]&&argc==5){NSString *name=[NSString stringWithUTF8String:argv[4]],*failure=nil;NSDictionary *recipe=recipes[name];NSArray *commands=recipe?TLaunchRecipeCommands(recipe,&failure):nil;if(!commands){fprintf(stderr,"termatica automation recipe: %s\n",(failure?:@"recipe not found").UTF8String);return 1;}return TAutomationRequest(@{@"command":@"automation",@"action":@"recipe",@"name":name,@"direction":recipe[@"direction"]?:@"horizontal",@"commands":commands},NO);}fputs("termatica automation recipe: invalid action or arguments\n",stderr);return 2;}
static int TRunAutomationCLI(int argc,const char *argv[]){if(argc<3){fputs("usage: termatica automation [status|new-tab|new-window|close|split|focus|send|run|key|activate|recipe]\n",stderr);return 2;}NSString *action=[[NSString stringWithUTF8String:argv[2]] lowercaseString];if([action isEqual:@"recipe"])return TRunLaunchRecipeCLI(argc,argv);NSMutableDictionary *request=[@{@"command":@"automation",@"action":action} mutableCopy];BOOL print=[action isEqual:@"status"];if([@[@"new-tab",@"new-window"] containsObject:action]){for(int i=3;i<argc;i++){NSString *flag=[NSString stringWithUTF8String:argv[i]];if(i+1>=argc||![@[@"--cwd",@"--command"] containsObject:flag])return 2;request[[flag isEqual:@"--cwd"]?@"cwd":@"text"]=[NSString stringWithUTF8String:argv[++i]];}}else if([action isEqual:@"close"]){if(argc!=4)return 2;request[@"target"]=[[NSString stringWithUTF8String:argv[3]] lowercaseString];}else if([action isEqual:@"split"]){request[@"direction"]=argc>=4?[[NSString stringWithUTF8String:argv[3]] lowercaseString]:@"horizontal";for(int i=4;i<argc;i++){NSString *flag=[NSString stringWithUTF8String:argv[i]];if(i+1>=argc||![@[@"--cwd",@"--command"] containsObject:flag])return 2;request[[flag isEqual:@"--cwd"]?@"cwd":@"text"]=[NSString stringWithUTF8String:argv[++i]];}}else if([action isEqual:@"focus"]){if(argc<4)return 2;request[@"target"]=[[NSString stringWithUTF8String:argv[3]] lowercaseString];if(argc>=5)request[@"index"]=@([[NSString stringWithUTF8String:argv[4]] integerValue]);}else if([action isEqual:@"send"]||[action isEqual:@"run"]){if(argc<4)return 2;NSMutableArray *parts=[NSMutableArray array];for(int i=3;i<argc;i++)[parts addObject:[NSString stringWithUTF8String:argv[i]]];request[@"text"]=[parts componentsJoinedByString:@" "];}else if([action isEqual:@"key"]){if(argc!=4)return 2;request[@"key"]=[[NSString stringWithUTF8String:argv[3]] lowercaseString];}else if(![@[@"status",@"activate"] containsObject:action]){fputs("termatica automation: unknown action\n",stderr);return 2;}return TAutomationRequest(request,print);}
...
static int TRunCLI(int argc, const char *argv[]) {
    ...
    NSDictionary *quick=@{@"sm":@"system-monitor",@"a":@"automation",@"b":@"benchmark",@"c":@"config",@"cf":@"config-file",@"u":@"update",@"r":@"reload",@"e":@"editor",@"x":@"run",@"h":@"help",@"v":@"version"};
    arg=quick[arg]?:arg;
    if([arg isEqual:@"help"]){
        fprintf(stdout,"Termatica %s\n\nCOMMANDS\n\nREMOTE & SYSTEM\n  ssh [action]       Manage, check, connect, split, and tile SSH profiles\n  automation ...     Control tabs, splits, focus, commands, input, and recipes\n  system-monitor     Show live system and process statistics\n\nCONFIGURATION\n  config [action]    Open settings or manage named configurations\n  config-file [path] Open or print the selected JSON configuration\n  reload             Reload saved configuration in the running app\n\nTOOLS\n  benchmark [all]    Benchmark Termatica or installed comparison terminals\n  editor <name> ...  Run Vim, Neovim, Emacs, Nano, Micro, or Helix\n  run <name> [text]  Run an enabled extension command\n  completions ...    Generate or install shell completions\n\nMAINTENANCE\n  update [check]     Install or check for the latest verified release\n  help               Show this command list\n  version            Print the app version\n",TCurrentVersion().UTF8String);return 0;
    }
    if([arg isEqual:@"version"]){fprintf(stdout,"Termatica %s\n",TCurrentVersion().UTF8String);return 0;}
    ...
    if([arg isEqual:@"ssh"])return TRunSSHCLI(argc,argv);
    if([arg isEqual:@"automation"])return TRunAutomationCLI(argc,argv);
    if([arg isEqual:@"system-monitor"]||[arg isEqual:@"monitor"])return TRunSystemMonitorCLI(argc,argv);
...
static NSRect TCursorRect(TRenderSnapshot *snapshot) {
    if(!snapshot.isValid||!snapshot.cursorVisible)return NSZeroRect;
    NSDictionary *style=snapshot.style;NSString *cursorStyle=style[@"cursorStyle"]?:@"block";CGFloat cellWidth=snapshot.metrics.cellWidth,cellHeight=snapshot.metrics.cellHeight,left=[style[@"left"] doubleValue],top=[style[@"top"] doubleValue],width=cellWidth,height=cellHeight,x=left+snapshot.cursorX*cellWidth,y=top+snapshot.cursorY*cellHeight,thickness=MAX(1,[style[@"cursorThickness"] doubleValue]);if([cursorStyle isEqual:@"bar"])width=MIN(cellWidth,thickness);else if([cursorStyle isEqual:@"underline"]){height=MIN(cellHeight,thickness);y+=cellHeight-height;}return NSMakeRect(x,y,MAX(1,width),MAX(1,height));
}

@interface TIntelCursorOverlayView : NSView
@end
@implementation TIntelCursorOverlayView
- (NSView *)hitTest:(NSPoint)point {return nil;}
@end

static BOOL TUseIntelCursorOverlay(void) {
#if defined(__x86_64__)
    return YES;
#else
    const char *forced=getenv("TERMATICA_FORCE_INTEL_CURSOR_OVERLAY");return forced&&*forced&&strcmp(forced,"0")!=0;
#endif
}

static NSInteger TAccumulatePreciseScroll(CGFloat raw,CGFloat cellHeight,BOOL began,CGFloat *accumulator) {
    if(!accumulator||fabs(raw)<0.001)return 0;
    if(began||(*accumulator!=0&&((*accumulator>0)!=(raw>0))))*accumulator=0;
    *accumulator+=raw/MAX(2.0,cellHeight*0.22);
    NSInteger lines=(NSInteger)trunc(*accumulator);*accumulator-=lines;return lines;
}
...
@property(nonatomic) BOOL activeTerminal;
@property BOOL verticalSplit;
@property BOOL splitBefore;
@property(weak) TTerminalView *splitAnchor;
@property(nonatomic) BOOL tiledRendering;
@property NSString *launchDirectory;
@property (copy) void (^splitShortcut)(void);
@property (copy) void (^verticalSplitShortcut)(void);
...
- (BOOL)captureKeybindingEvent:(NSEvent *)event;
- (BOOL)hasPendingKeybindingCapture;
...
- (void)recoverPresentationAfterLifecycleChange;
- (void)recoverPresentationAfterLifecycleChange:(NSString *)reason;
- (void)clearTerminal;
...
- (void)updateIntelCursorOverlayForSnapshot:(TRenderSnapshot *)snapshot;
...
static NSString *TShortcutForEvent(NSEvent *event) {
    if(!event||event.type!=NSEventTypeKeyDown)return nil;NSDictionary<NSNumber *,NSString *> *special=@{@36:@"return",@76:@"return",@48:@"tab",@51:@"delete",@53:@"escape",@123:@"left",@124:@"right",@125:@"down",@126:@"up",@115:@"home",@119:@"end",@116:@"page-up",@121:@"page-down",@117:@"forward-delete",@122:@"f1",@120:@"f2",@99:@"f3",@118:@"f4",@96:@"f5",@97:@"f6",@98:@"f7",@100:@"f8",@101:@"f9",@109:@"f10",@103:@"f11",@111:@"f12"};NSString *key=special[@(event.keyCode)];if(!key.length){key=event.charactersIgnoringModifiers.lowercaseString;if(!key.length)return nil;unichar value=[key characterAtIndex:0];if(value==' ')key=@"space";else if(value=='+')key=@"plus";else if(value<0x21||value==0x7f)return nil;else key=[key substringWithRange:[key rangeOfComposedCharacterSequenceAtIndex:0]];}NSEventModifierFlags flags=event.modifierFlags&NSEventModifierFlagDeviceIndependentFlagsMask;NSMutableArray<NSString *> *parts=[NSMutableArray array];if(flags&NSEventModifierFlagCommand)[parts addObject:@"cmd"];if(flags&NSEventModifierFlagShift)[parts addObject:@"shift"];if(flags&NSEventModifierFlagOption)[parts addObject:@"option"];if(flags&NSEventModifierFlagControl)[parts addObject:@"control"];[parts addObject:key];return [parts componentsJoinedByString:@"+"];
}
static NSString *TCanonicalShortcut(NSString *spec) {
    NSArray<NSString *> *raw=[spec.lowercaseString componentsSeparatedByString:@"+"];if(!raw.count)return @"";NSMutableSet<NSString *> *parts=[NSMutableSet setWithArray:raw];NSString *key=raw.lastObject?:@"";NSMutableArray<NSString *> *ordered=[NSMutableArray array];if([parts containsObject:@"cmd"]||[parts containsObject:@"command"])[ordered addObject:@"cmd"];if([parts containsObject:@"shift"])[ordered addObject:@"shift"];if([parts containsObject:@"option"]||[parts containsObject:@"alt"])[ordered addObject:@"option"];if([parts containsObject:@"control"]||[parts containsObject:@"ctrl"])[ordered addObject:@"control"];if([key isEqual:@" "])key=@"space";else if([key isEqual:@"+"])key=@"plus";if(key.length)[ordered addObject:key];return [ordered componentsJoinedByString:@"+"];
}
static BOOL TEventMatchesShortcut(NSEvent *event,NSString *spec) {return spec.length&&[(TShortcutForEvent(event)?:@"") isEqual:TCanonicalShortcut(spec)];}
...
@implementation TTerminalView {
    ...
    NSString *_keybindingCaptureToken;
    ...
    NSUInteger _mainKittyKeyboardFlags, _alternateKittyKeyboardFlags;
    NSMutableArray<NSNumber *> *_mainKittyKeyboardStack, *_alternateKittyKeyboardStack;
    ...
    TIntelCursorOverlayView *_intelCursorOverlay;
    NSUInteger _intelCursorOverlayUpdates;
    BOOL _metalFailed;
    NSUInteger _metalFailureCount;
    NSUInteger _metalQuarantineBypassCount;
    NSUInteger _metalRetryAttemptCount;
    NSUInteger _metalSuccessfulRetryCount;
    NSString *_requestedRenderer;
    NSString *_lastMetalFailureStage;
    CFAbsoluteTime _lastMetalFailureAt;
    CFAbsoluteTime _lastMetalRetryAt;
    NSUInteger _presentationRecoveryCount;
    NSMutableDictionary<NSString *,NSNumber *> *_presentationRecoveryReasons;
    NSNumber *_lastObservedScreenNumber;
    CGFloat _lastObservedBackingScale;
    NSUInteger _observedScreenChangeCount;
    NSUInteger _observedBackingScaleChangeCount;
    NSString *_lastPresentationRecovery;
    CFAbsoluteTime _lastPresentationRecoveryAt;
...
- (instancetype)initWithFrame:(NSRect)frame config:(TConfig *)config deferPresentation:(BOOL)deferPresentation {
    if ((self = [super initWithFrame:frame])) {
        ...
        if(TUseIntelCursorOverlay()){_intelCursorOverlay=[[TIntelCursorOverlayView alloc]initWithFrame:NSZeroRect];_intelCursorOverlay.wantsLayer=YES;_intelCursorOverlay.hidden=YES;_intelCursorOverlay.layer.zPosition=1000;_intelCursorOverlay.layer.actions=@{@"position":NSNull.null,@"bounds":NSNull.null,@"backgroundColor":NSNull.null,@"opacity":NSNull.null,@"hidden":NSNull.null};_intelCursorOverlay.accessibilityElement=NO;[self addSubview:_intelCursorOverlay positioned:NSWindowAbove relativeTo:nil];}
...
- (void)invalidateCursorPresentation {
...
- (void)updateIntelCursorOverlayForSnapshot:(TRenderSnapshot *)snapshot {
    if(!_intelCursorOverlay)return;if(!NSThread.isMainThread){__weak typeof(self) weakSelf=self;dispatch_async(dispatch_get_main_queue(),^{[weakSelf updateIntelCursorOverlayForSnapshot:snapshot];});return;}
    BOOL visible=!_metalBackend&&snapshot.isValid&&snapshot.cursorVisible&&self.activeTerminal&&self.window.isKeyWindow&&self.window.firstResponder==self;_intelCursorOverlayUpdates++;[CATransaction begin];[CATransaction setDisableActions:YES];if(!visible){_intelCursorOverlay.hidden=YES;[CATransaction commit];return;}NSRect frame=TCursorRect(snapshot);CGFloat scale=MAX((CGFloat)1,snapshot.metrics.scale),x0=round(NSMinX(frame)*scale)/scale,y0=round(NSMinY(frame)*scale)/scale,x1=round(NSMaxX(frame)*scale)/scale,y1=round(NSMaxY(frame)*scale)/scale;frame=NSMakeRect(x0,y0,MAX(1.0/scale,x1-x0),MAX(1.0/scale,y1-y0));NSString *style=snapshot.style[@"cursorStyle"]?:@"block";_intelCursorOverlay.frame=frame;_intelCursorOverlay.layer.backgroundColor=TColor([snapshot.style[@"cursor"] unsignedIntValue]).CGColor;_intelCursorOverlay.layer.opacity=[style isEqual:@"block"]?[snapshot.style[@"cursorBlockOpacity"] doubleValue]:0.96;_intelCursorOverlay.hidden=NO;[CATransaction commit];
}
- (void)presentTerminalSnapshot:(TRenderSnapshot *)snapshot {if(!snapshot.isValid||!_renderBackend)return;if(_metalBackend){_displaySnapshot=snapshot;[self updateIntelCursorOverlayForSnapshot:snapshot];}[_renderBackend presentSnapshot:snapshot];}
- (void)setActiveTerminal:(BOOL)activeTerminal {
    if(_activeTerminal==activeTerminal)return;
    _activeTerminal=activeTerminal;
    [self invalidateCursorPresentation];
}
- (void)setTiledRendering:(BOOL)tiledRendering {
    if(_tiledRendering==tiledRendering)return;
    _tiledRendering=tiledRendering;
    // A layer-backed AppKit view can otherwise retain pixels from its old,
    // full-window backing store while the first tiled frame is resized.
    // Drop that store at the ownership transition and repaint every cell.
    if(self.wantsLayer){[CATransaction begin];[CATransaction setDisableActions:YES];self.layer.contents=nil;[CATransaction commit];}
    @synchronized(self){[self markAllDamage];}
    if(_presentationReady)[self refreshTextView];
}
...
- (void)insertText:(id)value replacementRange:(NSRange)replacement{NSString *text=[value isKindOfClass:NSAttributedString.class]?[value string]:[value description];[self unmarkText];if(text.length){if(_kittyKeyboardFlags&8){NSString *points=(_kittyKeyboardFlags&16)?[self kittyTextCodepoints:text]:@"";[self sendString:points.length?[NSString stringWithFormat:@"\033[0;1;%@u",points]:@"\033[0;1u"];}else [self sendString:text];}_hasSelection=NO;[self setNeedsDisplay:YES];}
...
- (BOOL)becomeFirstResponder {BOOL accepted=[super becomeFirstResponder];if(accepted&&_focusReporting)[self sendString:@"\033[I"];if(accepted){[self updateSecureKeyboardInput];[self invalidateCursorPresentation];}return accepted;}
- (BOOL)resignFirstResponder {if(_focusReporting)[self sendString:@"\033[O"];[self disableSecureKeyboardInput];BOOL resigned=[super resignFirstResponder];if(resigned)[self invalidateCursorPresentation];return resigned;}
- (BOOL)acceptsFirstMouse:(NSEvent *)event {return YES;}
- (BOOL)isFlipped { return YES; }
- (BOOL)isOpaque { return NO; }
- (void)viewDidMoveToWindow {[super viewDidMoveToWindow];if(_presentationReady&&self.window)[self recoverPresentationAfterLifecycleChange:@"view-window"];}
- (void)viewDidChangeBackingProperties {[super viewDidChangeBackingProperties];[self recoverPresentationAfterLifecycleChange:@"view-backing-scale"];}
- (void)recoverPresentationAfterLifecycleChange {[self recoverPresentationAfterLifecycleChange:@"manual"];}
- (void)recoverPresentationAfterLifecycleChange:(NSString *)reason {
    if(!_presentationReady)return;
    _presentationRecoveryCount++;
    _lastPresentationRecovery=reason.length?[reason copy]:@"unknown";
    if(!_presentationRecoveryReasons)_presentationRecoveryReasons=[NSMutableDictionary dictionary];_presentationRecoveryReasons[_lastPresentationRecovery]=@([_presentationRecoveryReasons[_lastPresentationRecovery] unsignedIntegerValue]+1);
    NSScreen *screen=self.window.screen;NSNumber *screenNumber=screen.deviceDescription[@"NSScreenNumber"];CGFloat backingScale=screen?MAX(1,screen.backingScaleFactor):0;if(_lastObservedScreenNumber&&screenNumber&&![_lastObservedScreenNumber isEqual:screenNumber])_observedScreenChangeCount++;if(_lastObservedBackingScale>0&&backingScale>0&&fabs(_lastObservedBackingScale-backingScale)>0.001)_observedBackingScaleChangeCount++;if(screenNumber)_lastObservedScreenNumber=screenNumber;if(backingScale>0)_lastObservedBackingScale=backingScale;
    _lastPresentationRecoveryAt=CFAbsoluteTimeGetCurrent();
    [self configureRenderBackend];
    @synchronized(self){[self markAllDamage];}
...
- (void)configureRenderBackend {
    ...
    _requestedRenderer=[desired copy];
    ...
    if([desired isEqual:@"metal"]&&_metalFailed&&[_renderBackend.name isEqual:@"appkit"]){
        NSError *error=nil;if([_renderBackend configureWithMetrics:metrics error:&error]){_metalQuarantineBypassCount++;TLog(@"Metal retry quarantined; AppKit reconfigured (bypass %lu)",(unsigned long)_metalQuarantineBypassCount);return;}
        [_renderBackend shutdown];_renderBackend=nil;
    }
    ...
    if([desired isEqual:@"metal"]&&!_metalFailed){
        ...
        if(backend&&[backend configureWithMetrics:metrics error:&error]){
            ...
            _metalBackend=backend;_renderBackend=backend;self.layer.contents=nil;[self markAllDamage];[self refreshTextView];TLog(@"renderer selected: metal");return;
        }
...
- (void)fallbackToAppKitForError:(NSError *)error {
    ...
    __weak typeof(self) weakSelf=self;TAppKitRenderBackend *appkit=[[TAppKitRenderBackend alloc]initWithPresentHandler:^(TRenderSnapshot *snapshot){__strong typeof(weakSelf) self=weakSelf;if(!self)return;self->_displaySnapshot=snapshot;[self updateIntelCursorOverlayForSnapshot:snapshot];[self setNeedsDisplayInRect:self.bounds];}];
...
- (NSDictionary *)retryMetalRenderer {
    ...
    NSString *requested=_requestedRenderer?:self.config.renderer?:@"appkit",*before=_renderBackend.name?:@"none";
    if(![requested isEqual:@"metal"])return @{@"attempted":@NO,@"recovered":@NO,@"status":@"metal-not-requested",@"before":before,@"after":before};
    if(!_metalFailed)return @{@"attempted":@NO,@"recovered":@NO,@"status":@"not-quarantined",@"before":before,@"after":before};
    _metalRetryAttemptCount++;_lastMetalRetryAt=CFAbsoluteTimeGetCurrent();_metalFailed=NO;
    [self configureRenderBackend];
    NSString *after=_renderBackend.name?:@"none";BOOL recovered=[after isEqual:@"metal"]&&!_metalFailed;if(recovered)_metalSuccessfulRetryCount++;else if(!_metalFailed){_metalFailed=YES;_metalFailureCount++;_lastMetalFailureStage=@"initialization";_lastMetalFailureAt=CFAbsoluteTimeGetCurrent();}
    [self invalidateCursorPresentation];
    TLog(@"explicit Metal retry %@ (attempt %lu, renderer %@ -> %@)",recovered?@"recovered":@"remained quarantined",(unsigned long)_metalRetryAttemptCount,before,after);
    return @{@"attempted":@YES,@"recovered":@(recovered),@"status":recovered?@"recovered":@"still-quarantined",@"before":before,@"after":after};
}
- (uint64_t)presentFrameForBenchmark {
    @synchronized(self){[self markAllDamage];}
...
- (void)resizeGrid {
    ...
    NSUInteger oldCols=_cols;
    TCell *next = calloc(cols * rows, sizeof(TCell));
    uint8_t *nextUnderlines=calloc(cols*rows,sizeof(uint8_t));
    if(!next||!nextUnderlines){free(next);free(nextUnderlines);return;}
    NSMutableDictionary<NSNumber *,NSString *> *nextLinks=[NSMutableDictionary dictionary];
    ...
    if (_cells) {
        ...
        for (NSUInteger y = 0; y < copyRows; y++){
            NSUInteger physical=[self physicalRowForRow:y];
            memcpy(next + y * cols, [self cellsForRow:y], copyCols * sizeof(TCell));
            if(_underlineStyles)memcpy(nextUnderlines+y*cols,_underlineStyles+physical*oldCols,copyCols);
            if(_linksByCell.count)for(NSUInteger x=0;x<copyCols;x++){NSString *link=_linksByCell[@(physical*oldCols+x)];if(link)nextLinks[@(y*cols+x)]=link;}
        }
        free(_cells);
    }
    _cells = next; _cols = cols; _rows = rows;free(_rowOrder);_rowOrder=malloc(rows*sizeof(NSUInteger));[self resetRowOrder];free(_underlineStyles);_underlineStyles=nextUnderlines;_linksByCell=nextLinks;_historyBlankValid=NO;[self ensureBlankRow];
    for(NSUInteger y=0;y<rows;y++)for(NSUInteger x=0;x<cols;x++){
        TCell *cell=_cells+y*cols+x;BOOL validWide=(cell->flags&TWide)&&x+1<cols&&(_cells[y*cols+x+1].flags&TContinuation),validContinuation=(cell->flags&TContinuation)&&x&&(_cells[y*cols+x-1].flags&TWide);
        if((cell->flags&TWide)&&validWide){x++;continue;}
        if((cell->flags&(TWide|TContinuation))&&!validWide&&!validContinuation){*cell=blank;_underlineStyles[y*cols+x]=0;[_linksByCell removeObjectForKey:@(y*cols+x)];}
    }
...
- (void)executeCSI:(uint8_t)command prefix:(uint8_t)prefix intermediate:(uint8_t)intermediate parameters:(const int *)parameters count:(NSUInteger)count __attribute__((objc_direct)) {
    ...
    if((prefix=='>'||prefix=='=')&&command=='u'){
        NSUInteger flags=(NSUInteger)MAX(0,parameters[0])&31,mode=count>1?(NSUInteger)MAX(1,parameters[1]):1;
        if(prefix=='>'){NSMutableArray<NSNumber *> *stack=_alternateScreen?_alternateKittyKeyboardStack:_mainKittyKeyboardStack;if(stack.count>=32)[stack removeObjectAtIndex:0];[stack addObject:@(_kittyKeyboardFlags)];_kittyKeyboardFlags=flags;}
        else if(mode==2)_kittyKeyboardFlags|=flags;else if(mode==3)_kittyKeyboardFlags&=~flags;else _kittyKeyboardFlags=flags;
        if(_alternateScreen)_alternateKittyKeyboardFlags=_kittyKeyboardFlags;else _mainKittyKeyboardFlags=_kittyKeyboardFlags;return;
    }
    if(prefix=='<'&&command=='u'){NSMutableArray<NSNumber *> *stack=_alternateScreen?_alternateKittyKeyboardStack:_mainKittyKeyboardStack;NSUInteger pops=(NSUInteger)MAX(1,parameters[0]);if(pops>stack.count){[stack removeAllObjects];_kittyKeyboardFlags=0;}else while(pops--){_kittyKeyboardFlags=stack.lastObject.unsignedIntegerValue;[stack removeLastObject];}if(_alternateScreen)_alternateKittyKeyboardFlags=_kittyKeyboardFlags;else _mainKittyKeyboardFlags=_kittyKeyboardFlags;return;}
...
- (void)clearWideCellAtX:(NSUInteger)x row:(TCell *)row __attribute__((objc_direct)) {
    if(!row||x>=_cols)return;
    NSUInteger first=x,last=x;
    if(row[x].flags&TWide){last=MIN(_cols-1,x+1);}
    else if(row[x].flags&TContinuation){first=x?x-1:x;}
    else return;
    TCell blank=[self blankCell];NSUInteger physical=[self physicalRowForRow:_cursorY];
    for(NSUInteger column=first;column<=last;column++){
        row[column]=blank;
        if(_underlineStyles)_underlineStyles[physical*_cols+column]=0;
        if(_linksByCell.count)[_linksByCell removeObjectForKey:@(physical*_cols+column)];
    }
    [self markDamageX:first y:_cursorY width:last-first+1 height:1];
}
- (void)sanitizeWideCellsInRow:(TCell *)row logicalRow:(NSUInteger)logicalRow __attribute__((objc_direct)) {
    if(!row||logicalRow>=_rows)return;NSUInteger physical=[self physicalRowForRow:logicalRow];TCell blank=[self blankCell];
    for(NSUInteger x=0;x<_cols;x++){
        if(row[x].flags&TWide){if(x+1<_cols&&(row[x+1].flags&TContinuation)){x++;continue;}}
        else if(!(row[x].flags&TContinuation))continue;
        row[x]=blank;if(_underlineStyles)_underlineStyles[physical*_cols+x]=0;if(_linksByCell.count)[_linksByCell removeObjectForKey:@(physical*_cols+x)];[self markDamageX:x y:logicalRow width:1 height:1];
    }
}
- (void)sanitizeWideCellsInRow:(TCell *)row __attribute__((objc_direct)) {[self sanitizeWideCellsInRow:row logicalRow:_cursorY];}
...
- (void)putCodepoints:(const uint32_t *)codepoints count:(NSUInteger)count __attribute__((objc_direct)) {
    ...
    for(NSUInteger i=0;i<count;i++){
        ...
        TCell *row=[self cellsForRow:_cursorY];uint8_t previousFlags=row[_cursorX].flags;[self clearWideCellAtX:_cursorX row:row];
...
- (void)putCodepoint:(uint32_t)cp __attribute__((objc_direct)) {
    if(_cachedUnicodeRendering&&_cursorX){
        ...
        if(TMayJoinGrapheme(cp,base->ch,base->flags)){
            ...
            if(!joins){
                NSString *existing=[self stringForCodepoint:base->ch],*scalar=[self stringForCodepoint:cp];candidate=[existing stringByAppendingString:scalar];
                NSRange composed=[candidate rangeOfComposedCharacterSequenceAtIndex:0];joins=composed.location==0&&NSMaxRange(composed)==candidate.length;
            }
            if(joins&&base->ch!=' '&&!(base->flags&TContinuation)){
                uint32_t token=cachedToken;
                if(!token){if(!candidate){NSString *existing=[self stringForCodepoint:base->ch],*scalar=[self stringForCodepoint:cp];candidate=[existing stringByAppendingString:scalar];}token=[self internGrapheme:candidate];_graphemePairKeys[pairIndex]=pairKey;_graphemePairValues[pairIndex]=token;}
                base->ch=token;base->flags|=TCluster;
                if((TUnicodeWide(cp)||cp==0xFE0F||cp==0x200D)&&!(base->flags&TWide)&&baseX+1<_cols){NSUInteger continuationX=baseX+1;[self clearWideCellAtX:continuationX row:row];base->flags|=TWide;row[continuationX]=(TCell){.ch=0,.fg=base->fg,.bg=base->bg,.flags=TContinuation};NSUInteger physical=[self physicalRowForRow:_cursorY];if(_underlineStyles)_underlineStyles[physical*_cols+continuationX]=_underlineStyles[physical*_cols+baseX];NSNumber *baseLink=[self linkKeyForX:baseX y:_cursorY],*continuationLink=[self linkKeyForX:continuationX y:_cursorY];NSString *link=_linksByCell[baseLink];if(link)_linksByCell[continuationLink]=link;else[_linksByCell removeObjectForKey:continuationLink];if(_cursorX==continuationX)_cursorX++;}
...
- (void)eraseLine:(int)mode {
    ...
    TCell *row=[self cellsForRow:_cursorY];for(NSUInteger x=a;x<b;x++)row[x]=blank;[self sanitizeWideCellsInRow:row];if(_underlineStyles)memset(_underlineStyles+[self physicalRowForRow:_cursorY]*_cols+a,0,b-a);[self clearLinksInRow:_cursorY from:a to:b];[self markDamageX:a y:_cursorY width:b-a height:1];
}
- (void)eraseCharacters:(int)n { TCell b=[self blankCell],*row=[self cellsForRow:_cursorY];NSUInteger end=MIN(_cols,_cursorX+(NSUInteger)n);for(NSUInteger x=_cursorX;x<end;x++)row[x]=b;[self sanitizeWideCellsInRow:row];if(_underlineStyles)memset(_underlineStyles+[self physicalRowForRow:_cursorY]*_cols+_cursorX,0,end-_cursorX);[self clearLinksInRow:_cursorY from:_cursorX to:end];[self markDamageX:_cursorX y:_cursorY width:end-_cursorX height:1]; }
- (void)deleteCharacters:(int)n { NSUInteger count=MIN((NSUInteger)n,_cols-_cursorX);TCell b=[self blankCell],*row=[self cellsForRow:_cursorY];memmove(row+_cursorX,row+_cursorX+count,(_cols-_cursorX-count)*sizeof(TCell));for(NSUInteger x=_cols-count;x<_cols;x++)row[x]=b;[self sanitizeWideCellsInRow:row];if(_underlineStyles){uint8_t *styles=_underlineStyles+[self physicalRowForRow:_cursorY]*_cols;memmove(styles+_cursorX,styles+_cursorX+count,_cols-_cursorX-count);memset(styles+_cols-count,0,count);}[self clearLinksInRow:_cursorY from:_cursorX to:_cols];[self markDamageX:_cursorX y:_cursorY width:_cols-_cursorX height:1]; }
- (void)insertCharacters:(int)n { NSUInteger count=MIN((NSUInteger)n,_cols-_cursorX);TCell b=[self blankCell],*row=[self cellsForRow:_cursorY];memmove(row+_cursorX+count,row+_cursorX,(_cols-_cursorX-count)*sizeof(TCell));for(NSUInteger x=_cursorX;x<_cursorX+count;x++)row[x]=b;[self sanitizeWideCellsInRow:row];if(_underlineStyles){uint8_t *styles=_underlineStyles+[self physicalRowForRow:_cursorY]*_cols;memmove(styles+_cursorX+count,styles+_cursorX,_cols-_cursorX-count);memset(styles+_cursorX,0,count);}[self clearLinksInRow:_cursorY from:_cursorX to:_cols];[self markDamageX:_cursorX y:_cursorY width:_cols-_cursorX height:1]; }
...
- (void)finishOSC:(NSString *)osc __attribute__((objc_direct)) {
    ...
    if([osc hasPrefix:@"777;term-keybinding-capture;"]){NSString *token=[osc substringFromIndex:28];if(token.length&&token.length<=64){__weak typeof(self) weakSelf=self;void(^activate)(void)=^{__strong typeof(weakSelf) self=weakSelf;if(!self)return;self->_keybindingCaptureToken=[token copy];[self sendString:[NSString stringWithFormat:@"TERMATICABIND:READY:%@\n",token]];__weak typeof(self) expiringSelf=self;dispatch_after(dispatch_time(DISPATCH_TIME_NOW,35*NSEC_PER_SEC),dispatch_get_main_queue(),^{__strong typeof(expiringSelf) self=expiringSelf;if(self&&[self->_keybindingCaptureToken isEqual:token])self->_keybindingCaptureToken=nil;});};if(NSThread.isMainThread)activate();else dispatch_async(dispatch_get_main_queue(),activate);}return;}
    if([osc hasPrefix:@"777;term-keybinding-cancel;"]){NSString *token=[osc substringFromIndex:27];__weak typeof(self) weakSelf=self;void(^cancel)(void)=^{__strong typeof(weakSelf) self=weakSelf;if(self&&[self->_keybindingCaptureToken isEqual:token])self->_keybindingCaptureToken=nil;};if(NSThread.isMainThread)cancel();else dispatch_async(dispatch_get_main_queue(),cancel);return;}
...
- (TRenderSnapshot *)renderSnapshot {
    @synchronized(self){
        for(NSUInteger row=0;row<_rows;row++)[self sanitizeWideCellsInRow:[self cellsForRow:row] logicalRow:row];
        ...
        BOOL cursorActive=self.activeTerminal,cursorFocused=cursorActive&&self.window.isKeyWindow&&self.window.firstResponder==self,pinnedInlineCursor=_inlineViewportMode&&_inlineViewportTop>0&&_cursorY>=_inlineViewportTop,cursorShown=_cursorVisible&&(!_cursorBlink||_cursorBlinkVisible)&&(_historyOffset==0||pinnedInlineCursor)&&cursorFocused;
        NSFont *fallback=[NSFont monospacedSystemFontOfSize:self.config.fontSize weight:NSFontWeightRegular];
        NSDictionary *style=@{@"foreground":@(TRGB(self.config.foreground)),@"background":@(TRGB(self.config.background)),@"backgroundAlpha":@0,@"cursor":@(TRGB(self.config.cursor)),@"accent":@(TRGB(self.config.accent)),@"selection":@(TRGB(self.config.selection)),@"plainPalette":_cachedPlainPalette,@"colorize":@(self.config.colorizePlainText),@"font":_font?:fallback,@"boldFont":_boldFont?:_font?:fallback,@"italicFont":_italicFont?:_font?:fallback,@"left":@(self.config.padding+self.leadingOverlayInset),@"top":@(self.config.padding+self.topContentInset),@"cursorStyle":_applicationCursorStyle?:self.config.cursorStyle?:@"block",@"cursorFocused":@(cursorFocused),@"cursorThickness":@(self.config->cursorThickness),@"cursorBlockOpacity":@(self.config->cursorBlockOpacity),@"cursorInactiveOpacity":@(self.config->cursorInactiveOpacity),@"scrollbarWidth":@(self.config->scrollbarWidth),@"scrollbarMargin":@(self.config->scrollbarMargin),@"scrollbarMinimumThumb":@(self.config->scrollbarMinimumThumb),@"scrollbarOpacity":@(self.config->scrollbarOpacity),@"glow":@(self.config.glow),@"scanlines":@(self.config.scanlines),@"scanlineSpacing":@(self.config->scanlineSpacing),@"scanlineThickness":@(self.config->scanlineThickness),@"vignette":@(self.config.vignette),@"vignetteLayers":@(self.config->vignetteLayers),@"tiled":@(self.tiledRendering)};
...
- (NSRect)pixelAlignedBlockRect:(TBlockElementRect)part cellX:(CGFloat)cellX cellY:(CGFloat)cellY width:(CGFloat)width height:(CGFloat)height scale:(CGFloat)scale {
    scale=MAX((CGFloat)1,scale);CGFloat x0=round((cellX+part.x*width)*scale)/scale,x1=round((cellX+(part.x+part.width)*width)*scale)/scale,y0=round((cellY+part.y*height)*scale)/scale,y1=round((cellY+(part.y+part.height)*height)*scale)/scale;return NSMakeRect(x0,y0,x1-x0,y1-y0);
}
- (void)drawBlockUnderlineAtX:(CGFloat)x y:(CGFloat)y width:(CGFloat)width font:(NSFont *)font foreground:(uint32_t)foreground style:(uint8_t)style scale:(CGFloat)scale {
    scale=MAX((CGFloat)1,scale);CGFloat thickness=MAX(1.0/scale,font.underlineThickness)*(style==2?2:1),position=0;[TColor(foreground)setFill];if(style<=2){NSRectFill(NSMakeRect(x,y-thickness,width,thickness));return;}while(position<width){CGFloat length=style==4?1:(style==5&&fmod(position,9)>=6?1:4);length=MIN(length,width-position);NSRectFill(NSMakeRect(x+position,y-thickness,length,thickness));position+=style==4?3:(style==5?(length==1?3:6):6);}
}
- (void)drawRect:(NSRect)dirtyRect {
    ...
    uint32_t defaultForeground=[style[@"foreground"] unsignedIntValue],defaultBackground=[style[@"background"] unsignedIntValue],cursorColor=[style[@"cursor"] unsignedIntValue],accent=[style[@"accent"] unsignedIntValue],selectionColor=[style[@"selection"] unsignedIntValue];NSString *cursorStyle=style[@"cursorStyle"]?:@"block";BOOL drawCursor=snapshot.cursorVisible;if(_intelCursorOverlay&&!_metalBackend)drawCursor=NO;BOOL blockCursor=drawCursor&&[cursorStyle isEqual:@"block"];
    [[TColor(defaultBackground) colorWithAlphaComponent:[style[@"backgroundAlpha"] doubleValue]]setFill];NSRectFillUsingOperation(dirtyRect,NSCompositingOperationCopy);
    ...
    for(NSInteger y=firstRow;y<lastRow;y++){[NSGraphicsContext saveGraphicsState];NSRectClip(NSMakeRect(left,top+y*cellHeight,columns*cellWidth,cellHeight));const TCell *line=allCells+y*columns;BOOL inToken=NO;NSUInteger token=0;
        ...
        NSInteger x=firstColumn;while(x<lastColumn){NSUInteger index=y*columns+x;TCell cell=line[x];BOOL inverse=(cell.flags&TInverse)!=0,cursorCell=blockCursor&&(NSUInteger)y==snapshot.cursorY&&(NSUInteger)x==snapshot.cursorX;uint32_t background=cursorCell?cursorColor:(inverse?(cell.fg==TDefaultColor?defaultForeground:cell.fg):(cell.bg==TDefaultColor?defaultBackground:cell.bg));if(!cursorCell&&searched[index])background=accent;BOOL paintsBackground=cursorCell||TRenderShouldPaintCellBackground(cell.bg,TDefaultColor,inverse,background,defaultBackground);NSInteger kind=cursorCell?4:(selected[index]?1:(searched[index]?3:(paintsBackground?2:0))),start=x;x++;while(x<lastColumn){NSUInteger nextIndex=y*columns+x;TCell next=line[x];BOOL nextInverse=(next.flags&TInverse)!=0,nextCursor=blockCursor&&(NSUInteger)y==snapshot.cursorY&&(NSUInteger)x==snapshot.cursorX;uint32_t nextBackground=nextCursor?cursorColor:(nextInverse?(next.fg==TDefaultColor?defaultForeground:next.fg):(next.bg==TDefaultColor?defaultBackground:next.bg));if(!nextCursor&&searched[nextIndex])nextBackground=accent;BOOL nextPaintsBackground=nextCursor||TRenderShouldPaintCellBackground(next.bg,TDefaultColor,nextInverse,nextBackground,defaultBackground);NSInteger nextKind=nextCursor?4:(selected[nextIndex]?1:(searched[nextIndex]?3:(nextPaintsBackground?2:0)));if(nextKind!=kind||((kind==2||kind==4)&&nextBackground!=background))break;x++;}if(kind){[TColor(kind==4?cursorColor:(kind==1?selectionColor:(kind==3?accent:background)))setFill];NSRectFill(NSMakeRect(left+start*cellWidth,top+y*cellHeight,(x-start)*cellWidth,cellHeight));}}
        if(drawCursor&&(NSUInteger)y==snapshot.cursorY&&!blockCursor){[TColor(cursorColor)setFill];NSRectFill(TCursorRect(snapshot));}
        x=firstColumn;while(x<lastColumn){NSUInteger index=y*columns+x;TCell cell=line[x];if(cell.flags&TContinuation){x++;continue;}BOOL inverse=(cell.flags&TInverse)!=0,cursorCell=blockCursor&&(NSUInteger)y==snapshot.cursorY&&(NSUInteger)x==snapshot.cursorX;uint32_t foreground=cursorCell||searched[index]?defaultBackground:(inverse?(cell.bg==TDefaultColor?defaultBackground:cell.bg):(cell.fg==TDefaultColor?plainForegrounds[x]:cell.fg));uint8_t flags=(cell.flags&TStyleMask)|(linked[index]?TUnderline:0),underline=underlines[index];TBlockElementRect blockRects[2];NSUInteger blockCount=TBlockElementRects(cell.ch,blockRects);if(blockCount){[TColor(foreground)setFill];CGFloat cellX=left+x*cellWidth,cellY=top+y*cellHeight;for(NSUInteger block=0;block<blockCount;block++)NSRectFill([self pixelAlignedBlockRect:blockRects[block] cellX:cellX cellY:cellY width:cellWidth height:cellHeight scale:snapshot.metrics.scale]);if(flags&TUnderline){NSFont *font=(flags&TBold)?style[@"boldFont"]:((flags&TItalic)?style[@"italicFont"]:style[@"font"]);if(!font)font=[NSFont monospacedSystemFontOfSize:11 weight:NSFontWeightRegular];[self drawBlockUnderlineAtX:cellX y:cellY+font.ascender-font.underlinePosition width:cellWidth font:font foreground:foreground style:MAX((uint8_t)1,underline) scale:snapshot.metrics.scale];}x++;continue;}if((cell.flags&(TWide|TCluster))||cell.ch>=0x80){NSString *text=[self stringForSnapshot:snapshot codepoint:cell.ch];NSUInteger span=(cell.flags&TWide)?2:1;[NSGraphicsContext saveGraphicsState];NSRectClip(NSMakeRect(left+x*cellWidth,top+y*cellHeight,span*cellWidth,cellHeight));[text drawAtPoint:NSMakePoint(left+x*cellWidth,top+y*cellHeight) withAttributes:[self wideSnapshotTextAttributes:snapshot foreground:foreground flags:flags shadow:shadow underlineStyle:underline]];[NSGraphicsContext restoreGraphicsState];x+=span;continue;}NSInteger start=x;NSUInteger length=0;BOOL hasGlyph=NO;while(x<lastColumn){NSUInteger nextIndex=y*columns+x;TCell next=line[x];if(next.flags&(TWide|TCluster|TContinuation)||next.ch>=0x80)break;BOOL nextInverse=(next.flags&TInverse)!=0,nextCursor=blockCursor&&(NSUInteger)y==snapshot.cursorY&&(NSUInteger)x==snapshot.cursorX;uint32_t nextForeground=nextCursor||searched[nextIndex]?defaultBackground:(nextInverse?(next.bg==TDefaultColor?defaultBackground:next.bg):(next.fg==TDefaultColor?plainForegrounds[x]:next.fg));uint8_t nextFlags=(next.flags&TStyleMask)|(linked[nextIndex]?TUnderline:0);uint8_t nextUnderline=underlines[nextIndex];if(nextForeground!=foreground||nextFlags!=flags||nextUnderline!=underline)break;uint32_t codepoint=next.ch?:' ';if(codepoint!=' ')hasGlyph=YES;length=TAppendUTF16(glyphs,length,codepoint);x++;}if(hasGlyph&&length){NSString *text=CFBridgingRelease(CFStringCreateWithCharactersNoCopy(NULL,glyphs,length,kCFAllocatorNull));[NSGraphicsContext saveGraphicsState];NSRectClip(NSMakeRect(left+start*cellWidth,top+y*cellHeight,(x-start)*cellWidth,cellHeight));[text drawAtPoint:NSMakePoint(left+start*cellWidth,top+y*cellHeight) withAttributes:[self snapshotTextAttributes:snapshot foreground:foreground flags:flags shadow:shadow underlineStyle:underline]];[NSGraphicsContext restoreGraphicsState];}}[NSGraphicsContext restoreGraphicsState];
    }
...
- (BOOL)shouldForwardApplicationMouseWithModifiers:(NSEventModifierFlags)modifiers {
    // A child that explicitly enables a terminal mouse mode owns ordinary mouse
    // input. Shift is the conventional local-selection/context-menu escape hatch,
    // while Command remains reserved for Termatica links and tiled-pane dragging.
    return _mouseTrackingMode&&!(modifiers&(NSEventModifierFlagShift|NSEventModifierFlagCommand));
}
...
- (void)scrollWheel:(NSEvent *)event {
    ...
    if(event.hasPreciseScrollingDeltas){BOOL began=(event.phase&NSEventPhaseBegan)||(event.momentumPhase&NSEventPhaseBegan);lines=TAccumulatePreciseScroll(raw,_cellHeight,began,&_scrollAccumulator);if(!lines&&(event.phase&(NSEventPhaseEnded|NSEventPhaseCancelled))&&fabs(_scrollAccumulator)>=0.12){lines=_scrollAccumulator>0?1:-1;_scrollAccumulator=0;}}
...
- (NSInteger)kittyModifierForFlags:(NSEventModifierFlags)flags {
    NSEventModifierFlags mods=flags&NSEventModifierFlagDeviceIndependentFlagsMask;NSUInteger value=0;if(mods&NSEventModifierFlagShift)value|=1;if(mods&NSEventModifierFlagOption)value|=2;if(mods&NSEventModifierFlagControl)value|=4;if(mods&NSEventModifierFlagCommand)value|=8;if(mods&NSEventModifierFlagCapsLock)value|=64;return (NSInteger)value+1;
}
- (NSString *)kittyTextCodepoints:(NSString *)text {
    NSData *data=[text dataUsingEncoding:NSUTF32LittleEndianStringEncoding];const uint32_t *values=data.bytes;NSMutableArray<NSString *> *parts=[NSMutableArray array];for(NSUInteger index=0;index<data.length/4;index++){uint32_t value=values[index];if(value>=32&&!(value>=127&&value<=159))[parts addObject:[NSString stringWithFormat:@"%u",value]];}return [parts componentsJoinedByString:@":"];
}
- (NSString *)kittySequenceForEvent:(NSEvent *)event eventType:(NSUInteger)eventType code:(uint32_t)forcedCode {
    uint32_t code=forcedCode?:[self kittyCodeForKey:event.keyCode];if(!code)code=[self firstScalar:event.charactersIgnoringModifiers.lowercaseString];if(!code&&event.characters.length)code=[self firstScalar:event.characters];
    NSMutableString *key=[NSMutableString stringWithFormat:@"%u",code];NSEventModifierFlags mods=event.modifierFlags&NSEventModifierFlagDeviceIndependentFlagsMask;
    if((_kittyKeyboardFlags&4)&&(mods&NSEventModifierFlagShift)){uint32_t shifted=[self firstScalar:event.characters];if(shifted&&shifted!=code)[key appendFormat:@":%u",shifted];}
    NSInteger modifier=[self kittyModifierForFlags:event.modifierFlags];NSMutableString *sequence=[NSMutableString stringWithFormat:@"\033[%@;%ld",key,(long)modifier];if((_kittyKeyboardFlags&2)&&eventType!=1)[sequence appendFormat:@":%lu",(unsigned long)eventType];
    if((_kittyKeyboardFlags&8)&&(_kittyKeyboardFlags&16)&&eventType!=3){NSString *text=[self kittyTextCodepoints:event.characters];if(text.length)[sequence appendFormat:@";%@",text];}
    [sequence appendString:@"u"];return sequence;
}
- (NSString *)functionalKeySequenceForKeyCode:(unsigned short)key modifier:(NSInteger)modifier {
...
- (BOOL)hasPendingKeybindingCapture {return _keybindingCaptureToken.length>0;}
- (BOOL)captureKeybindingEvent:(NSEvent *)event {
    if(!_keybindingCaptureToken.length)return NO;NSString *token=_keybindingCaptureToken;_keybindingCaptureToken=nil;NSString *shortcut=TShortcutForEvent(event);if([shortcut isEqual:@"escape"])shortcut=@"";[self sendString:[NSString stringWithFormat:@"TERMATICABIND:VALUE:%@:%@\n",token,shortcut?:@""]];return YES;
}
- (void)startCursorBlink {
    ...
    __weak typeof(self) weakSelf=self;
    dispatch_source_set_event_handler(_blinkTimer,^{__strong typeof(weakSelf) self=weakSelf;if(!self)return;self->_cursorBlinkVisible=!self->_cursorBlinkVisible;[self invalidateCursorPresentation];});
    dispatch_resume(_blinkTimer);
}
- (void)keyDown:(NSEvent *)e {
    if(_cursorBlink&&!_cursorBlinkVisible){_cursorBlinkVisible=YES;[self invalidateCursorPresentation];}
    ...
    if(e.modifierFlags&NSEventModifierFlagCommand){if((e.modifierFlags&NSEventModifierFlagShift)&&[e.charactersIgnoringModifiers.lowercaseString isEqual:@"f"]){[self searchScrollback];return;}if((e.modifierFlags&NSEventModifierFlagShift)&&[e.charactersIgnoringModifiers.lowercaseString isEqual:@"d"]){void(^cb)(void)=self.verticalSplitShortcut;if(cb)cb();return;}if([e.charactersIgnoringModifiers.lowercaseString isEqual:@"d"]){void(^cb)(void)=self.splitShortcut;if(cb)cb();return;}if([e.charactersIgnoringModifiers.lowercaseString isEqual:@"]"]){void(^cb)(void)=self.nextSplitShortcut;if(cb)cb();return;}if([e.charactersIgnoringModifiers.lowercaseString isEqual:@"["]){void(^cb)(void)=self.prevSplitShortcut;if(cb)cb();return;}[super keyDown:e];return;}
    NSString *s=nil; unsigned short k=e.keyCode;NSEventModifierFlags mods=e.modifierFlags&NSEventModifierFlagDeviceIndependentFlagsMask;NSInteger modifier=[self kittyModifierForFlags:mods];NSUInteger eventType=e.isARepeat&&(_kittyKeyboardFlags&2)?2:1;
    ...
    if(_kittyKeyboardFlags&8){[self sendString:[self kittySequenceForEvent:e eventType:eventType code:0]];_hasSelection=NO;[self setNeedsDisplay:YES];return;}
    if((_kittyKeyboardFlags&1)&&(k==53||(mods&(NSEventModifierFlagOption|NSEventModifierFlagControl)))){[self sendString:[self kittySequenceForEvent:e eventType:eventType code:0]];_hasSelection=NO;[self setNeedsDisplay:YES];return;}
...
- (void)keyUp:(NSEvent *)e {
    if(!(_kittyKeyboardFlags&2))return;NSEventModifierFlags mods=e.modifierFlags&NSEventModifierFlagDeviceIndependentFlagsMask;uint32_t functional=[self kittyCodeForKey:e.keyCode];BOOL c0=e.keyCode==36||e.keyCode==76||e.keyCode==48||e.keyCode==51;if(!(_kittyKeyboardFlags&8)&&(c0||(!functional&&!((_kittyKeyboardFlags&1)&&(e.keyCode==53||(mods&(NSEventModifierFlagOption|NSEventModifierFlagControl)))))))return;[self sendString:[self kittySequenceForEvent:e eventType:3 code:functional]];
}
- (void)flagsChanged:(NSEvent *)e {
    NSEventModifierFlags current=e.modifierFlags&NSEventModifierFlagDeviceIndependentFlagsMask;if(!(_kittyKeyboardFlags&8)){[super flagsChanged:e];return;}uint32_t code=0;NSEventModifierFlags flag=0;switch(e.keyCode){case 56:code=57441;flag=NSEventModifierFlagShift;break;case 60:code=57447;flag=NSEventModifierFlagShift;break;case 59:code=57442;flag=NSEventModifierFlagControl;break;case 62:code=57448;flag=NSEventModifierFlagControl;break;case 58:code=57443;flag=NSEventModifierFlagOption;break;case 61:code=57449;flag=NSEventModifierFlagOption;break;case 55:code=57444;flag=NSEventModifierFlagCommand;break;case 54:code=57450;flag=NSEventModifierFlagCommand;break;case 57:code=57358;flag=NSEventModifierFlagCapsLock;break;default:[super flagsChanged:e];return;}NSUInteger eventType=(current&flag)?1:3;[self sendString:[self kittySequenceForEvent:e eventType:eventType code:code]];
}
- (NSString *)workingDirectory {
    if (_pid > 0) {
...
- (NSDictionary *)diagnosticState {
    @synchronized(self){return @{
      ...
      @"requestedRenderer":_requestedRenderer?:self.config.renderer?:@"appkit",@"renderer":_renderBackend.name?:@"none",@"renderGeneration":@(_renderBackend.lastPresentedGeneration),
      @"metalFailed":@(_metalFailed),@"metalQuarantined":@(_metalFailed),@"metalRetryAvailable":@(_metalFailed&&[(_requestedRenderer?:self.config.renderer) isEqual:@"metal"]),@"metalRetryRequiresExplicitAction":@(_metalFailed),@"metalFailureCount":@(_metalFailureCount),
      @"metalQuarantineBypassCount":@(_metalQuarantineBypassCount),@"metalRetryAttemptCount":@(_metalRetryAttemptCount),@"metalSuccessfulRetryCount":@(_metalSuccessfulRetryCount),@"lastMetalFailureStage":_lastMetalFailureStage?:@"none",@"secondsSinceMetalFailure":@(_lastMetalFailureAt>0?MAX(0,CFAbsoluteTimeGetCurrent()-_lastMetalFailureAt):-1),@"secondsSinceMetalRetry":@(_lastMetalRetryAt>0?MAX(0,CFAbsoluteTimeGetCurrent()-_lastMetalRetryAt):-1),
      @"presentationRecoveryCount":@(_presentationRecoveryCount),@"presentationRecoveryReasons":_presentationRecoveryReasons.copy?:@{},@"observedScreenChangeCount":@(_observedScreenChangeCount),@"observedBackingScaleChangeCount":@(_observedBackingScaleChangeCount),@"lastPresentationRecovery":_lastPresentationRecovery?:@"none",@"secondsSincePresentationRecovery":@(_lastPresentationRecoveryAt>0?MAX(0,CFAbsoluteTimeGetCurrent()-_lastPresentationRecoveryAt):-1),
      @"metalFrameChecksum":@(_metalBackend.lastFrameChecksum),@"metalFrameVaried":@(_metalBackend.lastFrameVariedPixels),@"rendererSurfaceHidden":@(_metalBackend?_metalBackend.presentationLayer.hidden:NO),@"snapshotBuildMs":@(_lastSnapshotBuildMilliseconds),
      @"metalSnapshotWaitMs":@(_metalBackend.lastSnapshotWaitMilliseconds),@"metalCPUEncodeMs":@(_metalBackend.lastCPUEncodeMilliseconds),@"metalGPUExecutionMs":@(_metalBackend.lastGPUExecutionMilliseconds),
      @"metalGPUCompletionMs":@(_metalBackend.lastGPUCompletionMilliseconds),@"metalPresentIntervalMs":@(_metalBackend.lastPresentIntervalMilliseconds)
    };}
}
...
@property (copy) void (^scrollTabs)(NSInteger direction);
@end

@implementation TTabRailView
- (BOOL)isFlipped{return YES;}
- (BOOL)isOpaque{return NO;}
- (void)scrollWheel:(NSEvent *)event {CGFloat delta=fabs(event.scrollingDeltaY)>=fabs(event.scrollingDeltaX)?event.scrollingDeltaY:event.scrollingDeltaX;if(fabs(delta)<0.01)return;void(^handler)(NSInteger)=self.scrollTabs;if(handler)handler(delta>0?-1:1);}
...
static CGFloat TScreenAlignedValue(CGFloat value,CGFloat scale) {return round(value*MAX(1,scale))/MAX(1,scale);}
static NSRect TUsableScreenFrame(NSRect visibleFrame,CGFloat inset,CGFloat scale) {
    CGFloat horizontal=MIN(MAX(0,inset),MAX(0,(NSWidth(visibleFrame)-320)/2)),vertical=MIN(MAX(0,inset),MAX(0,(NSHeight(visibleFrame)-200)/2));
    NSRect frame=NSInsetRect(visibleFrame,horizontal,vertical);frame.origin.x=TScreenAlignedValue(frame.origin.x,scale);frame.origin.y=TScreenAlignedValue(frame.origin.y,scale);frame.size.width=MAX(1,TScreenAlignedValue(NSMaxX(visibleFrame)-horizontal-frame.origin.x,scale));frame.size.height=MAX(1,TScreenAlignedValue(NSMaxY(visibleFrame)-vertical-frame.origin.y,scale));return frame;
}
static NSRect TConstrainWindowFrame(NSRect frame,NSRect usable,NSSize minimum,CGFloat scale) {
    frame.size.width=MIN(NSWidth(usable),MAX(MIN(minimum.width,NSWidth(usable)),NSWidth(frame)));frame.size.height=MIN(NSHeight(usable),MAX(MIN(minimum.height,NSHeight(usable)),NSHeight(frame)));
    frame.origin.x=MIN(MAX(NSMinX(usable),frame.origin.x),NSMaxX(usable)-NSWidth(frame));frame.origin.y=MIN(MAX(NSMinY(usable),frame.origin.y),NSMaxY(usable)-NSHeight(frame));
    frame.origin.x=TScreenAlignedValue(frame.origin.x,scale);frame.origin.y=TScreenAlignedValue(frame.origin.y,scale);frame.size.width=TScreenAlignedValue(frame.size.width,scale);frame.size.height=TScreenAlignedValue(frame.size.height,scale);return frame;
}
static NSRect TInitialWindowFrameForScreen(NSRect visibleFrame,NSSize configured,NSSize minimum,CGFloat inset,CGFloat scale) {
    NSRect usable=TUsableScreenFrame(visibleFrame,inset,scale);CGFloat width=MAX(configured.width,floor(NSWidth(usable)*0.84)),height=MAX(configured.height,floor(NSHeight(usable)*0.82));NSRect frame=NSMakeRect(NSMidX(usable)-width/2,NSMidY(usable)-height/2,width,height);return TConstrainWindowFrame(frame,usable,minimum,scale);
}
static NSRect TRemapWindowFrameToScreen(NSRect frame,NSRect previousVisible,NSRect nextVisible,NSSize minimum,CGFloat inset,CGFloat scale) {
    if(NSWidth(previousVisible)<1||NSHeight(previousVisible)<1)return TInitialWindowFrameForScreen(nextVisible,frame.size,minimum,inset,scale);NSRect previous=TUsableScreenFrame(previousVisible,inset,scale),next=TUsableScreenFrame(nextVisible,inset,scale);
    CGFloat width=NSWidth(frame)*NSWidth(next)/NSWidth(previous),height=NSHeight(frame)*NSHeight(next)/NSHeight(previous);NSPoint center=NSMakePoint(NSMidX(frame),NSMidY(frame));NSRect mapped=NSMakeRect(center.x-width/2,center.y-height/2,width,height);return TConstrainWindowFrame(mapped,next,minimum,scale);
}
...
- (instancetype)initWithConfig:(TConfig *)config extensions:(TExtensionHost *)extensions directory:(NSString *)directory command:(NSString *)command;
- (void)addTab;
- (TTerminalView *)addTabWithDirectory:(NSString *)directory command:(NSString *)command;
- (void)addCommandTTerminal;
- (void)addSplitWithVerticalDivider:(BOOL)verticalDivider;
- (TTerminalView *)addSplitWithVerticalDivider:(BOOL)verticalDivider directory:(NSString *)directory command:(NSString *)command;
- (void)focusAdjacentSplit:(NSInteger)direction;
- (void)closeTab;
- (void)selectTabNumber:(NSInteger)number;
- (BOOL)focusAutomationTab:(NSInteger)number;
- (BOOL)focusAutomationPane:(NSInteger)number;
- (NSArray<TTerminalView *> *)terminalsInGroupOfTerminal:(TTerminalView *)terminal;
- (NSDictionary *)automationStatus;
- (void)reloadConfig;
...
- (void)layoutTabs;
- (BOOL)moveLayoutPositionOfTerminal:(TTerminalView *)first toTerminal:(TTerminalView *)second;
- (void)handleDisplayParametersChanged;
@end

@implementation TWindowController { NSString *_cwd; NSView *_root; NSVisualEffectView *_effect; NSView *_tileSurface; CAShapeLayer *_tileSurfaceLayer; TTabRailView *_tabRail; TTabEdgeView *_tabEdge; NSMutableArray<TTabButton *> *_tabButtons; NSMutableArray<TTerminalView *> *_hyprlandRootOrder; BOOL _animateTabLayout; BOOL _hyprlandApplied,_adjustingScreenFrame; NSRect _preHyprlandFrame,_preHyprlandVisibleFrame,_lastScreenVisibleFrame; TTerminalView *_draggingTerminal; TTerminalView *_dragPositionTarget; NSPoint _dragOffset; TTerminalView *_enteringTerminal; NSTimer *_tabHideTimer; NSTrackingArea *_tabHoverArea; NSRect _tabRailTargetFrame; BOOL _tabRailVisible; BOOL _mouseInTabArea; BOOL _revealRailAfterLayout; }
- (instancetype)initWithConfig:(TConfig *)config extensions:(TExtensionHost *)extensions {return [self initWithConfig:config extensions:extensions directory:nil command:nil];}
- (instancetype)initWithConfig:(TConfig *)config extensions:(TExtensionHost *)extensions directory:(NSString *)directory command:(NSString *)command {
    if((self=[super initWithWindow:nil])){_config=config;_extensions=extensions;_terminals=[NSMutableArray array];_tabButtons=[NSMutableArray array];
        TTerminalView *terminal=[self newTerminalDeferred:!config.hyprlandLayout];terminal.launchDirectory=directory;[_terminals addObject:terminal];[terminal startShell];if(command.length)[self sendAutomationCommand:command toTerminal:terminal];self.terminal=terminal;self.extensions.activeTerminal=terminal;
        ...
        window.delegate=(id)self;window.title=@"Termatica";window.titleVisibility=NSWindowTitleHidden;window.titlebarAppearsTransparent=YES;window.styleMask|=NSWindowStyleMaskFullSizeContentView;window.minSize=NSMakeSize(config->minimumWindowWidth,config->minimumWindowHeight);window.tabbingMode=NSWindowTabbingModeDisallowed;window.movableByWindowBackground=NO;window.restorable=NO;NSScreen *launchScreen=NSApp.keyWindow.screen?:NSScreen.mainScreen?:NSScreen.screens.firstObject;if(launchScreen){CGFloat scale=launchScreen.backingScaleFactor?:1;_lastScreenVisibleFrame=launchScreen.visibleFrame;[window setFrame:TInitialWindowFrameForScreen(launchScreen.visibleFrame,NSMakeSize(config->initialWindowWidth,config->initialWindowHeight),window.minSize,config.screenInset,scale) display:NO];}else [window center];
        ...
        _tileSurface=[[NSView alloc]initWithFrame:_root.bounds];_tileSurface.autoresizingMask=NSViewWidthSizable|NSViewHeightSizable;_tileSurface.wantsLayer=YES;_tileSurface.hidden=YES;_tileSurfaceLayer=[CAShapeLayer layer];_tileSurfaceLayer.actions=@{@"path":NSNull.null,@"fillColor":NSNull.null,@"position":NSNull.null,@"bounds":NSNull.null};[_tileSurface.layer addSublayer:_tileSurfaceLayer];[_root addSubview:_tileSurface];
        _tabRail=[[TTabRailView alloc]initWithFrame:NSZeroRect];_tabRail.config=config;_tabRail.wantsLayer=YES;__weak typeof(self) railWeakSelf=self;_tabRail.scrollTabs=^(NSInteger direction){__strong typeof(railWeakSelf) self=railWeakSelf;if(!self||!self.terminals.count)return;NSUInteger current=[self.terminals indexOfObject:self.terminal];if(current==NSNotFound)current=0;NSInteger next=((NSInteger)current+direction+(NSInteger)self.terminals.count)%(NSInteger)self.terminals.count;[self selectTabNumber:next+1];};[_root addSubview:_tabRail];
...
- (TTerminalView *)rootTerminalForTerminal:(TTerminalView *)terminal {TTerminalView *root=terminal;NSUInteger depth=0;while(root.splitAnchor&&[_terminals containsObject:root.splitAnchor]&&depth++<_terminals.count)root=root.splitAnchor;return root;}
- (NSArray<TTerminalView *> *)terminalsInGroupOfTerminal:(TTerminalView *)terminal {TTerminalView *root=[self rootTerminalForTerminal:terminal];if(!root)return @[];NSMutableArray<TTerminalView *> *group=[NSMutableArray array];for(TTerminalView *item in _terminals)if([self rootTerminalForTerminal:item]==root)[group addObject:item];return group;}
- (NSArray<TTerminalView *> *)visibleLayoutTerminals {return self.config.hyprlandLayout?[_terminals copy]:[self terminalsInGroupOfTerminal:self.terminal];}
- (BOOL)usesTiledLayout {return self.config.hyprlandLayout?_terminals.count>1:[self terminalsInGroupOfTerminal:self.terminal].count>1;}
- (BOOL)requiresStableTiledCompositing {return [self usesTiledLayout]&&(self.config.backgroundOpacity<0.999||self.config.windowOpacity<0.999||(self.config.blur&&self.config.hyprlandBlur));}
- (BOOL)tabRailAvailable {return _terminals.count>1&&!self.config.hyprlandLayout;}
- (BOOL)animationsEnabled {return self.config.tabAnimations&&!NSWorkspace.sharedWorkspace.accessibilityDisplayShouldReduceMotion;}
...
- (void)revealTabRail {
    if(![self tabRailAvailable]||NSIsEmptyRect(_tabRailTargetFrame)){[self suppressTabRail];return;}[_tabHideTimer invalidate];_tabHideTimer=nil;BOOL wasVisible=_tabRailVisible,animated=[self animationsEnabled];_tabRailVisible=YES;[self updateTerminalTabInsets];[_tabRail.layer removeAllAnimations];[_tabEdge.layer removeAllAnimations];_tabRail.hidden=NO;if(!wasVisible){NSRect start=animated?NSOffsetRect(_tabRailTargetFrame,-6,0):_tabRailTargetFrame;_tabRail.frame=start;_tabRail.alphaValue=animated?0.65:1;}_tabEdge.hidden=NO;CAMediaTimingFunction *settle=[CAMediaTimingFunction functionWithControlPoints:0.16f :1.0f :0.3f :1.0f];[NSAnimationContext runAnimationGroup:^(NSAnimationContext *context){context.duration=animated?[self animationDuration:self.config->terminalAnimationDuration]:0;context.timingFunction=settle;_tabRail.animator.frame=_tabRailTargetFrame;_tabRail.animator.alphaValue=1;_tabEdge.animator.alphaValue=0;} completionHandler:^{if(self->_tabRailVisible)self->_tabEdge.hidden=YES;}];[self scheduleTabRailHide];
}
- (void)hideTabRail {
    if(!_tabRailVisible||_terminals.count<2)return;if(_mouseInTabArea){[self scheduleTabRailHide];return;}[_tabHideTimer invalidate];_tabHideTimer=nil;BOOL animated=[self animationsEnabled];_tabRailVisible=NO;NSRect collapsed=_tabRailTargetFrame;collapsed.origin.x=-NSWidth(collapsed)+self.config->tabCollapsedPeek;NSRect exitFrame=animated?NSOffsetRect(_tabRailTargetFrame,-6,0):collapsed;_tabEdge.hidden=NO;_tabEdge.alphaValue=0;[_tabRail.layer removeAllAnimations];[_tabEdge.layer removeAllAnimations];CAMediaTimingFunction *settle=[CAMediaTimingFunction functionWithControlPoints:0.16f :1.0f :0.3f :1.0f];[NSAnimationContext runAnimationGroup:^(NSAnimationContext *context){context.duration=animated?[self animationDuration:self.config->terminalAnimationDuration]:0;context.timingFunction=settle;_tabRail.animator.frame=exitFrame;_tabRail.animator.alphaValue=0;_tabEdge.animator.alphaValue=self.config->tabEdgeOpacity;} completionHandler:^{if(!self->_tabRailVisible){self->_tabRail.hidden=YES;self->_tabRail.frame=collapsed;[self updateTerminalTabInsets];}}];
}
...
- (void)assignFrame:(NSRect)frame toTerminal:(TTerminalView *)terminal frames:(NSMutableArray<NSValue *> *)frames {
    ...
    NSRect own=frame;
    for(TTerminalView *child in children){BOOL vertical=child.verticalSplit,before=child.splitBefore;CGFloat extent=vertical?NSWidth(own):NSHeight(own),gap=MIN(self.config.tileGap,MAX(0,extent-2)),half=MAX(1,floor((extent-gap)/2));NSRect childFrame=own;if(vertical){if(before){childFrame.size.width=half;own.origin.x=NSMinX(own)+half+gap;own.size.width=MAX(1,extent-gap-half);}else{own.size.width=half;childFrame.origin.x=NSMaxX(own)+gap;childFrame.size.width=MAX(1,extent-gap-half);}}else{if(before){childFrame.origin.y=NSMaxY(own)-half;childFrame.size.height=half;own.size.height=MAX(1,extent-gap-half);}else{own.origin.y=NSMaxY(own)-half;own.size.height=half;childFrame.size.height=MAX(1,extent-gap-half);}}[self assignFrame:childFrame toTerminal:child frames:frames];}
    frames[index]=[NSValue valueWithRect:own];
}
- (NSArray<TTerminalView *> *)orderedHyprlandRoots {
    NSMutableArray<TTerminalView *> *roots=[NSMutableArray array];for(TTerminalView *item in _terminals)if([self rootTerminalForTerminal:item]==item)[roots addObject:item];
    if(!_hyprlandRootOrder)_hyprlandRootOrder=[NSMutableArray array];NSIndexSet *stale=[_hyprlandRootOrder indexesOfObjectsPassingTest:^BOOL(TTerminalView *item,NSUInteger index,BOOL *stop){return ![roots containsObject:item];}];if(stale.count)[_hyprlandRootOrder removeObjectsAtIndexes:stale];for(TTerminalView *root in roots)if(![_hyprlandRootOrder containsObject:root])[_hyprlandRootOrder addObject:root];return [_hyprlandRootOrder copy];
}
- (NSArray<NSValue *> *)hyprlandFrames {
    ...
    NSMutableArray<TTerminalView *> *roots=[NSMutableArray array];
    if(self.config.hyprlandLayout)[roots addObjectsFromArray:[self orderedHyprlandRoots]];
    else{TTerminalView *root=[self rootTerminalForTerminal:self.terminal];if(root)[roots addObject:root];}
    ...
    CGFloat scale=self.window.screen.backingScaleFactor?:NSScreen.mainScreen.backingScaleFactor?:1,usableHeight=MAX(1,h-gap*(rows-1)),cellHeight=usableHeight/rows;
    for(NSUInteger i=0;i<roots.count;i++){NSUInteger row=i/columns,column=i%columns,countInRow=MIN(columns,roots.count-row*columns);CGFloat usableWidth=MAX(1,w-gap*(countInRow-1)),cellWidth=usableWidth/countInRow,x0=column*(cellWidth+gap),x1=x0+cellWidth,top=row*(cellHeight+gap),bottom=top+cellHeight;x0=round(x0*scale)/scale;x1=round(x1*scale)/scale;top=round(top*scale)/scale;bottom=round(bottom*scale)/scale;[self assignFrame:NSMakeRect(x0,h-bottom,MAX(1,x1-x0),MAX(1,bottom-top)) toTerminal:roots[i] frames:frames];}
    return frames;
}
- (BOOL)moveLayoutPositionOfTerminal:(TTerminalView *)first toTerminal:(TTerminalView *)second {
    if(!first||!second||first==second||![_terminals containsObject:first]||![_terminals containsObject:second])return NO;TTerminalView *firstRoot=[self rootTerminalForTerminal:first],*secondRoot=[self rootTerminalForTerminal:second];
    if(self.config.hyprlandLayout&&firstRoot&&secondRoot&&firstRoot!=secondRoot){NSMutableArray<TTerminalView *> *order=[[self orderedHyprlandRoots] mutableCopy];NSUInteger source=[order indexOfObject:firstRoot],destination=[order indexOfObject:secondRoot];if(source==NSNotFound||destination==NSNotFound)return NO;[order removeObjectAtIndex:source];[order insertObject:firstRoot atIndex:MIN(destination,order.count)];_hyprlandRootOrder=order;return YES;}
    if(first.splitAnchor==second){first.splitBefore=!first.splitBefore;return YES;}
    if(second.splitAnchor==first){second.splitBefore=!second.splitBefore;return YES;}
    return NO;
}
- (void)updateEffectMask {
    if(!_effect)return;
    [CATransaction begin];[CATransaction setDisableActions:YES];
    if(![self usesTiledLayout]){_effect.layer.mask=nil;[CATransaction commit];return;}
    CAShapeLayer *mask=[_effect.layer.mask isKindOfClass:CAShapeLayer.class]?(CAShapeLayer *)_effect.layer.mask:[CAShapeLayer layer];mask.actions=@{@"path":NSNull.null,@"position":NSNull.null,@"bounds":NSNull.null,@"frame":NSNull.null,@"fillColor":NSNull.null};
    CGMutablePathRef path=CGPathCreateMutable();
    for(TTerminalView *terminal in [self visibleLayoutTerminals])CGPathAddRoundedRect(path,NULL,NSRectToCGRect(terminal.frame),self.config->tileCornerRadius,self.config->tileCornerRadius);
    mask.frame=_effect.bounds;mask.path=path;mask.fillColor=NSColor.blackColor.CGColor;_effect.layer.mask=mask;[CATransaction commit];CGPathRelease(path);
}
- (void)beginDraggingTerminal:(TTerminalView *)terminal event:(NSEvent *)event {
    ...
    [self focusTerminal:terminal];_draggingTerminal=terminal;_dragPositionTarget=nil;
...
- (void)dragTerminal:(TTerminalView *)terminal event:(NSEvent *)event {
    ...
    NSArray<NSValue *> *slots=[self hyprlandFrames];NSSet<TTerminalView *> *visible=[NSSet setWithArray:[self visibleLayoutTerminals]];
    TTerminalView *target=nil;CGFloat bestOverlap=0;
    for(NSUInteger index=0;index<_terminals.count&&index<slots.count;index++){
        TTerminalView *candidate=_terminals[index];if(candidate==terminal||![visible containsObject:candidate])continue;NSRect slot=slots[index].rectValue;if(NSIsEmptyRect(slot))continue;
    ...
    if(target&&target!=_dragPositionTarget){
        if([self moveLayoutPositionOfTerminal:terminal toTerminal:target]){_dragPositionTarget=target;_animateTabLayout=YES;[self layoutTabs];}
    }else if(!target){
        _dragPositionTarget=nil;
    }
    [self updateEffectMask];
}
- (void)endDraggingTerminal:(TTerminalView *)terminal event:(NSEvent *)event {
    if(terminal!=_draggingTerminal)return;terminal.autoresizingMask=NSViewWidthSizable|NSViewHeightSizable;terminal.layer.zPosition=0;terminal.layer.borderWidth=0;terminal.layer.borderColor=nil;_draggingTerminal=nil;_dragPositionTarget=nil;_animateTabLayout=YES;[self layoutTabs];[self focusTerminal:terminal];
}
- (TTerminalView *)newTerminal {
    return [self newTerminalDeferred:NO];
}
- (TTerminalView *)newTerminalDeferred:(BOOL)deferred {
    TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:NSZeroRect config:self.config deferPresentation:deferred];terminal.autoresizingMask=NSViewWidthSizable|NSViewHeightSizable;__weak typeof(self) weakSelf=self;__weak TTerminalView *weakTerminal=terminal;terminal.titleChanged=^(NSString *title){__strong typeof(weakSelf) self=weakSelf;if(self&&self.terminal==weakTerminal)self.window.title=title.length?title:@"Termatica";};terminal.cwdChanged=^(NSString *cwd){__strong typeof(weakSelf) self=weakSelf;if(self&&self.terminal==weakTerminal)self->_cwd=cwd;};terminal.focused=^{__strong typeof(weakSelf) self=weakSelf;if(self&&weakTerminal)[self focusTerminal:weakTerminal];};terminal.tileDragBegan=^(TTerminalView *tile,NSEvent *event){__strong typeof(weakSelf) self=weakSelf;if(self)[self beginDraggingTerminal:tile event:event];};terminal.tileDragMoved=^(TTerminalView *tile,NSEvent *event){__strong typeof(weakSelf) self=weakSelf;if(self)[self dragTerminal:tile event:event];};terminal.tileDragEnded=^(TTerminalView *tile,NSEvent *event){__strong typeof(weakSelf) self=weakSelf;if(self)[self endDraggingTerminal:tile event:event];};terminal.accessibilityHelp=@"Command-drag, or drag from the top padding, to rearrange the visible terminal panes.";terminal.splitShortcut=^{__strong typeof(weakSelf) self=weakSelf;if(self)[self addSplitWithVerticalDivider:NO];};terminal.verticalSplitShortcut=^{__strong typeof(weakSelf) self=weakSelf;if(self)[self addSplitWithVerticalDivider:YES];};terminal.nextSplitShortcut=^{__strong typeof(weakSelf) self=weakSelf;if(self)[self focusAdjacentSplit:1];};terminal.prevSplitShortcut=^{__strong typeof(weakSelf) self=weakSelf;if(self)[self focusAdjacentSplit:-1];};return terminal;
}
- (void)focusAdjacentSplit:(NSInteger)direction {NSArray<TTerminalView *> *group=[self visibleLayoutTerminals];if(group.count<2)return;NSUInteger index=[group indexOfObject:self.terminal];if(index==NSNotFound)index=0;NSInteger next=((NSInteger)index+direction+(NSInteger)group.count)%(NSInteger)group.count;[self focusTerminal:group[(NSUInteger)next]];}
...
- (void)focusTerminal:(TTerminalView *)terminal {if(!terminal||![_terminals containsObject:terminal])return;if(self.terminal!=terminal){TTerminalView *oldRoot=[self rootTerminalForTerminal:self.terminal];BOOL wasTiled=[self usesTiledLayout];self.terminal=terminal;_cwd=[terminal workingDirectory];TTerminalView *nextRoot=[self rootTerminalForTerminal:terminal];BOOL nowTiled=[self usesTiledLayout];if(wasTiled!=nowTiled)[self applyAppearance];else if(oldRoot!=nextRoot){[self rebuildTabs];[self layoutTabs];}else [self updateTabSelectionAnimated:YES];}for(TTerminalView *item in _terminals)item.activeTerminal=NO;[self.window displayIfNeeded];BOOL accepted=[self.window makeFirstResponder:terminal];terminal.activeTerminal=accepted&&self.window.firstResponder==terminal;self.extensions.activeTerminal=terminal;[terminal setNeedsDisplay:YES];TLog(@"cursor owner terminal=%lu responder=%@ key=%@",(unsigned long)([_terminals indexOfObject:terminal]+1),terminal.activeTerminal?@"yes":@"no",self.window.isKeyWindow?@"yes":@"no");}
- (void)animateLaunchReveal {
...
- (void)animateNewTerminal:(TTerminalView *)terminal {
    if(![self animationsEnabled])return;CAMediaTimingFunction *ease=[CAMediaTimingFunction functionWithControlPoints:0.16f :1.0f :0.3f :1.0f];
    if([self usesTiledLayout]){if([self requiresStableTiledCompositing]){terminal.layer.mask=nil;[terminal.layer removeAnimationForKey:@"termatica.hypr.snap"];}else TAnimateCenterReveal(terminal,[self animationDuration:self.config->terminalAnimationDuration],self.config->tileCornerRadius,@"termatica.terminal.center",nil);return;}
    terminal.layer.mask=nil;[terminal.layer removeAllAnimations];[terminal releaseAnimationLayer];NSUInteger index=[_terminals indexOfObject:terminal];if(_tabButtons.count>1&&index!=NSNotFound&&index<_tabButtons.count){TTabButton *button=_tabButtons[index];[button.layer removeAllAnimations];CABasicAnimation *move=[CABasicAnimation animationWithKeyPath:@"transform.translation.x"];move.fromValue=@(-3);move.toValue=@0;move.duration=[self animationDuration:self.config->layoutAnimationDuration];move.timingFunction=ease;[button.layer addAnimation:move forKey:@"termatica.tab.add.move"];CABasicAnimation *fade=[CABasicAnimation animationWithKeyPath:@"opacity"];fade.fromValue=@0.65;fade.toValue=@1;fade.duration=[self animationDuration:self.config->layoutAnimationDuration];fade.timingFunction=ease;[button.layer addAnimation:fade forKey:@"termatica.tab.add.fade"];}
}
- (void)cancelTileAnimation {_enteringTerminal=nil;}
- (void)sendAutomationCommand:(NSString *)command toTerminal:(TTerminalView *)terminal {if(!command.length)return;dispatch_after(dispatch_time(DISPATCH_TIME_NOW,100*NSEC_PER_MSEC),dispatch_get_main_queue(),^{[terminal sendString:[command stringByAppendingString:@"\n"]];});}
- (void)addTab {[self addTabWithDirectory:nil command:nil];}
- (void)addCommandTTerminal {if(!self.config.hyprlandLayout){[self addTab];return;}TTerminalView *anchor=self.terminal;NSUInteger depth=0;while(anchor.splitAnchor&&[_terminals containsObject:anchor.splitAnchor]&&depth++<_terminals.count)anchor=anchor.splitAnchor;[self addSplitWithVerticalDivider:(depth%2)==0];}
- (TTerminalView *)addTabWithDirectory:(NSString *)directory command:(NSString *)command {[self cancelTileAnimation];BOOL animate=_terminals.count>0&&_terminals.count<9,wasTiled=[self usesTiledLayout];TTerminalView *anchor=self.terminal,*anchorRoot=[self rootTerminalForTerminal:anchor],*terminal=[self newTerminal];terminal.launchDirectory=directory.length?directory:(anchor?[anchor workingDirectory]:_cwd);[terminal startShell];NSUInteger anchorIndex=[_terminals indexOfObject:anchor],insertionIndex=anchorIndex==NSNotFound?_terminals.count:anchorIndex+1;[_terminals insertObject:terminal atIndex:insertionIndex];if(self.config.hyprlandLayout){NSMutableArray<TTerminalView *> *order=[[self orderedHyprlandRoots] mutableCopy];[order removeObject:terminal];NSUInteger rootIndex=[order indexOfObject:anchorRoot];[order insertObject:terminal atIndex:rootIndex==NSNotFound?order.count:rootIndex+1];_hyprlandRootOrder=order;}[_root addSubview:terminal positioned:NSWindowBelow relativeTo:_tabRail];self.terminal=terminal;self.extensions.activeTerminal=terminal;_animateTabLayout=NO;if(wasTiled!=[self usesTiledLayout])[self applyAppearance];else{[self rebuildTabs];[self layoutTabs];}if(animate)[self animateNewTerminal:terminal];[self focusTerminal:terminal];[self sendAutomationCommand:command toTerminal:terminal];return terminal;}
- (void)addSplitWithVerticalDivider:(BOOL)verticalDivider {[self addSplitWithVerticalDivider:verticalDivider directory:nil command:nil];}
- (TTerminalView *)addSplitWithVerticalDivider:(BOOL)verticalDivider directory:(NSString *)directory command:(NSString *)command {[self cancelTileAnimation];TTerminalView *anchor=self.terminal;if(!anchor)return [self addTabWithDirectory:directory command:command];BOOL animate=_terminals.count<9,wasTiled=[self usesTiledLayout];TTerminalView *terminal=[self newTerminal];terminal.launchDirectory=directory.length?directory:([anchor workingDirectory]?:_cwd);terminal.verticalSplit=verticalDivider;terminal.splitAnchor=anchor;[terminal startShell];NSUInteger index=[_terminals indexOfObject:anchor],insertionIndex=index==NSNotFound?_terminals.count:index+1;if(index!=NSNotFound){for(NSUInteger candidateIndex=index+1;candidateIndex<_terminals.count;candidateIndex++){TTerminalView *parent=_terminals[candidateIndex].splitAnchor;NSUInteger depth=0;while(parent&&depth++<_terminals.count){if(parent==anchor){insertionIndex=candidateIndex+1;break;}parent=parent.splitAnchor;}}}[_terminals insertObject:terminal atIndex:insertionIndex];if(animate&&[self animationsEnabled])_enteringTerminal=terminal;[_root addSubview:terminal positioned:NSWindowBelow relativeTo:_tabRail];self.terminal=terminal;self.extensions.activeTerminal=terminal;_animateTabLayout=animate;if(wasTiled!=[self usesTiledLayout])[self applyAppearance];else{[self rebuildTabs];[self layoutTabs];}[self focusTerminal:terminal];[self sendAutomationCommand:command toTerminal:terminal];return terminal;}
- (void)closeTab {[self cancelTileAnimation];TTerminalView *closing=self.terminal;if(!closing)return;if(_terminals.count<=1){[_terminals removeAllObjects];[_hyprlandRootOrder removeAllObjects];self.terminal=nil;self.extensions.activeTerminal=nil;[closing stopShellTerminating:YES];[closing removeFromSuperview];[self.window close];return;}BOOL wasTiled=[self usesTiledLayout],animate=_terminals.count<=9;NSUInteger index=[_terminals indexOfObject:closing],rootPosition=_hyprlandRootOrder?[_hyprlandRootOrder indexOfObject:closing]:NSNotFound;NSMutableArray<TTerminalView *> *children=[NSMutableArray array];for(TTerminalView *item in _terminals)if(item.splitAnchor==closing)[children addObject:item];TTerminalView *parent=closing.splitAnchor,*promoted=children.firstObject;if(!parent&&promoted){promoted.splitAnchor=nil;promoted.splitBefore=NO;for(NSUInteger i=1;i<children.count;i++)children[i].splitAnchor=promoted;}else for(TTerminalView *child in children)child.splitAnchor=parent;if(rootPosition!=NSNotFound){[_hyprlandRootOrder removeObject:closing];if(!parent&&promoted)[_hyprlandRootOrder insertObject:promoted atIndex:MIN(rootPosition,_hyprlandRootOrder.count)];}[_terminals removeObjectAtIndex:index];[closing stopShellTerminating:YES];[closing removeFromSuperview];self.terminal=promoted?:_terminals[MIN(index,_terminals.count-1)];self.extensions.activeTerminal=self.terminal;_animateTabLayout=animate;if(wasTiled!=[self usesTiledLayout])[self applyAppearance];else{[self rebuildTabs];[self layoutTabs];}[self focusTerminal:self.terminal];}
...
- (void)selectTabNumber:(NSInteger)number {
    ...
    TTerminalView *next=_terminals[(NSUInteger)index];[self focusTerminal:next];[self updateTabSelectionAnimated:[self animationsEnabled]];[next setNeedsDisplay:YES];
}
- (NSArray<TTerminalView *> *)automationRoots {NSMutableArray *roots=[NSMutableArray array];for(TTerminalView *terminal in _terminals)if([self rootTerminalForTerminal:terminal]==terminal)[roots addObject:terminal];return roots;}
- (BOOL)focusAutomationTab:(NSInteger)number {NSArray *roots=[self automationRoots];if(number<1||number>(NSInteger)roots.count)return NO;[self focusTerminal:roots[(NSUInteger)number-1]];return YES;}
- (BOOL)focusAutomationPane:(NSInteger)number {NSArray *panes=[self visibleLayoutTerminals];if(number<1||number>(NSInteger)panes.count)return NO;[self focusTerminal:panes[(NSUInteger)number-1]];return YES;}
- (NSDictionary *)automationStatus {NSArray *roots=[self automationRoots],*panes=[self visibleLayoutTerminals];NSUInteger activeRoot=[roots indexOfObject:[self rootTerminalForTerminal:self.terminal]],activePane=[panes indexOfObject:self.terminal];return @{@"tabs":@(roots.count),@"panes":@(panes.count),@"activeTab":@(activeRoot==NSNotFound?0:activeRoot+1),@"activePane":@(activePane==NSNotFound?0:activePane+1)};}
- (void)rebuildTabs {
...
- (void)layoutTabs {
    CGFloat w=_root.bounds.size.width,h=_root.bounds.size.height;BOOL tile=[self usesTiledLayout],animate=_animateTabLayout&&[self animationsEnabled],stableTiledCompositing=[self requiresStableTiledCompositing];NSArray<NSValue *> *slots=tile?[self hyprlandFrames]:@[];NSSet<TTerminalView *> *visible=[NSSet setWithArray:[self visibleLayoutTerminals]];CAMediaTimingFunction *ease=[CAMediaTimingFunction functionWithControlPoints:0.11f :1.0f :0.2f :1.0f];TTerminalView *entering=_enteringTerminal;
    for(NSUInteger i=0;i<_terminals.count;i++){
        TTerminalView *terminal=_terminals[i];BOOL shown=[visible containsObject:terminal];NSRect target=tile?slots[i].rectValue:NSMakeRect(0,0,w,h),prior=terminal.frame;
        terminal.hidden=!shown;terminal.leadingOverlayInset=0;terminal.topContentInset=self.config.topBar?0:6;terminal.activeTerminal=shown&&terminal==self.terminal;
        if(terminal!=_draggingTerminal){terminal.frame=target;[terminal resizeGrid];}
        if(tile&&shown)TLog(@"tile %lu frame %.0f,%.0f %.0fx%.0f anchor %@",(unsigned long)i+1,target.origin.x,target.origin.y,target.size.width,target.size.height,terminal.splitAnchor?[NSString stringWithFormat:@"%lu",(unsigned long)[_terminals indexOfObject:terminal.splitAnchor]+1]:@"root");
        if(terminal==_draggingTerminal)continue;
        BOOL animateTerminal=shown&&tile&&animate&&!stableTiledCompositing&&(!entering||terminal==entering);
        terminal.wantsLayer=(shown&&tile)||animateTerminal||[terminal usesMetalRenderer];terminal.tiledRendering=shown&&tile;terminal.layer.cornerRadius=shown&&tile?self.config->tileCornerRadius:0;terminal.layer.masksToBounds=shown&&tile;[terminal.layer removeAnimationForKey:@"termatica.hypr.snap"];if(stableTiledCompositing)terminal.layer.mask=nil;
        if(animateTerminal&&NSWidth(prior)>0&&NSHeight(prior)>0&&!NSEqualRects(prior,target)){CGFloat dx=NSMidX(prior)-NSMidX(target),dy=NSMidY(prior)-NSMidY(target);CABasicAnimation *snap=[CABasicAnimation animationWithKeyPath:@"transform"];snap.fromValue=[NSValue valueWithCATransform3D:CATransform3DMakeTranslation(dx,dy,0)];snap.toValue=[NSValue valueWithCATransform3D:CATransform3DIdentity];snap.duration=[self animationDuration:self.config->layoutAnimationDuration];snap.timingFunction=ease;[terminal.layer addAnimation:snap forKey:@"termatica.hypr.snap"];}
    }
    CGMutablePathRef surfacePath=CGPathCreateMutable();NSUInteger surfaceCount=0;if(tile){for(TTerminalView *terminal in [self visibleLayoutTerminals])if(!terminal.hidden){CGPathAddRoundedRect(surfacePath,NULL,NSRectToCGRect(terminal.frame),self.config->tileCornerRadius,self.config->tileCornerRadius);surfaceCount++;}}else if(!NSIsEmptyRect(_root.bounds)){CGPathAddRect(surfacePath,NULL,NSRectToCGRect(_root.bounds));surfaceCount=1;}[CATransaction begin];[CATransaction setDisableActions:YES];_tileSurfaceLayer.frame=_tileSurface.bounds;_tileSurfaceLayer.path=surfacePath;_tileSurfaceLayer.fillColor=self.config.background.CGColor;_tileSurface.hidden=!surfaceCount;[CATransaction commit];CGPathRelease(surfacePath);
    if(entering&&animate&&!stableTiledCompositing&&![entering isHidden])TAnimateCenterReveal(entering,[self animationDuration:self.config->terminalAnimationDuration],self.config->tileCornerRadius,@"termatica.terminal.center",nil);
    ...
    CGFloat margin=self.config->tabRailMargin,inset=self.config->tabButtonInset,topInset=MAX(self.config->tabMaximumHeight+margin*2,_root.safeAreaInsets.top+margin),available=MAX(1,h-topInset-margin),minimumItem=MAX(self.config->tabMinimumHeight,self.config->tabButtonFontSize+4);NSUInteger capacity=MAX((NSUInteger)1,(NSUInteger)floor(MAX(1,available-inset*2)/minimumItem)),visibleCount=MIN(_terminals.count,capacity),active=[_terminals indexOfObject:self.terminal],start=0;if(visibleCount<_terminals.count){NSUInteger half=visibleCount/2;start=active>half?active-half:0;if(start+visibleCount>_terminals.count)start=_terminals.count-visibleCount;}CGFloat itemHeight=MIN(self.config->tabMaximumHeight,floor(MAX(1,available-inset*2)/visibleCount));itemHeight=MAX(1,itemHeight);CGFloat railWidth=self.config.tabRailWidth,railHeight=MIN(available,inset*2+itemHeight*visibleCount);_tabRail.tabCount=visibleCount;_tabRailTargetFrame=NSMakeRect(margin,MAX(margin,h-topInset-railHeight-margin),railWidth,railHeight);NSUInteger visibleIndex=0;for(NSUInteger i=0;i<_tabButtons.count;i++){BOOL buttonVisible=i>=start&&i<start+visibleCount;_tabButtons[i].hidden=!buttonVisible;if(buttonVisible){CGFloat y=inset+(visibleCount-1-visibleIndex++)*itemHeight;_tabButtons[i].frame=NSMakeRect(inset,y,MAX(1,railWidth-inset*2),itemHeight);}}[self updateTabHoverArea];TLog(@"tab rail frame %.0f,%.0f %.0fx%.0f showing %lu/%lu",_tabRailTargetFrame.origin.x,_tabRailTargetFrame.origin.y,_tabRailTargetFrame.size.width,_tabRailTargetFrame.size.height,(unsigned long)visibleCount,(unsigned long)_terminals.count);BOOL reveal=_revealRailAfterLayout;_revealRailAfterLayout=NO;_animateTabLayout=NO;if(reveal)[self revealTabRail];else if(_tabRailVisible)_tabRail.frame=_tabRailTargetFrame;else{NSRect collapsed=_tabRailTargetFrame;collapsed.origin.x=-NSWidth(collapsed)+self.config->tabCollapsedPeek;_tabRail.frame=collapsed;}[_root addSubview:_tabRail positioned:NSWindowAbove relativeTo:nil];[_root addSubview:_tabEdge positioned:NSWindowAbove relativeTo:nil];[_tabRail setNeedsDisplay:YES];
}
...
- (void)windowDidResignKey:(NSNotification *)notification {for(TTerminalView *terminal in _terminals)[terminal invalidateCursorPresentation];}
- (void)adaptWindowToCurrentScreen {
    if(!self.window||!_root||!self.config||_adjustingScreenFrame||(self.window.styleMask&NSWindowStyleMaskFullScreen))return;NSScreen *screen=self.window.screen?:NSScreen.mainScreen;if(!screen)return;NSRect visible=screen.visibleFrame;if(NSWidth(visible)<320||NSHeight(visible)<200)return;CGFloat scale=screen.backingScaleFactor?:1;NSRect target;
    if(self.config.hyprlandLayout)target=TUsableScreenFrame(visible,self.config.screenInset,scale);
    else target=TRemapWindowFrameToScreen(self.window.frame,_lastScreenVisibleFrame,visible,self.window.minSize,self.config.screenInset,scale);
    _lastScreenVisibleFrame=visible;if(NSEqualRects(self.window.frame,target))return;_adjustingScreenFrame=YES;[self.window setFrame:target display:YES animate:NO];_adjustingScreenFrame=NO;[self layoutTabs];
}
- (void)handleDisplayParametersChanged {[self adaptWindowToCurrentScreen];for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange:@"display-parameters"];}
- (void)windowDidChangeScreen:(NSNotification *)notification {[self adaptWindowToCurrentScreen];for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange:@"window-screen"];}
- (void)windowDidChangeBackingProperties:(NSNotification *)notification {for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange:@"window-backing-scale"];}
- (void)windowDidChangeOcclusionState:(NSNotification *)notification {if(self.window.occlusionState&NSWindowOcclusionStateVisible)for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange:@"window-visible"];}
- (void)routeKeyEvent:(NSEvent *)event {if(self.terminal)[self.terminal keyDown:event];}
- (BOOL)captureKeybindingEvent:(NSEvent *)event {TTerminalView *focused=self.terminal;if([focused captureKeybindingEvent:event])return YES;for(TTerminalView *terminal in _terminals)if(terminal!=focused&&terminal.hasPendingKeybindingCapture&&[terminal captureKeybindingEvent:event])return YES;return NO;}
...
- (void)applyAppearance {
    ...
    BOOL animateWindow=self.window.isVisible&&[self animationsEnabled];
    if(self.config.hyprlandLayout){
        if(!_hyprlandApplied&&_preHyprlandFrame.size.width==0){_preHyprlandFrame=self.window.frame;_preHyprlandVisibleFrame=_lastScreenVisibleFrame;}
        ...
        if(visibleFrame.size.width>=480&&visibleFrame.size.height>=280){
            NSRect target=TUsableScreenFrame(visibleFrame,self.config.screenInset,screen.backingScaleFactor?:1);
            if(!NSEqualRects(self.window.frame,target))[self.window setFrame:target display:YES animate:animateWindow];
            _lastScreenVisibleFrame=visibleFrame;
        }
        _hyprlandApplied=YES;
    }else if(_hyprlandApplied){if(_preHyprlandFrame.size.width>0){NSScreen *screen=self.window.screen?:NSScreen.mainScreen;NSRect visible=screen?screen.visibleFrame:_preHyprlandVisibleFrame;CGFloat scale=screen?screen.backingScaleFactor:1;NSRect restored=TRemapWindowFrameToScreen(_preHyprlandFrame,_preHyprlandVisibleFrame,visible,self.window.minSize,self.config.screenInset,scale);[self.window setFrame:restored display:YES animate:animateWindow];_lastScreenVisibleFrame=visible;}_hyprlandApplied=NO;_preHyprlandFrame=NSZeroRect;_preHyprlandVisibleFrame=NSZeroRect;}
...
static void TApplyMenuShortcut(NSMenuItem *item,NSString *spec) {if(!spec.length){item.keyEquivalent=@"";item.keyEquivalentModifierMask=0;return;}NSArray<NSString *> *parts=[spec.lowercaseString componentsSeparatedByString:@"+"];NSEventModifierFlags mask=0;NSString *key=parts.lastObject;for(NSString *part in parts){if([part isEqual:@"cmd"]||[part isEqual:@"command"])mask|=NSEventModifierFlagCommand;else if([part isEqual:@"shift"])mask|=NSEventModifierFlagShift;else if([part isEqual:@"option"]||[part isEqual:@"alt"])mask|=NSEventModifierFlagOption;else if([part isEqual:@"control"]||[part isEqual:@"ctrl"])mask|=NSEventModifierFlagControl;}NSDictionary<NSString *,NSString *> *special=@{@"space":@" ",@"return":@"\r",@"tab":@"\t",@"delete":@"\x7f",@"left":[NSString stringWithFormat:@"%C",(unichar)NSLeftArrowFunctionKey],@"right":[NSString stringWithFormat:@"%C",(unichar)NSRightArrowFunctionKey],@"up":[NSString stringWithFormat:@"%C",(unichar)NSUpArrowFunctionKey],@"down":[NSString stringWithFormat:@"%C",(unichar)NSDownArrowFunctionKey],@"home":[NSString stringWithFormat:@"%C",(unichar)NSHomeFunctionKey],@"end":[NSString stringWithFormat:@"%C",(unichar)NSEndFunctionKey],@"page-up":[NSString stringWithFormat:@"%C",(unichar)NSPageUpFunctionKey],@"page-down":[NSString stringWithFormat:@"%C",(unichar)NSPageDownFunctionKey],@"forward-delete":[NSString stringWithFormat:@"%C",(unichar)NSDeleteFunctionKey]};if([key isEqual:@"plus"])key=@"+";else if([key hasPrefix:@"f"]&&[[key substringFromIndex:1] integerValue]>=1&&[[key substringFromIndex:1] integerValue]<=12)key=[NSString stringWithFormat:@"%C",(unichar)(NSF1FunctionKey+[[key substringFromIndex:1] integerValue]-1)];else key=special[key]?:key;item.keyEquivalent=key?:@"";item.keyEquivalentModifierMask=mask;}
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (NSDictionary *)rendererFieldReport;
- (NSDictionary *)rendererRetryReport;
- (NSDictionary *)automationReply:(NSDictionary *)request;
- (void)recordFieldEvent:(NSString *)reason;
- (void)reloadAllIfChanged;
- (NSDictionary *)configReloadDiagnostics;
- (TWindowController *)newWindowWithDirectory:(NSString *)directory command:(NSString *)command;
@end
...
- (void)sendEvent:(NSEvent *)event {
    ...
    if(event.type==NSEventTypeKeyDown){NSWindow *captureWindow=event.window?:NSApp.keyWindow;TWindowController *captureController=[captureWindow.windowController isKindOfClass:TWindowController.class]?(TWindowController *)captureWindow.windowController:[(TAppDelegate *)self.delegate active];if(captureController&&[captureController captureKeybindingEvent:event])return;TAppDelegate *delegate=(TAppDelegate *)self.delegate;NSDictionary *keys=delegate.config.keybindings;NSArray<NSArray<NSString *> *> *actions=@[@[@"openConfig",@"openConfig:"],@[@"newWindow",@"newWindow:"],@[@"newTab",@"newTab:"],@[@"closeTab",@"closeTab:"],@[@"clearTerminal",@"clearTerminal:"],@[@"searchScrollback",@"searchScrollback:"],@[@"previousPrompt",@"previousPrompt:"],@[@"nextPrompt",@"nextPrompt:"],@[@"reload",@"reloadConfig:"],@[@"copy",@"copy:"],@[@"paste",@"paste:"],@[@"selectAll",@"selectAll:"],@[@"splitHorizontal",@"splitHorizontal:"],@[@"splitVertical",@"splitVertical:"],@[@"nextSplit",@"nextSplit:"],@[@"previousSplit",@"previousSplit:"],@[@"zoomIn",@"zoomIn:"],@[@"zoomOut",@"zoomOut:"],@[@"zoomReset",@"zoomReset:"]];for(NSArray<NSString *> *binding in actions)if(TEventMatchesShortcut(event,keys[binding[0]])){if([binding[0] isEqual:@"newTab"]&&event.isARepeat)return;if([self sendAction:NSSelectorFromString(binding[1]) to:nil from:self])return;}for(NSInteger index=1;index<=9;index++){NSString *name=[NSString stringWithFormat:@"tab%ld",(long)index];if(TEventMatchesShortcut(event,keys[name])){NSMenuItem *sender=[NSMenuItem new];sender.tag=index;if([self sendAction:@selector(selectTab:) to:delegate from:sender])return;}}NSEventModifierFlags mods=event.modifierFlags&NSEventModifierFlagDeviceIndependentFlagsMask;if(!(mods&NSEventModifierFlagCommand)){NSWindow *target=event.window?:NSApp.keyWindow;TWindowController *controller=[target.windowController isKindOfClass:TWindowController.class]?(TWindowController *)target.windowController:[delegate active];if(controller&&target==controller.window){[controller routeKeyEvent:event];return;}}}
    [super sendEvent:event];
}
@end

@implementation TAppDelegate {int _cliSocket;dispatch_source_t _cliSource;NSMutableArray *_configSources;BOOL _configReloadPending;NSString *_lastAppliedConfigFingerprint;NSUInteger _configReloadApplications,_configReloadSkips;NSMutableDictionary<NSString *,NSNumber *> *_fieldEventCounts;NSMutableSet<NSNumber *> *_observedDisplayCounts,*_observedScales,*_observedRefreshRates;NSUInteger _lastDisplayCount,_displayCountChanges,_displayParameterEvents;CFAbsoluteTime _fieldStartedAt;NSString *_fieldSessionIdentifier;}
- (void)recordFieldEvent:(NSString *)reason {
    if(!_fieldStartedAt)_fieldStartedAt=CFAbsoluteTimeGetCurrent();if(!_fieldSessionIdentifier)_fieldSessionIdentifier=NSUUID.UUID.UUIDString.lowercaseString;if(!_fieldEventCounts)_fieldEventCounts=[NSMutableDictionary dictionary];if(!_observedDisplayCounts)_observedDisplayCounts=[NSMutableSet set];if(!_observedScales)_observedScales=[NSMutableSet set];if(!_observedRefreshRates)_observedRefreshRates=[NSMutableSet set];
    NSString *event=reason.length?reason:@"unknown";_fieldEventCounts[event]=@([_fieldEventCounts[event] unsignedIntegerValue]+1);NSUInteger count=NSScreen.screens.count;if(_lastDisplayCount&&count!=_lastDisplayCount)_displayCountChanges++;_lastDisplayCount=count;[_observedDisplayCounts addObject:@(count)];if([event isEqual:NSApplicationDidChangeScreenParametersNotification])_displayParameterEvents++;
    for(NSScreen *screen in NSScreen.screens){CGFloat scale=MAX(1,screen.backingScaleFactor);NSUInteger fps=[screen respondsToSelector:@selector(maximumFramesPerSecond)]?screen.maximumFramesPerSecond:60;[_observedScales addObject:@(scale)];[_observedRefreshRates addObject:@(fps)];}
}
- (void)recoverPresentations:(NSNotification *)notification {
    NSString *reason=notification.name.length?notification.name:@"application-lifecycle";
    if(!NSThread.isMainThread){__weak typeof(self) weakSelf=self;dispatch_async(dispatch_get_main_queue(),^{[weakSelf recordFieldEvent:reason];for(TWindowController *window in weakSelf.windows){if([reason isEqual:NSApplicationDidChangeScreenParametersNotification])[window handleDisplayParametersChanged];else for(TTerminalView *terminal in window.terminals)[terminal recoverPresentationAfterLifecycleChange:reason];}});return;}
    [self recordFieldEvent:reason];
    for(TWindowController *window in self.windows){if([reason isEqual:NSApplicationDidChangeScreenParametersNotification])[window handleDisplayParametersChanged];else for(TTerminalView *terminal in window.terminals)[terminal recoverPresentationAfterLifecycleChange:reason];}
}
- (NSDictionary *)rendererFieldReport {
    if(!_fieldStartedAt)[self recordFieldEvent:@"report-initialization"];
    NSMutableArray *displays=[NSMutableArray array];NSUInteger displayIndex=0;for(NSScreen *screen in NSScreen.screens){CGFloat scale=MAX(1,screen.backingScaleFactor);NSUInteger fps=[screen respondsToSelector:@selector(maximumFramesPerSecond)]?screen.maximumFramesPerSecond:60;[displays addObject:@{@"index":@(displayIndex++),@"main":@(screen==NSScreen.mainScreen),@"pixelWidth":@((NSInteger)llround(screen.frame.size.width*scale)),@"pixelHeight":@((NSInteger)llround(screen.frame.size.height*scale)),@"scale":@(scale),@"maximumFPS":@(fps)}];}
    NSMutableArray *panes=[NSMutableArray array];NSUInteger windowIndex=0,metalPanes=0,appkitPanes=0,quarantinedPanes=0,mismatchPanes=0,totalFailures=0,totalAttempts=0,totalSuccessfulRetries=0,screenTransitionEvents=0,backingScaleEvents=0,observedScreenChanges=0,observedBackingScaleChanges=0;
    for(TWindowController *controller in self.windows){NSUInteger paneIndex=0;for(TTerminalView *terminal in controller.terminals){NSDictionary *state=terminal.diagnosticState,*reasons=[state[@"presentationRecoveryReasons"] isKindOfClass:NSDictionary.class]?state[@"presentationRecoveryReasons"]:@{};NSString *requested=state[@"requestedRenderer"]?:@"appkit",*renderer=state[@"renderer"]?:@"none";BOOL quarantined=[state[@"metalQuarantined"] boolValue];NSUInteger paneDisplay=[NSScreen.screens indexOfObject:terminal.window.screen];screenTransitionEvents+=[reasons[@"window-screen"] unsignedIntegerValue];backingScaleEvents+=[reasons[@"window-backing-scale"] unsignedIntegerValue]+[reasons[@"view-backing-scale"] unsignedIntegerValue];observedScreenChanges+=[state[@"observedScreenChangeCount"] unsignedIntegerValue];observedBackingScaleChanges+=[state[@"observedBackingScaleChangeCount"] unsignedIntegerValue];metalPanes+=[renderer isEqual:@"metal"];appkitPanes+=[renderer isEqual:@"appkit"];quarantinedPanes+=quarantined;mismatchPanes+=![requested isEqual:renderer];totalFailures+=[state[@"metalFailureCount"] unsignedIntegerValue];totalAttempts+=[state[@"metalRetryAttemptCount"] unsignedIntegerValue];totalSuccessfulRetries+=[state[@"metalSuccessfulRetryCount"] unsignedIntegerValue];[panes addObject:@{@"window":@(windowIndex),@"pane":@(paneIndex++),@"displayIndex":@(paneDisplay==NSNotFound?-1:(NSInteger)paneDisplay),@"active":state[@"activeTerminal"]?:@NO,@"firstResponder":state[@"firstResponder"]?:@NO,@"requestedRenderer":requested,@"renderer":renderer,@"rows":state[@"rows"]?:@0,@"columns":state[@"columns"]?:@0,@"renderGeneration":state[@"renderGeneration"]?:@0,@"metalFailed":state[@"metalFailed"]?:@NO,@"metalQuarantined":@(quarantined),@"metalRetryAvailable":state[@"metalRetryAvailable"]?:@NO,@"metalRetryRequiresExplicitAction":state[@"metalRetryRequiresExplicitAction"]?:@NO,@"metalFailureCount":state[@"metalFailureCount"]?:@0,@"metalQuarantineBypassCount":state[@"metalQuarantineBypassCount"]?:@0,@"metalRetryAttemptCount":state[@"metalRetryAttemptCount"]?:@0,@"metalSuccessfulRetryCount":state[@"metalSuccessfulRetryCount"]?:@0,@"lastMetalFailureStage":state[@"lastMetalFailureStage"]?:@"none",@"secondsSinceMetalFailure":state[@"secondsSinceMetalFailure"]?:@(-1),@"secondsSinceMetalRetry":state[@"secondsSinceMetalRetry"]?:@(-1),@"presentationRecoveryCount":state[@"presentationRecoveryCount"]?:@0,@"presentationRecoveryReasons":reasons,@"observedScreenChangeCount":state[@"observedScreenChangeCount"]?:@0,@"observedBackingScaleChangeCount":state[@"observedBackingScaleChangeCount"]?:@0,@"lastPresentationRecovery":state[@"lastPresentationRecovery"]?:@"none"}];}windowIndex++;}
    NSUInteger minimumDisplays=NSUIntegerMax,maximumDisplays=0;for(NSNumber *value in _observedDisplayCounts){minimumDisplays=MIN(minimumDisplays,value.unsignedIntegerValue);maximumDisplays=MAX(maximumDisplays,value.unsignedIntegerValue);}if(minimumDisplays==NSUIntegerMax)minimumDisplays=displays.count;
    NSDictionary *evidence=@{@"evidenceSource":@"observed-runtime-events",@"fieldSessionIdentifier":_fieldSessionIdentifier?:@"unknown-session",@"sessionSeconds":@(MAX(0,CFAbsoluteTimeGetCurrent()-_fieldStartedAt)),@"processArchitecture":TRuntimeArchitecture(),@"translated":@(TProcessIsTranslated()),@"minimumDisplayCount":@(minimumDisplays),@"maximumDisplayCount":@(maximumDisplays),@"observedDisplayCountCount":@(_observedDisplayCounts.count),@"observedScaleCount":@(_observedScales.count),@"observedRefreshRateCount":@(_observedRefreshRates.count),@"displayParameterEvents":@(_displayParameterEvents),@"displayCountChanges":@(_displayCountChanges),@"wakeEvents":@([_fieldEventCounts[NSWorkspaceDidWakeNotification] unsignedIntegerValue]),@"screenWakeEvents":@([_fieldEventCounts[NSWorkspaceScreensDidWakeNotification] unsignedIntegerValue]),@"screenTransitionEvents":@(screenTransitionEvents),@"backingScaleEvents":@(backingScaleEvents),@"observedScreenChanges":@(observedScreenChanges),@"observedBackingScaleChanges":@(observedBackingScaleChanges),@"metalPanes":@(metalPanes),@"quarantinedPanes":@(quarantinedPanes)};NSDictionary *qualification=TFieldQualification(evidence);
    NSString *generated=[NSISO8601DateFormatter stringFromDate:NSDate.date timeZone:NSTimeZone.localTimeZone formatOptions:NSISO8601DateFormatWithInternetDateTime];NSDictionary *info=NSBundle.mainBundle.infoDictionary;return @{@"ok":@YES,@"schemaVersion":@4,@"generatedAt":generated?:@"unknown",@"privacy":@{@"terminalContentIncluded":@NO,@"workingDirectoriesIncluded":@NO,@"configPathIncluded":@NO,@"displayNamesIncluded":@NO},@"rolloutPolicy":@{@"defaultRenderer":@"appkit",@"metalOptIn":@YES,@"appkitAvailable":@YES,@"defaultChangeEligible":qualification[@"defaultChangeEligible"]?:@NO,@"defaultChangeBlocker":@"field-qualification-gates-open"},@"fieldQualification":qualification,@"fieldEvidence":evidence,@"health":@{@"metalPanes":@(metalPanes),@"appkitPanes":@(appkitPanes),@"quarantinedPanes":@(quarantinedPanes),@"rendererMismatchPanes":@(mismatchPanes),@"metalFailureCount":@(totalFailures),@"metalRetryAttemptCount":@(totalAttempts),@"metalSuccessfulRetryCount":@(totalSuccessfulRetries)},@"app":@{@"version":[info[@"CFBundleShortVersionString"] description]?:TCurrentVersion(),@"build":[info[@"CFBundleVersion"] description]?:@"unknown",@"requestedRenderer":self.config.renderer?:@"appkit",@"activeConfig":TActiveConfigName(self.config),@"font":self.config.fontName?:@"unknown",@"fontSize":@(self.config.fontSize),@"windowCount":@(self.windows.count),@"paneCount":@(panes.count)},@"system":@{@"operatingSystem":NSProcessInfo.processInfo.operatingSystemVersionString?:@"unknown",@"processArchitecture":TRuntimeArchitecture(),@"translated":@(TProcessIsTranslated()),@"displayCount":@(displays.count)},@"displays":displays,@"panes":panes};
}
- (NSDictionary *)rendererRetryReport {
    NSMutableArray *results=[NSMutableArray array];NSUInteger requested=0,attempted=0,recovered=0,stillQuarantined=0,unchanged=0,windowIndex=0;
    for(TWindowController *controller in self.windows){NSUInteger paneIndex=0;for(TTerminalView *terminal in controller.terminals){NSDictionary *before=terminal.diagnosticState;if([before[@"requestedRenderer"] isEqual:@"metal"])requested++;NSDictionary *result=[terminal retryMetalRenderer];BOOL didAttempt=[result[@"attempted"] boolValue],didRecover=[result[@"recovered"] boolValue];attempted+=didAttempt;recovered+=didAttempt&&didRecover;stillQuarantined+=didAttempt&&!didRecover;unchanged+=!didAttempt;[results addObject:@{@"window":@(windowIndex),@"pane":@(paneIndex++),@"attempted":@(didAttempt),@"recovered":@(didRecover),@"status":result[@"status"]?:@"unknown",@"before":result[@"before"]?:@"none",@"after":result[@"after"]?:@"none"}];}windowIndex++;}
    return @{@"ok":@(stillQuarantined==0),@"requestedMetalPanes":@(requested),@"attempted":@(attempted),@"recovered":@(recovered),@"stillQuarantined":@(stillQuarantined),@"unchanged":@(unchanged),@"panes":results};
}
...
#pragma clang diagnostic pop
- (void)applicationDidFinishLaunching:(NSNotification *)notification {TLog(@"application delegate started");_cliSocket=-1;_config=[TConfig new];TLog(@"configuration ready");_windows=[NSMutableArray array];[self recordFieldEvent:@"application-launch"];TWindowController *controller=[[TWindowController alloc]initWithConfig:self.config extensions:nil];[self.windows addObject:controller];controller.window.initialFirstResponder=controller.terminal;[controller.window makeFirstResponder:controller.terminal];[controller showWindow:nil];dispatch_async(dispatch_get_main_queue(),^{[controller animateLaunchReveal];});_extensions=[TExtensionHost new];_extensions.config=_config;controller.extensions=_extensions;_extensions.activeTerminal=controller.terminal;[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(recoverPresentations:) name:NSApplicationDidChangeScreenParametersNotification object:nil];[NSWorkspace.sharedWorkspace.notificationCenter addObserver:self selector:@selector(recoverPresentations:) name:NSWorkspaceDidWakeNotification object:nil];[NSWorkspace.sharedWorkspace.notificationCenter addObserver:self selector:@selector(recoverPresentations:) name:NSWorkspaceScreensDidWakeNotification object:nil];dispatch_after(dispatch_time(DISPATCH_TIME_NOW,100*NSEC_PER_MSEC),dispatch_get_main_queue(),^{TInstallConfiguredPlugins(self.config);[self.config reload];self->_lastAppliedConfigFingerprint=TActiveConfigFingerprint();[self buildMenu];[self startCLIListener];[self.extensions loadExtensions];[self startConfigWatcher];});dispatch_after(dispatch_time(DISPATCH_TIME_NOW,10*NSEC_PER_SEC),dispatch_get_main_queue(),^{[self checkForUpdatesOnLaunch];});}
- (void)startConfigWatcher {for(dispatch_source_t source in _configSources?:@[])dispatch_source_cancel(source);_configSources=[NSMutableArray array];NSMutableOrderedSet<NSString *> *paths=[NSMutableOrderedSet orderedSetWithArray:@[TConfigDirectoryPath(),TConfigProfilesPath(),TConfigSelectionPath(),self.config.path?:TLegacyConfigPath()]];__weak typeof(self) weakSelf=self;for(NSString *path in paths){int fd=open(path.fileSystemRepresentation,O_EVTONLY);if(fd<0){TLog(@"config watcher skipped unavailable path %@",path);continue;}dispatch_source_t source=dispatch_source_create(DISPATCH_SOURCE_TYPE_VNODE,fd,DISPATCH_VNODE_WRITE|DISPATCH_VNODE_DELETE|DISPATCH_VNODE_RENAME|DISPATCH_VNODE_EXTEND|DISPATCH_VNODE_ATTRIB|DISPATCH_VNODE_LINK|DISPATCH_VNODE_REVOKE,dispatch_get_global_queue(QOS_CLASS_UTILITY,0));dispatch_source_set_event_handler(source,^{dispatch_async(dispatch_get_main_queue(),^{__strong typeof(weakSelf) self=weakSelf;if(!self||self->_configReloadPending)return;self->_configReloadPending=YES;dispatch_after(dispatch_time(DISPATCH_TIME_NOW,40*NSEC_PER_MSEC),dispatch_get_main_queue(),^{__strong typeof(weakSelf) self=weakSelf;if(!self)return;self->_configReloadPending=NO;[self reloadAllIfChanged];[self startConfigWatcher];});});});dispatch_source_set_cancel_handler(source,^{close(fd);});[_configSources addObject:source];dispatch_resume(source);}TLog(@"config watcher active for %lu paths",(unsigned long)_configSources.count);}
- (void)applicationWillTerminate:(NSNotification *)notification {[NSNotificationCenter.defaultCenter removeObserver:self];[NSWorkspace.sharedWorkspace.notificationCenter removeObserver:self];for(dispatch_source_t source in _configSources?:@[])dispatch_source_cancel(source);_configSources=nil;if(_cliSource){dispatch_source_cancel(_cliSource);_cliSource=nil;}else if(_cliSocket>=0){close(_cliSocket);TRemoveOwnedSocket(TCLISocketPath());_cliSocket=-1;}}
- (BOOL)applicationShouldTerminateAfterLastWindowClosed:(NSApplication *)sender{return YES;}
- (TWindowController *)newWindowWithDirectory:(NSString *)directory command:(NSString *)command {TWindowController *controller=[[TWindowController alloc]initWithConfig:self.config extensions:self.extensions directory:directory command:command];[self.windows addObject:controller];controller.window.initialFirstResponder=controller.terminal;[controller.window makeFirstResponder:controller.terminal];[controller animateLaunchReveal];[controller showWindow:nil];return controller;}
- (void)newWindow:(id)sender {[self newWindowWithDirectory:nil command:nil];}
- (TWindowController *)active {return (TWindowController *)NSApp.keyWindow.windowController?:self.windows.lastObject;}
- (void)newTab:(id)sender {TLog(@"new terminal shortcut");[[self active] addCommandTTerminal];}
...
- (void)splitHorizontal:(id)sender{[[self active] addSplitWithVerticalDivider:NO];}
- (void)splitVertical:(id)sender{[[self active] addSplitWithVerticalDivider:YES];}
- (void)nextSplit:(id)sender{[[self active] focusAdjacentSplit:1];}
- (void)previousSplit:(id)sender{[[self active] focusAdjacentSplit:-1];}
...
- (void)reloadAll {NSDictionary *priorBindings=self.config.keybindings;[self.config ensureEditableFile];[self.config reload];TInstallConfiguredPlugins(self.config);[self.config reload];if(![priorBindings isEqualToDictionary:self.config.keybindings])[self buildMenu];[self.extensions loadExtensions];for(TWindowController *window in self.windows)[window reloadConfig];_lastAppliedConfigFingerprint=TActiveConfigFingerprint();_configReloadApplications++;TLog(@"config reload applied fingerprint=%@ count=%lu",_lastAppliedConfigFingerprint,(unsigned long)_configReloadApplications);}
- (void)reloadAllIfChanged {NSString *fingerprint=TActiveConfigFingerprint();if(_lastAppliedConfigFingerprint&&[_lastAppliedConfigFingerprint isEqual:fingerprint]){_configReloadSkips++;TLog(@"config reload skipped unchanged fingerprint=%@ count=%lu",fingerprint,(unsigned long)_configReloadSkips);return;}[self reloadAll];}
- (NSDictionary *)configReloadDiagnostics {return @{@"applications":@(_configReloadApplications),@"skips":@(_configReloadSkips),@"fingerprint":_lastAppliedConfigFingerprint?:@""};}
- (NSString *)automationDirectory:(id)value {if(![value isKindOfClass:NSString.class]||![value length])return nil;NSString *path=[value stringByExpandingTildeInPath].stringByStandardizingPath;BOOL directory=NO;return [NSFileManager.defaultManager fileExistsAtPath:path isDirectory:&directory]&&directory?path:nil;}
- (NSDictionary *)automationReply:(NSDictionary *)request {
    NSString *action=[request[@"action"] isKindOfClass:NSString.class]?[request[@"action"] lowercaseString]:@"",*text=[request[@"text"] isKindOfClass:NSString.class]?request[@"text"]:nil,*rawDirectory=[request[@"cwd"] isKindOfClass:NSString.class]?request[@"cwd"]:nil,*directory=[self automationDirectory:rawDirectory];if(rawDirectory.length&&!directory)return @{@"ok":@NO,@"error":@"cwd must be an existing directory"};if(text.length>4096)return @{@"ok":@NO,@"error":@"text exceeds the 4096 character limit"};TWindowController *controller=[self active];
    if([action isEqual:@"status"]){NSMutableArray *windows=[NSMutableArray array];for(TWindowController *window in self.windows)[windows addObject:[window automationStatus]];NSUInteger active=[self.windows indexOfObject:controller];return @{@"ok":@YES,@"schemaVersion":@1,@"windows":windows,@"activeWindow":@(active==NSNotFound?0:active+1),@"privacy":@{@"terminalContentIncluded":@NO,@"scrollbackIncluded":@NO,@"commandHistoryIncluded":@NO}};}
    if([action isEqual:@"activate"]){[NSApp activateIgnoringOtherApps:YES];[controller.window makeKeyAndOrderFront:nil];[controller focusTerminal:controller.terminal];return @{@"ok":@YES};}
    if([action isEqual:@"new-window"]){[self newWindowWithDirectory:directory command:text];return @{@"ok":@YES};}
    if(!controller)return @{@"ok":@NO,@"error":@"no terminal window is available"};
    if([action isEqual:@"close"]){NSString *target=[request[@"target"] isKindOfClass:NSString.class]?[request[@"target"] lowercaseString]:@"";if(![@[@"pane",@"tab",@"window"] containsObject:target])return @{@"ok":@NO,@"error":@"close target must be pane, tab, or window"};NSArray<TTerminalView *> *closing=[target isEqual:@"window"]?[controller.terminals copy]:([target isEqual:@"tab"]?[controller terminalsInGroupOfTerminal:controller.terminal]:@[controller.terminal]);NSUInteger total=0;for(TWindowController *window in self.windows)total+=window.terminals.count;if(closing.count>=total)return @{@"ok":@NO,@"error":@"cannot close the final terminal through automation"};for(TTerminalView *terminal in [closing copy])if([controller.terminals containsObject:terminal]){[controller focusTerminal:terminal];[controller closeTab];}if(!controller.terminals.count)[self.windows removeObject:controller];return @{@"ok":@YES,@"closed":target};}
    if([action isEqual:@"new-tab"]){[controller addTabWithDirectory:directory command:text];return @{@"ok":@YES};}
    if([action isEqual:@"split"]){NSString *direction=[request[@"direction"] isKindOfClass:NSString.class]?[request[@"direction"] lowercaseString]:@"horizontal";if(![@[@"horizontal",@"vertical"] containsObject:direction])return @{@"ok":@NO,@"error":@"split direction must be horizontal or vertical"};[controller addSplitWithVerticalDivider:[direction isEqual:@"vertical"] directory:directory command:text];return @{@"ok":@YES};}
    if([action isEqual:@"focus"]){NSString *target=[request[@"target"] isKindOfClass:NSString.class]?[request[@"target"] lowercaseString]:@"";BOOL ok=NO;if([target isEqual:@"next"]||[target isEqual:@"previous"]){[controller focusAdjacentSplit:[target isEqual:@"next"]?1:-1];ok=YES;}else if([target isEqual:@"tab"])ok=[controller focusAutomationTab:[request[@"index"] integerValue]];else if([target isEqual:@"pane"])ok=[controller focusAutomationPane:[request[@"index"] integerValue]];return ok?@{@"ok":@YES}:@{@"ok":@NO,@"error":@"focus target or index is invalid"};}
    if([action isEqual:@"send"]||[action isEqual:@"run"]){if(!text.length)return @{@"ok":@NO,@"error":@"text is required"};[controller.terminal sendString:[action isEqual:@"run"]?[text stringByAppendingString:@"\n"]:text];return @{@"ok":@YES};}
    if([action isEqual:@"key"]){NSDictionary *keys=@{@"enter":@"\r",@"return":@"\r",@"escape":@"\033",@"tab":@"\t",@"up":@"\033[A",@"down":@"\033[B",@"right":@"\033[C",@"left":@"\033[D",@"home":@"\033[H",@"end":@"\033[F",@"page-up":@"\033[5~",@"page-down":@"\033[6~",@"ctrl-c":@"\003",@"ctrl-d":@"\004"};NSString *value=keys[[request[@"key"] lowercaseString]];if(!value)return @{@"ok":@NO,@"error":@"unsupported key"};[controller.terminal sendString:value];return @{@"ok":@YES};}
    if([action isEqual:@"recipe"]){NSArray *commands=[request[@"commands"] isKindOfClass:NSArray.class]?request[@"commands"]:@[];NSString *direction=[request[@"direction"] isEqual:@"vertical"]?@"vertical":@"horizontal";if(!commands.count||commands.count>9)return @{@"ok":@NO,@"error":@"recipe must contain 1 to 9 commands"};for(id command in commands)if(![command isKindOfClass:NSString.class]||[command length]>4096)return @{@"ok":@NO,@"error":@"recipe command is invalid"};[controller.terminal sendString:[commands[0] stringByAppendingString:@"\n"]];for(NSUInteger i=1;i<commands.count;i++)[controller addSplitWithVerticalDivider:[direction isEqual:@"vertical"] directory:nil command:commands[i]];return @{@"ok":@YES,@"panes":@(commands.count)};}
    return @{@"ok":@NO,@"error":@"unknown automation action"};
}
- (void)handleCLIRequest:(NSDictionary *)request {NSString *command=request[@"command"];if([command isEqual:@"automation"])TSendCLIReply(request[@"replyPath"],[self automationReply:request]);else if([command isEqual:@"newTab"])[self automationReply:@{@"action":@"new-tab"}];else if([command isEqual:@"newWindow"])[self automationReply:@{@"action":@"new-window"}];else if([command isEqual:@"splitHorizontal"])[self automationReply:@{@"action":@"split",@"direction":@"horizontal"}];else if([command isEqual:@"splitVertical"])[self automationReply:@{@"action":@"split",@"direction":@"vertical"}];else if([command isEqual:@"sendText"])[self automationReply:@{@"action":@"send",@"text":request[@"text"]?:@""}];else if([command isEqual:@"reload"])[self reloadAll];else if([command isEqual:@"reloadIfChanged"]){[self reloadAllIfChanged];TSendCLIReply(request[@"replyPath"],@{@"ok":@YES,@"config":[self configReloadDiagnostics]});}else if([command isEqual:@"activate"])[self automationReply:@{@"action":@"activate"}];else if([command isEqual:@"run"]&&![[self active] executeExtensionNamed:request[@"name"] query:request[@"query"]])TLog(@"extension command not found: %@",request[@"name"]);else if([command isEqual:@"rendererReport"])TSendCLIReply(request[@"replyPath"],[self rendererFieldReport]);else if([command isEqual:@"rendererRetry"])TSendCLIReply(request[@"replyPath"],[self rendererRetryReport]);else if([command isEqual:@"benchmark"]){TWindowController *controller=[self active];NSUInteger terminals=0;for(TWindowController *window in self.windows)terminals+=window.terminals.count;NSDictionary *reply=controller?TRunInAppBenchmark(self.config,controller.terminal,self.windows.count,terminals):@{@"ok":@NO};TSendCLIReply(request[@"replyPath"],reply);}else if([command isEqual:@"showBenchmark"]){NSString *path=[request[@"artifactPath"] isKindOfClass:NSString.class]?request[@"artifactPath"]:nil;BOOL directory=NO;if(path.length&&[NSFileManager.defaultManager fileExistsAtPath:path isDirectory:&directory]&&directory){self.benchmarkResults=[[TBenchmarkResultsController alloc]initWithArtifactPath:path current:[request[@"current"] isKindOfClass:NSDictionary.class]?request[@"current"]:@{}];[NSApp activateIgnoringOtherApps:YES];[self.benchmarkResults showWindow:nil];[self.benchmarkResults.window makeKeyAndOrderFront:nil];}}else if([command isEqual:@"benchmarkDiagnostics"])TSendCLIReply(request[@"replyPath"],self.benchmarkResults?[self.benchmarkResults diagnostics]:@{@"ok":@NO,@"error":@"benchmark window is not open"});else if([command isEqual:@"quit"])[NSApp terminate:nil];}
- (void)buildMenu {
...
@interface TAutomationStatusScriptCommand : NSScriptCommand
@end
@implementation TAutomationStatusScriptCommand
- (id)performDefaultImplementation {NSDictionary *reply=[(TAppDelegate *)NSApp.delegate automationReply:@{@"action":@"status"}];NSData *json=[NSJSONSerialization dataWithJSONObject:reply options:NSJSONWritingSortedKeys error:nil];return [[NSString alloc]initWithData:json encoding:NSUTF8StringEncoding];}
@end
@interface TOneWordAutomationScriptCommand : NSScriptCommand
@end
@implementation TOneWordAutomationScriptCommand
- (id)performDefaultImplementation {TAppDelegate *delegate=(TAppDelegate *)NSApp.delegate;NSString *name=self.commandDescription.commandName.lowercaseString,*direct=[self.directParameter isKindOfClass:NSString.class]?self.directParameter:@"";NSMutableDictionary *request=[NSMutableDictionary dictionary];NSDictionary *actions=@{@"newterminaltab":@"new-tab",@"newterminalwindow":@"new-window",@"splitterminal":@"split",@"splitterminalvertically":@"split",@"sendterminaltext":@"send",@"runterminalcommand":@"run",@"pressterminalkey":@"key"};NSString *action=actions[name];if(action){request[@"action"]=action;if([name isEqual:@"splitterminal"]||[name isEqual:@"splitterminalvertically"])request[@"direction"]=[name hasSuffix:@"vertically"]?@"vertical":@"horizontal";if([action isEqual:@"key"])request[@"key"]=direct;else if(direct.length)request[@"text"]=direct;}else if([name isEqual:@"focusterminal"]){NSArray *raw=[direct componentsSeparatedByCharactersInSet:NSCharacterSet.whitespaceCharacterSet];NSMutableArray *parts=[NSMutableArray array];for(NSString *part in raw)if(part.length)[parts addObject:part];request[@"action"]=@"focus";request[@"target"]=parts.count?parts[0]:@"";if(parts.count>1)request[@"index"]=@([parts[1] integerValue]);}else if([name isEqual:@"launchsshrecipe"]){NSString *failure=nil;NSDictionary *recipe=TLaunchRecipeNamed(direct);NSArray *commands=recipe?TLaunchRecipeCommands(recipe,&failure):nil;if(!commands){self.scriptErrorNumber=-1708;self.scriptErrorString=failure?:@"launch recipe not found";return nil;}request[@"action"]=@"recipe";request[@"commands"]=commands;request[@"direction"]=recipe[@"direction"]?:@"horizontal";}NSDictionary *reply=[delegate automationReply:request];if(![reply[@"ok"] boolValue]){self.scriptErrorNumber=-1708;self.scriptErrorString=reply[@"error"]?:@"automation failed";return nil;}return @YES;}
@end
#if TERMATICA_BENCHMARKS
...
static int TRunTerminalSelfTest(void) {
    ...
    CGFloat scrollAccumulator=0;CGFloat scrollUnit=18*0.22;if(TAccumulatePreciseScroll(scrollUnit*0.65,18,YES,&scrollAccumulator)!=0||TAccumulatePreciseScroll(-scrollUnit*0.65,18,NO,&scrollAccumulator)!=0||TAccumulatePreciseScroll(-scrollUnit*0.65,18,NO,&scrollAccumulator)!=-1)return 213;
    if(TRenderShouldPaintCellBackground(0x101216,TDefaultColor,NO,0x101216,0x101216)||!TRenderShouldPaintCellBackground(0x20242C,TDefaultColor,NO,0x20242C,0x101216)||!TRenderShouldPaintCellBackground(TDefaultColor,TDefaultColor,YES,0xD8DEE9,0x101216))return 214;
    NSRect laptopVisible=NSMakeRect(0,23,1440,877),largeExternalVisible=NSMakeRect(-3440,0,3440,1400);NSSize configuredWindow=NSMakeSize(580,350),minimumWindow=NSMakeSize(480,280);NSRect laptopWindow=TInitialWindowFrameForScreen(laptopVisible,configuredWindow,minimumWindow,18,2),fallbackWindow=TRemapWindowFrameToScreen(NSMakeRect(0,0,580,350),NSZeroRect,laptopVisible,minimumWindow,18,2),largeUsable=TUsableScreenFrame(largeExternalVisible,18,1),externalWindow=TRemapWindowFrameToScreen(laptopWindow,laptopVisible,largeExternalVisible,minimumWindow,18,1),hyprlandExternal=TUsableScreenFrame(largeExternalVisible,18,1),returnedWindow=TRemapWindowFrameToScreen(externalWindow,largeExternalVisible,laptopVisible,minimumWindow,18,2);
    if(!NSContainsRect(TUsableScreenFrame(laptopVisible,18,2),laptopWindow)||!NSContainsRect(TUsableScreenFrame(laptopVisible,18,2),fallbackWindow)||NSWidth(laptopWindow)<NSWidth(TUsableScreenFrame(laptopVisible,18,2))*0.83||NSHeight(laptopWindow)<NSHeight(TUsableScreenFrame(laptopVisible,18,2))*0.81)return 204;
    if(!NSContainsRect(largeUsable,externalWindow)||NSWidth(externalWindow)<=NSWidth(laptopWindow)*1.8||NSHeight(externalWindow)<=NSHeight(laptopWindow)*1.3||!NSEqualRects(hyprlandExternal,largeUsable))return 205;
    if(!NSContainsRect(TUsableScreenFrame(laptopVisible,18,2),returnedWindow)||fabs(NSWidth(returnedWindow)-NSWidth(laptopWindow))>1||fabs(NSHeight(returnedWindow)-NSHeight(laptopWindow))>1)return 206;
    ...
    NSEvent *capturedShortcut=[NSEvent keyEventWithType:NSEventTypeKeyDown location:NSZeroPoint modifierFlags:NSEventModifierFlagCommand|NSEventModifierFlagShift timestamp:0 windowNumber:0 context:nil characters:@"T" charactersIgnoringModifiers:@"t" isARepeat:NO keyCode:17];if(![[TShortcutForEvent(capturedShortcut) description] isEqual:@"cmd+shift+t"]||!TEventMatchesShortcut(capturedShortcut,@"shift+command+t"))return 215;[fresh startDiagnosticInputCapture];[fresh consumeData:[@"\033]777;term-keybinding-capture;TESTTOKEN\a" dataUsingEncoding:NSUTF8StringEncoding]];NSString *readyReply=[[NSString alloc]initWithData:[fresh finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding];if(![readyReply isEqual:@"TERMATICABIND:READY:TESTTOKEN\n"]||!fresh.hasPendingKeybindingCapture)return 216;[fresh startDiagnosticInputCapture];if(![fresh captureKeybindingEvent:capturedShortcut])return 217;NSString *captureReply=[[NSString alloc]initWithData:[fresh finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding];if(![captureReply isEqual:@"TERMATICABIND:VALUE:TESTTOKEN:cmd+shift+t\n"]||[fresh captureKeybindingEvent:capturedShortcut]||fresh.hasPendingKeybindingCapture)return 218;NSEvent *commandReturn=[NSEvent keyEventWithType:NSEventTypeKeyDown location:NSZeroPoint modifierFlags:NSEventModifierFlagCommand timestamp:0 windowNumber:0 context:nil characters:@"\r" charactersIgnoringModifiers:@"\r" isARepeat:NO keyCode:36];[fresh startDiagnosticInputCapture];[fresh consumeData:[@"\033]777;term-keybinding-capture;RETURNTEST\a" dataUsingEncoding:NSUTF8StringEncoding]];[fresh finishDiagnosticInputCapture];[fresh startDiagnosticInputCapture];if(![fresh captureKeybindingEvent:commandReturn]||![[[NSString alloc]initWithData:[fresh finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding] isEqual:@"TERMATICABIND:VALUE:RETURNTEST:cmd+return\n"]||!TEventMatchesShortcut(commandReturn,@"cmd+return"))return 219;
    ...
    fresh.activeTerminal=YES;TRenderSnapshot *cursorSeed=fresh.renderSnapshot;TRenderSnapshot *cursorBefore=[[TRenderSnapshot alloc]initWithGeneration:cursorSeed.generation metrics:cursorSeed.metrics cells:cursorSeed.cells underlineStyles:cursorSeed.underlineStyles selectionMask:cursorSeed.selectionMask searchMask:cursorSeed.searchMask linkMask:cursorSeed.linkMask graphemes:cursorSeed.graphemes style:cursorSeed.style links:cursorSeed.links images:cursorSeed.images cursorX:cursorSeed.cursorX cursorY:cursorSeed.cursorY cursorVisible:YES historyCount:cursorSeed.historyCount historyOffset:cursorSeed.historyOffset fullDamage:YES damagedRows:NSMakeRange(0,cursorSeed.metrics.rows)];NSRect cursorBeforeFrame=TCursorRect(cursorBefore);if(fabs(NSWidth(cursorBeforeFrame)-2)>0.01){fprintf(stderr,"terminal-self-test failed stage=cursor-composition-show frame=%s style=%s thickness=%.2f\n",NSStringFromRect(cursorBeforeFrame).UTF8String,[cursorBefore.style[@"cursorStyle"] UTF8String],[cursorBefore.style[@"cursorThickness"] doubleValue]);return 74;}
    [fresh consumeData:[@"\033[D" dataUsingEncoding:NSUTF8StringEncoding]];TRenderSnapshot *cursorAfterSeed=fresh.renderSnapshot;TRenderSnapshot *cursorAfter=[[TRenderSnapshot alloc]initWithGeneration:cursorAfterSeed.generation metrics:cursorAfterSeed.metrics cells:cursorAfterSeed.cells underlineStyles:cursorAfterSeed.underlineStyles selectionMask:cursorAfterSeed.selectionMask searchMask:cursorAfterSeed.searchMask linkMask:cursorAfterSeed.linkMask graphemes:cursorAfterSeed.graphemes style:cursorAfterSeed.style links:cursorAfterSeed.links images:cursorAfterSeed.images cursorX:cursorAfterSeed.cursorX cursorY:cursorAfterSeed.cursorY cursorVisible:YES historyCount:cursorAfterSeed.historyCount historyOffset:cursorAfterSeed.historyOffset fullDamage:YES damagedRows:NSMakeRange(0,cursorAfterSeed.metrics.rows)];NSRect cursorAfterFrame=TCursorRect(cursorAfter);if(fabs((NSMinX(cursorAfterFrame)-NSMinX(cursorBeforeFrame))+cursorAfter.metrics.cellWidth)>0.01||!NSEqualSizes(cursorBeforeFrame.size,cursorAfterFrame.size))return 75;
    fresh.activeTerminal=NO;
    ...
    TTerminalView *resizeScroll=[[TTerminalView alloc]initWithFrame:NSMakeRect(0,0,420,240) config:freshConfig];NSUInteger beforeResizeRows=[resizeScroll.diagnosticState[@"rows"] unsignedIntegerValue],beforeResizeColumns=[resizeScroll.diagnosticState[@"columns"] unsignedIntegerValue];NSMutableString *preResize=[NSMutableString string];for(NSUInteger i=0;i<beforeResizeRows+2;i++)[preResize appendFormat:@"pre-resize-%lu\r\n",(unsigned long)i];[resizeScroll consumeData:[preResize dataUsingEncoding:NSUTF8StringEncoding]];[resizeScroll setFrameSize:NSMakeSize(720,240)];TRenderSnapshot *resized=resizeScroll.renderSnapshot;if(resized.metrics.columns<=beforeResizeColumns)return 92;NSMutableString *alternateResize=[NSMutableString stringWithString:@"\033[?1049h"];for(NSUInteger i=0;i<resized.metrics.rows+2;i++)[alternateResize appendFormat:@"alternate-resize-%lu 漢🙂\r\n",(unsigned long)i];[resizeScroll consumeData:[alternateResize dataUsingEncoding:NSUTF8StringEncoding]];TRenderSnapshot *resizeScrolled=resizeScroll.renderSnapshot;if(!resizeScrolled.isValid||![[resizeScroll visibleText] containsString:@"alternate-resize-"])return 93;
    ...
    TTerminalView *graphemeLayout=[[TTerminalView alloc]initWithFrame:NSMakeRect(0,0,800,500) config:config];[graphemeLayout consumeData:[@"A👩🏽‍💻B क्षिC" dataUsingEncoding:NSUTF8StringEncoding]];TRenderSnapshot *graphemeSnapshot=graphemeLayout.renderSnapshot;const TCell *graphemeCells=graphemeSnapshot.cells.bytes;BOOL graphemeColumns=graphemeCells&&graphemeSnapshot.cursorX==7&&graphemeCells[0].ch=='A'&&(graphemeCells[1].flags&(TCluster|TWide))==(TCluster|TWide)&&(graphemeCells[2].flags&TContinuation)&&graphemeCells[3].ch=='B'&&graphemeCells[4].ch==' '&&(graphemeCells[5].flags&TCluster)&&!(graphemeCells[5].flags&TWide)&&graphemeCells[6].ch=='C';if(!graphemeColumns||![[graphemeLayout visibleText] isEqual:@"A👩🏽‍💻B क्षिC"]){fprintf(stderr,"grapheme layout failed text='%s' cursor=%lu\n",graphemeLayout.visibleText.UTF8String,(unsigned long)graphemeSnapshot.cursorX);return 90;}
    TTerminalView *codexRedraw=[[TTerminalView alloc]initWithFrame:NSMakeRect(0,0,800,500) config:config];[codexRedraw consumeData:[@"\033[2J\033[H\033[?2026hCodex preparing response with wide 漢🙂 tail\033[2;1Hstable-row\033[?2026l" dataUsingEncoding:NSUTF8StringEncoding]];[codexRedraw consumeData:[@"\033[?2026h\033[1;1HOK\033[K\033[2;1H漢\033[2GZ\033[K\033[?2026l" dataUsingEncoding:NSUTF8StringEncoding]];TRenderSnapshot *codexSnapshot=codexRedraw.renderSnapshot;const TCell *codexCells=codexSnapshot.cells.bytes;NSString *codexText=codexRedraw.visibleText;BOOL codexCellsClean=codexCells&&codexSnapshot.metrics.rows>=2&&codexSnapshot.metrics.columns>=2&&codexCells[0].ch=='O'&&codexCells[1].ch=='K'&&codexCells[codexSnapshot.metrics.columns].ch==' '&&codexCells[codexSnapshot.metrics.columns+1].ch=='Z'&&!(codexCells[codexSnapshot.metrics.columns].flags&(TWide|TContinuation))&&!(codexCells[codexSnapshot.metrics.columns+1].flags&(TWide|TContinuation));if(!codexCellsClean||![codexText isEqual:@"OK\n Z"]||[codexRedraw.diagnosticState[@"synchronizedUpdates"] boolValue]){fprintf(stderr,"codex redraw cleanup failed text='%s' cells-clean=%d state=%s\n",codexText.UTF8String,codexCellsClean,codexRedraw.diagnosticState.description.UTF8String);return 89;}
    TTerminalView *wideMetadata=[[TTerminalView alloc]initWithFrame:NSMakeRect(0,0,800,500) config:config];[wideMetadata consumeData:[@"\033]8;;https://example.com\a\033[4:3m漢\033[0m\033]8;;\a\033[2GZ" dataUsingEncoding:NSUTF8StringEncoding]];TRenderSnapshot *metadataSnapshot=wideMetadata.renderSnapshot;const TCell *metadataCells=metadataSnapshot.cells.bytes;const uint8_t *metadataUnderlines=metadataSnapshot.underlineStyles.bytes,*metadataLinks=metadataSnapshot.linkMask.bytes;BOOL metadataClean=metadataCells&&metadataUnderlines&&metadataLinks&&metadataCells[0].ch==' '&&metadataCells[1].ch=='Z'&&!(metadataCells[0].flags&(TWide|TContinuation))&&!metadataUnderlines[0]&&!metadataLinks[0]&&!metadataLinks[1];if(!metadataClean){fprintf(stderr,"wide metadata cleanup failed text='%s'\n",wideMetadata.visibleText.UTF8String);return 91;}
    TTerminalView *emojiUpgrade=[[TTerminalView alloc]initWithFrame:NSMakeRect(0,0,800,500) config:config];[emojiUpgrade consumeData:[@"x漢Z\033[H✈️" dataUsingEncoding:NSUTF8StringEncoding]];TRenderSnapshot *emojiSnapshot=emojiUpgrade.renderSnapshot;const TCell *emojiCells=emojiSnapshot.cells.bytes;BOOL emojiPairClean=emojiCells&&(emojiCells[0].flags&(TCluster|TWide))==(TCluster|TWide)&&(emojiCells[1].flags&TContinuation)&&emojiCells[2].ch==' '&&!(emojiCells[2].flags&(TWide|TContinuation))&&emojiCells[3].ch=='Z';if(!emojiPairClean||![[emojiUpgrade visibleText] isEqual:@"✈️ Z"]){fprintf(stderr,"emoji width upgrade cleanup failed text='%s'\n",emojiUpgrade.visibleText.UTF8String);return 103;}
    ...
    [terminal consumeData:[@"\033[>7u\033[=8;2u\033[=2;3u" dataUsingEncoding:NSUTF8StringEncoding]];[terminal startDiagnosticInputCapture];[terminal consumeData:[@"\033[?u" dataUsingEncoding:NSUTF8StringEncoding]];if(![[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding] isEqual:@"\033[?13u"])return 68;
    [terminal consumeData:[@"\033[<u\033[=1u\033[?1049h" dataUsingEncoding:NSUTF8StringEncoding]];[terminal startDiagnosticInputCapture];[terminal consumeData:[@"\033[?u" dataUsingEncoding:NSUTF8StringEncoding]];if(![[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding] isEqual:@"\033[?0u"])return 69;
    [terminal consumeData:[@"\033[=2u\033[?1049l" dataUsingEncoding:NSUTF8StringEncoding]];[terminal startDiagnosticInputCapture];[terminal consumeData:[@"\033[?u" dataUsingEncoding:NSUTF8StringEncoding]];if(![[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding] isEqual:@"\033[?1u"])return 70;
    [terminal consumeData:[@"\033[=31u" dataUsingEncoding:NSUTF8StringEncoding]];NSEvent *kittyPress=[NSEvent keyEventWithType:NSEventTypeKeyDown location:NSZeroPoint modifierFlags:NSEventModifierFlagShift timestamp:0 windowNumber:0 context:nil characters:@"A" charactersIgnoringModifiers:@"a" isARepeat:NO keyCode:0];NSEvent *kittyRepeat=[NSEvent keyEventWithType:NSEventTypeKeyDown location:NSZeroPoint modifierFlags:NSEventModifierFlagShift timestamp:0 windowNumber:0 context:nil characters:@"A" charactersIgnoringModifiers:@"a" isARepeat:YES keyCode:0];NSEvent *kittyRelease=[NSEvent keyEventWithType:NSEventTypeKeyUp location:NSZeroPoint modifierFlags:NSEventModifierFlagShift timestamp:0 windowNumber:0 context:nil characters:@"A" charactersIgnoringModifiers:@"a" isARepeat:NO keyCode:0];[terminal startDiagnosticInputCapture];[terminal keyDown:kittyPress];[terminal keyDown:kittyRepeat];[terminal keyUp:kittyRelease];[terminal insertText:@"å" replacementRange:NSMakeRange(NSNotFound,0)];NSString *kittyReport=[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding];if(![kittyReport isEqual:@"\033[97:65;2;65u\033[97:65;2:2;65u\033[97:65;2:3u\033[0;1;229u"])return 71;[terminal consumeData:[@"\033[=0u" dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    if([terminal shouldForwardApplicationMouseWithModifiers:NSEventModifierFlagShift]||[terminal shouldForwardApplicationMouseWithModifiers:NSEventModifierFlagCommand])return 92;
    ...
    [terminal scrollByLines:-100000];
    NSEventModifierFlags none=0;NSPoint mousePoint=NSMakePoint(160,120),mousePoint2=NSMakePoint(200,145);NSEvent *leftDown=[NSEvent mouseEventWithType:NSEventTypeLeftMouseDown location:mousePoint modifierFlags:none timestamp:1 windowNumber:0 context:nil eventNumber:20 clickCount:1 pressure:1],*leftUp=[NSEvent mouseEventWithType:NSEventTypeLeftMouseUp location:mousePoint modifierFlags:none timestamp:2 windowNumber:0 context:nil eventNumber:21 clickCount:1 pressure:0],*rightDown=[NSEvent mouseEventWithType:NSEventTypeRightMouseDown location:mousePoint modifierFlags:none timestamp:3 windowNumber:0 context:nil eventNumber:22 clickCount:1 pressure:1],*rightUp=[NSEvent mouseEventWithType:NSEventTypeRightMouseUp location:mousePoint modifierFlags:none timestamp:4 windowNumber:0 context:nil eventNumber:23 clickCount:1 pressure:0],*middleDown=[NSEvent mouseEventWithType:NSEventTypeOtherMouseDown location:mousePoint modifierFlags:none timestamp:5 windowNumber:0 context:nil eventNumber:24 clickCount:1 pressure:1],*middleUp=[NSEvent mouseEventWithType:NSEventTypeOtherMouseUp location:mousePoint modifierFlags:none timestamp:6 windowNumber:0 context:nil eventNumber:25 clickCount:1 pressure:0];
    [terminal startDiagnosticInputCapture];[terminal mouseDown:leftDown];[terminal mouseUp:leftUp];[terminal rightMouseDown:rightDown];[terminal rightMouseUp:rightUp];[terminal otherMouseDown:middleDown];[terminal otherMouseUp:middleUp];NSString *buttonReport=[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding];if(![buttonReport containsString:@"\033[<0;"]||![buttonReport containsString:@"m\033[<2;"]||![buttonReport containsString:@"m\033[<1;"]||![buttonReport hasSuffix:@"m"]){fprintf(stderr,"mouse button report mismatch: %s\n",buttonReport.UTF8String);return 83;}
    NSEvent *shiftDown=[NSEvent mouseEventWithType:NSEventTypeLeftMouseDown location:mousePoint modifierFlags:NSEventModifierFlagShift timestamp:6.5 windowNumber:0 context:nil eventNumber:28 clickCount:1 pressure:1];[terminal startDiagnosticInputCapture];[terminal mouseDown:shiftDown];if([terminal finishDiagnosticInputCapture].length||![terminal.diagnosticState[@"selecting"] boolValue])return 93;[terminal mouseUp:leftUp];
    [terminal consumeData:[@"\033[?1002h" dataUsingEncoding:NSUTF8StringEncoding]];NSEvent *rightDrag=[NSEvent mouseEventWithType:NSEventTypeRightMouseDragged location:mousePoint2 modifierFlags:none timestamp:7 windowNumber:0 context:nil eventNumber:26 clickCount:1 pressure:1];[terminal startDiagnosticInputCapture];[terminal rightMouseDown:rightDown];[terminal rightMouseDragged:rightDrag];[terminal rightMouseDragged:rightDrag];[terminal rightMouseUp:rightUp];NSString *dragReport=[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding];if(![dragReport containsString:@"\033[<2;"]||!([dragReport componentsSeparatedByString:@"\033[<34;"].count==2)||![dragReport hasSuffix:@"m"])return 84;
    [terminal consumeData:[@"\033[?1003h" dataUsingEncoding:NSUTF8StringEncoding]];NSEvent *move=[NSEvent mouseEventWithType:NSEventTypeMouseMoved location:mousePoint2 modifierFlags:none timestamp:8 windowNumber:0 context:nil eventNumber:27 clickCount:0 pressure:0];[terminal startDiagnosticInputCapture];[terminal mouseMoved:move];[terminal mouseMoved:move];NSString *motionReport=[[NSString alloc]initWithData:[terminal finishDiagnosticInputCapture] encoding:NSUTF8StringEncoding];if(![motionReport hasPrefix:@"\033[<35;"]||[motionReport componentsSeparatedByString:@"\033[<"].count!=2)return 85;
    [terminal consumeData:[@"\033[?1003l\033[?1000h\033[?1006l" dataUsingEncoding:NSUTF8StringEncoding]];[terminal startDiagnosticInputCapture];[terminal mouseDown:leftDown];[terminal mouseUp:leftUp];NSData *legacyReport=[terminal finishDiagnosticInputCapture];const uint8_t *legacyBytes=legacyReport.bytes;if(legacyReport.length!=12||legacyBytes[3]!=(uint8_t)32||legacyBytes[9]!=(uint8_t)(32+3))return 86;
    [terminal consumeData:[@"\033[?1000l\033[?9h" dataUsingEncoding:NSUTF8StringEncoding]];[terminal startDiagnosticInputCapture];[terminal mouseDown:leftDown];[terminal mouseUp:leftUp];if([terminal finishDiagnosticInputCapture].length!=6)return 87;[terminal consumeData:[@"\033[?9l" dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    TBlockElementRect fullBlock[2],cornerBlock[2];if(TBlockElementRects(0x2588,fullBlock)!=1||fullBlock[0].x!=0||fullBlock[0].y!=0||fullBlock[0].width!=1||fullBlock[0].height!=1||TBlockElementRects(0x2599,cornerBlock)!=2||TBlockElementRects('A',cornerBlock)!=0)return 94;
    TTerminalView *claudeLogo=[[TTerminalView alloc]initWithFrame:NSMakeRect(0,0,800,500) config:config];[claudeLogo consumeData:[@"▐▛███▜▌\r\n▝▜█████▛▘\r\n  ▘▘ ▝▝" dataUsingEncoding:NSUTF8StringEncoding]];if(![[claudeLogo visibleText] containsString:@"▐▛███▜▌\n▝▜█████▛▘\n  ▘▘ ▝▝"])return 95;
    ...
    for(NSString *normalRenderer in @[@"appkit",@"metal"]){TConfig *normalTabConfig=[TConfig new];normalTabConfig.renderer=normalRenderer;normalTabConfig.hyprlandLayout=NO;normalTabConfig.tabAnimations=YES;normalTabConfig.shell=@"/usr/bin/true";normalTabConfig.shellArguments=@[];TWindowController *normalTabs=[[TWindowController alloc]initWithConfig:normalTabConfig extensions:nil];[normalTabs.window orderFront:nil];[normalTabs.window makeKeyWindow];for(NSUInteger cycle=0;cycle<4;cycle++){[normalTabs addTab];[normalTabs layoutTabs];NSUInteger visible=0,active=0;NSView *launchSurface=[normalTabs valueForKey:@"tileSurface"];CAShapeLayer *launchSurfaceLayer=[normalTabs valueForKey:@"tileSurfaceLayer"];NSColor *launchColor=[NSColor colorWithCGColor:launchSurfaceLayer.fillColor],*expectedColor=normalTabConfig.background;launchColor=[launchColor colorUsingColorSpace:NSColorSpace.sRGBColorSpace];expectedColor=[expectedColor colorUsingColorSpace:NSColorSpace.sRGBColorSpace];if(launchSurface.hidden||!launchSurfaceLayer.path||!launchColor||!expectedColor||fabs(launchColor.redComponent-expectedColor.redComponent)>0.001||fabs(launchColor.greenComponent-expectedColor.greenComponent)>0.001||fabs(launchColor.blueComponent-expectedColor.blueComponent)>0.001||fabs(launchColor.alphaComponent-expectedColor.alphaComponent)>0.001){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 218;}for(TTerminalView *item in normalTabs.terminals){visible+=!item.hidden;active+=item.activeTerminal;BOOL expectsLayer=[item.diagnosticState[@"renderer"] isEqual:@"metal"];if(item.wantsLayer!=expectsLayer||item.layer.mask||item.layer.animationKeys.count||item.isOpaque||[item.renderSnapshot.style[@"backgroundAlpha"] doubleValue]!=0){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 77;}}if(normalTabs.terminals.count!=cycle+2||visible!=1||active!=1){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 78;}}
        [normalTabs selectTabNumber:1];NSUInteger selectedVisible=0;for(TTerminalView *item in normalTabs.terminals){selectedVisible+=!item.hidden;BOOL expectsLayer=[item.diagnosticState[@"renderer"] isEqual:@"metal"];if(item.wantsLayer!=expectsLayer||item.layer.mask||item.layer.animationKeys.count){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 79;}}if(normalTabs.terminal!=normalTabs.terminals.firstObject||selectedVisible!=1){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 80;}[normalTabs selectTabNumber:2];TTerminalView *selectedBeforeInsert=normalTabs.terminal,*followingBeforeInsert=normalTabs.terminals[2];[normalTabs addTab];TTerminalView *ordinaryTab=normalTabs.terminal;NSUInteger selectedIndex=[normalTabs.terminals indexOfObject:selectedBeforeInsert],ordinaryIndex=[normalTabs.terminals indexOfObject:ordinaryTab],followingIndex=[normalTabs.terminals indexOfObject:followingBeforeInsert];if(ordinaryIndex!=selectedIndex+1||followingIndex!=ordinaryIndex+1||ordinaryTab.splitAnchor){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 197;}NSArray<TTabButton *> *orderedButtons=[normalTabs valueForKey:@"tabButtons"];if(ordinaryIndex>=orderedButtons.count||NSMidY(orderedButtons[selectedIndex].frame)<=NSMidY(orderedButtons[ordinaryIndex].frame)){for(TTerminalView *cleanup in normalTabs.terminals)[cleanup stopShellTerminating:YES];[normalTabs.window close];return 198;}for(TTerminalView *item in normalTabs.terminals)[item stopShellTerminating:YES];[normalTabs.window close];}
    TConfig *shortcutConfig=[TConfig new];shortcutConfig.renderer=@"appkit";shortcutConfig.hyprlandLayout=NO;shortcutConfig.tabAnimations=NO;shortcutConfig.shell=@"/usr/bin/true";shortcutConfig.shellArguments=@[];TWindowController *shortcutSplits=[[TWindowController alloc]initWithConfig:shortcutConfig extensions:nil];[shortcutSplits.window orderFront:nil];[shortcutSplits.window makeKeyWindow];TAppDelegate *shortcutDelegate=[TAppDelegate new];shortcutDelegate.windows=[NSMutableArray arrayWithObject:shortcutSplits];TTerminalView *firstShortcutRoot=shortcutSplits.terminal;[shortcutSplits addTab];TTerminalView *secondShortcutRoot=shortcutSplits.terminal;[shortcutSplits focusTerminal:firstShortcutRoot];[shortcutDelegate newTab:nil];TTerminalView *commandTTab=shortcutSplits.terminal;NSArray<TTerminalView *> *commandTGroup=[shortcutSplits visibleLayoutTerminals];NSUInteger commandTVisible=0;for(TTerminalView *item in shortcutSplits.terminals)commandTVisible+=!item.hidden;NSUInteger firstShortcutIndex=[shortcutSplits.terminals indexOfObject:firstShortcutRoot],commandTIndex=[shortcutSplits.terminals indexOfObject:commandTTab],secondShortcutIndex=[shortcutSplits.terminals indexOfObject:secondShortcutRoot];if(commandTTab.splitAnchor||commandTGroup.count!=1||commandTVisible!=1||!firstShortcutRoot.hidden||!secondShortcutRoot.hidden||firstShortcutIndex+1!=commandTIndex||commandTIndex+1!=secondShortcutIndex)return 200;[shortcutSplits focusTerminal:secondShortcutRoot];[shortcutSplits addSplitWithVerticalDivider:NO];TTerminalView *commandShiftTLower=shortcutSplits.terminal;NSArray<TTerminalView *> *commandShiftTGroup=[shortcutSplits visibleLayoutTerminals];NSUInteger commandShiftTVisible=0;for(TTerminalView *item in shortcutSplits.terminals)commandShiftTVisible+=!item.hidden;if(commandShiftTLower.splitAnchor!=secondShortcutRoot||commandShiftTLower.verticalSplit||commandShiftTGroup.count!=2||commandShiftTVisible!=2||!firstShortcutRoot.hidden||!commandTTab.hidden||fabs(NSWidth(commandShiftTLower.frame)-NSWidth(secondShortcutRoot.frame))>1||!NSIsEmptyRect(NSIntersectionRect(commandShiftTLower.frame,secondShortcutRoot.frame)))return 201;NSRect focusedHalfBefore=secondShortcutRoot.frame,siblingHalfBefore=commandShiftTLower.frame;[shortcutSplits focusTerminal:secondShortcutRoot];[shortcutSplits addSplitWithVerticalDivider:NO];TTerminalView *nestedHorizontal=shortcutSplits.terminal;NSArray<TTerminalView *> *nestedGroup=[shortcutSplits visibleLayoutTerminals];CGFloat nestedGap=shortcutConfig.tileGap,nestedHeight=NSHeight(secondShortcutRoot.frame)+NSHeight(nestedHorizontal.frame)+nestedGap;if(nestedHorizontal.splitAnchor!=secondShortcutRoot||nestedHorizontal.verticalSplit||nestedGroup.count!=3||!NSEqualRects(commandShiftTLower.frame,siblingHalfBefore)||!NSContainsRect(focusedHalfBefore,secondShortcutRoot.frame)||!NSContainsRect(focusedHalfBefore,nestedHorizontal.frame)||fabs(nestedHeight-NSHeight(focusedHalfBefore))>1||fabs(NSWidth(secondShortcutRoot.frame)-NSWidth(focusedHalfBefore))>1||fabs(NSWidth(nestedHorizontal.frame)-NSWidth(focusedHalfBefore))>1||!NSIsEmptyRect(NSIntersectionRect(secondShortcutRoot.frame,nestedHorizontal.frame))||!NSIsEmptyRect(NSIntersectionRect(commandShiftTLower.frame,nestedHorizontal.frame)))return 202;for(TTerminalView *item in shortcutSplits.terminals)[item stopShellTerminating:YES];[shortcutSplits.window close];
    TConfig *hyprCommandConfig=[TConfig new];hyprCommandConfig.renderer=@"appkit";hyprCommandConfig.hyprlandLayout=YES;hyprCommandConfig.tabAnimations=NO;hyprCommandConfig.shell=@"/usr/bin/true";hyprCommandConfig.shellArguments=@[];TWindowController *hyprCommandTabs=[[TWindowController alloc]initWithConfig:hyprCommandConfig extensions:nil];[hyprCommandTabs.window orderFront:nil];[hyprCommandTabs.window makeKeyWindow];TAppDelegate *hyprCommandDelegate=[TAppDelegate new];hyprCommandDelegate.windows=[NSMutableArray arrayWithObject:hyprCommandTabs];TTerminalView *hyprRoot=hyprCommandTabs.terminal;[hyprCommandDelegate newTab:nil];TTerminalView *hyprRight=hyprCommandTabs.terminal;[hyprCommandDelegate newTab:nil];TTerminalView *hyprLowerRight=hyprCommandTabs.terminal;[hyprCommandDelegate newTab:nil];TTerminalView *hyprLowerRightSide=hyprCommandTabs.terminal;[hyprCommandTabs layoutTabs];if(hyprCommandTabs.terminals.count!=4||[hyprCommandTabs visibleLayoutTerminals].count!=4||hyprCommandTabs.terminal!=hyprLowerRightSide||hyprRight.splitAnchor!=hyprRoot||!hyprRight.verticalSplit||hyprLowerRight.splitAnchor!=hyprRight||hyprLowerRight.verticalSplit||hyprLowerRightSide.splitAnchor!=hyprLowerRight||!hyprLowerRightSide.verticalSplit||NSMinX(hyprRight.frame)<=NSMinX(hyprRoot.frame)||NSMinY(hyprRight.frame)<=NSMinY(hyprLowerRight.frame)||NSMinX(hyprLowerRightSide.frame)<=NSMinX(hyprLowerRight.frame))return 211;for(NSUInteger i=0;i<hyprCommandTabs.terminals.count;i++){TTerminalView *item=hyprCommandTabs.terminals[i];if(item.hidden)return 211;for(NSUInteger j=i+1;j<hyprCommandTabs.terminals.count;j++)if(!NSIsEmptyRect(NSIntersectionRect(item.frame,hyprCommandTabs.terminals[j].frame)))return 211;}for(TTerminalView *item in hyprCommandTabs.terminals)[item stopShellTerminating:YES];[hyprCommandTabs.window close];
    TConfig *mixedShortcutConfig=[TConfig new];mixedShortcutConfig.renderer=@"appkit";mixedShortcutConfig.hyprlandLayout=YES;mixedShortcutConfig.tabAnimations=NO;mixedShortcutConfig.shell=@"/usr/bin/true";mixedShortcutConfig.shellArguments=@[];TWindowController *mixedShortcutSplits=[[TWindowController alloc]initWithConfig:mixedShortcutConfig extensions:nil];[mixedShortcutSplits.window orderFront:nil];[mixedShortcutSplits.window makeKeyWindow];TTerminalView *mixedRoot=mixedShortcutSplits.terminal;[mixedShortcutSplits addTab];TTerminalView *mixedRight=mixedShortcutSplits.terminal;NSRect leftHalfBefore=mixedRoot.frame,rightHalfBefore=mixedRight.frame;[mixedShortcutSplits focusTerminal:mixedRoot];[mixedShortcutSplits addSplitWithVerticalDivider:NO];TTerminalView *mixedLowerLeft=mixedShortcutSplits.terminal;NSArray<TTerminalView *> *mixedGroup=[mixedShortcutSplits visibleLayoutTerminals];CGFloat mixedGap=mixedShortcutConfig.tileGap,mixedHeight=NSHeight(mixedRoot.frame)+NSHeight(mixedLowerLeft.frame)+mixedGap;if(mixedRight.splitAnchor||mixedLowerLeft.splitAnchor!=mixedRoot||mixedLowerLeft.verticalSplit||mixedGroup.count!=3||!NSEqualRects(mixedRight.frame,rightHalfBefore)||!NSContainsRect(leftHalfBefore,mixedRoot.frame)||!NSContainsRect(leftHalfBefore,mixedLowerLeft.frame)||fabs(mixedHeight-NSHeight(leftHalfBefore))>1||fabs(NSWidth(mixedRoot.frame)-NSWidth(leftHalfBefore))>1||fabs(NSWidth(mixedLowerLeft.frame)-NSWidth(leftHalfBefore))>1||!NSIsEmptyRect(NSIntersectionRect(mixedRoot.frame,mixedLowerLeft.frame))||!NSIsEmptyRect(NSIntersectionRect(mixedRight.frame,mixedLowerLeft.frame)))return 203;for(TTerminalView *item in mixedShortcutSplits.terminals)[item stopShellTerminating:YES];[mixedShortcutSplits.window close];
    TConfig *splitConfig=[TConfig new];splitConfig.renderer=@"appkit";splitConfig.hyprlandLayout=NO;splitConfig.tabAnimations=NO;splitConfig.shell=@"/usr/bin/true";splitConfig.shellArguments=@[];TWindowController *splits=[[TWindowController alloc]initWithConfig:splitConfig extensions:nil];[splits.window orderFront:nil];[splits.window makeKeyWindow];TTerminalView *splitRoot=splits.terminal;[splits addSplitWithVerticalDivider:NO];TTerminalView *lower=splits.terminal;if(lower.splitAnchor!=splitRoot||lower.verticalSplit||fabs(NSWidth(lower.frame)-NSWidth(splitRoot.frame))>1||!NSIsEmptyRect(NSIntersectionRect(lower.frame,splitRoot.frame)))return 166;NSRect upperPosition=splitRoot.frame,lowerPosition=lower.frame;NSArray<TTerminalView *> *ordinarySplitIdentity=[splits.terminals copy];if(![splits moveLayoutPositionOfTerminal:splitRoot toTerminal:lower])return 209;[splits layoutTabs];if(!NSEqualRects(splitRoot.frame,lowerPosition)||!NSEqualRects(lower.frame,upperPosition)||lower.splitAnchor!=splitRoot||![splits.terminals isEqualToArray:ordinarySplitIdentity])return 209;if(![splits moveLayoutPositionOfTerminal:splitRoot toTerminal:lower])return 210;[splits layoutTabs];if(!NSEqualRects(splitRoot.frame,upperPosition)||!NSEqualRects(lower.frame,lowerPosition))return 210;[splits focusTerminal:splitRoot];[splits addSplitWithVerticalDivider:YES];TTerminalView *right=splits.terminal;NSArray<TTerminalView *> *splitGroup=[splits visibleLayoutTerminals];if(right.splitAnchor!=splitRoot||!right.verticalSplit||splitGroup.count!=3||!NSIsEmptyRect(NSIntersectionRect(right.frame,splitRoot.frame)))return 167;for(NSUInteger i=0;i<splitGroup.count;i++){if(splitGroup[i].hidden||!NSContainsRect(splits.window.contentView.bounds,splitGroup[i].frame))return 168;for(NSUInteger j=i+1;j<splitGroup.count;j++)if(!NSIsEmptyRect(NSIntersectionRect(splitGroup[i].frame,splitGroup[j].frame)))return 169;}[splits revealTabRail];if(![[splits valueForKey:@"tabRailVisible"] boolValue]||![splits valueForKey:@"tabHideTimer"])return 176;[splits hideTabRail];if([[splits valueForKey:@"tabRailVisible"] boolValue])return 177;splitConfig.tabAutoHide=NO;[splits revealTabRail];if(![[splits valueForKey:@"tabRailVisible"] boolValue]||[splits valueForKey:@"tabHideTimer"])return 178;[splits addTab];TTerminalView *ordinary=splits.terminal;NSUInteger ordinaryVisible=0;for(TTerminalView *item in splits.terminals)ordinaryVisible+=!item.hidden;if(ordinary.splitAnchor||ordinaryVisible!=1||[splits usesTiledLayout])return 170;[splits selectTabNumber:1];NSUInteger restoredVisible=0;for(TTerminalView *item in splits.terminals)restoredVisible+=!item.hidden;if(restoredVisible!=3||![splits usesTiledLayout])return 171;[splits.window setContentSize:NSMakeSize(480,280)];for(NSUInteger i=0;i<14;i++)[splits addTab];NSArray<TTabButton *> *railButtons=[splits valueForKey:@"tabButtons"];TTabRailView *rail=[splits valueForKey:@"tabRail"];NSMutableArray<TTabButton *> *shownButtons=[NSMutableArray array];for(TTabButton *button in railButtons)if(!button.hidden)[shownButtons addObject:button];if(!shownButtons.count||shownButtons.count>=railButtons.count||rail.tabCount!=shownButtons.count)return 172;for(NSUInteger i=0;i<shownButtons.count;i++){if(!NSContainsRect(rail.bounds,shownButtons[i].frame))return 173;for(NSUInteger j=i+1;j<shownButtons.count;j++)if(!NSIsEmptyRect(NSIntersectionRect(shownButtons[i].frame,shownButtons[j].frame)))return 174;}TTerminalView *beforeScroll=splits.terminal;if(!rail.scrollTabs)return 179;rail.scrollTabs(-1);if(splits.terminal==beforeScroll)return 179;for(TTerminalView *item in splits.terminals)[item stopShellTerminating:YES];[splits.window close];
    TConfig *tiledConfig=[TConfig new];tiledConfig.renderer=@"appkit";tiledConfig.hyprlandLayout=YES;tiledConfig.tabAnimations=YES;tiledConfig.blur=YES;tiledConfig.hyprlandBlur=YES;tiledConfig.backgroundOpacity=0.37;tiledConfig.background=[NSColor colorWithSRGBRed:0.12 green:0.18 blue:0.26 alpha:tiledConfig.backgroundOpacity];tiledConfig.shell=@"/usr/bin/true";tiledConfig.shellArguments=@[];
    TWindowController *tiles=[[TWindowController alloc]initWithConfig:tiledConfig extensions:nil];[tiles addTab];[tiles.window orderFront:nil];[tiles.window makeKeyWindow];
    if(tiles.terminals.count!=2)return 18;NSUInteger splitCursorCount=0;for(TTerminalView *tile in tiles.terminals)splitCursorCount+=[tile.diagnosticState[@"compositedCursorVisible"] boolValue];if(splitCursorCount!=(tiles.window.isKeyWindow?1:0)||[tiles.terminals.firstObject.diagnosticState[@"compositedCursorVisible"] boolValue])return 216;NSVisualEffectView *stableEffect=[tiles valueForKey:@"effect"];CAShapeLayer *initialEffectMask=(CAShapeLayer *)stableEffect.layer.mask;CAShapeLayer *initialSurfaceLayer=[tiles valueForKey:@"tileSurfaceLayer"];if(!stableEffect||![initialEffectMask isKindOfClass:CAShapeLayer.class]||!CGPathEqualToPath(initialEffectMask.path,initialSurfaceLayer.path)||initialEffectMask.animationKeys.count||initialEffectMask.actions[@"path"]!=NSNull.null||initialEffectMask.actions[@"position"]!=NSNull.null||initialEffectMask.actions[@"bounds"]!=NSNull.null)return 215;for(TTerminalView *tile in tiles.terminals)if(tile.layer.mask||[tile.layer animationForKey:@"termatica.hypr.snap"])return 215;
    ...
    TTerminalView *leftRoot=tiles.terminals[0],*rightRoot=tiles.terminals[1];[leftRoot consumeData:[@"POSITION-LEFT-CONTENT" dataUsingEncoding:NSUTF8StringEncoding]];[rightRoot consumeData:[@"POSITION-RIGHT-CONTENT" dataUsingEncoding:NSUTF8StringEncoding]];NSRect leftRootBefore=leftRoot.frame,rightRootBefore=rightRoot.frame;NSArray<TTerminalView *> *identityOrder=[tiles.terminals copy];NSEvent *halfDragDown=[NSEvent mouseEventWithType:NSEventTypeLeftMouseDown location:NSMakePoint(NSMidX(leftRootBefore),NSMidY(leftRootBefore)) modifierFlags:NSEventModifierFlagCommand timestamp:0.1 windowNumber:tiles.window.windowNumber context:nil eventNumber:101 clickCount:1 pressure:1],*halfDragMove=[NSEvent mouseEventWithType:NSEventTypeLeftMouseDragged location:NSMakePoint(NSMidX(rightRootBefore),NSMidY(rightRootBefore)) modifierFlags:NSEventModifierFlagCommand timestamp:0.2 windowNumber:tiles.window.windowNumber context:nil eventNumber:102 clickCount:1 pressure:1],*halfDragUp=[NSEvent mouseEventWithType:NSEventTypeLeftMouseUp location:NSMakePoint(NSMidX(rightRootBefore),NSMidY(rightRootBefore)) modifierFlags:NSEventModifierFlagCommand timestamp:0.3 windowNumber:tiles.window.windowNumber context:nil eventNumber:103 clickCount:1 pressure:0];[leftRoot mouseDown:halfDragDown];[leftRoot mouseDragged:halfDragMove];[leftRoot mouseUp:halfDragUp];if(!NSEqualRects(leftRoot.frame,rightRootBefore)||!NSEqualRects(rightRoot.frame,leftRootBefore)||![tiles.terminals isEqualToArray:identityOrder]||leftRoot.splitAnchor||rightRoot.splitAnchor||![[leftRoot visibleText] containsString:@"POSITION-LEFT-CONTENT"]||![[rightRoot visibleText] containsString:@"POSITION-RIGHT-CONTENT"])return 207;if(![tiles moveLayoutPositionOfTerminal:leftRoot toTerminal:rightRoot])return 208;[tiles layoutTabs];if(!NSEqualRects(leftRoot.frame,leftRootBefore)||!NSEqualRects(rightRoot.frame,rightRootBefore))return 208;tiles.terminal=leftRoot;[tiles addSplitWithVerticalDivider:NO];[tiles layoutTabs];TTerminalView *quarter=tiles.terminal;CGFloat quarterHeight=NSHeight(leftRoot.frame)+NSHeight(quarter.frame)+tiledConfig.tileGap;
    if(tiles.terminals.count!=3||!NSEqualRects(rightRoot.frame,rightRootBefore)||!NSContainsRect(leftRootBefore,leftRoot.frame)||!NSContainsRect(leftRootBefore,quarter.frame)||fabs(quarterHeight-NSHeight(leftRootBefore))>1||fabs(NSWidth(quarter.frame)-NSWidth(leftRootBefore))>1||!NSIsEmptyRect(NSIntersectionRect(leftRoot.frame,quarter.frame))||!NSIsEmptyRect(NSIntersectionRect(rightRoot.frame,quarter.frame)))return 52;
    NSView *uniformSurface=[tiles valueForKey:@"tileSurface"];CAShapeLayer *uniformSurfaceLayer=[tiles valueForKey:@"tileSurfaceLayer"];NSColor *surfaceColor=[NSColor colorWithCGColor:uniformSurfaceLayer.fillColor];NSColor *expectedSurface=[tiledConfig.background colorUsingColorSpace:NSColorSpace.sRGBColorSpace],*actualSurface=[surfaceColor colorUsingColorSpace:NSColorSpace.sRGBColorSpace];if(uniformSurface.hidden||!uniformSurfaceLayer.path||!actualSurface||fabs(actualSurface.redComponent-expectedSurface.redComponent)>0.001||fabs(actualSurface.greenComponent-expectedSurface.greenComponent)>0.001||fabs(actualSurface.blueComponent-expectedSurface.blueComponent)>0.001||fabs(actualSurface.alphaComponent-expectedSurface.alphaComponent)>0.001)return 212;CAShapeLayer *splitEffectMask=(CAShapeLayer *)stableEffect.layer.mask;if(!CGPathEqualToPath(splitEffectMask.path,uniformSurfaceLayer.path)||splitEffectMask.animationKeys.count)return 215;uint32_t uniformBackground=TRGB(tiledConfig.background);for(TTerminalView *tile in tiles.terminals){TRenderSnapshot *snapshot=tile.renderSnapshot;if([snapshot.style[@"backgroundAlpha"] doubleValue]!=0||[snapshot.style[@"background"] unsignedIntValue]!=uniformBackground||tile.isOpaque||tile.layer.mask||[tile.layer animationForKey:@"termatica.hypr.snap"])return 212;}
    for(TTerminalView *focusTarget in @[leftRoot,rightRoot,quarter,leftRoot,quarter]){
        [tiles focusTerminal:focusTarget];
        NSUInteger expectedCursorCount=tiles.window.isKeyWindow?1:0,immediateActiveCount=0,immediateCursorCount=0,immediateCompositedCount=0;for(TTerminalView *tile in tiles.terminals){immediateActiveCount+=tile.activeTerminal;immediateCursorCount+=tile.renderSnapshot.cursorVisible;immediateCompositedCount+=[tile.diagnosticState[@"compositedCursorVisible"] boolValue];}
        if(tiles.window.firstResponder!=focusTarget||immediateActiveCount!=1||immediateCursorCount!=expectedCursorCount||immediateCompositedCount!=expectedCursorCount)return 68;
        ...
        NSUInteger activeCount=0,visibleCursorCount=0,displayCursorCount=0,compositedCursorCount=0;
    ...
    if(TUseIntelCursorOverlay()){NSUInteger intelCursorOverlayUpdates=0,intelCursorOverlays=0;for(TTerminalView *tile in tiles.terminals){intelCursorOverlayUpdates+=[tile.diagnosticState[@"intelCursorOverlayUpdates"] unsignedIntegerValue];intelCursorOverlays+=[tile.diagnosticState[@"intelCursorOverlayVisible"] boolValue];}if(!intelCursorOverlayUpdates||intelCursorOverlays!=(tiles.window.isKeyWindow?1:0))return 219;}
    ...
    if(!NSEqualSizes(quarter.frame.size,quarterSlot.size)||!NSEqualSizes(leftRoot.frame.size,rootQuarterSlot.size)||!NSEqualSizes(rightRoot.frame.size,halfSlot.size)||NSMinX(quarter.frame)<=NSMinX(quarterSlot)||NSMinX(leftRoot.frame)<=NSMinX(rootQuarterSlot)||NSMinX(rightRoot.frame)>=NSMinX(halfSlot)||quarter.splitAnchor!=quarterAnchor||rightRoot.splitAnchor||![tiles.terminals isEqualToArray:mixedIdentityOrder])return 53;
    CABasicAnimation *quarterMotion=(CABasicAnimation *)[quarter.layer animationForKey:@"termatica.hypr.snap"],*rightMotion=(CABasicAnimation *)[rightRoot.layer animationForKey:@"termatica.hypr.snap"];BOOL expectsMotion=[tiles animationsEnabled]&&![tiles requiresStableTiledCompositing];if(expectsMotion&&!quarterMotion&&!rightMotion)return 56;if(!expectsMotion&&(quarterMotion||rightMotion))return 56;
    if(expectsMotion)for(CABasicAnimation *motion in @[quarterMotion?:rightMotion,rightMotion?:quarterMotion]){CATransform3D from=[motion.fromValue CATransform3DValue];if(fabs(from.m11-1)>0.0001||fabs(from.m22-1)>0.0001||fabs(from.m33-1)>0.0001)return 175;}
    ...
    TConfig *denseConfig=[TConfig new];denseConfig.renderer=@"appkit";denseConfig.hyprlandLayout=YES;denseConfig.tabAnimations=YES;denseConfig.blur=NO;denseConfig.shell=@"/usr/bin/true";denseConfig.shellArguments=@[];TWindowController *dense=[[TWindowController alloc]initWithConfig:denseConfig extensions:nil];[dense.window orderFront:nil];[dense.window setContentSize:NSMakeSize(1200,800)];TTerminalView *denseRoot=dense.terminal;[denseRoot consumeData:[@"DENSE-ROOT-CONTENT" dataUsingEncoding:NSUTF8StringEncoding]];for(NSUInteger index=1;index<16;index++){[dense addTab];[dense.terminal consumeData:[[NSString stringWithFormat:@"DENSE-%02lu-CONTENT",(unsigned long)index] dataUsingEncoding:NSUTF8StringEncoding]];}[dense layoutTabs];if(dense.terminals.count!=16)return 96;CGFloat denseRootArea=NSWidth(denseRoot.frame)*NSHeight(denseRoot.frame),denseMinimumWidth=CGFLOAT_MAX,denseMinimumHeight=CGFLOAT_MAX;for(NSUInteger i=0;i<dense.terminals.count;i++){TTerminalView *item=dense.terminals[i];denseMinimumWidth=MIN(denseMinimumWidth,NSWidth(item.frame));denseMinimumHeight=MIN(denseMinimumHeight,NSHeight(item.frame));if(item.hidden||!NSContainsRect(dense.window.contentView.bounds,item.frame)||!item.renderSnapshot.isValid){fprintf(stderr,"hyprland dense invalid index=%lu hidden=%d frame=%s bounds=%s snapshot=%s\n",(unsigned long)i,item.hidden,NSStringFromRect(item.frame).UTF8String,NSStringFromRect(dense.window.contentView.bounds).UTF8String,item.diagnosticState.description.UTF8String);return 97;}for(NSUInteger j=i+1;j<dense.terminals.count;j++)if(!NSIsEmptyRect(NSIntersectionRect(item.frame,dense.terminals[j].frame)))return 98;}if(denseMinimumWidth<250||denseMinimumHeight<160){fprintf(stderr,"hyprland dense undersized minimum=%.1fx%.1f\n",denseMinimumWidth,denseMinimumHeight);return 99;}while(dense.terminals.count>4){[dense focusTerminal:dense.terminals.lastObject];[dense closeTab];}[dense layoutTabs];CGFloat expandedArea=NSWidth(denseRoot.frame)*NSHeight(denseRoot.frame);if(dense.terminals.count!=4||expandedArea<denseRootArea*3||![[denseRoot visibleText] containsString:@"DENSE-ROOT-CONTENT"]||denseRoot.renderSnapshot.metrics.viewportWidth!=NSWidth(denseRoot.frame)||denseRoot.renderSnapshot.metrics.viewportHeight!=NSHeight(denseRoot.frame)){fprintf(stderr,"hyprland close-expand invalid count=%lu before=%.1f after=%.1f frame=%s snapshot=%s text=%s\n",(unsigned long)dense.terminals.count,denseRootArea,expandedArea,NSStringFromRect(denseRoot.frame).UTF8String,denseRoot.diagnosticState.description.UTF8String,denseRoot.visibleText.UTF8String);return 100;}for(NSUInteger i=0;i<dense.terminals.count;i++){TTerminalView *item=dense.terminals[i];if(item.hidden||NSWidth(item.frame)<500||NSHeight(item.frame)<350||!NSContainsRect(dense.window.contentView.bounds,item.frame))return 101;for(NSUInteger j=i+1;j<dense.terminals.count;j++)if(!NSIsEmptyRect(NSIntersectionRect(item.frame,dense.terminals[j].frame)))return 102;}for(TTerminalView *item in dense.terminals)[item stopShellTerminating:YES];[dense.window close];
    fprintf(stdout,"terminal-self-test ok history=%lu rows=%lu columns=%lu keyboard=kitty+legacy mouse=default-tui-clicks+shift-selection+buttons+drag+motion+tui-wheel selection=document-anchored wheel=normal+hyprland+direction-stable codex-redraw=wide-cell-clean+metadata-clean+emoji-upgrade-pair-clean+cell-clipped+cursor-frame-atomic+intel-overlay-isolated claude-logo=seamless-block-geometry graphemes=column-stable resize=alternate-scroll-safe+monitor-adaptive tabs=surface-stable+selected-below+command-t-hyprland-dwindle+inactive-cursor-cleared layout=command-t-vertical-horizontal-alternation+nested-half-preserves-sibling+hyprland-pane-local+independent-mixed-splits+balanced-16-pane-close-expand+position-move-preserves-tile-identity+shape+content tint=window-owned-all-layouts+launch-stable+tabs-stable+splits-stable+windows-stable+focus-stable+renderer-independent+default-bg-idempotent motion=opaque-translation-only+translucent-static rail=top-to-bottom+bounded\n",(unsigned long)[initial[@"history"] unsignedIntegerValue],(unsigned long)[initial[@"rows"] unsignedIntegerValue],(unsigned long)[initial[@"columns"] unsignedIntegerValue]);
    return 0;
}

static int TRunRendererReliabilitySelfTest(void) {
    [TApplication sharedApplication];
    NSArray<NSString *> *requestedRenderers=getenv("CI")?@[@"appkit"]:@[@"appkit",@"metal"];
    NSUInteger focusSwitches=0,lifecycleRecoveries=0,metalRuns=0;
    for(NSString *requested in requestedRenderers){
        TConfig *config=[TConfig new];config.renderer=requested;config.blur=NO;config.hyprlandLayout=YES;config.tabAnimations=NO;config.shell=@"/usr/bin/true";config.shellArguments=@[];
        TWindowController *controller=[[TWindowController alloc]initWithConfig:config extensions:nil];[controller addTab];[controller addTab];[controller.window orderFront:nil];[controller.window makeKeyWindow];for(TTerminalView *terminal in controller.terminals)[terminal preparePresentation];[controller layoutTabs];for(TTerminalView *terminal in controller.terminals)[terminal recoverPresentationAfterLifecycleChange];NSDate *readyDeadline=[NSDate dateWithTimeIntervalSinceNow:0.08];while(readyDeadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.002]];
        if(controller.terminals.count!=3){[controller.window close];fprintf(stderr,"renderer-reliability-self-test failed stage=terminals requested=%s\n",requested.UTF8String);return 110;}
        BOOL metalActive=[controller.terminals.firstObject.diagnosticState[@"renderer"] isEqual:@"metal"];if([requested isEqual:@"metal"]&&metalActive)metalRuns++;
        for(NSUInteger index=0;index<controller.terminals.count;index++)[controller.terminals[index] consumeData:[[NSString stringWithFormat:@"PHASE5-PANE-%lu λ漢🙂",(unsigned long)index] dataUsingEncoding:NSUTF8StringEncoding]];
        for(NSUInteger cycle=0;cycle<120;cycle++){
            TTerminalView *target=controller.terminals[cycle%controller.terminals.count];[controller focusTerminal:target];focusSwitches++;
            ...
            NSUInteger active=0,cursors=0,displayCursors=0,compositedCursors=0;for(TTerminalView *terminal in controller.terminals){active+=terminal.activeTerminal;cursors+=terminal.renderSnapshot.cursorVisible;displayCursors+=[terminal.diagnosticState[@"displayCursorVisible"] boolValue];compositedCursors+=[terminal.diagnosticState[@"compositedCursorVisible"] boolValue];}
            NSUInteger expectedCursors=controller.window.isKeyWindow?1:0;if(controller.terminal!=target||controller.window.firstResponder!=target||active!=1||cursors!=expectedCursors||displayCursors!=expectedCursors||compositedCursors!=expectedCursors){[controller.window close];fprintf(stderr,"renderer-reliability-self-test failed stage=cursor requested=%s cycle=%lu active=%lu snapshots=%lu displayed=%lu composited=%lu key=%s\n",requested.UTF8String,(unsigned long)cycle,(unsigned long)active,(unsigned long)cursors,(unsigned long)displayCursors,(unsigned long)compositedCursors,controller.window.isKeyWindow?"yes":"no");return 111;}
        }
        ...
        for(NSUInteger index=0;index<controller.terminals.count;index++){TTerminalView *terminal=controller.terminals[index];if(![[terminal visibleText] containsString:[NSString stringWithFormat:@"PHASE5-PANE-%lu",(unsigned long)index]]){[controller.window close];fprintf(stderr,"renderer-reliability-self-test failed stage=state requested=%s pane=%lu\n",requested.UTF8String,(unsigned long)index);return 112;}if([terminal.diagnosticState[@"renderer"] isEqual:@"metal"]){NSDictionary *scheduler=[terminal metalSchedulerDiagnosticsForRendererSelfTest];if([scheduler[@"pendingSnapshots"] unsignedIntegerValue]>1||[scheduler[@"inFlightFrames"] unsignedIntegerValue]>1||[scheduler[@"generationReversals"] unsignedIntegerValue]){[controller.window close];fprintf(stderr,"renderer-reliability-self-test failed stage=scheduler stats=%s\n",scheduler.description.UTF8String);return 113;}}[terminal stopShellTerminating:YES];}
        [controller.window close];
    }
    fprintf(stdout,"renderer-reliability-self-test ok renderers=%lu metal-active=%lu panes=3 focus-switches=%lu lifecycle-recoveries=%lu state-preserved=yes cursor-owner=exactly-one\n",(unsigned long)requestedRenderers.count,(unsigned long)metalRuns,(unsigned long)focusSwitches,(unsigned long)lifecycleRecoveries);return 0;
}

static int TRunPhase6FieldSelfTest(void) {
    [TApplication sharedApplication];NSArray<NSString *> *requestedRenderers=getenv("CI")?@[@"appkit"]:@[@"appkit",@"metal"];
    NSUInteger recoveries=0,metalRuns=0;NSArray<NSString *> *reasons=@[@"window-screen",@"window-backing-scale",@"window-visible",NSWorkspaceDidWakeNotification,NSWorkspaceScreensDidWakeNotification,NSApplicationDidChangeScreenParametersNotification];
    for(NSString *requested in requestedRenderers){TConfig *config=[TConfig new];config.renderer=requested;config.blur=NO;config.tabAnimations=NO;config.shell=@"/usr/bin/true";config.shellArguments=@[];NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,920,620) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];terminal.activeTerminal=YES;terminal.autoresizingMask=NSViewWidthSizable|NSViewHeightSizable;window.contentView=terminal;[window makeFirstResponder:terminal];[window orderFront:nil];[window makeKeyWindow];NSString *actual=terminal.diagnosticState[@"renderer"];
        if([requested isEqual:@"metal"]&&![actual isEqual:@"metal"]){if([terminal.diagnosticState[@"metalFailed"] boolValue]){[window close];continue;}[window close];fputs("phase6-field-self-test failed stage=metal-selection\n",stderr);return 140;}if([actual isEqual:@"metal"])metalRuns++;
        [terminal consumeData:[@"PHASE6-FIELD ASCII λ漢字🙂 👩🏽‍💻 é \033[38;2;89;194;255mcolour\033[0m\r\n" dataUsingEncoding:NSUTF8StringEncoding]];[terminal setMarkedText:@"かな漢字" selectedRange:NSMakeRange(4,0) replacementRange:NSMakeRange(NSNotFound,0)];if(!terminal.hasMarkedText){[window close];fputs("phase6-field-self-test failed stage=ime-marked\n",stderr);return 141;}[terminal unmarkText];if(terminal.hasMarkedText){[window close];fputs("phase6-field-self-test failed stage=ime-unmark\n",stderr);return 142;}terminal.accessibilityValue=terminal.visibleText;if(![[terminal.accessibilityValue description] containsString:@"PHASE6-FIELD"]){[window close];fputs("phase6-field-self-test failed stage=accessibility-value\n",stderr);return 143;}
        NSUInteger initialRecoveries=[terminal.diagnosticState[@"presentationRecoveryCount"] unsignedIntegerValue];for(NSUInteger cycle=0;cycle<48;cycle++){if(cycle%8==0)[window setContentSize:NSMakeSize(820+(cycle%4)*37,540+(cycle%5)*29)];NSString *reason=reasons[cycle%reasons.count];[terminal recoverPresentationAfterLifecycleChange:reason];recoveries++;NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:0.012];while(deadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.002]];}
        NSDictionary *state=terminal.diagnosticState;BOOL content=[terminal.visibleText containsString:@"PHASE6-FIELD"],recoveryCount=[state[@"presentationRecoveryCount"] unsignedIntegerValue]>=initialRecoveries+48,reason=[reasons containsObject:state[@"lastPresentationRecovery"]],renderer=[state[@"renderer"] isEqual:actual],cursor=terminal.renderSnapshot.cursorVisible==window.isKeyWindow;if(!content||!recoveryCount||!reason||!renderer||!cursor){[window close];fprintf(stderr,"phase6-field-self-test failed stage=continuity requested=%s state=%s\n",requested.UTF8String,state.description.UTF8String);return 144;}if([actual isEqual:@"metal"]){uint64_t generation=[terminal presentFrameForBenchmark];if(!TWaitForMetalGeneration(terminal,generation,3)){[window close];fputs("phase6-field-self-test failed stage=metal-present\n",stderr);return 145;}NSDictionary *scheduler=[terminal metalSchedulerDiagnosticsForRendererSelfTest];if([scheduler[@"pendingSnapshots"] unsignedIntegerValue]>1||[scheduler[@"maximumInFlightFrames"] unsignedIntegerValue]>1||[scheduler[@"generationReversals"] unsignedIntegerValue]){[window close];fprintf(stderr,"phase6-field-self-test failed stage=scheduler state=%s\n",scheduler.description.UTF8String);return 146;}}[window close];}
    TAppDelegate *delegate=[TAppDelegate new];delegate.config=[TConfig new];delegate.windows=[NSMutableArray array];NSDictionary *report=[delegate rendererFieldReport];NSData *reportData=[NSJSONSerialization dataWithJSONObject:report options:0 error:nil];NSString *reportText=reportData?[[NSString alloc]initWithData:reportData encoding:NSUTF8StringEncoding]:nil;NSDictionary *privacy=report[@"privacy"];if(![report[@"ok"] boolValue]||[report[@"schemaVersion"] unsignedIntegerValue]!=4||[privacy[@"terminalContentIncluded"] boolValue]||[privacy[@"workingDirectoriesIncluded"] boolValue]||[privacy[@"configPathIncluded"] boolValue]||[privacy[@"displayNamesIncluded"] boolValue]||[reportText containsString:@"PHASE6-FIELD"]){fputs("phase6-field-self-test failed stage=privacy-report\n",stderr);return 147;}
    fprintf(stdout,"phase6-field-self-test ok renderers=%lu metal-active=%lu lifecycle-recoveries=%lu unicode=yes ime=yes accessibility-value=yes state-preserved=yes privacy-report=no-terminal-content\n",(unsigned long)requestedRenderers.count,(unsigned long)metalRuns,(unsigned long)recoveries);return 0;
}

static int TRunPhase7ProductionSelfTest(void) {
    [TApplication sharedApplication];TConfig *config=[TConfig new];config.renderer=@"metal";config.blur=NO;config.tabAnimations=NO;config.shell=@"/usr/bin/true";config.shellArguments=@[];
    setenv("TERMATICA_METAL_FORCE_FAILURE","1",1);NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,900,600) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];window.contentView=terminal;[window orderFront:nil];unsetenv("TERMATICA_METAL_FORCE_FAILURE");
    NSDictionary *initial=terminal.diagnosticState;if(![initial[@"requestedRenderer"] isEqual:@"metal"]||![initial[@"renderer"] isEqual:@"appkit"]||![initial[@"metalQuarantined"] boolValue]||[initial[@"metalFailureCount"] unsignedIntegerValue]!=1||![initial[@"lastMetalFailureStage"] isEqual:@"initialization"]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=initial-quarantine state=%s\n",initial.description.UTF8String);return 148;}
    [terminal consumeData:[@"PHASE7-QUARANTINE ASCII λ漢字🙂 👩🏽‍💻 é\r\n" dataUsingEncoding:NSUTF8StringEncoding]];NSUInteger baseline=[initial[@"metalQuarantineBypassCount"] unsignedIntegerValue];for(NSUInteger cycle=0;cycle<64;cycle++)[terminal recoverPresentationAfterLifecycleChange:[NSString stringWithFormat:@"phase7-cycle-%lu",(unsigned long)cycle]];
    NSDictionary *quarantined=terminal.diagnosticState;if(![quarantined[@"renderer"] isEqual:@"appkit"]||[quarantined[@"metalFailureCount"] unsignedIntegerValue]!=1||[quarantined[@"metalQuarantineBypassCount"] unsignedIntegerValue]<baseline+64||![quarantined[@"metalRetryRequiresExplicitAction"] boolValue]||![[terminal visibleText] containsString:@"PHASE7-QUARANTINE"]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=bounded-recovery state=%s\n",quarantined.description.UTF8String);return 149;}
    config.renderer=@"appkit";[terminal reloadAppearance];NSDictionary *reset=terminal.diagnosticState;if(![reset[@"requestedRenderer"] isEqual:@"appkit"]||![reset[@"renderer"] isEqual:@"appkit"]||[reset[@"metalQuarantined"] boolValue]||![[terminal visibleText] containsString:@"PHASE7-QUARANTINE"]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=explicit-reset state=%s\n",reset.description.UTF8String);return 150;}
    NSUInteger metalRuns=0,runtimeRecoveries=0;if(!getenv("CI")){config.renderer=@"metal";[terminal reloadAppearance];NSDictionary *metal=terminal.diagnosticState;if([metal[@"renderer"] isEqual:@"metal"]){metalRuns=1;uint64_t generation=[terminal presentFrameForBenchmark];if(!TWaitForMetalGeneration(terminal,generation,3)){[window close];fputs("phase7-production-self-test failed stage=metal-retry-present\n",stderr);return 151;}setenv("TERMATICA_METAL_FORCE_COMMAND_FAILURE","1",1);[terminal presentFrameForBenchmark];NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:3];while([terminal.diagnosticState[@"renderer"] isEqual:@"metal"]&&deadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];unsetenv("TERMATICA_METAL_FORCE_COMMAND_FAILURE");NSDictionary *runtime=terminal.diagnosticState;if(![runtime[@"renderer"] isEqual:@"appkit"]||![runtime[@"metalQuarantined"] boolValue]||![runtime[@"lastMetalFailureStage"] isEqual:@"runtime"]||[runtime[@"metalFailureCount"] unsignedIntegerValue]!=2||![[terminal visibleText] containsString:@"PHASE7-QUARANTINE"]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=runtime-fallback state=%s\n",runtime.description.UTF8String);return 152;}NSUInteger runtimeBaseline=[runtime[@"metalQuarantineBypassCount"] unsignedIntegerValue];for(NSUInteger cycle=0;cycle<32;cycle++){[terminal recoverPresentationAfterLifecycleChange:@"phase7-runtime-quarantine"];runtimeRecoveries++;}NSDictionary *stable=terminal.diagnosticState;if([stable[@"metalFailureCount"] unsignedIntegerValue]!=2||[stable[@"metalQuarantineBypassCount"] unsignedIntegerValue]<runtimeBaseline+32){[window close];fprintf(stderr,"phase7-production-self-test failed stage=runtime-bounded state=%s\n",stable.description.UTF8String);return 153;}}}
    TAppDelegate *delegate=[TAppDelegate new];delegate.config=[TConfig new];delegate.windows=[NSMutableArray array];NSDictionary *report=[delegate rendererFieldReport],*policy=report[@"rolloutPolicy"],*privacy=report[@"privacy"];if([report[@"schemaVersion"] unsignedIntegerValue]!=4||![policy[@"defaultRenderer"] isEqual:@"appkit"]||![policy[@"metalOptIn"] boolValue]||[policy[@"defaultChangeEligible"] boolValue]||[privacy[@"terminalContentIncluded"] boolValue]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=report-policy report=%s\n",report.description.UTF8String);return 154;}
    [window close];fprintf(stdout,"phase7-production-self-test ok initialization-failures=1 quarantine-recoveries=64 runtime-metal-active=%lu runtime-recoveries=%lu retry=explicit-action state-preserved=yes default=appkit report-schema=4\n",(unsigned long)metalRuns,(unsigned long)runtimeRecoveries);return 0;
}

static int TRunPhase8RecoverySelfTest(void) {
    [TApplication sharedApplication];BOOL ci=getenv("CI")!=NULL;NSMutableArray<NSWindow *> *nativeWindows=[NSMutableArray array];NSMutableArray<TTerminalView *> *terminals=[NSMutableArray array];
    for(NSUInteger index=0;index<3;index++){TConfig *config=[TConfig new];config.renderer=(ci&&index!=1)?@"appkit":@"metal";config.blur=NO;config.tabAnimations=NO;config.shell=@"/usr/bin/true";config.shellArguments=@[];if(index==1)setenv("TERMATICA_METAL_FORCE_FAILURE","1",1);NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,760,480) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];window.contentView=terminal;[window orderFront:nil];if(index==1)unsetenv("TERMATICA_METAL_FORCE_FAILURE");[terminal consumeData:[[NSString stringWithFormat:@"PHASE8-PANE-%lu λ漢🙂\r\n",(unsigned long)index] dataUsingEncoding:NSUTF8StringEncoding]];[nativeWindows addObject:window];[terminals addObject:terminal];}
    TTerminalView *left=terminals[0],*failed=terminals[1],*right=terminals[2];NSDictionary *failedState=failed.diagnosticState;if(![failedState[@"renderer"] isEqual:@"appkit"]||![failedState[@"metalQuarantined"] boolValue]||![failedState[@"metalRetryAvailable"] boolValue]){for(NSWindow *window in nativeWindows)[window close];fprintf(stderr,"phase8-recovery-self-test failed stage=forced-quarantine state=%s\n",failedState.description.UTF8String);return 155;}
    NSString *leftRenderer=left.diagnosticState[@"renderer"],*rightRenderer=right.diagnosticState[@"renderer"];NSUInteger leftFailures=[left.diagnosticState[@"metalFailureCount"] unsignedIntegerValue],rightFailures=[right.diagnosticState[@"metalFailureCount"] unsignedIntegerValue],leftAttempts=[left.diagnosticState[@"metalRetryAttemptCount"] unsignedIntegerValue],rightAttempts=[right.diagnosticState[@"metalRetryAttemptCount"] unsignedIntegerValue];
    TWindowController *controller=[TWindowController new];controller.terminals=terminals;TAppDelegate *delegate=[TAppDelegate new];delegate.config=failed.config;delegate.windows=[NSMutableArray arrayWithObject:controller];NSDictionary *before=[delegate rendererFieldReport],*beforeHealth=before[@"health"];if([before[@"schemaVersion"] unsignedIntegerValue]!=4||[beforeHealth[@"quarantinedPanes"] unsignedIntegerValue]<1||[before[@"panes"] count]!=3){for(NSWindow *window in nativeWindows)[window close];fputs("phase8-recovery-self-test failed stage=aggregate-before\n",stderr);return 156;}
    setenv("TERMATICA_METAL_FORCE_FAILURE","1",1);NSDictionary *blockedRetry=[delegate rendererRetryReport];unsetenv("TERMATICA_METAL_FORCE_FAILURE");NSDictionary *blockedState=failed.diagnosticState,*blockedLeft=left.diagnosticState,*blockedRight=right.diagnosticState;BOOL blockedIsolation=[leftRenderer isEqual:blockedLeft[@"renderer"]]&&[rightRenderer isEqual:blockedRight[@"renderer"]]&&leftFailures==[blockedLeft[@"metalFailureCount"] unsignedIntegerValue]&&rightFailures==[blockedRight[@"metalFailureCount"] unsignedIntegerValue]&&leftAttempts==[blockedLeft[@"metalRetryAttemptCount"] unsignedIntegerValue]&&rightAttempts==[blockedRight[@"metalRetryAttemptCount"] unsignedIntegerValue];BOOL blockedAccounting=[blockedRetry[@"attempted"] unsignedIntegerValue]==1&&[blockedRetry[@"recovered"] unsignedIntegerValue]==0&&[blockedRetry[@"stillQuarantined"] unsignedIntegerValue]==1&&[blockedRetry[@"unchanged"] unsignedIntegerValue]==2&&[blockedState[@"metalRetryAttemptCount"] unsignedIntegerValue]==1&&[blockedState[@"metalQuarantined"] boolValue];
    if(!blockedIsolation||!blockedAccounting){for(NSWindow *window in nativeWindows)[window close];fprintf(stderr,"phase8-recovery-self-test failed stage=isolated-failed-retry retry=%s state=%s\n",blockedRetry.description.UTF8String,blockedState.description.UTF8String);return 157;}
    NSDictionary *retry=[delegate rendererRetryReport];NSDictionary *afterFailed=failed.diagnosticState,*afterLeft=left.diagnosticState,*afterRight=right.diagnosticState;BOOL isolated=[leftRenderer isEqual:afterLeft[@"renderer"]]&&[rightRenderer isEqual:afterRight[@"renderer"]]&&leftFailures==[afterLeft[@"metalFailureCount"] unsignedIntegerValue]&&rightFailures==[afterRight[@"metalFailureCount"] unsignedIntegerValue]&&leftAttempts==[afterLeft[@"metalRetryAttemptCount"] unsignedIntegerValue]&&rightAttempts==[afterRight[@"metalRetryAttemptCount"] unsignedIntegerValue];BOOL content=[[left visibleText] containsString:@"PHASE8-PANE-0"]&&[[failed visibleText] containsString:@"PHASE8-PANE-1"]&&[[right visibleText] containsString:@"PHASE8-PANE-2"];BOOL accounting=[retry[@"attempted"] unsignedIntegerValue]==1&&[retry[@"unchanged"] unsignedIntegerValue]==2&&[afterFailed[@"metalRetryAttemptCount"] unsignedIntegerValue]==2;
    if(!isolated||!content||!accounting){for(NSWindow *window in nativeWindows)[window close];fprintf(stderr,"phase8-recovery-self-test failed stage=isolated-retry retry=%s state=%s\n",retry.description.UTF8String,afterFailed.description.UTF8String);return 158;}
    NSDictionary *after=[delegate rendererFieldReport],*health=after[@"health"];if([health[@"metalRetryAttemptCount"] unsignedIntegerValue]!=2||[health[@"metalSuccessfulRetryCount"] unsignedIntegerValue]!=[retry[@"recovered"] unsignedIntegerValue]){for(NSWindow *window in nativeWindows)[window close];fputs("phase8-recovery-self-test failed stage=aggregate-after\n",stderr);return 159;}
    for(NSWindow *window in nativeWindows)[window close];fprintf(stdout,"phase8-recovery-self-test ok panes=3 failed-retry-isolated=yes attempts=2 recovered=%lu still-quarantined=%lu healthy-panes-unchanged=yes state-preserved=yes report-schema=4\n",(unsigned long)[retry[@"recovered"] unsignedIntegerValue],(unsigned long)[retry[@"stillQuarantined"] unsignedIntegerValue]);return 0;
}

static int TRunPhase9FieldQualificationSelfTest(void) {
    [TApplication sharedApplication];NSMutableArray<NSWindow *> *nativeWindows=[NSMutableArray array];NSMutableArray<TTerminalView *> *terminals=[NSMutableArray array];NSArray *renderers=getenv("CI")?@[@"appkit",@"appkit"]:@[@"metal",@"appkit"];
    for(NSUInteger index=0;index<renderers.count;index++){TConfig *config=[TConfig new];config.renderer=renderers[index];config.blur=NO;config.tabAnimations=NO;config.shell=@"/usr/bin/true";config.shellArguments=@[];NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,820,520) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];window.contentView=terminal;[window orderFront:nil];[terminal consumeData:[[NSString stringWithFormat:@"PHASE9-FIELD-PANE-%lu λ漢🙂 👩🏽‍💻\r\n",(unsigned long)index] dataUsingEncoding:NSUTF8StringEncoding]];for(NSUInteger cycle=0;cycle<20;cycle++){[terminal recoverPresentationAfterLifecycleChange:@"window-screen"];[terminal recoverPresentationAfterLifecycleChange:@"window-backing-scale"];}[nativeWindows addObject:window];[terminals addObject:terminal];}
    TWindowController *controller=[TWindowController new];controller.terminals=terminals;TAppDelegate *delegate=[TAppDelegate new];delegate.config=terminals.firstObject.config;delegate.windows=[NSMutableArray arrayWithObject:controller];[delegate recordFieldEvent:@"phase9-start"];[delegate recoverPresentations:[NSNotification notificationWithName:NSWorkspaceDidWakeNotification object:nil]];[delegate recoverPresentations:[NSNotification notificationWithName:NSWorkspaceScreensDidWakeNotification object:nil]];[delegate recoverPresentations:[NSNotification notificationWithName:NSApplicationDidChangeScreenParametersNotification object:nil]];
    NSDictionary *report=[delegate rendererFieldReport],*evidence=report[@"fieldEvidence"],*qualification=report[@"fieldQualification"],*gates=qualification[@"gates"],*privacy=report[@"privacy"];NSData *data=[NSJSONSerialization dataWithJSONObject:report options:0 error:nil];NSString *text=data?[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]:@"";BOOL contentPreserved=YES;for(NSUInteger index=0;index<terminals.count;index++)contentPreserved&=[[terminals[index] visibleText] containsString:[NSString stringWithFormat:@"PHASE9-FIELD-PANE-%lu",(unsigned long)index]];
    BOOL factual=[report[@"schemaVersion"] unsignedIntegerValue]==4&&[qualification[@"evidenceSource"] isEqual:@"observed-runtime-events"]&&![qualification[@"syntheticEvidenceAccepted"] boolValue]&&![qualification[@"defaultChangeEligible"] boolValue]&&![gates[@"physicalVisualInspectionConfirmed"] boolValue]&&[evidence[@"screenTransitionEvents"] unsignedIntegerValue]>=40&&[evidence[@"backingScaleEvents"] unsignedIntegerValue]>=40&&[evidence[@"wakeEvents"] unsignedIntegerValue]>=1&&[evidence[@"screenWakeEvents"] unsignedIntegerValue]>=1&&[evidence[@"displayParameterEvents"] unsignedIntegerValue]>=1;
    BOOL private=![privacy[@"terminalContentIncluded"] boolValue]&&![privacy[@"workingDirectoriesIncluded"] boolValue]&&![privacy[@"configPathIncluded"] boolValue]&&![privacy[@"displayNamesIncluded"] boolValue]&&![text containsString:@"PHASE9-FIELD-PANE"];
    if(!factual||!private||!contentPreserved){for(NSWindow *window in nativeWindows)[window close];fprintf(stderr,"phase9-field-self-test failed stage=live-report report=%s\n",report.description.UTF8String);return 160;}
    NSDictionary *syntheticEvidence=@{@"evidenceSource":@"synthetic-test",@"processArchitecture":TRuntimeArchitecture(),@"translated":@(TProcessIsTranslated()),@"metalPanes":@2,@"quarantinedPanes":@0,@"maximumDisplayCount":@2,@"displayCountChanges":@3,@"observedBackingScaleChanges":@4,@"observedScreenChanges":@5,@"wakeEvents":@2,@"screenWakeEvents":@2,@"observedRefreshRateCount":@2};NSDictionary *synthetic=TFieldQualification(syntheticEvidence);NSArray *blockers=synthetic[@"blockers"];BOOL refusesSynthetic=[synthetic[@"evidenceSource"] isEqual:@"synthetic-test"]&&![synthetic[@"syntheticEvidenceAccepted"] boolValue]&&![synthetic[@"defaultChangeEligible"] boolValue]&&[blockers containsObject:@"physicalVisualInspectionConfirmed"]&&([blockers containsObject:@"intelNativeObserved"]||[blockers containsObject:@"appleSiliconNativeObserved"]);
    for(NSWindow *window in nativeWindows)[window close];if(!refusesSynthetic){fprintf(stderr,"phase9-field-self-test failed stage=synthetic-boundary qualification=%s\n",synthetic.description.UTF8String);return 161;}fprintf(stdout,"phase9-field-self-test ok panes=2 screen-events=%lu scale-events=%lu wake-events=%lu privacy=yes synthetic-proof-rejected=yes default=appkit report-schema=4\n",(unsigned long)[evidence[@"screenTransitionEvents"] unsignedIntegerValue],(unsigned long)[evidence[@"backingScaleEvents"] unsignedIntegerValue],(unsigned long)([evidence[@"wakeEvents"] unsignedIntegerValue]+[evidence[@"screenWakeEvents"] unsignedIntegerValue]));return 0;
}

static NSDictionary *TPhase10FixtureReport(NSString *architecture,NSDictionary *overrides) {
    NSMutableDictionary *evidence=[@{@"evidenceSource":@"observed-runtime-events",@"fieldSessionIdentifier":[NSString stringWithFormat:@"phase10-%@-session",architecture],@"sessionSeconds":@900,@"processArchitecture":architecture,@"translated":@NO,@"minimumDisplayCount":@1,@"maximumDisplayCount":@1,@"observedDisplayCountCount":@1,@"observedScaleCount":@1,@"observedRefreshRateCount":@1,@"displayParameterEvents":@0,@"displayCountChanges":@0,@"wakeEvents":@0,@"screenWakeEvents":@0,@"screenTransitionEvents":@0,@"backingScaleEvents":@0,@"observedScreenChanges":@0,@"observedBackingScaleChanges":@0,@"metalPanes":@1,@"quarantinedPanes":@0} mutableCopy];[evidence addEntriesFromDictionary:overrides?:@{}];return @{@"schemaVersion":@4,@"generatedAt":@"2026-08-13T00:00:00Z",@"app":@{@"version":@"1.12.0",@"build":@"62"},@"fieldEvidence":evidence};
}

static int TRunPhase10CampaignSelfTest(void) {
    NSDictionary *apple=TFieldCampaignEnvelope(TPhase10FixtureReport(@"arm64",@{@"maximumDisplayCount":@2,@"observedDisplayCountCount":@2,@"displayCountChanges":@1,@"observedScaleCount":@2,@"observedRefreshRateCount":@2,@"displayParameterEvents":@3,@"wakeEvents":@1,@"screenTransitionEvents":@1,@"backingScaleEvents":@1,@"observedScreenChanges":@1,@"observedBackingScaleChanges":@1})),*intel=TFieldCampaignEnvelope(TPhase10FixtureReport(@"x86_64",@{@"screenWakeEvents":@1}));NSDictionary *valid=TValidateFieldCampaignEnvelope(apple);if(![valid[@"ok"] boolValue]||[valid[@"digest"] length]!=64){fprintf(stderr,"phase10-campaign-self-test failed stage=valid-envelope result=%s\n",valid.description.UTF8String);return 162;}
    NSMutableDictionary *tampered=[apple mutableCopy];NSMutableDictionary *payload=[apple[@"payload"] mutableCopy],*evidence=[payload[@"fieldEvidence"] mutableCopy];evidence[@"displayCountChanges"]=@99;payload[@"fieldEvidence"]=evidence;tampered[@"payload"]=payload;NSDictionary *tamperResult=TValidateFieldCampaignEnvelope(tampered);if([tamperResult[@"ok"] boolValue]||![tamperResult[@"error"] isEqual:@"digest-mismatch"]){fprintf(stderr,"phase10-campaign-self-test failed stage=tamper result=%s\n",tamperResult.description.UTF8String);return 163;}
    NSDictionary *synthetic=TFieldCampaignEnvelope(TPhase10FixtureReport(@"arm64",@{@"evidenceSource":@"synthetic-test",@"maximumDisplayCount":@2,@"displayCountChanges":@1,@"observedRefreshRateCount":@2,@"wakeEvents":@1,@"observedScreenChanges":@1,@"observedBackingScaleChanges":@1})),*syntheticResult=TValidateFieldCampaignEnvelope(synthetic);if([syntheticResult[@"ok"] boolValue]||![syntheticResult[@"error"] isEqual:@"non-observed-source"]){fprintf(stderr,"phase10-campaign-self-test failed stage=synthetic result=%s\n",syntheticResult.description.UTF8String);return 164;}
    NSDictionary *appleReexport=TFieldCampaignEnvelope(TPhase10FixtureReport(@"arm64",@{@"sessionSeconds":@1200,@"maximumDisplayCount":@2,@"observedDisplayCountCount":@2,@"displayCountChanges":@2,@"observedScaleCount":@2,@"observedRefreshRateCount":@2,@"displayParameterEvents":@4,@"wakeEvents":@1,@"screenTransitionEvents":@2,@"backingScaleEvents":@2,@"observedScreenChanges":@2,@"observedBackingScaleChanges":@2}));NSDictionary *aggregate=TAggregateFieldCampaignReports(@[apple,intel,apple,appleReexport,tampered,synthetic]),*gates=aggregate[@"gates"];NSArray *blockers=aggregate[@"blockers"];BOOL correct=[aggregate[@"inputReports"] unsignedIntegerValue]==6&&[aggregate[@"acceptedReports"] unsignedIntegerValue]==2&&[aggregate[@"duplicateReports"] unsignedIntegerValue]==2&&[aggregate[@"rejectedReports"] count]==2&&[aggregate[@"fieldEvidenceComplete"] boolValue]&&![aggregate[@"defaultRendererChangeEligible"] boolValue]&&[aggregate[@"operatorReviewRequired"] boolValue]&&[blockers isEqual:@[@"physicalVisualInspectionConfirmed"]]&&[gates[@"appleSiliconNativeObserved"] boolValue]&&[gates[@"intelNativeObserved"] boolValue];NSData *data=TCanonicalJSON(aggregate);NSString *text=data?[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding]:@"";BOOL private=![text containsString:@"terminal text"]&&![text containsString:@"/Users/"]&&![text containsString:@"display name"];
    if(!correct||!private){fprintf(stderr,"phase10-campaign-self-test failed stage=aggregate result=%s\n",aggregate.description.UTF8String);return 165;}fprintf(stdout,"phase10-campaign-self-test ok accepted=2 duplicate=2 rejected=2 architectures=arm64+x86_64 tamper=rejected synthetic=rejected authenticity=not-asserted operator-review=required default=appkit\n");return 0;
}

static NSString *TPhase5Executable(NSString *name) {
    if([name hasPrefix:@"/"])return [NSFileManager.defaultManager isExecutableFileAtPath:name]?name:nil;
    NSString *path=NSProcessInfo.processInfo.environment[@"PATH"]?:@"/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin";
    for(NSString *directory in [path componentsSeparatedByString:@":"]){NSString *candidate=[directory stringByAppendingPathComponent:name];if([NSFileManager.defaultManager isExecutableFileAtPath:candidate])return candidate;}
    return nil;
}

static BOOL TPhase5WaitForOutput(TTerminalView *terminal,NSArray<NSString *> *needles,NSTimeInterval timeout) {
    NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:timeout];while(deadline.timeIntervalSinceNow>0){NSString *text=terminal.visibleText;for(NSString *needle in needles)if([text localizedCaseInsensitiveContainsString:needle])return YES;[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];}return NO;
}

static int TRunTUICompatibilitySelfTest(void) {
    [TApplication sharedApplication];
    NSString *fixture=[NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"termatica-phase5-%d.txt",getpid()]];NSString *fixtureText=@"TERMATICAPHASE5\nUnicode λ 漢字 🙂 👩🏽‍💻\nANSI colours and resize-safe terminal content\n";if(![fixtureText writeToFile:fixture atomically:YES encoding:NSUTF8StringEncoding error:nil])return 120;
    NSString *tmuxSocket=[NSString stringWithFormat:@"termatica-phase5-%d",getpid()];
    NSArray<NSDictionary *> *cases=@[
      @{@"name":@"zsh",@"command":@"zsh",@"args":@[@"-f"],@"needles":@[@"%"],@"input":@"printf '\\033[?1049hTERMATICAPHASE5-SHELL λ漢🙂\\033[?1049lSHELL-RESTORED\\n'; exit\r",@"after":@"SHELL-RESTORED"},
      @{@"name":@"resize",@"command":@"zsh",@"args":@[@"-f",@"-c",@"trap 'print -r -- WINCH-$(stty size)' WINCH; print -r -- RESIZE-READY-$(stty size); while true; do sleep 1; done"],@"needles":@[@"RESIZE-READY-"],@"resize":@YES},
      @{@"name":@"vim",@"command":@"vim",@"args":@[@"-Nu",@"NONE",@"-n",fixture],@"needles":@[@"TERMATICAPHASE5",@"~"],@"input":@":q!\r"},
      @{@"name":@"nvim",@"command":@"nvim",@"args":@[@"--clean",@"-n",fixture],@"needles":@[@"TERMATICAPHASE5",@"~"],@"input":@":q!\r"},
      @{@"name":@"tmux",@"command":@"tmux",@"args":@[@"-L",tmuxSocket,@"new-session",@"printf 'TERMATICAPHASE5-TMUX\\n'; exec zsh -f"],@"needles":@[@"TERMATICAPHASE5-TMUX"],@"input":@"exit\r"},
      @{@"name":@"less",@"command":@"less",@"args":@[fixture],@"needles":@[@"TERMATICAPHASE5"],@"input":@"q"},
      @{@"name":@"man",@"command":@"man",@"args":@[@"printf"],@"needles":@[@"PRINTF"],@"input":@"q"},
      @{@"name":@"top",@"command":@"top",@"args":@[@"-l",@"1",@"-n",@"1"],@"needles":@[@"Processes",@"Load Avg"]},
      @{@"name":@"htop",@"command":@"htop",@"args":@[],@"needles":@[@"F1",@"Help",@"CPU"],@"input":@"q"},
      @{@"name":@"ssh",@"command":@"ssh",@"args":@[@"-V"],@"needles":@[@"OpenSSH"]},
      @{@"name":@"codex",@"command":@"codex",@"args":@[@"--help"],@"needles":@[@"Print help",@"--no-alt-screen"]},
      @{@"name":@"codex-ui",@"command":@"codex",@"renderer":@"appkit",@"args":@[@"--no-alt-screen",@"--sandbox",@"read-only"],@"needles":@[@"OpenAI Codex",@"Ask Codex to do anything"],@"input":@"\003"},
      @{@"name":@"herdr-appkit",@"command":@"env",@"renderer":@"appkit",@"args":@[[NSString stringWithFormat:@"HERDR_SOCKET_PATH=/tmp/termatica-herdr-appkit-%d.sock",getpid()],[NSString stringWithFormat:@"HERDR_CLIENT_SOCKET_PATH=/tmp/termatica-herdr-appkit-client-%d.sock",getpid()],[NSString stringWithFormat:@"HERDR_CONFIG_PATH=/tmp/termatica-herdr-appkit-%d.toml",getpid()],@"herdr",@"--no-session"],@"needles":@[@"No workspaces yet",@"terminal workspace manager"],@"input":@"\003"},
      @{@"name":@"herdr-metal",@"command":@"env",@"renderer":@"metal",@"args":@[[NSString stringWithFormat:@"HERDR_SOCKET_PATH=/tmp/termatica-herdr-metal-%d.sock",getpid()],[NSString stringWithFormat:@"HERDR_CLIENT_SOCKET_PATH=/tmp/termatica-herdr-metal-client-%d.sock",getpid()],[NSString stringWithFormat:@"HERDR_CONFIG_PATH=/tmp/termatica-herdr-metal-%d.toml",getpid()],@"herdr",@"--no-session"],@"needles":@[@"No workspaces yet",@"terminal workspace manager"],@"input":@"\003"},
      @{@"name":@"claude",@"command":@"claude",@"args":@[@"--help"],@"needles":@[@"update|upgrade",@"available"]},
      @{@"name":@"gemini",@"command":@"gemini",@"args":@[@"--help"],@"needles":@[@"Show version number",@"Show help"]},
      @{@"name":@"opencode",@"command":@"opencode",@"args":@[@"--help"],@"needles":@[@"opencode",@"Usage"]},
      @{@"name":@"pi",@"command":@"pi",@"args":@[@"--help"],@"needles":@[@"Built-in Tool Names",@"Read file contents"]},
      @{@"name":@"nano",@"command":@"nano",@"args":@[fixture],@"needles":@[@"TERMATICAPHASE5",@"Write Out"],@"input":@"\030"},
      @{@"name":@"btop",@"command":@"btop",@"args":@[],@"needles":@[@"cpu",@"mem",@"proc"],@"input":@"q"},
      @{@"name":@"yazi",@"command":@"yazi",@"args":@[fixture],@"needles":@[@"termatica-phase5"],@"input":@"q"},
      @{@"name":@"emacs",@"command":@"emacs",@"args":@[@"-Q",@"--no-window-system",fixture],@"needles":@[@"TERMATICAPHASE5"],@"input":@"\030\003"}
    ];
    NSMutableArray<NSString *> *passed=[NSMutableArray array],*skipped=[NSMutableArray array];
    for(NSDictionary *item in cases){NSString *name=item[@"name"],*executable=TPhase5Executable(item[@"command"]);if(!executable){[skipped addObject:name];continue;}TConfig *config=[TConfig new];config.renderer=item[@"renderer"]?:@"metal";config.blur=NO;config.shellIntegration=NO;config.shell=executable;config.shellArguments=item[@"args"];
        NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,900,600) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];terminal.activeTerminal=YES;window.contentView=terminal;[window makeFirstResponder:terminal];[window orderFront:nil];if(![terminal.diagnosticState[@"renderer"] isEqual:config.renderer]){[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=renderer-unavailable app=%s requested=%s actual=%s\n",name.UTF8String,config.renderer.UTF8String,[terminal.diagnosticState[@"renderer"] UTF8String]);return 124;}if(![terminal startShell]){[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=launch app=%s\n",name.UTF8String);return 121;}
        BOOL observed=TPhase5WaitForOutput(terminal,item[@"needles"],8);if(!observed){NSString *visible=terminal.visibleText;[terminal stopShellTerminating:YES];[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=output app=%s text=%s\n",name.UTF8String,visible.UTF8String);return 122;}
        if([item[@"resize"] boolValue]){NSUInteger beforeRows=[terminal.diagnosticState[@"rows"] unsignedIntegerValue],beforeColumns=[terminal.diagnosticState[@"columns"] unsignedIntegerValue];[terminal setFrameSize:NSMakeSize(1100,700)];if(!TPhase5WaitForOutput(terminal,@[@"WINCH-"],2)||([terminal.diagnosticState[@"rows"] unsignedIntegerValue]==beforeRows&&[terminal.diagnosticState[@"columns"] unsignedIntegerValue]==beforeColumns)){[terminal stopShellTerminating:YES];[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=resize app=%s\n",name.UTF8String);return 125;}}
        NSString *input=item[@"input"];if(input.length)[terminal sendString:input];NSString *after=item[@"after"];if(after.length&&!TPhase5WaitForOutput(terminal,@[after],2)){[terminal stopShellTerminating:YES];[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=restore app=%s\n",name.UTF8String);return 123;}NSDate *settle=[NSDate dateWithTimeIntervalSinceNow:0.08];while(settle.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:settle];[terminal stopShellTerminating:YES];[window close];[passed addObject:name];
    }
    NSString *tmux=TPhase5Executable(@"tmux");if(tmux)TRunTask(tmux,@[@"-L",tmuxSocket,@"kill-server"],nil,nil);[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stdout,"tui-compat-self-test ok passed=%s skipped=%s renderers=appkit+metal real-pty=yes\n",[passed componentsJoinedByString:@","].UTF8String,skipped.count?[skipped componentsJoinedByString:@","].UTF8String:"none");return 0;
}

static int TRunConfigWatcherSelfTest(void) {
    TConfig *config=[TConfig new];NSDictionary *themeFixture=@{@"background":@"#080B0D",@"foreground":@"#BFD3DE",@"cursor":@"#F2FAF8",@"accent":@"#67B7F7",@"panel":@"#0A1016",@"muted":@"#668092",@"selection":@"#23445A",@"palette":TStandardPaletteHex(),@"appearance":@{@"backgroundOpacity":@0.28,@"windowOpacity":@1,@"blur":@"on",@"hyprlandBlur":@"on"}};NSData *themeData=[NSJSONSerialization dataWithJSONObject:themeFixture options:0 error:nil];NSString *themePath=[[TEnsureDirectory(@"themes") stringByAppendingPathComponent:@"self-test"] stringByAppendingPathExtension:@"json"];if(![themeData writeToFile:themePath options:NSDataWritingAtomic error:nil])return 183;NSMutableDictionary *themed=[TReadJSONDictionaryAtPath(config.path) mutableCopy];themed[@"theme"]=@"self-test";NSMutableDictionary *themeColors=[themed[@"colors"] mutableCopy];for(NSString *key in @[@"background",@"foreground",@"cursor",@"accent",@"panel",@"muted",@"selection",@"palette"])themeColors[key]=@"theme";themed[@"colors"]=themeColors;NSMutableDictionary *themeTabs=[themed[@"tabs"] mutableCopy];themeTabs[@"hyprlandBlur"]=@"theme";themed[@"tabs"]=themeTabs;if(!TWriteJSONDictionary(themed,config.path,nil))return 183;[config reload];if(TRGB(config.background)!=0x080B0D||TRGB(config.foreground)!=0xBFD3DE||TRGB(config.panel)!=0x0A1016||!config.hyprlandBlur){fprintf(stderr,"config-watcher-self-test failed stage=theme-inheritance background=%06x foreground=%06x panel=%06x blur=%d\n",TRGB(config.background),TRGB(config.foreground),TRGB(config.panel),config.hyprlandBlur);return 183;}TAppDelegate *delegate=[TAppDelegate new];delegate.config=config;delegate.windows=[NSMutableArray array];[delegate reloadAll];[delegate reloadAllIfChanged];[delegate reloadAllIfChanged];NSDictionary *baseline=[delegate configReloadDiagnostics];if([baseline[@"applications"] unsignedIntegerValue]!=1||[baseline[@"skips"] unsignedIntegerValue]!=2){fprintf(stderr,"config-watcher-self-test failed stage=unchanged-dedupe diagnostics=%s\n",baseline.description.UTF8String);return 182;}[delegate startConfigWatcher];
    NSUInteger expectedApplications=1;double maximumReloadMilliseconds=0;
    for(NSNumber *size in @[@18,@23]){
        NSMutableDictionary *dictionary=[TReadJSONDictionaryAtPath(config.path) mutableCopy];dictionary[@"fontSize"]=size;NSError *error=nil;
        CFAbsoluteTime writeStarted=CFAbsoluteTimeGetCurrent();
        if(!TWriteJSONDictionary(dictionary,config.path,&error)){fprintf(stderr,"config-watcher-self-test failed stage=write error=%s\n",error.localizedDescription.UTF8String);return 180;}
        expectedApplications++;NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:0.5];
        while((config.fontSize!=size.doubleValue||[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue]<expectedApplications)&&deadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];
        maximumReloadMilliseconds=MAX(maximumReloadMilliseconds,(CFAbsoluteTimeGetCurrent()-writeStarted)*1000.0);NSDate *settled=[NSDate dateWithTimeIntervalSinceNow:0.12];while(settled.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:settled];NSDictionary *diagnostics=[delegate configReloadDiagnostics];
        if(config.fontSize!=size.doubleValue||[diagnostics[@"applications"] unsignedIntegerValue]!=expectedApplications||maximumReloadMilliseconds>350){fprintf(stderr,"config-watcher-self-test failed stage=reload expected=%.0f actual=%.0f latency-ms=%.1f diagnostics=%s\n",size.doubleValue,config.fontSize,maximumReloadMilliseconds,diagnostics.description.UTF8String);return 181;}
    }
    NSData *validData=[NSData dataWithContentsOfFile:config.path];NSData *invalidData=[@"{\"fontSize\":" dataUsingEncoding:NSUTF8StringEncoding];if(![invalidData writeToFile:config.path options:NSDataWritingAtomic error:nil])return 184;expectedApplications++;NSDate *invalidDeadline=[NSDate dateWithTimeIntervalSinceNow:2];while([[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue]<expectedApplications&&invalidDeadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];if(config.fontSize!=23||TReadJSONDictionaryAtPath(config.path)||[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue]!=expectedApplications){fprintf(stderr,"config-watcher-self-test failed stage=invalid-preservation font=%.0f diagnostics=%s\n",config.fontSize,delegate.configReloadDiagnostics.description.UTF8String);return 184;}if(![validData writeToFile:config.path options:NSDataWritingAtomic error:nil])return 185;expectedApplications++;NSDate *recoveryDeadline=[NSDate dateWithTimeIntervalSinceNow:2];while((!TReadJSONDictionaryAtPath(config.path)||[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue]<expectedApplications)&&recoveryDeadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];if(config.fontSize!=23||[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue]!=expectedApplications)return 185;
    NSDictionary *diagnostics=[delegate configReloadDiagnostics];fprintf(stdout,"config-watcher-self-test ok repeated-atomic-edits=2 maximum-reload-ms=%.1f invalid-preserved=yes last-good-recovered=yes theme-inheritance=yes selected-file-rearmed=yes applied=%lu unchanged-skipped=%lu\n",maximumReloadMilliseconds,(unsigned long)[diagnostics[@"applications"] unsignedIntegerValue],(unsigned long)[diagnostics[@"skips"] unsignedIntegerValue]);return 0;
}

static uint64_t TCampaignRandom(uint64_t *state) {uint64_t value=*state;value^=value<<13;value^=value>>7;value^=value<<17;*state=value;return value;}
static NSString *TValidateAppControlCampaign(TAppDelegate *delegate) {
    if(!delegate.windows.count)return @"no windows remain";NSDictionary *status=[delegate automationReply:@{@"action":@"status"}];if(![status[@"ok"] boolValue]||[status[@"windows"] count]!=delegate.windows.count)return @"automation status topology mismatch";
    for(TWindowController *controller in delegate.windows){if(!controller.terminals.count||controller.terminals.count>16)return @"terminal count outside campaign bounds";if(![controller.terminals containsObject:controller.terminal])return @"active terminal is not owned by its window";[controller layoutTabs];for(TTerminalView *terminal in controller.terminals){if(NSWidth(terminal.frame)>0&&NSHeight(terminal.frame)>0){TRenderSnapshot *snapshot=terminal.renderSnapshot;NSDate *deadline=[NSDate dateWithTimeIntervalSinceNow:0.35];while(!snapshot.isValid&&deadline.timeIntervalSinceNow>0){[terminal resizeGrid];[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];snapshot=terminal.renderSnapshot;}if(!snapshot.isValid)return [NSString stringWithFormat:@"invalid visible render snapshot after layout settled frame=%@ state=%@",NSStringFromRect(terminal.frame),terminal.diagnosticState];}if(terminal.splitAnchor&&(![controller.terminals containsObject:terminal.splitAnchor]||terminal.splitAnchor==terminal))return @"invalid split anchor";TTerminalView *ancestor=terminal;NSUInteger depth=0;while(ancestor.splitAnchor){ancestor=ancestor.splitAnchor;if(++depth>controller.terminals.count)return @"split anchor cycle";}}}
    return nil;
}
static int TRunAppControlCampaign(NSUInteger requestedActions,uint64_t requestedSeed) {
    [TApplication sharedApplication];NSUInteger actions=MAX((NSUInteger)200,MIN((NSUInteger)2000,requestedActions?:500));uint64_t seed=requestedSeed?:0x5445524d41544943ULL,state=seed;TConfig *config=[TConfig new];[config setPlugin:@"hyprland-layout" enabled:NO];config.shell=@"/bin/zsh";config.shellArguments=@[@"-f"];config.tabAnimations=NO;config.blur=NO;config.glow=config.scanlines=config.vignette=0;TAppDelegate *delegate=[TAppDelegate new];delegate.config=config;delegate.windows=[NSMutableArray array];delegate.extensions=[TExtensionHost new];delegate.extensions.config=config;NSApp.delegate=delegate;[delegate newWindowWithDirectory:nil command:@"printf 'campaign-ready\\n'"];[delegate reloadAll];
    NSMutableDictionary *profileA=[TReadJSONDictionaryAtPath(config.path) mutableCopy],*profileB=[profileA mutableCopy];profileA[@"renderer"]=@"metal";profileA[@"fontName"]=@"SF Mono";profileA[@"fontSize"]=@11;profileA[@"padding"]=@10;profileB[@"renderer"]=@"appkit";profileB[@"fontName"]=@"Monaco";profileB[@"fontSize"]=@12;profileB[@"padding"]=@12;NSError *profileError=nil;if(!TWriteJSONDictionary(profileA,TCanonicalConfigPath(@"campaign-a.json"),&profileError)||!TWriteJSONDictionary(profileB,TCanonicalConfigPath(@"campaign-b.json"),&profileError)||!TWriteSelectedConfigFilename(@"campaign-a.json",&profileError)){fprintf(stderr,"app-control-campaign failed stage=profiles error=%s\n",profileError.localizedDescription.UTF8String);return 191;}[delegate reloadAllIfChanged];[delegate reloadAllIfChanged];
    BOOL codexAvailable=TPhase5Executable(@"codex")!=nil;if(codexAvailable){[delegate automationReply:@{@"action":@"new-tab",@"text":@"codex --no-alt-screen"}];NSDate *startup=[NSDate dateWithTimeIntervalSinceNow:1.5];while(startup.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.02]];for(NSString *name in @[@"campaign-b.json",@"campaign-a.json"]){TWriteSelectedConfigFilename(name,nil);NSUInteger before=[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue];[delegate reloadAllIfChanged];[delegate reloadAllIfChanged];NSUInteger after=[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue];if(after!=before+1){fprintf(stderr,"app-control-campaign failed stage=codex-config-dedupe profile=%s before=%lu after=%lu\n",name.UTF8String,(unsigned long)before,(unsigned long)after);return 192;}}[delegate automationReply:@{@"action":@"key",@"key":@"ctrl-c"}];}
    NSUInteger openedWindows=1,openedTabs=codexAvailable?1:0,openedSplits=0,closed=0,configSwitches=2,resizes=0,commands=1,focuses=0;NSArray *commandsToRun=@[@"printf 'unicode λ漢字🙂 é\\n'",@"printf '\\033[?2026hSYNC-LONG-0123456789\\rSYNC-SHORT\\033[K\\033[?2026l\\n'",@"tmux -V 2>/dev/null || true",@"ssh -V 2>&1",@"codex --help 2>/dev/null | head -n 2",@"vim --version 2>/dev/null | head -n 1",@"top -l 1 -n 0 2>/dev/null | head -n 2"];
    for(NSUInteger index=0;index<actions;index++){@autoreleasepool{NSUInteger total=0;for(TWindowController *window in delegate.windows)total+=window.terminals.count;NSUInteger operation=(NSUInteger)(TCampaignRandom(&state)%10);NSDictionary *reply=@{@"ok":@YES};
        if(operation==0&&delegate.windows.count<3){reply=[delegate automationReply:@{@"action":@"new-window",@"text":commandsToRun[TCampaignRandom(&state)%commandsToRun.count]}];openedWindows++;commands++;}
        else if(operation==1&&total<14){reply=[delegate automationReply:@{@"action":@"new-tab",@"text":commandsToRun[TCampaignRandom(&state)%commandsToRun.count]}];openedTabs++;commands++;}
        else if(operation==2&&total<14){reply=[delegate automationReply:@{@"action":@"split",@"direction":TCampaignRandom(&state)&1?@"vertical":@"horizontal",@"text":commandsToRun[TCampaignRandom(&state)%commandsToRun.count]}];openedSplits++;commands++;}
        else if(operation==3&&total>1){NSArray *targets=@[@"pane",@"tab",@"window"];NSString *target=targets[TCampaignRandom(&state)%targets.count];reply=[delegate automationReply:@{@"action":@"close",@"target":target}];if([reply[@"ok"] boolValue])closed++;else reply=@{@"ok":@YES};}
        else if(operation==4){reply=[delegate automationReply:@{@"action":@"focus",@"target":TCampaignRandom(&state)&1?@"next":@"previous"}];focuses++;}
        else if(operation==5){TWindowController *active=[delegate active];CGFloat width=640+(TCampaignRandom(&state)%561),height=380+(TCampaignRandom(&state)%421);[active.window setContentSize:NSMakeSize(width,height)];[active layoutTabs];resizes++;}
        else if(operation==6){NSString *name=(configSwitches++&1)?@"campaign-a.json":@"campaign-b.json";TWriteSelectedConfigFilename(name,nil);NSUInteger before=[[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue];[delegate reloadAllIfChanged];[delegate reloadAllIfChanged];if([[delegate configReloadDiagnostics][@"applications"] unsignedIntegerValue]!=before+1){fprintf(stderr,"app-control-campaign failed stage=config-dedupe action=%lu\n",(unsigned long)index);return 193;}}
        else if(operation==7){reply=[delegate automationReply:@{@"action":@"run",@"text":commandsToRun[TCampaignRandom(&state)%commandsToRun.count]}];commands++;}
        else if(operation==8){reply=[delegate automationReply:@{@"action":@"send",@"text":@"printf 'input-path-ok\\n'"}];[delegate automationReply:@{@"action":@"key",@"key":@"enter"}];commands++;}
        else {reply=[delegate automationReply:@{@"action":@"status"}];}
        if(![reply[@"ok"] boolValue]){fprintf(stderr,"app-control-campaign failed stage=action index=%lu operation=%lu error=%s\n",(unsigned long)index,(unsigned long)operation,[reply[@"error"] UTF8String]);return 194;}NSDate *tick=[NSDate dateWithTimeIntervalSinceNow:0.006];while(tick.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:tick];NSString *failure=TValidateAppControlCampaign(delegate);if(failure){fprintf(stderr,"app-control-campaign failed stage=invariant index=%lu reason=%s\n",(unsigned long)index,failure.UTF8String);return 195;}}}
    NSDate *settle=[NSDate dateWithTimeIntervalSinceNow:0.8];while(settle.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.02]];NSString *failure=TValidateAppControlCampaign(delegate);if(failure){fprintf(stderr,"app-control-campaign failed stage=final reason=%s\n",failure.UTF8String);return 196;}NSDictionary *reloads=[delegate configReloadDiagnostics];for(TWindowController *controller in [delegate.windows copy]){for(TTerminalView *terminal in [controller.terminals copy])[terminal stopShellTerminating:YES];[controller.window close];}fprintf(stdout,"app-control-campaign ok seed=%llu actions=%lu windows-opened=%lu tabs-opened=%lu splits-opened=%lu closed=%lu focuses=%lu resizes=%lu commands=%lu config-switches=%lu reloads-applied=%lu duplicate-reloads-skipped=%lu codex=%s real-pty=yes\n",(unsigned long long)seed,(unsigned long)actions,(unsigned long)openedWindows,(unsigned long)openedTabs,(unsigned long)openedSplits,(unsigned long)closed,(unsigned long)focuses,(unsigned long)resizes,(unsigned long)commands,(unsigned long)configSwitches,(unsigned long)[reloads[@"applications"] unsignedIntegerValue],(unsigned long)[reloads[@"skips"] unsignedIntegerValue],codexAvailable?"exercised":"unavailable");return 0;
}

static int TRunPhase5Soak(NSTimeInterval requestedSeconds) {
    [TApplication sharedApplication];NSTimeInterval duration=MAX(1,MIN(1800,requestedSeconds>0?requestedSeconds:1800));TConfig *config=[TConfig new];config.renderer=@"metal";config.blur=NO;config.glow=config.scanlines=config.vignette=0;config.scrollback=10000;config.shell=@"/usr/bin/true";config.shellArguments=@[];
    NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,1000,700) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];terminal.activeTerminal=YES;window.contentView=terminal;[window orderFront:nil];if(![terminal.diagnosticState[@"renderer"] isEqual:@"metal"]){[window close];fputs("phase5-soak skipped metal=unavailable\n",stdout);return 0;}
    NSString *tinyPNG=@"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=";[terminal consumeData:[[NSString stringWithFormat:@"\033]1337;File=inline=1:%@\a",tinyPNG] dataUsingEncoding:NSUTF8StringEncoding]];NSData *pattern=[@"phase5 sustained ASCII λ漢字🙂 👩🏽‍💻 é \033[38;2;89;194;255mcolor\033[0m \033[4:3munderline\033[0m\r\n" dataUsingEncoding:NSUTF8StringEncoding];NSMutableData *chunk=[NSMutableData dataWithCapacity:16384];while(chunk.length+pattern.length<=16384)[chunk appendData:pattern];
    CFAbsoluteTime started=CFAbsoluteTimeGetCurrent(),deadline=started+duration;NSUInteger frames=0;uint64_t bytes=0;double memoryStart=TAppFootprintMiB(),memoryMaximum=memoryStart;while(CFAbsoluteTimeGetCurrent()<deadline){@autoreleasepool{[terminal consumeData:chunk];bytes+=chunk.length;uint64_t generation=[terminal presentFrameForBenchmarkWithScrollDelta:(frames%240)<120?1:-1];if(!TWaitForMetalGeneration(terminal,generation,2)){[window close];fprintf(stderr,"phase5-soak failed stage=frame-timeout frame=%lu\n",(unsigned long)frames);return 130;}if(frames%300==0){NSDictionary *cache=[terminal metalCacheDiagnosticsForRendererSelfTest],*scheduler=[terminal metalSchedulerDiagnosticsForRendererSelfTest];memoryMaximum=MAX(memoryMaximum,TAppFootprintMiB());if([cache[@"imageBytes"] unsignedIntegerValue]>[cache[@"imageBudget"] unsignedIntegerValue]||[scheduler[@"pendingSnapshots"] unsignedIntegerValue]>1||[scheduler[@"maximumInFlightFrames"] unsignedIntegerValue]>1||[scheduler[@"generationReversals"] unsignedIntegerValue]){[window close];fprintf(stderr,"phase5-soak failed stage=bounds cache=%s scheduler=%s\n",cache.description.UTF8String,scheduler.description.UTF8String);return 131;}}frames++;[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];}}
    NSDictionary *cache=[terminal metalCacheDiagnosticsForRendererSelfTest],*scheduler=[terminal metalSchedulerDiagnosticsForRendererSelfTest];double elapsed=CFAbsoluteTimeGetCurrent()-started,memoryEnd=TAppFootprintMiB();memoryMaximum=MAX(memoryMaximum,memoryEnd);BOOL valid=[terminal.diagnosticState[@"renderer"] isEqual:@"metal"]&&[scheduler[@"generationReversals"] unsignedIntegerValue]==0&&[scheduler[@"maximumInFlightFrames"] unsignedIntegerValue]<=1&&[cache[@"imageBytes"] unsignedIntegerValue]<=[cache[@"imageBudget"] unsignedIntegerValue]&&memoryMaximum<=256;[window close];if(!valid){fprintf(stderr,"phase5-soak failed stage=final memory-max-mib=%.1f cache=%s scheduler=%s\n",memoryMaximum,cache.description.UTF8String,scheduler.description.UTF8String);return 132;}fprintf(stdout,"phase5-soak ok seconds=%.1f frames=%lu mib=%.1f memory-start-mib=%.1f memory-max-mib=%.1f memory-end-mib=%.1f memory-limit-mib=256 cache-bytes=%lu max-in-flight=%lu reversals=%lu\n",elapsed,(unsigned long)frames,(double)bytes/1048576.0,memoryStart,memoryMaximum,memoryEnd,(unsigned long)[cache[@"totalCacheBytes"] unsignedIntegerValue],(unsigned long)[scheduler[@"maximumInFlightFrames"] unsignedIntegerValue],(unsigned long)[scheduler[@"generationReversals"] unsignedIntegerValue]);return 0;
}
...
static TRenderSnapshot *TParityFixture(TRenderSnapshot *seed,uint64_t generation,NSString *cursorStyle,BOOL cursorFocused,BOOL effects,BOOL tiled,BOOL partialDamage,CGFloat scale) {
    ...
    if(rows>14&&columns>10){const uint32_t logo[][10]={{0x2590,0x259B,0x2588,0x2588,0x2588,0x259C,0x258C,0,0,0},{0x259D,0x259C,0x2588,0x2588,0x2588,0x2588,0x2588,0x259B,0x2598,0},{' ',' ',0x2598,0x2598,' ',0x259D,0x259D,0,0,0}};for(NSUInteger row=0;row<3;row++)for(NSUInteger column=0;column<10&&logo[row][column];column++)cells[(12+row)*columns+column]=(TCell){.ch=logo[row][column],.fg=0xE58A5B,.bg=TDefaultColor};}
...
static BOOL TValidateParityFixture(NSString *name,TRenderSnapshot *snapshot,NSString **failure) {
    ...
    NSUInteger count=snapshot.metrics.rows*snapshot.metrics.columns;const TCell *cells=snapshot.cells.bytes;const uint8_t *underlines=snapshot.underlineStyles.bytes,*selection=snapshot.selectionMask.bytes,*search=snapshot.searchMask.bytes,*links=snapshot.linkMask.bytes;NSUInteger bold=0,italic=0,inverse=0,wide=0,continuation=0,cluster=0,blocks=0,selected=0,searched=0,linked=0;BOOL underlineStyles[6]={0};
    for(NSUInteger index=0;index<count;index++){uint8_t flags=cells[index].flags;TBlockElementRect blockRects[2];bold+=(flags&TBold)!=0;italic+=(flags&TItalic)!=0;inverse+=(flags&TInverse)!=0;wide+=(flags&TWide)!=0;continuation+=(flags&TContinuation)!=0;cluster+=(flags&TCluster)!=0;blocks+=TBlockElementRects(cells[index].ch,blockRects)>0;selected+=selection[index]!=0;searched+=search[index]!=0;linked+=links[index]!=0;if(underlines[index]<=5)underlineStyles[underlines[index]]=YES;}
    ...
    BOOL complete=bold&&italic&&inverse&&wide&&continuation&&cluster>=9&&blocks>=10&&selected&&searched&&linked&&allUnderlines&&snapshot.graphemes.count>=9&&scriptCorpus&&snapshot.images.count&&snapshot.links.count&&snapshot.cursorVisible&&snapshot.historyCount&&snapshot.historyOffset>0;
...
static TRenderSnapshot *TTransparentRedrawSnapshot(TRenderSnapshot *source,uint64_t generation,NSString *text,CGFloat alpha) {
    NSUInteger rows=source.metrics.rows,columns=source.metrics.columns,count=rows*columns;NSMutableData *cells=[NSMutableData dataWithLength:count*sizeof(TCell)],*underlines=[NSMutableData dataWithLength:count],*selection=[NSMutableData dataWithLength:count],*search=[NSMutableData dataWithLength:count],*links=[NSMutableData dataWithLength:count];TCell *values=cells.mutableBytes,blank={.ch=' ',.fg=TDefaultColor,.bg=TDefaultColor};for(NSUInteger index=0;index<count;index++)values[index]=blank;NSData *utf8=[text dataUsingEncoding:NSUTF8StringEncoding];const uint8_t *bytes=utf8.bytes;for(NSUInteger index=0;index<MIN(columns,utf8.length);index++)values[index]=(TCell){.ch=bytes[index],.fg=TDefaultColor,.bg=TDefaultColor};NSMutableDictionary *style=[source.style mutableCopy];style[@"backgroundAlpha"]=@(alpha);style[@"glow"]=@0;style[@"scanlines"]=@0;style[@"vignette"]=@0;return [[TRenderSnapshot alloc]initWithGeneration:generation metrics:source.metrics cells:cells underlineStyles:underlines selectionMask:selection searchMask:search linkMask:links graphemes:@[] style:style links:@{} images:@{} cursorX:MIN(text.length,columns-1) cursorY:0 cursorVisible:NO historyCount:0 historyOffset:0 fullDamage:YES damagedRows:NSMakeRange(0,rows)];
}

static BOOL TAppKitTransparentRedrawMatchesClean(TTerminalView *view,TRenderSnapshot *source) {
    NSUInteger width=(NSUInteger)ceil(source.metrics.viewportWidth),height=(NSUInteger)ceil(source.metrics.viewportHeight),stride=width*4;if(!width||!height)return NO;NSArray<NSString *> *updates=@[@"Codex streaming response with a long redraw tail",@"OK",@"status: checking renderer state",@"",@"prompt >",@"a much longer line that must replace every prior glyph",@"done"];
    for(NSNumber *alphaValue in @[@0,@0.28]){CGFloat alpha=alphaValue.doubleValue;NSMutableData *incremental=[NSMutableData dataWithLength:stride*height],*clean=[NSMutableData dataWithLength:stride*height];CGColorSpaceRef colorSpace=CGColorSpaceCreateDeviceRGB();CGContextRef incrementalContext=colorSpace?CGBitmapContextCreate(incremental.mutableBytes,width,height,8,stride,colorSpace,(CGBitmapInfo)(kCGImageAlphaPremultipliedFirst|kCGBitmapByteOrder32Little)):NULL;CGContextRef cleanContext=colorSpace?CGBitmapContextCreate(clean.mutableBytes,width,height,8,stride,colorSpace,(CGBitmapInfo)(kCGImageAlphaPremultipliedFirst|kCGBitmapByteOrder32Little)):NULL;if(colorSpace)CGColorSpaceRelease(colorSpace);if(!incrementalContext||!cleanContext){if(incrementalContext)CGContextRelease(incrementalContext);if(cleanContext)CGContextRelease(cleanContext);return NO;}NSGraphicsContext *graphics=[NSGraphicsContext graphicsContextWithCGContext:incrementalContext flipped:YES];[NSGraphicsContext saveGraphicsState];NSGraphicsContext.currentContext=graphics;TRenderSnapshot *finalFrame=nil;for(NSUInteger cycle=0;cycle<28;cycle++){NSString *text=updates[cycle%updates.count];finalFrame=TTransparentRedrawSnapshot(source,9001+cycle,text,alpha);[view setDisplaySnapshotForRendererSelfTest:finalFrame];CGFloat top=[finalFrame.style[@"top"] doubleValue];[view drawRect:cycle?NSMakeRect(0,top,view.bounds.size.width,finalFrame.metrics.cellHeight):view.bounds];}[NSGraphicsContext restoreGraphicsState];CGContextFlush(incrementalContext);graphics=[NSGraphicsContext graphicsContextWithCGContext:cleanContext flipped:YES];[NSGraphicsContext saveGraphicsState];NSGraphicsContext.currentContext=graphics;[view setDisplaySnapshotForRendererSelfTest:finalFrame];[view drawRect:view.bounds];[NSGraphicsContext restoreGraphicsState];CGContextFlush(cleanContext);CGContextRelease(incrementalContext);CGContextRelease(cleanContext);if(![incremental isEqual:clean])return NO;}return YES;
}
...
static CGImageRef TCreateCacheFixtureImage(NSUInteger index) {
...
static int TRunRendererSchedulerSelfTest(void) {
    ...
    [terminal presentSnapshotForRendererSelfTest:TCacheFixtureSnapshot(base,3999,base.images)];NSDictionary *stats=[terminal metalSchedulerDiagnosticsForRendererSelfTest];BOOL valid=[stats[@"pendingSnapshots"] unsignedIntegerValue]<=1&&[stats[@"maximumInFlightFrames"] unsignedIntegerValue]<=1&&[stats[@"coalescedSnapshots"] unsignedIntegerValue]>0&&[stats[@"generationReversals"] unsignedIntegerValue]==0&&[stats[@"lastAcceptedGeneration"] unsignedLongLongValue]==last&&[stats[@"lastSubmittedGeneration"] unsignedLongLongValue]==last;if(!valid){[window close];fprintf(stderr,"renderer-scheduler-self-test failed stage=bounded-monotonic stats=%s\n",stats.description.UTF8String);return 104;}
    TRenderMetrics resized=base.metrics;resized.viewportWidth+=17;resized.viewportHeight+=13;TRenderSnapshot *resizeSnapshot=[[TRenderSnapshot alloc]initWithGeneration:last+2 metrics:resized cells:base.cells underlineStyles:base.underlineStyles selectionMask:base.selectionMask searchMask:base.searchMask linkMask:base.linkMask graphemes:base.graphemes style:base.style links:base.links images:base.images cursorX:base.cursorX cursorY:base.cursorY cursorVisible:base.cursorVisible historyCount:base.historyCount historyOffset:base.historyOffset fullDamage:YES damagedRows:NSMakeRange(0,resized.rows)];CFAbsoluteTime resizeStart=CFAbsoluteTimeGetCurrent();if(![terminal configureRendererForSnapshotSelfTest:resizeSnapshot]||![terminal.diagnosticState[@"rendererSurfaceHidden"] boolValue]){[window close];fputs("renderer-scheduler-self-test failed stage=resize-configure\n",stderr);return 105;}[terminal presentSnapshotForRendererSelfTest:TCacheFixtureSnapshot(base,last+1,base.images)];NSDate *staleDeadline=[NSDate dateWithTimeIntervalSinceNow:0.05];while(staleDeadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.002]];if([terminal.diagnosticState[@"renderGeneration"] unsignedLongLongValue]>=last+1||![terminal.diagnosticState[@"rendererSurfaceHidden"] boolValue]){[window close];fputs("renderer-scheduler-self-test failed stage=stale-resize-frame\n",stderr);return 106;}[terminal presentSnapshotForRendererSelfTest:resizeSnapshot];if(!TWaitForMetalGeneration(terminal,last+2,2)){[window close];fputs("renderer-scheduler-self-test failed stage=resize-timeout\n",stderr);return 107;}NSDate *revealDeadline=[NSDate dateWithTimeIntervalSinceNow:0.1];while([terminal.diagnosticState[@"rendererSurfaceHidden"] boolValue]&&revealDeadline.timeIntervalSinceNow>0)[NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.002]];if([terminal.diagnosticState[@"rendererSurfaceHidden"] boolValue]){[window close];fputs("renderer-scheduler-self-test failed stage=resize-reveal\n",stderr);return 108;}double resizeMilliseconds=(CFAbsoluteTimeGetCurrent()-resizeStart)*1000.0;stats=[terminal metalSchedulerDiagnosticsForRendererSelfTest];BOOL displayLinkActive=[stats[@"displayLinkActive"] boolValue];[window close];fprintf(stdout,"renderer-scheduler-self-test ok burst=%llu coalesced=%lu max-in-flight=%lu reversals=%lu stale-resize-dropped=yes resize-ms=%.3f display-link=%s\n",(unsigned long long)(last-first+1),(unsigned long)[stats[@"coalescedSnapshots"] unsignedIntegerValue],(unsigned long)[stats[@"maximumInFlightFrames"] unsignedIntegerValue],(unsigned long)[stats[@"generationReversals"] unsignedIntegerValue],resizeMilliseconds,displayLinkActive?"active":"immediate-fallback");return 0;
}
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if(argc>1&&!strcmp(argv[1],"--renderer-reliability-self-test"))return TRunRendererReliabilitySelfTest();
        if(argc>1&&!strcmp(argv[1],"--phase6-field-self-test"))return TRunPhase6FieldSelfTest();
        if(argc>1&&!strcmp(argv[1],"--phase7-production-self-test"))return TRunPhase7ProductionSelfTest();
        if(argc>1&&!strcmp(argv[1],"--phase8-recovery-self-test"))return TRunPhase8RecoverySelfTest();
        if(argc>1&&!strcmp(argv[1],"--phase9-field-self-test"))return TRunPhase9FieldQualificationSelfTest();
        if(argc>1&&!strcmp(argv[1],"--phase10-campaign-self-test"))return TRunPhase10CampaignSelfTest();
        if(argc>1&&!strcmp(argv[1],"--tui-compat-self-test"))return TRunTUICompatibilitySelfTest();
        if(argc>1&&!strcmp(argv[1],"--config-watcher-self-test"))return TRunConfigWatcherSelfTest();
        if(argc>1&&!strcmp(argv[1],"--app-control-campaign"))return TRunAppControlCampaign(argc>2?(NSUInteger)strtoull(argv[2],NULL,10):500,argc>3?strtoull(argv[3],NULL,0):0);
        if(argc>1&&!strcmp(argv[1],"--phase5-soak"))return TRunPhase5Soak(argc>2?strtod(argv[2],NULL):1800);
