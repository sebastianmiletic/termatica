...
- (void)ensureEditableFile {
    ...
    NSString *selectedPath=TCanonicalConfigPath(selected);NSDictionary *selectedDictionary=TReadJSONDictionaryAtPath(selectedPath);
    if(legacyRegular&&legacy){selectedDictionary=legacy;}
    if(!selectedDictionary)selectedDictionary=[self defaults];
    NSMutableDictionary *selectedNormalized=[self normalizedDictionary:selectedDictionary configName:nil];NSError *writeError=nil;if(![selectedNormalized isEqualToDictionary:selectedDictionary]&&!TWriteJSONDictionary(selectedNormalized,selectedPath,&writeError))TLog(@"could not prepare selected config %@: %@",selected,writeError.localizedDescription);
...
- (void)animateNewTerminal:(TTerminalView *)terminal {
    ...
    if([self usesTiledLayout]){TAnimateCenterReveal(terminal,[self animationDuration:self.config->terminalAnimationDuration],self.config->tileCornerRadius,@"termatica.terminal.center",nil);return;}
...
- (void)layoutTabs {
    ...
    for(NSUInteger i=0;i<_terminals.count;i++){
        ...
        BOOL animateTerminal=shown&&tile&&animate&&(!entering||terminal==entering);
        terminal.wantsLayer=(shown&&tile)||animateTerminal||[terminal usesMetalRenderer];terminal.layer.cornerRadius=shown&&tile?self.config->tileCornerRadius:0;terminal.layer.masksToBounds=shown&&tile;[terminal.layer removeAnimationForKey:@"termatica.hypr.snap"];
    ...
    if(entering&&animate&&![entering isHidden])TAnimateCenterReveal(entering,[self animationDuration:self.config->terminalAnimationDuration],self.config->tileCornerRadius,@"termatica.terminal.center",nil);
...
static int TRunTerminalSelfTest(void) {
    ...
    TConfig *tiledConfig=[TConfig new];tiledConfig.hyprlandLayout=YES;tiledConfig.tabAnimations=NO;tiledConfig.backgroundOpacity=0.37;tiledConfig.background=[NSColor colorWithSRGBRed:0.12 green:0.18 blue:0.26 alpha:tiledConfig.backgroundOpacity];tiledConfig.shell=@"/usr/bin/true";tiledConfig.shellArguments=@[];
...
static int TRunConfigWatcherSelfTest(void) {
    ...
    NSDictionary *diagnostics=[delegate configReloadDiagnostics];fprintf(stdout,"config-watcher-self-test ok repeated-atomic-edits=2 selected-file-rearmed=yes applied=%lu unchanged-skipped=%lu\n",(unsigned long)[diagnostics[@"applications"] unsignedIntegerValue],(unsigned long)[diagnostics[@"skips"] unsignedIntegerValue]);return 0;
}
