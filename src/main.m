...
- (void)viewDidMoveToWindow {[super viewDidMoveToWindow];if(_presentationReady&&self.window)[self recoverPresentationAfterLifecycleChange];}
- (void)viewDidChangeBackingProperties {[super viewDidChangeBackingProperties];[self recoverPresentationAfterLifecycleChange];}
- (void)recoverPresentationAfterLifecycleChange {
    if(!_presentationReady)return;
    [self configureRenderBackend];
    @synchronized(self){[self markAllDamage];}
...
- (void)windowDidChangeScreen:(NSNotification *)notification {for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange];}
- (void)windowDidChangeBackingProperties:(NSNotification *)notification {for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange];}
- (void)windowDidChangeOcclusionState:(NSNotification *)notification {if(self.window.occlusionState&NSWindowOcclusionStateVisible)for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange];}
...
- (void)recoverPresentations:(NSNotification *)notification {
    (void)notification;
    if(!NSThread.isMainThread){__weak typeof(self) weakSelf=self;dispatch_async(dispatch_get_main_queue(),^{[weakSelf recoverPresentations:nil];});return;}
    for(TWindowController *window in self.windows)for(TTerminalView *terminal in window.terminals)[terminal recoverPresentationAfterLifecycleChange];
}
