...
- (NSArray<NSValue *> *)hyprlandFrames {
    ...
    if(self.config.hyprlandLayout&&count){
        CGFloat scale=self.window.screen.backingScaleFactor?:NSScreen.mainScreen.backingScaleFactor?:1;NSUInteger columns=count>1?(NSUInteger)ceil(sqrt((double)count)):1,rows=(NSUInteger)ceil((double)count/columns);CGFloat usableHeight=MAX(1,h-gap*(rows-1)),cellHeight=usableHeight/rows;
        for(NSUInteger index=0;index<count;index++){NSUInteger row=index/columns,column=index%columns,countInRow=MIN(columns,count-row*columns);CGFloat usableWidth=MAX(1,w-gap*(countInRow-1)),cellWidth=usableWidth/countInRow,x0=column*(cellWidth+gap),x1=x0+cellWidth,top=row*(cellHeight+gap),bottom=top+cellHeight;x0=round(x0*scale)/scale;x1=round(x1*scale)/scale;top=round(top*scale)/scale;bottom=round(bottom*scale)/scale;frames[index]=[NSValue valueWithRect:NSMakeRect(x0,h-bottom,MAX(1,x1-x0),MAX(1,bottom-top))];}
        return frames;
    }
    NSMutableArray<TTerminalView *> *roots=[NSMutableArray array];TTerminalView *root=[self rootTerminalForTerminal:self.terminal];if(root)[roots addObject:root];
...
static int TRunTerminalSelfTest(void) {
    ...
    TTerminalView *leftRoot=tiles.terminals[0],*rightRoot=tiles.terminals[1];tiles.terminal=leftRoot;[tiles addCommandShiftTHorizontalSplit];[tiles layoutTabs];TTerminalView *quarter=tiles.terminal;
    if(tiles.terminals.count!=3||fabs(NSHeight(quarter.frame)-NSHeight(rightRoot.frame))>1||NSWidth(quarter.frame)>=NSWidth(rightRoot.frame))return 52;
