...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    for(NSInteger y=firstRow;y<lastRow;y++){[NSGraphicsContext saveGraphicsState];NSRectClip(NSMakeRect(left,top+y*cellHeight,columns*cellWidth,cellHeight));const TCell *line=allCells+y*columns;BOOL inToken=NO;NSUInteger token=0;
        ...
        if(snapshot.cursorVisible&&(NSUInteger)y==snapshot.cursorY&&!blockCursor){[TColor(cursorColor)setFill];NSRectFill(TCursorRect(snapshot));}
...
- (void)addCommandShiftTHorizontalSplit;
...
- (void)addCommandShiftTHorizontalSplit {[self addSplitWithVerticalDivider:NO];}
...
- (void)newVerticalTab:(id)sender {TLog(@"horizontal split of current tab shortcut");[[self active] addCommandShiftTHorizontalSplit];}
