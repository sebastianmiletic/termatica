...
- (void)reverseIndex {
    ...
    TCell blank=[self blankCell]; for(NSUInteger x=0;x<_cols;x++) _cells[_scrollTop*_cols+x]=blank;[_linksByCell removeAllObjects];
}
...
- (void)eraseDisplay:(int)mode {
    TCell blank=[self blankCell];
    if (mode==2 || mode==3) { for(NSUInteger i=0;i<_cols*_rows;i++) _cells[i]=blank;if(_underlineStyles)memset(_underlineStyles,0,_cols*_rows);[_linksByCell removeAllObjects];_rowOffset=0;if(mode==3)[self clearHistory];[self markAllDamage]; }
...
- (void)eraseLine:(int)mode {
    ...
    TCell *row=[self cellsForRow:_cursorY];for(NSUInteger x=a;x<b;x++)row[x]=blank;if(_underlineStyles)memset(_underlineStyles+((_rowOffset+_cursorY)%_rows)*_cols+a,0,b-a);[self clearLinksInRow:_cursorY from:a to:b];[self markDamageX:a y:_cursorY width:b-a height:1];
}
- (void)eraseCharacters:(int)n { TCell b=[self blankCell],*row=[self cellsForRow:_cursorY];NSUInteger end=MIN(_cols,_cursorX+(NSUInteger)n);for(NSUInteger x=_cursorX;x<end;x++)row[x]=b;if(_underlineStyles)memset(_underlineStyles+((_rowOffset+_cursorY)%_rows)*_cols+_cursorX,0,end-_cursorX);[self clearLinksInRow:_cursorY from:_cursorX to:end];[self markDamageX:_cursorX y:_cursorY width:end-_cursorX height:1]; }
...
static NSDictionary *TRunInAppBenchmark(TConfig *config,TTerminalView *active,NSUInteger windows,NSUInteger terminals){@autoreleasepool{
    double memoryBefore=TAppFootprintMiB();NSUInteger bytes=1024*1024;NSDictionary *patterns=@{@"ASCII":@"benchmark plain terminal text 0123456789 abcdefghijklmnopqrstuvwxyz\r\n",@"Unicode":@"Unicode λ漢字🙂 composed é emoji 👩‍💻\r\n",@"CSI":@"\033[38;2;89;194;255mcyan\033[0m \033[1mbold\033[0m \033[4:3munderline\033[0m \033[2Kbenchmark\r\n"};NSMutableDictionary *rates=[NSMutableDictionary dictionary];for(NSString *name in @[@"ASCII",@"Unicode",@"CSI"]){NSData *data=TBenchmarkPattern(patterns[name],bytes);rates[name]=@(TInAppThroughput(config,data));}
    ...
    NSMutableArray *lines=[NSMutableArray arrayWithObjects:@"",@"TERMATICA IN-APP BENCHMARK",[NSString stringWithFormat:@"Version             %@ (build %@)",TCurrentVersion(),build],[NSString stringWithFormat:@"Config              %@",TActiveConfigName(config)],[NSString stringWithFormat:@"Configured renderer %@",state[@"renderer"]?:config.renderer],[NSString stringWithFormat:@"Font                %@ %.1f",config.fontName,config.fontSize],[NSString stringWithFormat:@"Display refresh     %lu Hz",(unsigned long)displayFPS],[NSString stringWithFormat:@"App memory          %.1f MiB before / %.1f MiB after",memoryBefore,memoryAfter],[NSString stringWithFormat:@"Open state          %lu window(s), %lu terminal(s), unchanged",(unsigned long)windows,(unsigned long)terminals],@"",@"WORKLOAD                 RESULT",[NSString stringWithFormat:@"ASCII parser/model       %8.1f MiB/s",[rates[@"ASCII"] doubleValue]],[NSString stringWithFormat:@"Unicode parser/model     %8.1f MiB/s",[rates[@"Unicode"] doubleValue]],[NSString stringWithFormat:@"CSI parser/model         %8.1f MiB/s",[rates[@"CSI"] doubleValue]],[NSString stringWithFormat:@"Offscreen text paint     %8.1f FPS  (p50 %.3f ms, p95 %.3f ms)",[textFrame[@"fps"] doubleValue],[textFrame[@"p50"] doubleValue],[textFrame[@"p95"] doubleValue]],[NSString stringWithFormat:@"Offscreen image paint    %8.1f FPS  (p50 %.3f ms, p95 %.3f ms)",[imageFrame[@"fps"] doubleValue],[imageFrame[@"p50"] doubleValue],[imageFrame[@"p95"] doubleValue]],@"",@"Scope: isolated offscreen terminal in this running app, using the active visual config.",@"No PTY, tab, scrollback, process, or window was closed or replaced.",@"Paint FPS is warmed offscreen AppKit composition, not Metal FPS or display key-to-photon FPS.",nil];return @{@"ok":@YES,@"lines":lines};}}
...
- (void)handleCLIRequest:(NSDictionary *)request {NSString *command=request[@"command"];if([command isEqual:@"reload"])[self reloadAll];else if([command isEqual:@"run"]&&![[self active] executeExtensionNamed:request[@"name"] query:request[@"query"]])TLog(@"extension command not found: %@",request[@"name"]);else if([command isEqual:@"benchmark"]){TWindowController *controller=[self active];NSUInteger terminals=0;for(TWindowController *window in self.windows)terminals+=window.terminals.count;NSDictionary *reply=controller?TRunInAppBenchmark(self.config,controller.terminal,self.windows.count,terminals):@{@"ok":@NO,@"lines":@[@"No active Termatica terminal is available."]};TSendCLIReply(request[@"replyPath"],reply);}else if([command isEqual:@"quit"])[NSApp terminate:nil];}
- (void)buildMenu {
