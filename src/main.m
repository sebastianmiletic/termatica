...
static NSString *TCompletionScript(NSString *shell) {
    NSString *commands=@"benchmark bench renderer-report config config-file update reload editor run completions help version b rr c cf u r e x h v",*configActions=@"list get set create use rename delete",*editors=@"vim nvim emacs nano micro hx";
...
static int TRunCLI(int argc, const char *argv[]) {
    ...
    NSDictionary *quick=@{@"b":@"benchmark",@"c":@"config",@"cf":@"config-file",@"u":@"update",@"r":@"reload",@"rr":@"renderer-report",@"e":@"editor",@"x":@"run",@"h":@"help",@"v":@"version"};
    arg=quick[arg]?:arg;
...
- (NSDictionary *)diagnosticState {
    @synchronized(self){return @{
      ...
      @"metalFailed":@(_metalFailed),@"metalQuarantined":@(_metalFailed),@"metalRetryRequiresReselect":@(_metalFailed),@"metalFailureCount":@(_metalFailureCount),
      @"metalQuarantineBypassCount":@(_metalQuarantineBypassCount),@"lastMetalFailureStage":_lastMetalFailureStage?:@"none",@"secondsSinceMetalFailure":@(_lastMetalFailureAt>0?MAX(0,CFAbsoluteTimeGetCurrent()-_lastMetalFailureAt):-1),
...
- (NSDictionary *)rendererFieldReport {
    ...
    NSMutableArray *panes=[NSMutableArray array];NSUInteger windowIndex=0;for(TWindowController *controller in self.windows){NSUInteger paneIndex=0;for(TTerminalView *terminal in controller.terminals){NSDictionary *state=terminal.diagnosticState;[panes addObject:@{@"window":@(windowIndex),@"pane":@(paneIndex++),@"active":state[@"activeTerminal"]?:@NO,@"firstResponder":state[@"firstResponder"]?:@NO,@"requestedRenderer":state[@"requestedRenderer"]?:@"appkit",@"renderer":state[@"renderer"]?:@"none",@"rows":state[@"rows"]?:@0,@"columns":state[@"columns"]?:@0,@"renderGeneration":state[@"renderGeneration"]?:@0,@"metalFailed":state[@"metalFailed"]?:@NO,@"metalQuarantined":state[@"metalQuarantined"]?:@NO,@"metalRetryRequiresReselect":state[@"metalRetryRequiresReselect"]?:@NO,@"metalFailureCount":state[@"metalFailureCount"]?:@0,@"metalQuarantineBypassCount":state[@"metalQuarantineBypassCount"]?:@0,@"lastMetalFailureStage":state[@"lastMetalFailureStage"]?:@"none",@"secondsSinceMetalFailure":state[@"secondsSinceMetalFailure"]?:@(-1),@"presentationRecoveryCount":state[@"presentationRecoveryCount"]?:@0,@"lastPresentationRecovery":state[@"lastPresentationRecovery"]?:@"none"}];}windowIndex++;}
    NSString *generated=[NSISO8601DateFormatter stringFromDate:NSDate.date timeZone:NSTimeZone.localTimeZone formatOptions:NSISO8601DateFormatWithInternetDateTime];NSDictionary *info=NSBundle.mainBundle.infoDictionary;return @{@"ok":@YES,@"schemaVersion":@2,@"generatedAt":generated?:@"unknown",@"privacy":@{@"terminalContentIncluded":@NO,@"workingDirectoriesIncluded":@NO,@"configPathIncluded":@NO},@"rolloutPolicy":@{@"defaultRenderer":@"appkit",@"metalOptIn":@YES,@"appkitAvailable":@YES,@"defaultChangeEligible":@NO,@"defaultChangeBlocker":@"physical-and-multi-machine-field-gates-open"},@"app":@{@"version":[info[@"CFBundleShortVersionString"] description]?:TCurrentVersion(),@"build":[info[@"CFBundleVersion"] description]?:@"unknown",@"requestedRenderer":self.config.renderer?:@"appkit",@"activeConfig":TActiveConfigName(self.config),@"font":self.config.fontName?:@"unknown",@"fontSize":@(self.config.fontSize),@"windowCount":@(self.windows.count),@"paneCount":@(panes.count)},@"system":@{@"operatingSystem":NSProcessInfo.processInfo.operatingSystemVersionString?:@"unknown",@"displayCount":@(displays.count)},@"displays":displays,@"panes":panes};
}
...
static int TRunPhase7ProductionSelfTest(void) {
    ...
    NSDictionary *quarantined=terminal.diagnosticState;if(![quarantined[@"renderer"] isEqual:@"appkit"]||[quarantined[@"metalFailureCount"] unsignedIntegerValue]!=1||[quarantined[@"metalQuarantineBypassCount"] unsignedIntegerValue]<baseline+64||![quarantined[@"metalRetryRequiresReselect"] boolValue]||![[terminal visibleText] containsString:@"PHASE7-QUARANTINE"]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=bounded-recovery state=%s\n",quarantined.description.UTF8String);return 149;}
    ...
    TAppDelegate *delegate=[TAppDelegate new];delegate.config=[TConfig new];delegate.windows=[NSMutableArray array];NSDictionary *report=[delegate rendererFieldReport],*policy=report[@"rolloutPolicy"],*privacy=report[@"privacy"];if([report[@"schemaVersion"] unsignedIntegerValue]!=2||![policy[@"defaultRenderer"] isEqual:@"appkit"]||![policy[@"metalOptIn"] boolValue]||[policy[@"defaultChangeEligible"] boolValue]||[privacy[@"terminalContentIncluded"] boolValue]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=report-policy report=%s\n",report.description.UTF8String);return 154;}
    [window close];fprintf(stdout,"phase7-production-self-test ok initialization-failures=1 quarantine-recoveries=64 runtime-metal-active=%lu runtime-recoveries=%lu retry=explicit-reselect state-preserved=yes default=appkit report-schema=2\n",(unsigned long)metalRuns,(unsigned long)runtimeRecoveries);return 0;
}
...
static int TRunTUICompatibilitySelfTest(void) {
    ...
    for(NSDictionary *item in cases){NSString *name=item[@"name"],*executable=TPhase5Executable(item[@"command"]);if(!executable){[skipped addObject:name];continue;}TConfig *config=[TConfig new];config.renderer=@"metal";config.blur=NO;config.shellIntegration=NO;config.shell=executable;config.shellArguments=item[@"args"];
        ...
        BOOL observed=TPhase5WaitForOutput(terminal,item[@"needles"],3);if(!observed){NSString *visible=terminal.visibleText;[terminal stopShellTerminating:YES];[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=output app=%s text=%s\n",name.UTF8String,visible.UTF8String);return 122;}
