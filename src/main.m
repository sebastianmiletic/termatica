...
static NSString *TCompletionScript(NSString *shell) {
    NSString *commands=@"benchmark bench renderer-report renderer-qualification qualify renderer-retry retry config config-file update reload editor run completions help version b rr rq rt c cf u r e x h v",*configActions=@"list get set create use rename delete",*editors=@"vim nvim emacs nano micro hx";
...
static int TRunCLI(int argc, const char *argv[]) {
    ...
    NSDictionary *quick=@{@"b":@"benchmark",@"c":@"config",@"cf":@"config-file",@"u":@"update",@"r":@"reload",@"rr":@"renderer-report",@"rq":@"renderer-qualification",@"rt":@"renderer-retry",@"e":@"editor",@"x":@"run",@"h":@"help",@"v":@"version"};
    arg=quick[arg]?:arg;
...
- (NSDictionary *)rendererFieldReport {
    ...
    NSDictionary *evidence=@{@"evidenceSource":@"observed-runtime-events",@"sessionSeconds":@(MAX(0,CFAbsoluteTimeGetCurrent()-_fieldStartedAt)),@"processArchitecture":TRuntimeArchitecture(),@"translated":@(TProcessIsTranslated()),@"minimumDisplayCount":@(minimumDisplays),@"maximumDisplayCount":@(maximumDisplays),@"observedDisplayCountCount":@(_observedDisplayCounts.count),@"observedScaleCount":@(_observedScales.count),@"observedRefreshRateCount":@(_observedRefreshRates.count),@"displayParameterEvents":@(_displayParameterEvents),@"displayCountChanges":@(_displayCountChanges),@"wakeEvents":@([_fieldEventCounts[NSWorkspaceDidWakeNotification] unsignedIntegerValue]),@"screenWakeEvents":@([_fieldEventCounts[NSWorkspaceScreensDidWakeNotification] unsignedIntegerValue]),@"screenTransitionEvents":@(screenTransitionEvents),@"backingScaleEvents":@(backingScaleEvents),@"observedScreenChanges":@(observedScreenChanges),@"observedBackingScaleChanges":@(observedBackingScaleChanges),@"metalPanes":@(metalPanes),@"quarantinedPanes":@(quarantinedPanes)};NSDictionary *qualification=TFieldQualification(evidence);
