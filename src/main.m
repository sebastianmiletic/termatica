...
- (void)updateCursorOverlayForSnapshot:(TRenderSnapshot *)snapshot {
    ...
    BOOL visible=snapshot.isValid&&snapshot.cursorVisible&&(!_cursorBlink||_cursorBlinkVisible)&&self.activeTerminal&&self.window.isKeyWindow&&self.window.firstResponder==self;[self applyCursorOverlayForSnapshot:snapshot visible:visible];
}
...
- (TRenderSnapshot *)renderSnapshot {
    @synchronized(self){
        ...
        BOOL cursorActive=self.activeTerminal,cursorFocused=cursorActive&&self.window.isKeyWindow&&self.window.firstResponder==self,pinnedInlineCursor=_inlineViewportMode&&_inlineViewportTop>0&&_cursorY>=_inlineViewportTop,cursorShown=_cursorVisible&&(_historyOffset==0||pinnedInlineCursor)&&cursorFocused;
...
- (void)startCursorBlink {
    ...
    __weak typeof(self) weakSelf=self;
    dispatch_source_set_event_handler(_blinkTimer,^{__strong typeof(weakSelf) self=weakSelf;if(!self)return;self->_cursorBlinkVisible=!self->_cursorBlinkVisible;if(self->_displaySnapshot)[self updateCursorOverlayForSnapshot:self->_displaySnapshot];});
    dispatch_resume(_blinkTimer);
}
- (void)keyDown:(NSEvent *)e {
    if(_cursorBlink&&!_cursorBlinkVisible){_cursorBlinkVisible=YES;if(_displaySnapshot)[self updateCursorOverlayForSnapshot:_displaySnapshot];}
...
- (NSDictionary *)diagnosticState {
    @synchronized(self){return @{
      ...
      @"activeTerminal":@(self.activeTerminal),@"firstResponder":@(self.window.firstResponder==self),@"displayCursorVisible":@(_displaySnapshot.cursorVisible),@"cursorBlinkPhase":@(_cursorBlinkVisible),@"overlayCursorVisible":@(!_cursorOverlay.hidden),@"overlayCursorFrame":NSStringFromRect(_cursorOverlay.frame),
...
static int TRunTUICompatibilitySelfTest(void) {
    ...
    for(NSDictionary *item in cases){NSString *name=item[@"name"],*executable=TPhase5Executable(item[@"command"]);if(!executable){[skipped addObject:name];continue;}TConfig *config=[TConfig new];config.renderer=@"metal";config.blur=NO;config.shellIntegration=NO;config.shell=executable;config.shellArguments=item[@"args"];
        NSWindow *window=[[NSWindow alloc]initWithContentRect:NSMakeRect(0,0,900,600) styleMask:NSWindowStyleMaskBorderless backing:NSBackingStoreBuffered defer:NO];window.releasedWhenClosed=NO;TTerminalView *terminal=[[TTerminalView alloc]initWithFrame:window.contentView.bounds config:config];terminal.activeTerminal=YES;window.contentView=terminal;[window makeFirstResponder:terminal];[window orderFront:nil];if(![terminal.diagnosticState[@"renderer"] isEqual:@"metal"]){[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=metal-unavailable app=%s\n",name.UTF8String);return 124;}if(![terminal startShell]){[window close];[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stderr,"tui-compat-self-test failed stage=launch app=%s\n",name.UTF8String);return 121;}
    ...
    NSString *tmux=TPhase5Executable(@"tmux");if(tmux)TRunTask(tmux,@[@"-L",tmuxSocket,@"kill-server"],nil,nil);[[NSFileManager defaultManager] removeItemAtPath:fixture error:nil];fprintf(stdout,"tui-compat-self-test ok passed=%s skipped=%s renderer=metal real-pty=yes\n",[passed componentsJoinedByString:@","].UTF8String,skipped.count?[skipped componentsJoinedByString:@","].UTF8String:"none");return 0;
}
