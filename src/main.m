...
- (void)windowDidChangeScreen:(NSNotification *)notification {for(TTerminalView *terminal in _terminals)[terminal recoverPresentationAfterLifecycleChange:@"window-screen"];}
...
- (void)recoverPresentations:(NSNotification *)notification {
    ...
    if(!NSThread.isMainThread){__weak typeof(self) weakSelf=self;dispatch_async(dispatch_get_main_queue(),^{[weakSelf recordFieldEvent:reason];for(TWindowController *window in weakSelf.windows)for(TTerminalView *terminal in window.terminals)[terminal recoverPresentationAfterLifecycleChange:reason];});return;}
    [self recordFieldEvent:reason];
    for(TWindowController *window in self.windows)for(TTerminalView *terminal in window.terminals)[terminal recoverPresentationAfterLifecycleChange:reason];
}
- (NSDictionary *)rendererFieldReport {
