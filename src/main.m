...
static NSString *TCompletionScript(NSString *shell) {
    NSString *commands=@"benchmark bench renderer-report renderer-retry retry config config-file update reload editor run completions help version b rr rt c cf u r e x h v",*configActions=@"list get set create use rename delete",*editors=@"vim nvim emacs nano micro hx";
...
static int TRunCLI(int argc, const char *argv[]) {
    ...
    NSDictionary *quick=@{@"b":@"benchmark",@"c":@"config",@"cf":@"config-file",@"u":@"update",@"r":@"reload",@"rr":@"renderer-report",@"rt":@"renderer-retry",@"e":@"editor",@"x":@"run",@"h":@"help",@"v":@"version"};
    arg=quick[arg]?:arg;
...
- (NSDictionary *)diagnosticState {
    @synchronized(self){return @{
      ...
      @"presentationRecoveryCount":@(_presentationRecoveryCount),@"lastPresentationRecovery":_lastPresentationRecovery?:@"none",@"secondsSincePresentationRecovery":@(_lastPresentationRecoveryAt>0?MAX(0,CFAbsoluteTimeGetCurrent()-_lastPresentationRecoveryAt):-1),
...
- (void)recoverPresentations:(NSNotification *)notification {
    ...
    if(!NSThread.isMainThread){__weak typeof(self) weakSelf=self;dispatch_async(dispatch_get_main_queue(),^{for(TWindowController *window in weakSelf.windows)for(TTerminalView *terminal in window.terminals)[terminal recoverPresentationAfterLifecycleChange:reason];});return;}
...
static int TRunPhase7ProductionSelfTest(void) {
    ...
    TAppDelegate *delegate=[TAppDelegate new];delegate.config=[TConfig new];delegate.windows=[NSMutableArray array];NSDictionary *report=[delegate rendererFieldReport],*policy=report[@"rolloutPolicy"],*privacy=report[@"privacy"];if([report[@"schemaVersion"] unsignedIntegerValue]!=3||![policy[@"defaultRenderer"] isEqual:@"appkit"]||![policy[@"metalOptIn"] boolValue]||[policy[@"defaultChangeEligible"] boolValue]||[privacy[@"terminalContentIncluded"] boolValue]){[window close];fprintf(stderr,"phase7-production-self-test failed stage=report-policy report=%s\n",report.description.UTF8String);return 154;}
    [window close];fprintf(stdout,"phase7-production-self-test ok initialization-failures=1 quarantine-recoveries=64 runtime-metal-active=%lu runtime-recoveries=%lu retry=explicit-action state-preserved=yes default=appkit report-schema=3\n",(unsigned long)metalRuns,(unsigned long)runtimeRecoveries);return 0;
}
...
static int TRunPhase8RecoverySelfTest(void) {
    ...
    for(NSWindow *window in nativeWindows)[window close];fprintf(stdout,"phase8-recovery-self-test ok panes=3 failed-retry-isolated=yes attempts=2 recovered=%lu still-quarantined=%lu healthy-panes-unchanged=yes state-preserved=yes report-schema=3\n",(unsigned long)[retry[@"recovered"] unsignedIntegerValue],(unsigned long)[retry[@"stillQuarantined"] unsignedIntegerValue]);return 0;
}
