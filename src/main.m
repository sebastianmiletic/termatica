...
static NSString *TSaveConfigNamed(NSString *name,TConfig *config){if(!TValidConfigName(name))return @"[ INVALID ] use letters, numbers, dot, dash or underscore";NSMutableDictionary *active=[config normalizedDictionary:TReadActiveConfig(config) configName:name];NSError *error=nil;if(!TWriteJSONDictionary(active,TConfigProfilePath(name),&error)||!TWriteJSONDictionary(active,config.path,&error))return [NSString stringWithFormat:@"[ FAILED ] %@",error.localizedDescription?:@"could not save config"];[config ensureEditableFile];[config reload];TPostCLICommand(@"reload");return [NSString stringWithFormat:@"[ SAVED + CURRENT ] %@",name];}
...
- (void)reloadAppearance {
    ...
    [_attributeCache removeAllObjects];_cachedPlainPalette=nil;
    ...
    if(_cachedUnicodeRendering&&_cursorX){
        ...
        if(TMayJoinGrapheme(cp,base->ch,base->flags)){
            ...
            if(joins&&base->ch!=' '&&!(base->flags&TContinuation)){
                ...
                if(!token){NSString *existing=[self stringForCodepoint:base->ch];NSString *scalar=[self stringForCodepoint:cp];NSString *candidate=[existing stringByAppendingString:scalar];token=[self internGrapheme:candidate];_graphemePairKeys[pairIndex]=pairKey;_graphemePairValues[pairIndex]=token;}
...
- (TRenderSnapshot *)renderSnapshot {
    @synchronized(self){
        ...
        if(_historyOffset==0&&!_inlineViewportMode){NSUInteger physicalBase=_rowOffset*_cols;for(NSUInteger row=0;row<_rows;row++){memcpy(destination+row*_cols,_cells+((physicalBase+row*_cols)%(_rows*_cols)),_cols*sizeof(TCell));}}
        ...
        NSMutableData *underlines=[NSMutableData dataWithLength:cellCount];if(_underlineStyles){uint8_t *destinationUnderlines=underlines.mutableBytes;for(NSUInteger row=0;row<_rows;row++)memcpy(destinationUnderlines+row*_cols,_underlineStyles+((_rowOffset+row)%_rows)*_cols,_cols);}
