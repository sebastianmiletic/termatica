...
release: $(BIN) $(SHORTCLI) $(PLIST) $(ICON) $(THEMES) $(SHELL_INTEGRATION_RESOURCES)
	...
	@bytes=$$(find $(APP) -type f -exec stat -f '%z' {} + | awk '{s+=$$1} END {print s}'); \
	  test "$$bytes" -le 1105920 || { echo "Size limit exceeded: $$bytes bytes"; exit 1; }
...
check: release $(BENCH)
	@set -eux; tmp=$$(mktemp -d /tmp/termatica-check.XXXXXX); \
	  trap 'rm -rf "$$tmp"' EXIT; \
	  TERMATICA_CONFIG_DIR="$$tmp" $(CLI) --version | grep -q '^Termatica 1.3.2$$'; \
	  ...
	  test "$$(TERMATICA_CONFIG_DIR="$$tmp" $(SHORTCLI) v)" = 'Termatica 1.3.2'; \
	  ...
	  grep -q 'Update available: 1.3.2 -> v9.9.9' "$$tmp/update-check.out"; \
