...
check: release
	@set -eux; tmp=$$(mktemp -d /tmp/termatica-check.XXXXXX); \
	  trap 'rm -rf "$$tmp"' EXIT; \
	  TERMATICA_CONFIG_DIR="$$tmp" $(CLI) --version | grep -q '^Termatica 1.0.0$$'; \
	  ...
	  test "$$(TERMATICA_CONFIG_DIR="$$tmp" $(SHORTCLI) v)" = 'Termatica 1.0.0'; \
	  ...
	  grep -q 'Update available: 1.0.0 -> v9.9.9' "$$tmp/update-check.out"; \
